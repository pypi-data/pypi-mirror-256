...
def MNMST_representation_with_histology(network_his, network_exp, network_spat, lamb=10, gamma=10, dim=150):
    W0 = create_sppmi_mtx(network_his, 1)
    W1 = create_sppmi_mtx(network_exp, 1)
    W2 = create_sppmi_mtx(network_spat, 1)
    ...
    for epoch in epoch_iter:
        F0 = np.linalg.solve(a=B.T @ B, b=B.T @ W0)
        F1 = np.linalg.solve(a=B.T @ B, b=B.T @ W1)
        F2 = np.linalg.solve(a=B.T @ B, b=B.T @ W2)
        F0 = np.maximum(F0, 0)
        F1 = np.maximum(F1, 0)
        F2 = np.maximum(F2, 0)

        # Update B
        B = (((W0 @ F0.T) + (W1 @ F1.T) + (W2 @ F2.T) + Y2.T + mu * H.T) @
             np.linalg.inv((F1 @ F1.T + F2 @ F2.T + mu * np.eye(dim))))
        B = np.maximum(B, 0)
