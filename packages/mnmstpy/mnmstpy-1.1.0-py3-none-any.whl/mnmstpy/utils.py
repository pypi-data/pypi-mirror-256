...
def generate_spatial_weights_fixed_nbrs(
        ...
        nbr_object: NearestNeighbors = None,
        verbose: bool = True) -> Tuple[csr_matrix, csr_matrix]:
    distance_graph = generate_spatial_distance_graph(locations, nbr_object=nbr_object, num_neighbours=num_neighbours, radius=None)
...
def data_enhance(adata, k_nei, ratio=0.2):
    nbrs = NearestNeighbors(algorithm='ball_tree').fit(adata.obsm['spatial'])
    distances, indices = nbrs.kneighbors(n_neighbors=k_nei)
    median_cell_distance = np.median(distances)
    print(f"\nMedian distance to closest cell = {median_cell_distance}")
    weights_graph, distance_graph = generate_spatial_weights_fixed_nbrs(adata.obsm['spatial'], num_neighbours=k_nei,
                                                                        decay_type='reciprocal', nbr_object=nbrs,
                                                                        verbose=False)

    nbrhood_contribution = ratio
    ...
    return enhanced_data, weights_graph


def construct_initial_graph(x, initial_k_nei=15):
    n_spot = x.shape[0]
    init_W = cosine_similarity(x)
    cos_init = np.zeros([n_spot, n_spot])
    for i in range(n_spot):
        vec = init_W[i, :]
        distance = vec.argsort()[:: -1]
        for t in range(initial_k_nei + 1):
            y = distance[t]
