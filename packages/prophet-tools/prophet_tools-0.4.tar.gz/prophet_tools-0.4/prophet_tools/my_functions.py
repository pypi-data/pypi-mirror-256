...
def list_of_files(ppath,scan_subfolders=False) -> list:
    def list_of_files___getting_files(tt,i):
        tmp = tt[i][2]
        for f in tmp:
            folder_path = ppath_replace_slahses(tt[i][0])
            ffiles.append(f"{folder_path}/{f}")
    ...
    if tt == []:
        ...
        if scan_subfolders == True:
            for i in range(0,len(tt)):
                list_of_files___getting_files(tt,i)
        else:
            i = 0
            list_of_files___getting_files(tt,i)
...
def get_from_URL_via_response_page():
    with open("C:/Users/user/Downloads/_Удалить/response page.html", encoding='utf-8') as ffile:
        return ffile.read()
...
def colored_text(
    ...
    return f"\x1B[38;2;{color}{style}m" + text + "\x1B[0m"
