...
from evidi_fabric.fs import get_table_path, get_workspace_name, get_lakehouse_name
...
def clean_df(df:DataFrame, table_name:str, config:dict, workspace:str=None, lakehouse:str=None) -> DataFrame:
    df = df.dropDuplicates()
    df = add_currency_converted_cols(df, table_name, config, workspace, lakehouse)    
    return df

def add_currency_converted_cols(df:DataFrame, table_name:dict[str,str], config:dict, workspace:str=None, lakehouse:str=None) -> DataFrame:
    """
    ...
    if table_name in config.keys():
        ...
        if "convert_currency" in config_table:
            ...
            currency_table_path=get_table_path(currency_table_name, workspace=None, lakehouse=lakehouse)
            ...
            for convert_currency_col_info in config_currency["convert_currency_cols"]:
                ...
                df_merged=df_merged.alias("df") \
                            .join(df_currency.alias("currency"), join_clause, how="left") \
                            .select(f"df.*", f.col("currency.Rate") \
                                              .alias(f"{col_name}__RATE"))
                df_merged=df_merged.withColumn(f"{col_name}__CONV", f.col(col_name) * f.col(f"{col_name}__Rate"))
                df_merged=df_merged.withColumn(f"{col_name}__CONV_ERR", f.when(f.col(f"{col_name}__CONV").isNull(),1) \
                                                                         .otherwise(0))
    return df_merged
