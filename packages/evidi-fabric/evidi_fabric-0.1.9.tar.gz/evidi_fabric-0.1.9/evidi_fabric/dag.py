...
def extract_depends_on(table_infos, source_table_names:list[str]):
        ...
        if isinstance(depends_on, list):
            for item in depends_on:
                ...
                if isinstance(item, dict) and "table_sql" in item:
                    tables = find_all_tables_in_sql(item["table_sql"])
    ...
    for _, value in table_infos.items():
        if isinstance(value, dict):
            depends_on_list.extend(extract_depends_on(value, source_table_names))
...
def get_dependencies(config, source_table_names:list[str]) -> dict[str, list[str]]:
    ...
    for table_name, table_infos in config.items():
        depends_on_list = extract_depends_on(table_infos, source_table_names)
        if depends_on_list:
            relevant_dependencies[table_name] = depends_on_list
    return relevant_dependencies

def _get_DAG_from_dependencies(silver_tables_with_new_data:list, silver_dependencies:dict[str,list[str]], gold_dependencies:dict[str,list[str]]) -> dict:
    """
    ...
    silver_dependencies_tables=list(set([dependency for dependencies in silver_dependencies.values() for dependency in dependencies]))
    for dependency in silver_dependencies_tables:
        activity = {
            "name": dependency,
            "path": "STD__FO_A",  # Adjust the notebook path accordingly
            "timeoutPerCellInSeconds": 120,  # Adjust the timeout as needed
    ...
    silver_tables_with_new_data = [table for table in silver_tables_with_new_data if table not in silver_dependencies_tables]
    ...
    for table, dependency in gold_dependencies.items():
        activity = {
            "name": table,
            "path": "TRANS",  # Adjust the notebook path accordingly
            "timeoutPerCellInSeconds": 120,  # Adjust the timeout as needed
...
def get_DAG(config_silver:dict[str,list[str]], config_gold:dict[str,list[str]], tables:list[str]) -> dict:
    """
    From a list of tables and config files that entails dependency infos, retrieve a DAG
    """
    silver_dependencies = get_dependencies(config_silver, tables)
    silver_tables_with_new_data=list(set(list(silver_dependencies.keys())+tables))
    gold_dependencies = get_dependencies(config_gold, silver_tables_with_new_data)
    DAG = _get_DAG_from_dependencies(silver_tables_with_new_data, silver_dependencies, gold_dependencies)
    return DAG
