...
from evidi_fabric.fs import get_table_path, get_lakehouse_from_path
...
def clean_df(df:DataFrame, directory_info:dict[str,str], config:dict) -> DataFrame:
    df = df.dropDuplicates()
    df = add_currency_converted_cols(df, directory_info, config)    
    return df

def add_currency_converted_cols(df:DataFrame, directory_info:dict[str,str], config:dict) -> DataFrame:
    """
    ...
    lakehouse=get_lakehouse_from_path(directory_info["path"])
    df_merged=df
    if directory in config.keys():
        config_table=config[directory]
