...
def create_sql_views_from_sql(lakehouse:str, table_sql:str) -> None:
    """
    ...
    for table in tables:
        table_path=get_table_path(lakehouse,table)
        df_temp=spark.read.load(table_path)
        df_temp.createOrReplaceTempView(table)
...
if __name__=="__main__":
    ...
    table_sql = "SELECT sales_amount FROM transactions"
    lakehouse = "Decide_DK_Silver"
    create_sql_views_from_sql(lakehouse, table_sql)
