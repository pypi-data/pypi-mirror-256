<script lang="ts">
	...
	import type { SelectData, LikeData, PlayData } from "@gradio/utils";
    ...
    import RunSec from "./RunSec.svelte";
	...
	export let enable_security = false;
	...
	function hasCode(text: string): boolean {
		console.log("getting code",getCodeBlocks(text))
		console.log("the motherfucking raw text is:",text)
  		const codeRegex = /`.*`|\n\s*\n.*/;
						...
						<div class="message-row {layout} {j == 0 ? 'user-row' : 'bot-row'}">
							...
							{#if (likeable && j !== 0) || (show_copy_button && message && typeof message === "string")}
								...
								>
									...
									{#if runnable && typeof message =="string" && hasCode(message) && j == 1 }
										<RunSec
											j={value.length-1==i}
											enableSecurity={enable_security}
											handle_action={(selected) =>
											handle_play(i, j, message, selected)}
											/>
									{/if}
									{#if likeable && j == 1}
