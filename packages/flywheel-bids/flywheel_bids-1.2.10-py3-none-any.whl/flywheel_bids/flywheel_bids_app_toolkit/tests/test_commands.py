...
@patch('flywheel_bids.flywheel_bids_app_toolkit.commands.exec_command', return_value="--option1\n--option2\n--option3")
def test_validate_kwargs_catches_invalid(mock_exec):
    mock_app_context = MockBIDSAppContext("/path/to/bids/app", {
            "option1": "value1",
            "option2": "value2",
    ...
    assert str(
        excinfo.value) == "Gear cannot run the algorithm with invalid arguments. Exiting."
