...
def test_set_participant_info_for_command(mock_app_context):
    ...
    result = set_participant_info_for_command(mock_app_context, participant_info)

    assert result.subject_label == "01"
    assert result.session_label == "ses-01"
    assert result.run_label == "run-01"
    assert not hasattr(result, "valueless_key")
