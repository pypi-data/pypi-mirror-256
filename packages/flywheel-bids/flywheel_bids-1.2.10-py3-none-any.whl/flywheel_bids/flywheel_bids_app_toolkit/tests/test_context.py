...
@pytest.mark.parametrize(
    ...
    [
        (None, None, Path("/path/to/work_dir/bids"), 0),
        ("/a/tar.zip", None, Path("/a/tar"), 1),
        ("/another/archive", None, Path("/another/archive"), 0),
        (None, {"n_cpus": 6}, Path("/path/to/work_dir/bids"), 0),
        (
...
def test_BIDSAppContext(
    ...
    assert bids_app_context.analysis_output_dir == Path(bids_app_context.output_dir) / Path(
        bids_app_context.destination_id
    )
...
def test_BIDSAppContext_exits_for_no_archive(extended_gear_context):
    ...
    extended_gear_context.config.get.side_effect = lambda key: {"gear-post-processing-only": True}
...
@pytest.mark.parametrize(
    "args,expected",
    [
        (["--arg1", "value1", "--arg2", "value2"], {"--arg1": "value1", "--arg2":
            "value2"}),
        (["--arg1", "value1", "arg2"], {"--arg1": "value1"}),
        (["arg1", "--arg2", "value2"], {"--arg2": "value2"}),
        (["--arg1", "value1", "-Z"], {"--arg1": "value1", "-Z":None}) ,
        ([], {})
    ]
)
