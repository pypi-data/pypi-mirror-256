...
def package_output(
    ...
    zip_file_name = f"{gear_name}_{app_context.run_label}_{app_context.destination_id}.zip"
    zip_output(
    ...
    if app_context.save_intermediate_output:
        work_zip_file_name = f"{gear_name}_work_{app_context.run_label}_{app_context.destination_id}.zip"
        zip_output(
    ...
    if app_context.save_intermediate_files:
        selected_work_zip_file_name = (
            f"{gear_name}_work_selected_files_{app_context.run_label}_{app_context.destination_id}.zip"
        )
        excl_list = walk_tree_to_exclude(app_context.save_intermediate_files.split())
        zip_output(
            str(Path(app_context.work_dir).parent),
            str(Path(app_context.work_dir).name),
            selected_work_zip_file_name,
    ...
    if app_context.save_intermediate_folders:
        dirs = walk_tree_to_find_dirs(app_context.save_intermediate_folders.split())
        for dir in dirs:
            selected_work_zip_file_name = (
                f"{gear_name}_work_{str(Path(dir).name)}_{app_context.run_label}_{app_context.destination_id}.zip"
            )
            zip_output(str(Path(dir).parent), str(Path(dir).name), selected_work_zip_file_name)
    ...
    if Path(app_context.analysis_output_dir).exists():
        if app_context.keep_output:
            ...
            log.debug('removing output directory "%s"', str(app_context.analysis_output_dir))
...
def save_metadata(
    ...
    my_metadata.add_gear_info("results", context.destination["id"], **analysis_based_metadata)


def walk_tree_to_find_dirs(root_dir, include_dirs: List):
    matching_dirs = []
