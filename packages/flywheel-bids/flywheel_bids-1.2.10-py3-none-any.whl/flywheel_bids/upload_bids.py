            ...
            for proj_ses in proj_sess:
                ...
                for ses_acq in ses_acqs:
                    ...
                    for f in ses_acq["files"]:
                        ...
                        if (
                            compare_json_to_file(os.path.basename(file_info["full_filename"]), f["name"])
                        ...
                        elif compare_json_to_file(os.path.basename(file_info["full_filename"]), f["name"]) and (
                            file_does_not_exist(ses_acq["files"], file_info["full_filename"]) or overwrite
                        ):
        ...
        # Figure out which acquisition files within ACQUISITION should have JSON info attached...
            ...
            for f in acq["files"]:
                ...
                if (
                    ...
                    file_does_not_exist(acq["files"], file_info["full_filename"]) or overwrite
                ):
...
def determine_upload_status(overwrite, bc_context, field_to_check, fname):
    ...
    if not overwrite and bc_context[field_to_check].get("files", []):
        existing_files = [f["name"] for f in bc_context[field_to_check].get("files", [])]
...
def fill_in_properties(bc_context, path, use_template_defaults: bool, corrected_dir_name: str = None):
    ...
    for mi in meta_info["BIDS"]:
        if not use_template_defaults and meta_info["BIDS"][mi]:
            ...
            if corrected_dir_name:
                ...
                meta_info["BIDS"][mi] = os.path.join(path.split(os.sep)[:-1], corrected_dir_name)
            else:
...
def handle_acquisition(fw, session_id, acquisition_label, subject_name):
    ...
    if not found:
        logger.info(
            "Acquisition %s not found. Creating new acquisition for session %s." % (acquisition_label, session_id)
        )
...
def handle_project(
    ...
    if found:  # an existing project should stay the way it is
        ...
        if save_sidecar_as_metadata and not template_is_old:  # make the new template save sidecar data in metadata
            template.definitions["project"] = template.definitions["dataset_description_file"]
        ...
        project_id = fw.add_project({"label": project_label, "group": group_id}, inherit=True)
...
def handle_session(fw, project_id, session_name, subject_name):
    ...
    for es in existing_sessions:
        if (es["label"] == session_name) and (es["subject"]["code"] == subject_name):
            logger.info(
                "Session %s for subject %s was found. Adding data to existing session." % (session_name, subject_name)
            )
            # Session exists
            session = es
            found = True
...
def handle_subject(fw, project_id, subject_code):
    ...
    if not found:
        ...
        subject_id = fw.add_subject({"code": subject_code, "label": subject_code, "project": project_id})
...
def handle_subject_folder(
    ...
    bc_context["subject"] = handle_subject(fw, bc_context["project"]["id"], subject_code)
    ...
    if subject_files:
        for fname in subject.get("files"):
            ...
            bc_context["parent_container_type"] = "project"  # TODO: should this be "subject"?
            ...
            bc_context["file"] = bidsify_flywheel.process_matching_templates(bc_context, template, upload=True)
    ...
    for session_label in sessions:
        # Create Session
        bc_context["session"] = handle_session(fw, bc_context["project"]["id"], session_label, subject_code)
        ...
        for foldername in folders:
            ...
            if not check_modality_supported(foldername):
                ...
                deeper_modality_check = subject[session_label][foldername].get("files")[0]
            ...
            for fname in subject[session_label][foldername].get("files"):
                ...
                # Create acquisition
                bc_context["acquisition"] = handle_acquisition(fw, bc_context["session"]["id"], acq_label, subject_code)
                ### Upload file
                # define full filename
                #   NOTE: If session_label equals 'ses-', session label is not
                #       actually present within the original directory structure
                ...
                upload = determine_upload_status(overwrite, bc_context, "acquisition", fname)
                ...
                if upload:
                    ...
                    bc_context["file"] = upload_acquisition_file(fw, bc_context, full_fname, fname)
                    # Update the bc_context for this file
                    bc_context["container_type"] = "file"
                    ...
                    if bc_context["file"].get("info") and ".json" not in fname:
                        ...
                        meta_info = fill_in_properties(bc_context, full_path, use_template_defaults)
                        # Upload the meta info onto the project file
                        fw.set_acquisition_file_info(bc_context["acquisition"]["id"], fname, meta_info)
...
def upload_acquisition_file(fw, bc_context, full_fname, fname):
    ...
    if classification:
        ...
        fw.modify_acquisition_file_classification(bc_context["acquisition"]["id"], bc_context["file"]["name"], update)

    # Return file
    return fw.get_acquisition_file_info(bc_context["acquisition"]["id"], fname).to_dict()
        ...
        if session_label and session_label in bids_hierarchy[project_label_cli][subject_label]:
            bids_hierarchy = {
                project_label_cli: {
                    ...
                    subject_label: {
                        "files": [],
                        session_label: bids_hierarchy[project_label_cli][subject_label][session_label],
                    },
                }
            }
        elif session_label:
...
def upload_bids_dir(
    ...
    if "project" in template.definitions and "Acknowledgements" in template.definitions["project"]:
    ...
    for proj_label in bids_hierarchy:
        ...
        bc_context["project"] = bidsify_flywheel.process_matching_templates(bc_context, template, upload=True)
        fw.modify_project(
        ...
        # TODO: will this only upload the last file in the project?  It does save a list of all of the
        #    json files.  These files are in the BIDS specification:
        ...
        #        samples.json
        try:
            upload = determine_upload_status(overwrite, bc_context, "project", fname)

            if upload:
                # Upload project file
                bc_context["file"] = upload_project_file(fw, bc_context, full_fname, fname)
                # Update the bc_context for this file
                bc_context["container_type"] = "file"
                ...
                if ".json" not in fname:
                    ...
                    meta_info = fill_in_properties(bc_context, full_path, use_template_defaults)
                    # Upload the meta info onto the project file
                    fw.set_project_file_info(bc_context["project"]["id"], fname, meta_info)
        ...
        for dirr in dirs:
            ...
            # Upload project file
            bc_context["file"] = upload_project_file(fw, bc_context, full_zname, dirr + ".zip")
            ...
            meta_info = fill_in_properties(bc_context, full_path, use_template_defaults)
            # Upload the meta info onto the project file
            fw.set_project_file_info(bc_context["project"]["id"], dirr + ".zip", meta_info)
...
def parse_meta_files(fw, files_for_special_handling, save_sidecar_as_metadata=False, overwrite=False):
    """
...
def main():
    ...
    parser.add_argument("--api-key", dest="api_key", action="store", required=True, help="API key")
    parser.add_argument(
        "-g",
        dest="group_id",
        action="store",
