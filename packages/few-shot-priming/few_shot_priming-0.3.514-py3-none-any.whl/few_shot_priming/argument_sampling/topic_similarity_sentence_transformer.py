...
def calc_similarity_sentence_transformer(df_test, df_training):
    ...
    #df_test["topic-text"] = df_test.apply(lambda record: record["topic"] + " " + record["text"], axis=1)
    #df_training["topic-text"] = df_training.apply(lambda record: record["topic"] + " " + record["text"],  axis=1)
