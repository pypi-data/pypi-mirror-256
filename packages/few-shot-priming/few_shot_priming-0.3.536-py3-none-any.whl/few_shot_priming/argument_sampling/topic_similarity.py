...
def check_exists(instance, instances):
    for record in instances:
        if instance["unique_id"].values[0] == record["unique_id"].values[0]:
            return True
    return False
...
def sample_similar_examples(test_index, similarities, df_training, few_shot_size, experiment="ibmsc"):
    ...
    if experiment == "ibmsc":
        ...
        if few_shot_size == 2:
            class_few_shot_size = 1
        else:
            class_few_shot_size = few_shot_size // 3

        neutral_shot_size = class_few_shot_size
        pro_shot_size = class_few_shot_size
        con_shot_size = few_shot_size - pro_shot_size - neutral_shot_size
    ...
    for index in items_indices:
        ...
        if experiment == "vast" and len(neutral_instances) and neutral_instances_count < neutral_shot_size:
            ...
            if experiment =="ibmsc" or not check_exists(con_instance, instances):
            ...
            if experiment =="ibmsc" or not check_exists(pro_instance, instances):
                instances.append(pro_instance)
                pro_instances_count = pro_instances_count + 1
        if experiment == "ibmsc" and pro_instances_count + con_instances_count == few_shot_size:
            break
    ...
    #df_few_shots.sort_values("topic", inplace=True, ascending=False)
...
def save_similar_examples_ks( experiment, experiment_type, save_text=False, debug=False):
    ...
    dataset = load_splits(experiment, oversample=False, validate=is_validate, debug=debug)
    all_k_similar_instances = []
    for k in tqdm(get_ks()):
    ...
    path_similar_examples_ks = get_similar_examples_path(experiment, experiment_type)
    df_examples_ks = pd.concat(all_k_similar_instances)
    df_examples_ks.to_csv(path_similar_examples_ks, sep="\t", encoding="utf-8")


def load_similar_examples(experiment, experiment_type, topic_count=None, few_shot_size=16):
    path_examples = get_similar_examples_path(experiment, experiment_type, topic_count=topic_count)
    df_examples = pd.read_csv(path_examples, sep="\t")
    if topic_count:
