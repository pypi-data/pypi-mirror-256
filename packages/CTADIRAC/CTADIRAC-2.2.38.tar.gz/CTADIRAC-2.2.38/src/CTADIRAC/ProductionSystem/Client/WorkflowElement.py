...
class WorkflowElementDefinition:
    ...
    def __init__(self, we_type: str) -> None:
        ...
        self.constrained_job_keys: set[str] = {"catalogs", "moon", "group_size"}
    ...
    def initialize(self) -> None:
        ...
        if self.we_type == "mcsimulation":
            ...
            self.constrained_job_keys.remove("group_size")
...
class WorkflowElement(WorkflowElementDefinition):
    ...
    def set_constrained_job_attribute(self, key, value) -> None:
        ...
        if self.we_type in self.allowed_simulation_types:
            self.set_constrained_mc_job_attribute(key, value)
        elif self.we_type in self.allowed_processing_types:
            self.set_constrained_processing_job_attribute(key, value)
        else:
            setattr(self.job, key, value)

    def set_constrained_mc_job_attribute(self, key, value) -> None:
        """Set simulation job attribute with constraints"""
        if key == "catalogs":
    ...
    def set_constrained_processing_job_attribute(self, key, value) -> None:
        """Set processing job attribute with constraints"""
        if key == "catalogs":
            # remove whitespaces between catalogs if there are some and separate between commas
            setattr(self.job, key, json.dumps(value.replace(", ", ",").split(sep=",")))
        elif key == "group_size":
