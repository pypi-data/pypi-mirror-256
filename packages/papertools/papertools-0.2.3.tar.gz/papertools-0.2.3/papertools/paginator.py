...
class Paginator(discord.ui.View):
        ...
        if self.attachments:
            ...
            if isinstance(current, str):
                await interaction.message.edit(
                    content=current, embed=None, view=self, attachments=current_attachment
                )
            elif isinstance(current, discord.Embed):
                await interaction.message.edit(
                    content=None, embed=current, view=self, attachments=current_attachment
                )
            ...
            if isinstance(current, str):
                ...
                for item in current:
                    if isinstance(item, str):
                        ...
                        dct["embed"] = item
                await interaction.message.edit(
                    content=dct.get("content", None), embed=dct.get("embed", None), view=self
                )
    ...
    def add_button(
        self, action, /, *, label="", emoji=None, style=discord.ButtonStyle.grey, row=None
    ):
        defer = self.defer
