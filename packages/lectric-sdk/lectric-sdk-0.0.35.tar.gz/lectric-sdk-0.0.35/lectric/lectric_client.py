...
class LectricClient:
    ...
    @propagate_exception
    def hard_drop_collection(self, name: str, exact: bool=False) -> None:
        ...
        if not exact:
            ...
            return hard_delete_collection_exact_collection_hard_delete_delete.sync_detailed(client=self.client, collection_name=name)
    ...
    def soft_drop_collection(self, collection_name: str, exact: bool=False, deleter: str=None) -> None:
        ...
        Args:
            name (str): The name of the collection to be dropped.
            exact (bool, optional): The collection is exact. Defaults to False.
            deleter (str, optional): The user who triggered the soft-drop feature. Defaults to None.

        Raises:
            NotImplementedError: If exact is False.
    ...
    def soft_delete_entities(self, collection_name: str,
        ...
        Args:
            ...
            deleter (str): Name of the user who initiates the soft-delete. Defaults to None.
        Returns:
            _type_: None
        """
        if is_approx:
            ...
            if ids:
                return soft_delete_entries_exact_entries_soft_delete_delete.sync_detailed(
                    client=self.client, collection_name=collection_name, deleter=deleter,
                    json_body=BodySoftDeleteEntriesExactEntriesSoftDeleteDelete(entry_ids=ids)
                )
            if urls:
                return soft_delete_entries_exact_entries_soft_delete_delete.sync_detailed(
                    client=self.client, collection_name=collection_name, deleter=deleter,
                    json_body=BodySoftDeleteEntriesExactEntriesSoftDeleteDelete(urls=urls),
                )
            if file:
        ...
        if is_approx:
            ...
            if ids:
                return hard_delete_entries_exact_entries_hard_delete_delete.sync_detailed(
                    ...
                    json_body=BodyHardDeleteEntriesExactEntriesHardDeleteDelete(entry_ids=ids)
                )
            if urls:
                return hard_delete_entries_exact_entries_hard_delete_delete.sync_detailed(
                    client=self.client, collection_name=collection_name,
                    json_body=BodyHardDeleteEntriesExactEntriesHardDeleteDelete(urls=urls),
                )
            if file:
    ...
    def recover_collection(self, collection_name: str, is_approx: bool = False) -> None:
        ...
        return restore_collection_exact_collection_restore_patch.sync_detailed(client=self.client, collection_name=collection_name)

    @propagate_exception
    def recover_entries(self, collection_name: str,
                ids: Union[List[str], List[int]]=None,
                is_approx: bool=True) -> None:
        if is_approx:
            raise NotImplementedError("Approximate collections not yet supported")
    ...
    def put_exact(self, collection_name: str,
        ...
        if uri:
            ...
            return put_entry_by_file_exact_hash_collection_name_file_put.sync_detailed(
                collection_name=collection_name, client=self.client, ingestor=ingestor,
    ...
    def lookup_exact(self, collection_name: str,
        ...
        if uri:
            return lookup_by_uri_exact_lookup_hash_uri_collection_name_get.sync_detailed(
                collection_name=collection_name, client=self.client, uri=uri,
            )
        elif file:
