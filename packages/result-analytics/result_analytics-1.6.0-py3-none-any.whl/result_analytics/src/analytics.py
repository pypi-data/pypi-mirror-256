...
class DataframeMetadata:
    ...
    def __init__(self, race: list[str]) -> None:
        self.race = race
...
class Analytics:
        ...
        for pdf_name in self.analyser.all_pdf:
            ...
            if filter_mode == "exclude":
                ...
                for index, tag in enumerate(pdf_name[:-1]):  # the [:-1] is to exclude the pdf name
                    if filters[index] is None or filters[index] == []:
                        continue
            ...
            if valid:
                ...
                analysis.metatdata = DataframeMetadata(race=pdf_name)
    ...
    def generate_stats(self, data: list):
        try:
            ...
            quantiles = [data[0], data[0], data[0]]
            stdev = 0
        return {
            "raw": data,
    ...
    def extract(self, aggregated_dataframes: dict, dimension: str, dimension_value: str):
        stats = {}
        if dimension_value not in aggregated_dataframes:
            return None
            ...
            for column in dataframe.columns:
                ...
                for elem in data:
                    ...
                    stats[column] = {
                        ...
                        "race": [*stats.get(column, {"race": []})["race"], dataframe.metatdata.to_json()],
                    }
        ...
        match dimension:
            case "athlete":
                ...
                cleaned_up_stats["ski_judge_base"]["total"] = self.generate_stats(stats["ski_base"]["raw"])
                cleaned_up_stats["ski_judge_deductions"]["total"] = self.generate_stats(stats["ski_deduction_total"]["raw"])

                for stat_name, stat in cleaned_up_stats.items():
                    stat["race"] = stats[stat_name]["race"]
            case _:
