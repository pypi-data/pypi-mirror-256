...
class Application(metaclass=Singleton):
    ...
    def _fetch_config_from_regscale(self, config: Optional[dict] = None) -> dict:
        ...
        """
        self.logger.debug(
            f"Provided config in _fetch_config_from_regscale is: {type(config)}"
        )
        token = config.get("token") or os.getenv("REGSCALE_TOKEN")
        domain = config.get("domain") or os.getenv("REGSCALE_DOMAIN")
        if domain is None or "http" not in domain:
            domain = (
                self.retrieve_domain()[:-1]
                if self.retrieve_domain().endswith("/")
                else self.retrieve_domain()
            )
        self.logger.info(f"Fetching config from {domain}...")
        if domain is not None and token is not None:
            try:
                response = requests.get(
                    url=f"{domain}/api/tenants/getDetailedCliConfig",
                    headers={
                ...
                if config := res_data.get("cliConfig"):
                    ...
                    parsed_dict["userId"] = res_data.get(
                        "userId"
                    ) or parse_user_id_from_jwt(self, token)
                    self.logger.debug(
                        f"updated domain, token and userId: {parsed_dict}"
                    )
    ...
    def _gen_config(self, config: Optional[Union[dict, str]] = None) -> dict:
        ...
        try:
            ...
            if self.templated is False:
                self.logger.debug(
                    f"Starting with {self.config_file}:{len(file_config)} and merging environment."
                )
            ...
            self.logger.error(
                f"ERROR: {self.config_file} has been encrypted! Please decrypt it before proceeding.\n"
            )
    ...
    def get_regscale_license(self, api) -> Response:
        """
