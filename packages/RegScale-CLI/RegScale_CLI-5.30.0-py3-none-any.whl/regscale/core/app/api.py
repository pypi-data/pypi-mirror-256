...
class Api:
    ...
    def __init__(
        ...
        defaults to 10 or "REGSCALE_TIMEOUT" from envar
        ...
        if app:
            ...
            warnings.warn(
                "The API class no longer requires Application object. "
                "Use 'Api()' instead.",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def get(
        ...
        try:
            ...
            self.logger.error(
                "Unable to connect to: %s Please verify the URL and try again.", url
            )
            return response
    ...
    def graph(
        ...
        try:
            ...
            self.logger.debug(
                "%i: %s - %s", response.status_code, response.text, response.reason
            )
