...
ALL_ASSESSMENTS_WB = "all_assessments.xlsx"
NEW_ASSESSMENTS_WB = "new_assessments.xlsx"
OLD_ASSESSMENTS_WB = "old_assessments.xlsx"
...
def new_assessment(path: Path) -> None:
    ...
    workbook.save(filename=os.path.join(path, NEW_ASSESSMENTS_WB))
    ...
    with pd.ExcelWriter(
        os.path.join(path, NEW_ASSESSMENTS_WB),
        mode="a",
        engine="openpyxl",
    ...
    workbook = load_workbook(os.path.join(path, NEW_ASSESSMENTS_WB))
    worksheet = workbook.active
    ...
    logger.info(
        "Your excel workbook has been created. Please open the new_assessments "
        "workbook and add new assessments."
    )
    return None
...
def all_assessments(regscale_id: int, regscale_module: str, path: Path) -> None:
    ...
    if (
        ...
        workbook.save(filename=os.path.join(path, ALL_ASSESSMENTS_WB))
        shutil.copy(
            os.path.join(path, ALL_ASSESSMENTS_WB),
            os.path.join(path, OLD_ASSESSMENTS_WB),
        )
        assessments_data = [
            [
                a["id"],
                a["title"],
                f"{a['leadAssessor']['lastName'].strip()}, {a['leadAssessor']['firstName'].strip()} ({a['leadAssessor']['userName'].strip()})",
        ...
        workbook2 = load_workbook(os.path.join(path, OLD_ASSESSMENTS_WB))
        worksheet2 = workbook2.active
        worksheet2.protection.sheet = True
        workbook2.save(filename=os.path.join(path, OLD_ASSESSMENTS_WB))

        # Adding Data Validation to ALL_ASSESSMENTS_WB file to be adjusted internally.
        workbook = load_workbook(os.path.join(path, ALL_ASSESSMENTS_WB))
        worksheet = workbook.active
        # lock worksheets containing data for dropdowns
...
def upload_data(path: Path) -> None:
    ...
    if os.path.isfile(os.path.join(path, NEW_ASSESSMENTS_WB)):
        new_files = os.path.join(path, NEW_ASSESSMENTS_WB)
        ...
        new = new.merge(
            accounts, how="left", on="LeadAssessor", validate="many_to_many"
        )
        new = new.merge(facilities, how="left", on="Facility", validate="many_to_many")
        new = new.merge(
            organizations, how="left", on="Organization", validate="many_to_many"
        )
        new = new.T.to_dict()
        new_assessments = [
            Assessment(
    ...
    if os.path.isfile(os.path.join(path, ALL_ASSESSMENTS_WB)):
        # Checking all_assessments file for differences before updating database

        df1 = pd.read_excel(
            os.path.join(path, OLD_ASSESSMENTS_WB), sheet_name=0, index_col="Id"
        )

        df2 = pd.read_excel(
            os.path.join(path, ALL_ASSESSMENTS_WB), sheet_name=0, index_col="Id"
        )
        ...
        diff = pd.read_csv(
            os.path.join(path, DIFFERENCES_FILE), header=0, sep=" ", index_col=None
        )
        ids = []
        ...
        updated_files = os.path.join(path, ALL_ASSESSMENTS_WB)
...
def delete_file(path: Path) -> int:
    ...
    logger.info(
        "%i file(s) have been deleted: %s", len(deleted_files), ", ".join(deleted_files)
    )
...
def get_assessment_types() -> pd.DataFrame:
    ...
    all_assessment_types = pd.DataFrame(
...
def adjust_column_widths_and_styles(
    ...
    :param list[str] editable_columns: List of rows to unlock for editing
    :param list[str] date_columns: List of columns to add date style to
    :param NamedStyle date_col_style: NamedStyle object to apply to date columns, defaults to None
    :return: None
    """
...
def create_data_validations(
    data_validations_info, workbook: Workbook, worksheet: Worksheet
) -> None:
    """
    ...
    :param worksheet: The worksheet object to add data validations to
    :return: None
    """
...
def post_and_save_assessments(
    app: Application, new_assessments: list[Assessment], workbook_path: Path
) -> None:
    """
    ...
    :return: None
    """
    new_assessments = Assessment.bulk_insert(
        app=app,
        assessments=new_assessments,
    )
    new_assessments_df = pd.DataFrame(
        [assessment.id for assessment in new_assessments], columns=["id_number"]
    )
    for file_name in [NEW_ASSESSMENTS_WB, ALL_ASSESSMENTS_WB]:
        with pd.ExcelWriter(
...
def map_pandas_timestamp(date_time: pd.Timestamp) -> Optional[str]:
    ...
    if isinstance(date_time, float):
        ...
        and not pd.isna(date_time)
        and not isinstance(date_time, str)
    ):
