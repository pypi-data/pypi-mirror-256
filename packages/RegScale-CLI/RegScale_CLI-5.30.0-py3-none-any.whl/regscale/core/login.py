...
def get_regscale_token(
    ...
    :param str domain: a string representing the RegScale domain,
                   checks environment REGSCALE_DOMAIN
    :param str mfa_token: MFA token to login with
    ...
    response = api.post(url=url, json=auth)
    logger.info(response.url)
    if response.status_code == 200:
        response_dict = response.json()
    elif response.status_code == 403:
        raise ValueError(
            "Unable to authenticate with RegScale. Please check your credentials."
        )
    elif response.status_code == 400 and mfa_token == "":
        raise ValueError(
            "Unable to authenticate with RegScale. Please try again with an MFA Token."
        )
    else:
        raise ValueError(
            f"Unable to authenticate with RegScale.\n{response.status_code}: {response.text}"
        )
