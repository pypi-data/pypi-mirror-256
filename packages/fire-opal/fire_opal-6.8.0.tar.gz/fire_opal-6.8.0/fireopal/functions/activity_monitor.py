...
def activity_monitor(
    ...
    if _check_for_remote_router():
        _EVENT_TRACKER.track_activity_monitor(
            user_email=_get_user_email(), limit=limit, offset=offset, status=status
        )
    # Row items.
...
def get_action_metadata(
    ...
    list[ActionMetadata]
        Action metadata.
    """
    check_argument(limit <= 50, "The limit cannot exceed 50.", {"limit": limit})
    if status is None:
        print(
    ...
    if _check_for_remote_router():
        _EVENT_TRACKER.track_get_action_metadata(
            user_email=_get_user_email(router),
            limit=limit,
            offset=offset,
            status=status,
        )
