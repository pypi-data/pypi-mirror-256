...
def list_root_files(filenames:Union[List[str], str], strict:bool=False):
    all_files = []
    ...
    for file in all_files:
        rf = ROOT.TFile(file)
        if not TFile.is_corrupt(rf):
            valid_files.append(file)
        elif strict:
            raise RuntimeError(f'Found empty/corrupted file "{file}"')
    return valid_files

def get_th1d_model(bins:Union[int, Sequence]=128,
    ...
    if isinstance(bins, Sequence):
        if bin_range is not None:
            raise ValueError('can not specify bin_range when bins is a sequence')
        bins = np.array(bins)
        nbinsx = len(bins)
        from quickstats.interface.cppyy.vectorize import as_pointer
        xbins = as_pointer(np.array(bins, dtype='float'))
        return ROOT.RDF.TH1DModel(name, title, nbinsx, xbins)
    if bin_range is None:
        xlow, xup = 0., 0.
    else:
        xlow, xup = bin_range
    return ROOT.RDF.TH1DModel(name, title, bins, xlow, xup)
