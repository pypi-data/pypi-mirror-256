...
class TableConnector(DataConnector):
    ...
    @staticmethod
    def _get_import_type(dtype, entity_name:str):
        """ Get an appropriate index type for a column of the given Pandas dtype.  The given entity_name will be used
            in an error message if the column type cannot be recognized. """
    ...
    def initialize_entities(self, entity_filter: Callable[[xi_types.Entity], bool], overwrite: bool = True) -> None:
        ...
        for entity in TableConnector._get_entities(self._data_container, entity_filter):
            ...
            if entity.name in self._data_container.__dict__ and\
                    not overwrite and not isinstance(entity, xi_types.DataFrame):
    ...
    def _initialize_scalar_or_param(self, entity: xi_types.ScalarBase):
        self._data_container.__dict__[entity.name] = entity.default

    def _initialize_index(self, entity: xi_types.Index):
        self._data_container.__dict__[entity.name] = pd.Index(
            dtype=xi_types.BASIC_PANDAS_DTYPE_MAP[entity.dtype],
            data=[])

    def _initialize_series(self, entity: xi_types.Series):
        self._data_container.__dict__[entity.name] = pd.Series(
            dtype=xi_types.BASIC_PANDAS_DTYPE_MAP[entity.dtype],
            index=xi_types.get_empty_index_for_entity(entity))

    def _initialize_data_frame(self, entity: xi_types.DataFrame, columns: Iterable[xi_types.Column], overwrite: bool):
        ...
        #
        if df is None:
            df = pd.DataFrame(index=xi_types.get_empty_index_for_entity(entity))
    ...
    def _import_data_frame_from_multiple_tables(self, entity: xi_types.DataFrame,
        ...
        if entity.index_names:
            ...
            pd_index = xi_types.get_empty_index_for_entity(entity)
