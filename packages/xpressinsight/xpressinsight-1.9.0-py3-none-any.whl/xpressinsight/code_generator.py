...
import shutil
import sys
from typing import Dict, List, TextIO, Tuple, Type, Union, ValuesView
...
from .entities import (BASIC_TYPE, BasicType, boolean, integer, string, real,
                       Entity, EntityBase, Param, Scalar, Index,
                       Series, DataFrame, Hidden, Manage, UnexpectedEntityTypeError)
...
class CodeGenerator:
    ...
    def __declaration_safe_sorted(cls, entities: List[EntityBase]) -> List[EntityBase]:
        ...
        #
        #
        entities_with_indexes = [e for e in entities if isinstance(e, (Series, DataFrame))]
        entities_without_indexes = [e for e in entities if not isinstance(e, (Series, DataFrame))]
...
model {quote_mos(self.app_cfg.name)}
    ...
    def write_data_func_header(self, func_name: str):
        """ Write the generic header of data functions. """
        self.file.write(f"""    public procedure {func_name}(
    ...
    def data_func_manage_index(self, index: Index, export: bool):
        """ Write the code to import or export an index (set). """
        func_name = 'exportset' if export else 'importset'
        set_ident = validate_ident(index.name)
        self.file.write(f'        {func_name}("{set_ident}", {set_ident})\n')

    @staticmethod
    def get_sql_index_set_list(index_list: Tuple[Index, ...]) -> str:
        """ Get SQL fragment listing index names/types """
        result = '['
        ...
        for i, index in enumerate(index_list):
            ...
            result += f'["{index.name}", {CodeGenerator.BASIC_SQL_TYPE_NAME_MAP[index.dtype]}]'

        return result + ']'

    def data_func_manage_array(self, array_name: str, index: Tuple[Index, ...], dtype: BASIC_TYPE, manage: Manage,
                               export: bool):
        """ Write code to import or export an array. """
        ...
        array_ident = validate_ident(array_name)
        self.file.write(f"""        SQLexecute({func_name}("{array_ident}",
            {CodeGenerator.BASIC_SQL_TYPE_NAME_MAP[dtype]}, {self.get_sql_index_set_list(index)}), {array_ident})
        checksqlstatus({err_msg}, ["{array_ident}"])\n""")

        if export and manage == Manage.INPUT:
            self.file.write(f'        delcell({array_name})\n')

    def write_data_func(self, func_name: str, entities: List[EntityBase], manage: Manage, export: bool):
        """ Write a data function to the Mosel source file. """
    ...
    def write_data_funcs(self):
        """ Write all the data functions to the Mosel source file. """
        self.write_data_func('AppRunner~exportparams', self.params, Manage.INPUT, export=True)
        self.write_data_func('AppRunner~importinputs', self.declared_entities, Manage.INPUT, export=False)
        self.write_data_func('AppRunner~exportinputs', self.declared_entities, Manage.INPUT, export=True)
        self.write_data_func('AppRunner~importresults', self.declared_entities, Manage.RESULT, export=False)
