...
from typing import Dict, Generator, List, Optional, Set, Tuple, Union, Type, Any
...
class XpriAttachmentUpdate:
    """
    Record of an attachment being updated.
    Note: newly-created attachments are not recorded in xpri_attachmentupdate records.
    """
...
class AppTestInterface(AppInterface):
    ...
    def _xpri_clear_attach_stat(self) -> None:
        #
        if self.attach_status == AttachStatus.IN_PROGRESS:
            _raise_runtime_error("Already in attachment operation.")
        self.attach_status = AttachStatus.IN_PROGRESS

    def _xpri_set_attach_stat(self, new_stat: AttachStatus) -> None:
        #
        if self.attach_status != AttachStatus.IN_PROGRESS:
            _raise_runtime_error("Not in attachment operation.")
        if new_stat == AttachStatus.IN_PROGRESS:
            _raise_runtime_error("Invalid status passed to _xpri_set_attach_stat.")
        self.attach_status = new_stat
        ...
        with self._lock:
            for upd in self._attachment_updates:
                ...
                if upd.update_type == AttachUpdateType.DELETE:
                    ...
                    #
                    #
                    print(f"ERROR: Unexpected update type: {upd.update_type}.", file=sys.stderr)
    ...
    def _xpri_get_attach(
        ...
        self.attach_status = self._xpri_get_attach_common(attach_cache, filename, destination_filename)
    ...
    def _xpri_get_attachs_by_tag(self, tag: str, destination_directory: str = None) -> Optional[List[Attachment]]:
        with self._lock:
            ...
            (status, atts) = self._xpri_list_attach_by_tag_common(attach_cache, tag)
    ...
    def _xpri_get_attach_filenames_by_tag(self, tag: str, destination_directory: str = None) -> List[str]:
        with self._lock:
            ...
            if self.attach_status == AttachStatus.OK:
                assert attachments is not None
    ...
    def _xpri_get_attach_by_tag(self, tag: str, destination_directory: str = None) -> Optional[Attachment]:
        with self._lock:
            ...
            (status, atts) = self._xpri_list_attach_by_tag_common(
