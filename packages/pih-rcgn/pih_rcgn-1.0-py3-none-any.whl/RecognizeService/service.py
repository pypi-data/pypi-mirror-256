...
def start(as_standalone: bool = False) -> None:
    if A.U.for_service(
        ...
        from pih.tools import ParameterList, ne, e, nn, j, js, b
        ...
        logger: Logger = Logger(MIO.create_output(A.CT_ME_WH.GROUP.DOCUMENTS_WORK_STACK))
        ...
        def service_call_handler(sc: SC, parameter_list: ParameterList) -> Any:
            ...
            if sc == SC.recognize_document:
                ...
                result: bool | PolibaseDocument | list[
                    MedicalDirectionDocument
                ] | None = recognize_document(
        ...
        def recognize_document(
            ...
            #
            if pdf_format_detected or image_format_detected:
                source_file_directory: str = A.PTH.get_file_directory(document_file_path)
                ...
                if nn(document_type):
                    ...
                    while True:
                        try:
                            page_list = (
                                Converter.pdf_to_pages_as_image_list(document_file_path)
                                if pdf_format_detected
                    ...
                    if len(page_list) > 0:
                        ...
                        if log_level >= 1:
                            ...
                            for scan_source_item in A.CT_SCN.Sources:
                                ...
                                if document_file_name.startswith(scan_source_item_value[0]):
                                ...
                                logger.write_image(
                                    j(
                                        (
                                            "Входящий документ: ",
                                            A.PTH.add_extension(
                                                document_file_name, document_file_extension
                                            ),
                                            " (",
                                            b(scan_source_title),
                                            ")",
                        ...
                        if document_type == A.CT_DT.POLIBASE:
                            ...
                            if ne(recognize_result):
                                polibase_scanned_document: PolibaseDocument | None = None
                                ...
                                if ne(polibase_document_recognize_result):
                                    ...
                                    polibase_document_description: PolibaseDocumentDescription = A.D.get(
                                        polibase_document_recognize_result.type
                                    )
                        ...
                        if document_type == A.CT_DT.MEDICAL_DIRECTION:
                            for page_index, page in enumerate(page_list):
                                ...
                                if ne(recognize_result):
                                    recognize_result_map[page_index] = recognize_result
                                    if recognize_result.type == A.CT_DT.MEDICAL_DIRECTION:
