...
# For signing
from neroimpacket import smb, nmb, ntlm, uuid
from neroimpacket import smb3structs as smb2
from neroimpacket.spnego import SPNEGO_NegTokenInit, TypesMech, MechTypes, SPNEGO_NegTokenResp, ASN1_AID, \
    ASN1_SUPPORTED_MECH
from neroimpacket.nt_errors import STATUS_NO_MORE_FILES, STATUS_NETWORK_NAME_DELETED, STATUS_INVALID_PARAMETER, \
...
from neroimpacket.dcerpc.v5.rpcrt import DCERPCServer
from neroimpacket.dcerpc.v5.dtypes import NULL
from neroimpacket.dcerpc.v5.srvs import NetrShareEnum, NetrShareEnumResponse, SHARE_INFO_1, NetrServerGetInfo, \
    NetrServerGetInfoResponse, NetrShareGetInfo, NetrShareGetInfoResponse
from neroimpacket.dcerpc.v5.wkst import NetrWkstaGetInfo, NetrWkstaGetInfoResponse
from neroimpacket.system_errors import ERROR_INVALID_LEVEL
...
class SimpleSMBServer:
    ...
    def __init__(self, listenAddress='0.0.0.0', listenPort=445, config=''):
        # Here we write a mini config for the server
        self.__smbConfig = configparser.ConfigParser()
        self.__smbConfig.add_section('global')
        self.__smbConfig.set('global', 'server_name', config.server_name)
        self.__smbConfig.set('global', 'server_os', config.server_os)
        self.__smbConfig.set('global', 'server_domain', config.server_domain)
        ...
        self.__smbConfig.set('IPC$', 'comment', 'Remote IPC')
        self.__smbConfig.set('IPC$', 'read only', 'yes')
        self.__smbConfig.set('IPC$', 'share type', '3')
        self.__smbConfig.set('IPC$', 'path', '/tmp')
