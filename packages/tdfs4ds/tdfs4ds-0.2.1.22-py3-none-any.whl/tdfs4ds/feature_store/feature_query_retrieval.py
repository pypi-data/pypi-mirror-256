...
def get_feature_versions(entity_name, features, domain=None, latest_version_only=True, version_lag=0):
    ...
    # Executing the query:
    df = tdml.DataFrame.from_query(query).to_pandas()
    results.update({row['FEATURE_NAME']: row['FEATURE_VERSION'] for i, row in df.iterrows() if row['FEATURE_NAME'] in [x.upper() for x in features]})
