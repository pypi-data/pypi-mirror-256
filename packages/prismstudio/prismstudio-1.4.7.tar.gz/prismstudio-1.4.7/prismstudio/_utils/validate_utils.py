    ...
    def type_checker(*args, **kwargs):
        try:
            ...
            for param_name, param_info in signature.parameters.items():
                ...
                arg = apply_validation_rules(arg, param_name, function.__name__)
...
def function_specific_validation(args_dict, function_name, class_name=None):
    if function_name == "fillna":
        ...
        if (function_name == "__init__"):
            ...
            interval = int(re.findall("\d+", args_dict["data_interval"])[0])
        ...
        if function_name in ["__init__", "run"]:
            frequency_validator(args_dict["frequency"], function_name)
...
def frequency_validator(frequency: str, function_name):
    if function_name in ["resample", "screen", "factor_backtest"]: business_pattern = "b?"
...
def data_interval_validator(period: str, _):
    period_pattern = r"([1-9][0-9]{0,3})?(D|W|M|Q|SA|A)"
...
def lookback_validator(lookback: Union[str, int], _):
    if isinstance(lookback, int) and (lookback < 0): raise PrismValueError('lookback should be positive')
    if isinstance(lookback, str): return period_validator(lookback)
    return f"{lookback}D"


def period_validator(period, _):
    period_pattern = r"([1-9][0-9]{0,3})?(D|W|M|Q|SA|A)"
...
def n_quarter_validator(n_quarter, _):
