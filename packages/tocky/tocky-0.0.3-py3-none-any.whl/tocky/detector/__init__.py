...
from tocky.ia import extract_page_index, get_djvu_pages, ocaid_to_djvu_url
...
def analyze_page_for_toc(
  ...
  def do_redo_ocr(min_conf: float):
    if elem.xpath('.//HIDDENTEXT/@x-re-ocrd') == ['true']:
      return
    new_ocr = ocr_djvu_page(ocaid, leaf_num)
...
def analyze_djvu_for_toc(djvu_url: str, index: int = None, allow_reocr=True):
    has_begun = False
    reocrs_done = 0
    last_result: tuple[str, str, TOCAnalysisResult] = None
    results: list[tuple[str, str, TOCAnalysisResult]] = []
    for page_name, elem in get_djvu_pages(djvu_url, 1, 28):
      ...
      if not has_begun and toc_analysis.is_toc and last_result and not last_result[2].reran_ocr:
        ...
        for line in elem.findall(".//LINE"):
            ...
            if words:
              last_word = words[-1]
              if has_roman_numeral := bool(re.search(r'\b([xvil]+)$', last_word.text)):
                recheck_previous = False
                break
