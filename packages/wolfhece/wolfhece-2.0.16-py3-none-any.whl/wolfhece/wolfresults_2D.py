...
class Wolfresults_2D(Element_To_Draw):
    ...
    def plot_h(self, xy:list[float], h_or_z:Literal['h', 'z', 'head'], toshow=False, figax = None):
        """
        ...
        for i in tqdm(range(nb)):
            self.read_oneresult(i)
            vals.append([self.get_values_from_xy(x,y) for x,y in xy])
        ...
        if h_or_z =='h':
            ...
            for i in range(len(xy)):
                ax.plot(times, [valsloc[i][0][which] for valsloc in vals], label=_('{}-{}'.format(xy[i][0], xy[i][1])))
        elif h_or_z =='z':
            which = 7
            for i in range(len(xy)):
                ax.plot(times, [valsloc[i][0][which] for valsloc in vals], label=_('{}-{}'.format(xy[i][0], xy[i][1])))
            ...
            for i in range(len(xy)):
                head = [valsloc[i][0][7]+valsloc[i][0][5]**2./2/9.81 for valsloc in vals]
                ax.plot(times, head, label=_('{}-{}'.format(xy[i][0], xy[i][1])))

        ax.set_xlabel(_('Time [s]'))
        ax.set_ylabel(_('Water depth [$m$]'))
        ax.legend(loc=2)
        fig.tight_layout()
