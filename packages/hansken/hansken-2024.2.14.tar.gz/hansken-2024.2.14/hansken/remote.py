...
class Connection:
    ...
    def create_image(self, **kwargs):
        ...
        """
        image = omit_empty({to_property_name(key): _safe_format(kwargs[key])
                            for key in kwargs if key not in IMAGE_READ_ONLY})
    ...
    def queue_cleaning(self, project_id, query=None, clean_priority='low'):
        """
        ...
        search_request = self.create_search_request(query=query, project_ids=multiple_projects)
        # single_project will either be a project id or None (in which case the step is left out of the url)
        response = self._session.post(self.url(self.path.projects, single_project, '/traces', '/clean'),
                                      json=search_request,
                                      params=omit_empty({'priority': clean_priority}))

        with drain(response):
            return _expect_ok(response, codes.accepted) is response
    ...
    def delete_traces(self, project_id, query=None):
        ...
        response = self._session.delete(self.url(self.path.projects, project_id, '/traces/search'),
                                        json=search_request)
    ...
    def extract(self, project_id, image_id, type='index', key=fetch, tools=None, configuration=None, query=None,
                pre_clean_priority=None, engine=None):
        """
    ...
    def export_project(self, project_id, user_export_key, image_keys=fetch,
                       query=None, include_priviliged=False, include_notes=False,
                       include_tags=False, include_entities=False, include_image_data=False,
                       image_id=None):
        """
        ...
        images = self.project_images(project_id)
        if image_keys is fetch:
            keys = {image['id']: b64encode(key) for image in images
                    if (key := self._maybe_fetch_key(image['id'], fetch))}
        else:
            keys = {image['id']: b64encode(key) for image in images
                    if (key := image_keys.get(image['id'], None))}
        ...
        response = self._session.post(self.url(self.path.projects, project_id, 'export'),
    ...
    def download_export(self, task_id, file_path):
        if os.path.exists(file_path):
            raise ValueError('Export file already exists: {}'.format(file_path))
    ...
    def prepare_project_import(self, project_id, user_export_key, export_file, image_keys=fetch):
        ...
        with open(export_file, 'rb') as f:
            m = MultipartEncoder(fields={'prepareImportRequest': json.dumps({'imageKeys': keys}),
                                         'file': ('filename', f)})

            # POST the file data to remote, expect a 201 (Created)
            response = self._session.post(self.url(self.path.projects, project_id, 'import', 'prepare'),
                                          data=m,
                                          headers={'Hansken-User-Export-Key': b64encode(user_export_key),
    ...
    def apply_project_import(self, project_id, user_export_key, task_id, image_keys=fetch,
        ...
        response = self._session.post(self.url(self.path.projects, project_id, 'import', 'apply'),
...
class FacetResult(Mapping):
    ...
    def __init__(self, source):
        ...
        self.counters = [FacetResult.Counter(
            label=counter['label'],
            value=counter['value'],
            count=counter['count'],
...
class ProjectContext:
    ...
    @_auto_open
    def queue_cleaning(self, query=None, clean_priority='low'):
        """
        Performs a search request built from the parameters, and puts the results on the cleaner queue with the
        given priority.
        ...
        :return: True on success (failure will likely result in an HTTPError)
        """
        return self.connection.queue_cleaning(project_id=self.project_id, query=query, clean_priority=clean_priority)
