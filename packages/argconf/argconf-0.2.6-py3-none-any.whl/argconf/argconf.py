...
def _resolve_references(conf, conf_fn, root_conf=None, parent_key=None):
    ...
    for k,v in conf.items():
        ...
        if isinstance(v, omegaconf.dictconfig.DictConfig):
            root_conf = full_key_set(root_conf, full_k, _resolve_references(v, conf_fn, root_conf,full_k))
        if isinstance(k, str) and k=="import":
            ...
            for sub_idx in range(0, len(path_elems)):
                for root_dir in ["conf", ".",  Path(conf_fn).parent]:
                    if Path(root_dir, *path_prefix, *path_elems[:len(path_elems)-sub_idx]).with_suffix('.yaml').exists():
                        sub_conf = OmegaConf.load(Path(root_dir, *path_prefix, *path_elems[:len(path_elems)-sub_idx]).with_suffix('.yaml'))
                        sub_ref = ".".join(path_elems[len(path_elems)-sub_idx:])
                        resolved_entry = full_key_get(sub_conf, sub_ref) if sub_ref!="" else sub_conf
            ...
            if parent_key is not None:
                ...
                root_conf = full_key_set(root_conf, parent_key, OmegaConf.merge(resolved_entry, full_key_get(root_conf, parent_key)))
            else:
                del root_conf[k]
...
def argconf_parse_hocon(conf_fn=None, ignore_unknown=False, parse_args=True, resolve_path=True):
    if conf_fn is None:
        # load config name
...
def argconf_parse(conf_fn=None, ignore_unknown=False, parse_args=True, resolve_path=True, backend=None):
    argv = sys.argv[1:] if conf_fn is not None else sys.argv[2:]
    if backend is None:
        if conf_fn is None:
            conf_fn = sys.argv[1]
