...
class InsulaDownloadJobResults(object):
    ...
    def __download_job_results(self, results: list, output_folder):
        for result in results:
            self.download_file(result, output_folder)

    def download_file(self, result, output_folder):
        ...
        run_request = requests.get(result['download'],
                                   headers=self.__insula_api_config.get_authorization_header())

        if run_request.status_code != 200:
            status['message'] = f"cant download file: {result['download']}, status: {run_request.status_code}"
