...
from .InsulaParamFilter import InsulaParamFilter, BLaBla
...
class InsulaWorkflowStepRunner(object):
    ...
    def __get_parameters_for_value(self, s):
        values = []
        for value in s['values']:
            ipf = InsulaParamFilter(value)
            if ipf.has_match():
                values.extend(
                    BLaBla.get_from_results(ipf, value, self.__results)
                )
            else:
    ...
    def __run_platform_processor_step(self, step):
        ...
        while attempt < 2:
            ...
            for s in step['params']:
                ...
                params_arr = []
                for a in self.__get_parameters_for_value(s):
                    params_arr.append(a['uri'])
            ...
            if run['status']['status'] != 'COMPLETED':
                ...
                run['results'] = (InsulaFilesJobResult(self.__insula_api_config)
                                  .get_result_from_job_status(insula_status))
                break
    ...
    def __run_download_job_result(self, step):
        ...
        for s in step['params']:
            ...
            for a in all_in:
                run['logs']['logs'].append(f'downloading: {a} save in {save_in}')
                status = downloader.download_file(a, save_in)
                ...
                run['logs']['logs'].append(
                    f'status: {status["success"]} message:{status["message"]}  for {a} save in {save_in}')
