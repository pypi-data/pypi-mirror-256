...
def send_success_webhook(url,fail_url, output, error_data, job):
    ...
    add_info_logs(job["id"], f"Getting levels for {job['data']['task_id']} and {list(job['data']['dag'].keys())[0]}")
    all_levels = get_levels_for_block(INTERIM_TASK_TABLE,job["data"]["task_id"],list(job['data']['dag'].keys())[0])
    for level in all_levels:
        data = {
            'level': level,
            'task_id':job['data']['task_id'],
            'algo_name': list(job['data']['dag'].keys())[0],
            'dag': job['data']['dag'],
        }
