...
def send_success_webhook(url,fail_url, output, error_data, job):
    ...
    for block in next_blocks:
        new_dag = create_next_dag(block, next_blocks, dag)
        add_info_logs(job["id"], f"Getting levels for {job['data']['task_id']} and {new_dag.keys()[0]}")
        all_levels = get_levels_for_block(INTERIM_TASK_TABLE,job["data"]["task_id"],new_dag.keys()[0])
        for level in all_levels:
            data = {
                'level': level,
                'task_id':job['data']['task_id'],
                'algo_name': list(dag.keys())[0],
                'dag': new_dag,
            }
