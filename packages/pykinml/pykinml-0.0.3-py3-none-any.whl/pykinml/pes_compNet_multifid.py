...
def my_loss_rel(ph, yh, expw, pl=None, yl=None, dEsq=1., dElsq=1., c=1.):
    if pl != None:
        loss = torch.mean((pl - yl) ** 2)/dEsq + c*torch.mean((ph - yh) ** 2)/dElsq
    else:
        loss = torch.mean(((ph - yh) ** 2) * expw)/dEsq
        #print('expw: ', expw)
        #print('loss: ', loss)
    return loss
...
def train(nn_pes, dpes, criterion, optimizer, args, verbose=False):
    ...
    while epoch != args.epochs:
            ...
            if args.wloss:
                ...
                if args.floss:
                    ...
                    if nn_pes.net.arch == 'hfonly' or args.refinehf:
                        #loss = my_loss(output, dpes.ybtr[b], None, None, dEsq)
                        #loss = my_loss(output, dpes.ybtr[b], None, None, dEsq=1.0)
                        loss = my_loss_rel(output, dpes.ybtr[b], dpes.expw[b], None, None, dEsq=1.0)
                    else:
                        if args.refinelf:
        ...
        if not args.nw and epoch % args.ntlpm == 0:
...
def parse_arguments_list():
    ...
    parser.add_argument('--ntlpm', type=int, default=1000,
                        help='specify save NN every <ntlpm> epochs')
    ...
    parser.add_argument('--savets', action='store_true', default=False, help='save test errors')
    parser.add_argument('--savevl', action='store_true', default=False, help='save validation errors')
    ...
    parser.add_argument('--write-hdf', action='store_true', default=False,
...
def prep_data(args, mf=None):
    ...
    if args.gen_byid:
        print('pes_compNet_multifid: prep_data: calling read_xidtxt with args.trtsid_name[0]:',args.trtsid_name[0])
        xid = read_xidtxt(args.trtsid_name[0])
        print('pes_compNet_multifid: prep_data: calling get_data with xid length:',len(xid))
        dpes.get_data(args, xid=xid)
    else:
        print('pes_compNet_multifid: prep_data: calling get_data with no xid spec')
        dpes.get_data(args)
    ...
    if args.read_tvtmsk:
        ...
        print('pes_compNet_multifid.py: prep_dat: tvt mask for testset was set based on the file: {}.'.format(args.trtsid_name))
    else:
        ...
        dpes.set_tvt_mask()
    print("pes_compNet_multifid.py: prep_dat: ntrdat:", dpes.ntrdat, "nvldat:", dpes.nvldat, "ntsdat:", dpes.ntsdat)
    ...
    print("pes_compNet_multifid.py: prep_dat: Prepare training, validation, and testing data")
    ...
    if dpes.nvldat > 0:
        print('pes_compNet_multifid.py: prep_data: nvldat: ',dpes.nvldat)
        print('pes_compNet_multifid.py: prep_data: calling dpes.prep_validation_data')
...
def train_pes(args, verbose=True):
    ...
    if args.epochs < 1:
        print('error::: the number of epochs should be > 0')
        sys.exit()
    ...
    if sae_late:
        if args.sae_fit:
            ...
            for b in range(len(dpes.ybtr)):
                ...
                for i in range(len(dpes.ybtr[b])):
                    #atom_count.append([dpes.ybtr_symb_data[b][i].count('C'), dpes.ybtr_symb_data[b][i].count('H')])
                    atom_count.append([Ccounts[i], Hcounts[i]])
                    total_energies.append(dpes.ybtr[b][i][0])
            atom_count = np.array(atom_count)
            #total_energies = list(zip(*dpes.meta))[0]
            ...
            for b in range(len(dpes.ybvl)):
                ...
                #dpes.ybvl[b] = dpes.ybvl[b] - Ccounts * sae_energies[0] - Hcounts * sae_energies[1]
                #print(b, 'dpes.ybvl: ', dpes.ybvl[b]) 
            for b in range(len(dpes.ybts)):
                ...
                #dpes.ybts[b] = dpes.ybts[b] - Ccounts * sae_energies[0] - Hcounts * sae_energies[1]
                #print(b, 'dpes.ybts: ', dpes.ybts[b])
            #for i in range(len(subtracted_energies)):
            #    dpes.xdat[i][dpes.num_nn] = [subtracted_energies[i]]
            ...
            #    print('Mean of subtracted energies LF (might be close to 0): ', np.mean(subtracted_energies))
            #    print('Range of subtracted energies LF: ', max(subtracted_energies) - min(subtracted_energies))
            #    energy_counter=0
            #    for b in range(len(dpes.ybtr_lf)):
            #        for i in range(len(dpes.ybtr_lf[b])):
            #            dpes.ybtr_lf[b][i] = subtracted_energies[energy_counter]
            #            energy_counter += 1
            #    for b in range(len(dpes.ybvl_lf)):
            ...
            #    for b in range(len(dpes.ybts_lf)):
            #        Cvalues, Ccounts = np.unique(dpes.indts_lf[b][0], return_counts=True)
            #        Hvalues, Hcounts = np.unique(dpes.indts_lf[b][1], return_counts=True)
            #        for j in range(len(dpes.ybts_lf[b])):
            #            dpes.ybts_lf[b][j] = dpes.ybts_lf[b][j] - Ccounts[j] * sae_energies[0] - Hcounts[j] * sae_energies[1]
        else:
...
#        print('Range of subtracted energies: ', max(all_trengs_shifted) - min(all_trengs_shifted))
    ...
    # if ismultifidelity:
    #     maxmin = [dpes.ymax_lf, dpes.ymin_lf]
    # else:
    #     maxmin = [dpes.ymax, dpes.ymin]
