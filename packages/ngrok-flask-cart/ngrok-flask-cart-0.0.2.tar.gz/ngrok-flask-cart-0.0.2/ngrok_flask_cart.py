...
def _run_ngrok(port, domain, auth_token):
    ...
    ngrok = subprocess.Popen([executable, "http", domain, str(port)])
...
def start_ngrok(port, domain, auth_token):
    ngrok_address = _run_ngrok(port, domain, auth_token)
    print(f" * Running on {ngrok_address}")
    print(f" * Traffic stats available on http://127.0.0.1:4040")


def run_with_ngrok(app, domain=None, auth_token=None):
    """
    ...
    def new_run(*args, **kwargs):
        ...
        thread = Timer(1, start_ngrok, args=(port, domain, auth_token))
