...
IMailConversationInternalSync_VERSION = "4.9.3438"
...
class IMailConversationInternalSync(BaseEndpoint):
    ...
    def sync(self, dry):
        ...
        __data__ = None
        __encoded__ = None
        __data__ = serder.BOOLEAN.encode(dry)
        __encoded__ = json.dumps(__data__)
        queryParams = {}

        response = requests.post(self.base + postUri, params=queryParams, verify=False, headers={
                                 'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IMailConversationInternalSync_VERSION}, data=__encoded__)
