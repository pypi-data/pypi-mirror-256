...
class parser:
    ...
    def readCombat(self, offset=0):
        ...
        with open(self.path, "r") as file:
            file.seek(offset)
            start = timer.time()
            # Trick to allow use of tell() when iterating by line.
    ...
    def readCombatFull(self, path, offset=0):
        self.resetParser(offset=offset)
        self.depthAnalysis = True
        self.setPath(path)
        self.readCombat(offset=offset)
        self.generatedUItables()
        return self.uiDictionary, self.dmgTableIndex, self.healTableIndex, self.uiInputDictionary, self.otherCombats, self.map, self.difficulty, self.damageChart, self.DPSChart, self.NPCDamageChart, self.NPCDPSChart

    def readCombatShallow(self, path, offset=0):
        self.resetParser(offset=offset)
        self.depthAnalysis = False
        self.setPath(path)
        self.readCombat(offset=offset)
