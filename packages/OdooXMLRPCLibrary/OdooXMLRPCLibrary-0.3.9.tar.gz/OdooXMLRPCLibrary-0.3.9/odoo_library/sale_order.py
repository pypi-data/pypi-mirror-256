...
# =================================================================================================================
    def confirm_sale_order(self, data):
        ...
        try:
            ...
            # Fetch sales order data
            sale_order_id = models.execute_kw(database, uid, password, 'sale.order', 'search', [['|', ["id", "=", order_id],["name", "=", order_id]]])

            if not sale_order_id:
                return jsonify({'error': f'Sale order with ID {order_id} not found'}), 404
            confirmRental = models.execute_kw(database, uid, password, 'sale.order', 'action_confirm', [[sale_order_id]])
            response = {
                            "code": 200,
                            "status": "success",
