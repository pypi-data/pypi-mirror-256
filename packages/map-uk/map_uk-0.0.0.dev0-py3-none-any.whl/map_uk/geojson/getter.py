...
from map_uk.utils.file_downloader import FileDownloader
...
class GeoJsonGetter:
    ...
    def get(self) -> dict:
        """Downloads the GeoJSON file if it does not exist locally. If simplification tolerance is provided the file is simplified accordingly.
        ...
        logging.debug("File %s already exists, so skipping download.", self.geojson_path)

        if not self.geojson_simplify_tolerance:
            with Path(self.geojson_path).open("r") as f:
                data = json.load(f)
                for d in data["features"]:
                    d["properties"]["geocode"] = d["properties"].pop("LAD23CD")
                    d["properties"]["ename"] = d["properties"].pop("LAD23NM")
                return data
        else:
    ...
    def _get_simplified_geojson_name(self) -> Path:
        ...
        new_file_name = (
            f"{file_name}-simplified-{self.geojson_simplify_tolerance}{file_ext}"
        )
    ...
    def _simplify(self) -> dict:
        ...
        logging.debug(
            "File %s already exists, so skipping simplification and returning the contents of that file instead.", simplified_json_path
        )
