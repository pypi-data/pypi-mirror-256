...
def create(project: str, environment: str, *resources: List[Resource]):
    ...
    with Progress(
        ...
        task = progress.add_task(
            f"Loading appliction for: [bold yellow]`{project}/{environment}`[/bold yellow]",
        )
        new_resources = []
    ...
    to_run = []
    if new_resources or replace_resources:
        options = []
        all_resources = []
    ...
    asyncio.run(_monitor_create_resource_operations(project, environment, to_run))


def clean(project: str, environment: str, *local_resources: List[Resource]):
    with Progress(
        SpinnerColumn(),
        TextColumn("[progress.description]{task.description}"),
    ) as progress:
        ...
        for remote_resource in remote_resources:
            if remote_resource.name not in keyed_local_resources:
    ...
    if to_delete_options:
        ...
        if not to_delete:
            ...
            return
    asyncio.run(_monitor_delete_resource_operations(project, environment, to_delete))
