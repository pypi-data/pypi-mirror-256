...
from launchflow.cli.templates import launch_template, main_template
...
def init(
    ...
    launch = launch_template.template(project.name, framework, resources)
    requirements = project_gen.requirements(framework, resources)
    main = main_template.template(framework)

    directory = os.path.abspath(directory)
    launch_py = os.path.join(directory, "launch.py")
    ...
    rich.print("  $ [green]launchflow apply launch:dev")
    print()
    print("To run your application run:")
    rich.print("  $ [green]launchflow run launch:dev")
    print()
    print("To deploy your application run:")
    rich.print("  $ [green]launchflow deploy launch:dev")
...
def create(
    ...
    try:
        ...
        with launchflow.allow(connection_failures=True):
            create(project, environment, *import_resources(resources))
