...
from opendapi.config import construct_dapi_source_sink_from_playbooks
...
class DBTColumn:
    ...
    def manifest_is_pii(self) -> bool:
        ...
        return self.manifest_info.get("meta", {}).get("contains_pii", False)
    ...
    def for_dapi(self) -> Dict:
        ...
        return {
            ...
            "description": self.manifest_description or PLACEHOLDER_TEXT,
...
class DbtDapiValidator(DapiValidator):
    """
    ...
    def get_dbt_projects(self) -> Dict[str, DBTProject]:
        ...
        for config_full_path in self.get_all_dbt_config_files():
            ...
            project_full_path = config_full_path.replace(f"/{self.DBT_CONFIG_YML}", "")
            artifacts_dir = f"{project_full_path}/{self.DBT_ARTIFACTS_DIR}"
            manifest_full_path = f"{artifacts_dir}/{self.DBT_MANIFEST_JSON}"
            catalog_full_path = f"{artifacts_dir}/{self.DBT_CATALOG_JSON}"

            config, manifest, catalog = None, None, None
            config = self._read_yaml_or_json(config_full_path)
            if os.path.exists(manifest_full_path):
                manifest = self._read_yaml_or_json(manifest_full_path)
            if os.path.exists(catalog_full_path):
                catalog = self._read_yaml_or_json(catalog_full_path)
    ...
    def selected_projects(self) -> List[DBTProject]:
        ...
        for project_config in config_settings.get("projects", {}).get("overrides", []):
            project_path = os.path.join(self.root_dir, project_config["project_path"])
            ...
            projects[project_path] = DBTProject(
                ...
                manifest=existing_project.manifest,
                catalog=existing_project.catalog,
    ...
    def base_template_for_autoupdate(self) -> Dict[str, Dict]:
        ...
        for table in self.get_dbt_models():
            ...
            result[table.construct_dapi_location()] = {
                ...
                "type": "entity",
                "description": table.manifest_table_description or PLACEHOLDER_TEXT,
                "owner_team_urn": self.get_owner_team_urn_for_table(table),
