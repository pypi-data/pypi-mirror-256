...
class BatchConsumerManager:
    def __init__(
        ...
        **kwargs,
        ...
        if not lst_subscribe_topic or any([not x or not isinstance(x, str) for x in lst_subscribe_topic]):
            raise Exception("lst_subscribe_topic: {} is not valid".format(lst_subscribe_topic))
        ...
        if 'num_messages' in kwargs and isinstance(kwargs.get('num_messages'), int):
            ...
            if not (10 <= num_messages <= 1000):
                raise Exception("num_messages: {} is not valid, num_messages must between 10, 1000 messages".format(num_messages))
        else:
            m_log.warning('set num_messages with default: {}'.format(num_messages))
        if 'consume_timeout' in kwargs and isinstance(kwargs.get('consume_timeout'), Number):
            consume_timeout = float(kwargs.get('consume_timeout'))
            if not (1.0 <= consume_timeout <= 30.0):
                raise Exception("consume_timeout: {} is not valid, consume_timeout must between 1, 30 seconds".format(consume_timeout))
        else:
            m_log.warning('set consume_timeout with default: {}'.format(consume_timeout))
        ...
        try:
            ...
            while True:
                if self.is_maintain():
                    ...
                    continue
                lst_msg = self.c.consume(num_messages=num_messages, timeout=consume_timeout)
    ...
    def manual_commit(self):
        ...
        ) or len(self.lst_processed_msg) >= 200:
    ...
    def process(self, lst_msg):
        try:
            self.message_handle(lst_msg=[json.loads(x.value().decode("utf-8")) for x in lst_msg])
            self.manual_commit()
        except Exception as e:
            m_log.error(
