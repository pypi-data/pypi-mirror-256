...
def gitCommitParse(commits: str) -> str:
    num = False
    inNum = False
    out = ""
    i = 0
    ...
    for v in commits:
        if v.isnumeric():
            ...
            if v == "f":
                num = False
                out = f"{out}{c.Fore.YELLOW}~{numVal}{c.Style.RESET_ALL} "
                f = int(numVal)
            elif v == "i":
                num = False
                out = f"{out}{c.Fore.GREEN}+{numVal}{c.Style.RESET_ALL} "
                i = int(numVal)
            elif v == "d":
                num = False
                out = f"{out}{c.Fore.RED}-{numVal}{c.Style.RESET_ALL} "
                d = int(numVal)
            else:
                inNum = False
