...
class IBLBaggingModel(IBLModel):
    ...
    def predict(self, X: pd.DataFrame, top_model: int = None) -> np.array:
        y_preds = []
        if top_model is None:
            for key in [model[0] for model in self.code_models]:
                self.code_model = self.code_models[key]['code_model']
                y_pred = super().predict(X)
                y_preds.append(y_pred)
        else:
            top_code_models = self.code_models[:top_model]
            for key in [model[0] for model in top_code_models]:
                self.code_model = self.code_models[key]['code_model']
                y_pred = super().predict(X)
                y_preds.append(y_pred)
