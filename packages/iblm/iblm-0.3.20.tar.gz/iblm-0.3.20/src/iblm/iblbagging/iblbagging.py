...
class IBLBaggingModel(IBLModel):
    ...
    def fit(
        ...
        # self.num_model 回の繰り返しで、各モデルに対するサンプリングを行う
        for _ in range(self.num_model):
            random_sample_size = np.random.randint(300, 501)
        ...
        for i, indices in enumerate(index_patterns, start=1):
            ...
            y_sampled = y[indices]
            key = f"model_{i}"
            self.code_model, prompt = super().fit(
                X_sampled,
                y_sampled,
                temperature,
                seed,
            ...
            #self.code_models[key] = {}
            #self.code_models[key]['code_model'] = self.code_model
            print(prompt)
            #print(f'{key}/n', self.code_model)
            y_pred = super().predict(X_sampled)
            metric_dict = super().evaluate(y_sampled, y_pred)
            #self.code_models[key]["metric_dict"] = metric_dict
