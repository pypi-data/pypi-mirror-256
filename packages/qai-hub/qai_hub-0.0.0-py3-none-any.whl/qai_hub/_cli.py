...
from qai_hub.public_rest_api import QAIHUB_CLIENT_ENV, _get_token, get_config_path

from .client import SourceModel

_HUB_URL = "https://app.qaihub.qualcomm.com"
...
def add_job_arguments(
    ...
    if job_type in ["compile", "compile_and_profile"]:
        job_parser.add_argument(
            "--model",
            action="store",
            help="Model ID of source framework model in Hub or path to a source framework model file on disk.",
            required=False,
        )
        job_parser.add_argument(
            "--compile_options",
...
def add_submit_profile_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    examples:
      ...
      {cmd_name} --model 9egnr5ox --device "Samsung Galaxy S23"
      ...
      {cmd_name} --clone np1wlnlg --profile_options " --compute_unit cpu"
    """
    )
...
def add_submit_compile_parser(subparsers: argparse._SubParsersAction):
    ...
    examples:
      ...
      # Re-submit with modifications
      {cmd_name} --clone np1wlnlg --compile_options " --quantize_full type=int8"
    """
    )
...
def parse_model(args: argparse.Namespace) -> hub.Model:
    model = args.model
    ...
    if os.path.isfile(model):
        ...
        if pattern.fullmatch(model) is not None:
            ...
            raise RuntimeError(
                f"Model {model} must be either a valid path to file on disk or a valid model hash on Hub."
            )
