...
def get_starforce_history(
    ...
    # if date is not None and date < STARFORCE_MINIMUM_DATETIME:
    #     raise ValueError("date must be after 2023-12-27 KST.")
    if isinstance(date, datetime):
        dates.is_valid(date, QueryableDate.STARFORCE)

    path = "/maplestory/v1/history/starforce"
    query = {
        "count": result_count,
        "date": dates.to_string(date) if date is not None else None,
        "cursor": page_cursor,
    }
...
def get_cube_usage_history(
    ...
    Args:
        result_count (int): 한번에 가져오려는 결과의 갯수. The number of results to fetch at once.
        date (datetime): 조회 기준일(KST). Reference date for the query (KST).
        page_cursor (str) : 페이징 처리를 위한 cursor. Cursor for paging.
    ...
    # if date is not None and date < CUBE_MINIMUM_DATETIME:
    #     raise ValueError("date must be after 2022-11-25 KST.")
    if isinstance(date, datetime):
        dates.is_valid(date, QueryableDate.CUBE)
...
def get_potential_history(
    ...
    Attributes:
        ...
        date: 조회 기준일 (KST) (cursor가 없는 경우 필수이며 cursor와 함께 사용 불가) e.g. "2024-01-25"
    ...
    # if date is not None and date < POTENTIAL_MINIMUM_DATETIME:
    #     raise ValueError("date must be after 2024-01-25 KST.")
    if isinstance(date, datetime):
        dates.is_valid(date, QueryableDate.POT)
