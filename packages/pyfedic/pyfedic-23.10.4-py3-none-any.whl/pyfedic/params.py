...
class Params(metaclass=ParamsBase):
    ...
    def expand_params(self):
        ...
        for param in [
            'median', 'mean', 'reg_size', 'elt_size',
            'itermax', 'diff_discr_min', 'normed_dU_min',
            'solver', 'reg_kind', 'meshes'
        ]:
        ...
        if self.init is not None:
            if isinstance(self.init, str):
                if '*' in self.init:
                    self.init = sorted(list(self._wd.glob(self.init)))
                    if len(self.init) == len(self.images):
                        self.init = self.init[1:]
                    is_set = self._is_set['sequential']
                    self.sequential = False
                    self._is_set['sequential'] = is_set
                else:
                    m, pd, _ = read_mesh(self._wd / self.init)
                    self.init = m, pd['U'].T
            elif isinstance(self.init[0], str):
                self.init = [self._wd / init for init in self.init]

        new_solver = []
        for solver in getattr(self, 'solver'):
            if solver == 'auto':
                if ndim == 2:
                    new_solver.append('spsolve')
                else:
                    new_solver.append('cgs')
            else:
                new_solver.append(solver)
        self._expanded['solver'] = tuple(new_solver)
