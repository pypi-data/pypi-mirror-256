...
class GenericCell:
    ...
    def dNdv(cls, v, points):
        ...
        """
        result = np.zeros((cls.n_nodes, len(points)), dtype='f8')
        for i, dN in enumerate(cls._dN[v]):
            result[i,:] = dN(*points.T)
        return result
...
class Cell(type):
    ...
    def __new__(cls, name, bases, classdict):
        ...
        if result.order == 1:
            result._N_data = np.zeros(((result.n_nodes,) + (2,) * result.ndim))
            for i, shape in enumerate(result.shapes):
                for k, v in shape.as_poly(result.symbols).as_dict().items():
                    result._N_data[(i,) + k] = v

            result._N_data = result._N_data.reshape((result.n_nodes, 2**result.ndim))

            result._dN_data = np.zeros(((result.ndim, result.n_nodes) + (2,) * result.ndim))
            ...
            result._dN_data = result._dN_data.reshape((result.ndim, result.n_nodes, 2**result.ndim))
        ...
        result.is_linear = result.n_nodes == result.ndim
...
class T4Q(GenericCell, metaclass=Cell):
    ...
    gauss_reduced = T4.gauss_reduced

    gauss = T4.gauss

    vertices = T4.vertices
...
class T3Q(GenericCell, metaclass=Cell):
    ...
    gauss_reduced = T3.gauss_reduced

    gauss = T3.gauss

    vertices = T3.vertices
