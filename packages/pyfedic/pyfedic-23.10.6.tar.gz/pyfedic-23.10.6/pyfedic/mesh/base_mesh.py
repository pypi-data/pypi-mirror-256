...
from ..cells import Cell, Q4, C8, P6, T3
...
class BaseMesh:
    ...
    def new(cls, nodes, cells_by_type):
        ...
        if len(cells_by_type) == 1:
            ...
            if cell_type not in [Q4, C8]:
                return mesh
            p = [np.diff(np.unique(mesh.nodes[:,x])) for x in range(3)]
            elt_size = np.unique(np.hstack(p))
            if elt_size.size == 1 and elt_size[0] % 1 == 0:
                elt_size = int(elt_size[0])
                nelems = tuple(len(x) for x in p[:mesh.ndim][::-1])
                x0, y0, z0 = mesh.nodes[0]
                xlims = int(x0+0.5), int(x0+0.5)+nelems[-1]*elt_size
                ylims = int(y0+0.5), int(y0+0.5)+nelems[-2]*elt_size
                if mesh.ndim == 2:
                    mesh = RegularMesh(xlims, ylims, elt_size=elt_size)
                else:
                    zlims = int(z0+0.5), int(z0+0.5)+nelems[-3]*elt_size
                    mesh = RegularMesh(xlims, ylims, zlims, elt_size=elt_size)
            return mesh
    ...
    def compute_nbh(self, order=1, use_cells=False, only_last=False):
        ...
        nbh0 = sparse.csr_array((adj.indices+1, indices, np.cumsum(np.hstack((0, nb)))), shape=(self.Nn, nb.max()), dtype='i8')
        ...
        while order > current_order:
            ...
            adj += sparse.coo_array(
                (
                    ...
                    (
                        np.hstack((adj2_col.astype('i8'), adj2_row.astype('i8'))),
                        np.hstack((adj2_row.astype('i8'), adj2_col.astype('i8'))),
                    )
                ), shape=(self.Nn,self.Nn)).tocsr()
            ...
            nbh = sparse.csr_array((adj.indices+1, indices, np.cumsum(np.hstack((0, nb)))), shape=(self.Nn, nb.max()), dtype='i8')
    ...
    def extrap_V(self, V, method='mean', inplace=False):
        ...
        while True:
            ...
            p = np.diff(nbh.indptr) - np.array(sparse.csr_array((s[nbh.data], nbh.indices, nbh.indptr), nbh.shape).sum(axis=1).T)[0]
            V[s] = 0

            V = np.vstack([sparse.csr_array(
            ...
            sel = sparse.coo_array(
                (
                    ...
                    (
                        ...
                        np.hstack((np.floor((n-1)/2),np.ceil((n-1)/2))).astype('i')
                    )
                ),
