...
def memoize(func):
    ...
    For efficiency purposes, the wrapper function only takes one argument,
    and that argument is an integer.
    :param func: The decorated function.
    :return: Inner function.
    """
    cache = {}
...
def read_data(filename: str = "data.txt", *, sep: str = "\n", sep2: str = None, _class: type = str, auto_annotate: bool = False):
    """
    Read data from a file.

    The data will be read from the file with the given filename (default: "data.txt").
    The data will be split by the given separator (default: "\n").
    ...
    This decorator will also add/edit the decorated function's first parameter's type hint according to the given type.
    ...
    :param auto_annotate: Whether to automatically annotate the decorated function's first parameter (dangerous!).
    :return: Inner function.
    """

    def decorator(func):
        if auto_annotate:
            processed_type = list[_class]
            if sep2 is not None:
                processed_type = list[processed_type]
    ...
    # If the decorator is used with no arguments, e.g. @read_data.
    ...
    def verify_type(err_message, expected_type, value, parameter_name=None):
        ...
        if not isinstance(value, expected_type):
            if isinstance(expected_type, types.UnionType):
                exp_types = tuple(f"{t.__name__}" for t in expected_type.__args__)
