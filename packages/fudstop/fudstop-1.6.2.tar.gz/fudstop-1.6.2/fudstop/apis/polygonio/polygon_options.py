...
class PolygonOptions:
    def __init__(self,user:str='chuck', database:str='markets', host:str='localhost', port:int=5432, password:str='fud'):
        ...
        self.session = None
        self.db = PolygonDatabase(user='chuck', database='markets', host='localhost', port=5432, password='fud')
...
# Asynchronous method to create an aiohttp session
    ...
    async def fetch_page(self, url):
        # This is just a placeholder function. You need to define how to fetch each page.
...
# Example usage
    ...
    async def option_trades(self, ticker:str,as_dataframe:bool=False):
        ...
        try:
            session = await self.fetch_page(url)
            
            data = await self.paginate_concurrent(url,session, as_dataframe=True)
            if data is not None:
                return data
        except Exception as e:
            print(f'Error')
    ...
    async def gpt_filter(self, ticker:str=None, strike:float=None, strike_min:float=None, strike_max:float=None, cp:str=None, expiry:str=None, expiry_min:str=None, expiry_max:str=None, dte_max:int=None, dte_min:int=None, 
        ...
        return chunks
    async def multi_snapshot(self, symbol_string:str):
