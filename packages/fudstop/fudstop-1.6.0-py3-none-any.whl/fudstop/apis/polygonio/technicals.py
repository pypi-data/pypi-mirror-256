...
from polygonio.models.option_models.universal_snapshot import OptionData
...
class Technicals:
    def __init__(self):
        ...
        self.db = PolygonDatabase(host='localhost', user='chuck', port=5432, password='fud', database='markets')
        self.macd_webhooks =  {
    ...
    async def run_technical_scanner(self, ticker, timespan):
        await self.poly.create_session()
        async with self.sema:
            ...
            if hasattr(macd_data, 'macd_value'):
                ...
                if 'No' not in macd_crossover and timespan in ['week', 'day', 'month']:
                    ...
                    asyncio.create_task(self.db.batch_insert_dataframe(df, table_name='feeds', unique_columns='ticker, status, description'))
