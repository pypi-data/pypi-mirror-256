...
@dataclass
class InstanceAccessProfile:
    id: str
    object_type: str
    who_has_access: str
    link_access: str
    owner_id: str
    definition_editors: list[str]
    definition_viewers: list[str]
    product_user: list[str]
    payment_option: str
    ...
    def is_explicit_product_user(self, user_id: str | None) -> bool:
        return user_id in self.product_user
    ...
    @classmethod
    def create(cls, id: str, object_type: str, owner_id: str) -> 'InstanceAccessProfile':
        def create_instance_access_profile() -> 'InstanceAccessProfile':
            api_response: Response = signed_requests.request(
                ...
                json={"id": id, "object_type": object_type, "owner_id": owner_id}
            )
            if api_response.status_code == 201:
                return cls(**api_response.json())
            raise Exception(f"Error creating instance access profile: {api_response.text}")
    ...
    def retrieve(cls, id: str) -> 'InstanceAccessProfile':
        def get_instance_access_profile() -> 'InstanceAccessProfile':
            ...
            if api_response.status_code == 200:
                return cls(**api_response.json())
            raise Exception(f"Error retrieving instance access profile: {api_response.text}")
        return INSTANCE_ACCESS_PROFILE_CACHE.get(
