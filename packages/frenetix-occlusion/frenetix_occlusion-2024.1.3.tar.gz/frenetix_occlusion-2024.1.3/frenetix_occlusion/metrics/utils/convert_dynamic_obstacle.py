...
def convert_prediction_to_dyn_obstacle(agent, key, prediction) -> DynamicObstacle:
    ...
    dynamic_obstacle = DynamicObstacle(key, agent.obstacle_type, shape, initial_state, prediction)
...
def convert_traj_to_dyn_obstacle(trajectory, vehicle_params, obstacle_id: int = 42) -> DynamicObstacle:
    ...
    state_list = [CustomState(time_step=int(time_step),  # Convert numpy int64 to Python int
                  ...
                  for time_step, position, orientation, velocity, acceleration in
                  zip(time_steps, positions, orientations, velocities, acceleration)]
    ...
    # convert prediction to dynamic obstacle
    dynamic_obstacle = DynamicObstacle(obstacle_id, ObstacleType.CAR, shape, initial_state, prediction)
