...
rule download_candidate_species:
    ...
    output:
        ...
        interest_fna = temp(os.path.join(
            config["params"]["align"]["download_dir"],
            f"library/{PROJECT}.fna")),
        acc2tax = os.path.join(
           config["params"]["align"]["download_dir"],
            f"acc2tax/{PROJECT}_acc2tax.txt"),                                  
    params:
...
if config["params"]["align"]["bowtie2"]["do"]:
    rule bowtie2_build_Index:
        output:
            expand(
                f"{config['params']['align']['bowtie2']['db']}/index/{PROJECT}/{PROJECT}{{ext}}",
                ext=[
                    ".1.bt2",
                    ".2.bt2",
                    ".3.bt2",
    ...
    rule bowtie2_lca_classifier:
        input:
            ...
            taxonomy_db = os.path.join(
                                        config["params"]["align"]["download_dir"],
                                        "ncbi.pkl"),
            acc2tax_db = os.path.join(config["params"]["align"]["download_dir"],f"{PROJECT}.db","IDENTITY"),
            interest_fna = temp(os.path.join(
            config["params"]["align"]["download_dir"],
            f"library/{PROJECT}.fna")),
        output:
...
if config["params"]["align"]["bwa"]["do"]:
    rule bwa_build_Index:
        output:
            expand(
                f"{config['params']['align']['bwa']['db']}/index/{PROJECT}/{PROJECT}{{ext}}",
                ext=[
                    ".amb",
                    ".ann",
                    ".bwt",
...
if config["params"]["align"]["bwa2"]["do"]:
    rule bwa2_build_Index:
        output:
            expand(
                f"{config['params']['align']['bwa2']['db']}/index/{PROJECT}/{PROJECT}{{ext}}",
                ext=[
                    ".amb",
                    ".ann",
                    # ".pac",
        ...
        benchmark:
            os.path.join(config["benchmarks"]["classifier"],
                        "bwa2/bulid_database_benchmark.tsv")
        resources:
            mem_mb=(
                lambda wildcards: os.stat(os.path.join(config["params"]["align"]["download_dir"],
                f"library/{PROJECT}.fna")).st_size /1024 /1024 * 5
            ),
        threads: 20
        log:
            os.path.join(config["logs"]["classifier"],
                        "bwa2_db/bwa2_db_build.log")
        shell:
            """
                if [ -s "{input.interest_fna}" ]; then
                    bwa-mem2 index {input.interest_fna} -p {params.bwa2_db_name}
                else
                    echo "Error: {input.interest_fna} is empty or does not exist."
                fi
            """
    rule bwa2_alignment:
        output:
        ...
        input:
            krak2_extracted_bam = os.path.join(
                config["output"]["classifier"],
                "rmhost_extracted_classified_output/{sample}/{sample}_kraken2_extracted_classified.bam"),
            db=rules.bwa2_build_Index.output,
            krak_screened_fastq = os.path.join(
                config["output"]["classifier"],
                "rmhost_extracted_classified_output/{sample}/{sample}_kraken2_screen.fastq"),
            krak_screened_r1_fastq = os.path.join(
        ...
        resources:
            mem_mb=(
                lambda wildcards: os.stat(os.path.join(config["params"]["align"]["download_dir"],
                f"library/{PROJECT}.fna")).st_size  /1024 /1024  * 8
            ),
        threads: 20
        log:
            os.path.join(config["logs"]["classifier"],
        ...
        shell:
            """
            if [ -s "{input.krak_screened_fastq}" ]; then
                bwa-mem2 mem -a -t {threads} {params.bwa2_db_name} {input.krak_screened_fastq} | samtools sort --threads {threads} -o {output.bwa_aligned_sorted_bam}
            else
                bwa-mem2 mem -a -t {threads} {params.bwa2_db_name} -p {input.krak_screened_r1_fastq} {input.krak_screened_r2_fastq} | samtools sort --threads {threads} -o {output.bwa_aligned_sorted_bam}
...
if config["params"]["align"]["minimap2"]["do"]:
    ...
    rule minimap2_alignment:
        ...
        shell:
            """
            if [ -s "{input.krak_screened_fastq}" ]; then
                minimap2 -ax sr -A2 -B1 -O4,24 -E2,1 -k10 -w8 -t {threads} {input.minimap2_db} {input.krak_screened_fastq} | samtools sort > {output.minimap2_aligned_sorted_bam}
            else
                minimap2 -ax sr -A2 -B1 -O4,24 -E2,1 -k10 -w8 -t {threads} {input.minimap2_db} -p {input.krak_screened_r1_fastq} {input.krak_screened_r2_fastq} | samtools sort > {output.minimap2_aligned_sorted_bam}
    ...
    rule minimap2_lca_classifier:
        ...
        shell:
            """
            cd {params.minimap2_output}

            sam2lca --dbdir {params.sam2lca_db_name} analyze -b -i 0.8 -a {params.project} ./Aligned.minimap2.sortedByCoord.out.bam
            """
    rule minimap2_aligned_all:
        input:
            expand(os.path.join(
                config["output"]["classifier"],"minimap2_align/{sample}/Aligned.minimap2.sortedByCoord.out.sam2lca.bam"),sample=SAMPLES_ID_LIST)
