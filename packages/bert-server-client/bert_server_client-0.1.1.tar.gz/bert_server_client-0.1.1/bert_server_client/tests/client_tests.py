...
from bert_server_client.schema.embedding import EmbeddingRequest, Embedding, EmbeddingData, EmbeddingUsage
...
def test_send_request_valid(bert_client, mock_zmq_context):
    ...
    embedding_data = [EmbeddingData(object='embedding', index=0, embedding=[0.1, 0.2, 0.3])]
    embedding_usage = EmbeddingUsage(prompt_tokens=5, total_tokens=10)
    embedding = Embedding(object='response', data=embedding_data, model='test-model', usage=embedding_usage)
