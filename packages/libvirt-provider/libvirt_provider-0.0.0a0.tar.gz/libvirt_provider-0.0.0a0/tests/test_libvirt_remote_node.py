...
class TestLibvirtRemote(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        username = os.environ.get("LIBVIRT_REMOTE_USERNAME", "rasmunk")
        password = os.environ.get("LIBVIRT_REMOTE_PASSWORD", "")
        private_key_file = os.environ.get(
            "LIBVIRT_REMOTE_PRIVATE_KEY_FILE",
            "/home/rasmunk/.ssh/id_ed25519",
        )
        public_key_file = os.environ.get(
            "LIBVIRT_REMOTE_PUBLIC_KEY_FILE",
            "/home/rasmunk/.ssh/id_ed25519.pub",
        )
        if not public_key_file and private_key_file:
            public_key_file = f"{private_key_file}.pub"
        hostname = os.environ.get("LIBVIRT_REMOTE_HOSTNAME", "fire-login0.cluster")
        port = os.environ.get("LIBVIRT_REMOTE_PORT", 22)
