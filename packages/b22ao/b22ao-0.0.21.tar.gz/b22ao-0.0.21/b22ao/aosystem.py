...
class AreaDetector:
    ...
    def __init__(self, pv_base):
        ...
        self.state_pv = pv_base + AD_STATE_PV
    ...
    def acquire(self, attempt=0):
        ...
        while True:
            ...
            self.request_frame()
            if caget(self.state_pv, as_string=True) == "Error":
                logger.error("Detector IOC in error state after acquisition. Retrying")
                caput(self.acquire_pv ,"Done") # 'stop' button
                return self.acquire(attempt+1)
