...
class ExecutionReport(BaseModel):
    testMode: StrictBool
    status: str = "FAILURE"
    customOutput: Optional[CustomModelOutput]
...
class VectorDataParameter(BaseModel):
    type: StrictStr
    sourceCodeName: StrictStr
    value: Optional[VectorDataValue]
...
class BatchVectorDataValue(BaseModel):
    ...
    indexUnit: DataUnit
    aggregationFunction: Optional[StrictStr]
    indexAggregationFunction: Optional[StrictStr]
    batchType: BatchType
...
class StoredBatchVectorDataParameter(BaseModel):
    type: StrictStr
    sourceCodeName: StrictStr
    value: Optional[BatchVectorDataValue]
...
class TimeDataParametersList(BaseModel):
    ...
    @property
    def readable_value(self) -> Optional[List[StrictStr]]:
        return [value.reference for value in self.value.dataList]

    @property
    def technical_value(self) -> Optional[List[StrictStr]]:
...
class ExecutionContext(BaseModel):
    type: StrictStr
    initiator: StrictStr
    lastSuccessfulExecutionInstant: Optional[datetime]
    currentExecutionInstant: datetime
    customData: Optional[Any]
    uploadEventId: Optional[StrictStr]
