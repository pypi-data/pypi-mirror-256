...
"""
    ...
    def create_endpoint(
        ...
        if "POST" in methods:
            ...
            if any(
                issubclass(param.annotation, BaseModel) for param in params.values()
            ):
                model_type = next(
                    (
                        param.annotation
                ...
                async def wrapper(body: model_type = Body(...)):
                ...
                async def wrapper(body: request_model = Body(...)):
                    # Convert Pydantic model to dict and unpack as function arguments
                    kwargs = body.dict()
                    return await func(**kwargs)

            self.app.post(endpoint_route, response_model=BaseModel)(wrapper)
