...
"""
    ...
    def create_endpoint(
        ...
        if "POST" in methods:
            ...
            async def post_wrapper(body: request_model = Body(...)) -> T:
                return await func(**body.dict())

            self.app.post(endpoint_route, response_model=BaseModel)(post_wrapper)

        if "GET" in methods:

            async def get_wrapper(**kwargs: Any) -> T:
                return await func(**kwargs)

            get_wrapper.__annotations__ = {}
            for name, param in inspect.signature(func).parameters.items():
                if param.annotation != inspect.Parameter.empty:
                    # For each parameter, create a dependency that extracts the query parameter
                    dependency = Depends(
                        Query(
                            default=(
                    ...
                    # Use the dependency as an annotation for FastAPI to interpret
                    get_wrapper.__annotations__[name] = (param.annotation, dependency)

            # Register the dynamically annotated function with FastAPI
            self.app.get(endpoint_route, response_model=BaseModel)(get_wrapper)
