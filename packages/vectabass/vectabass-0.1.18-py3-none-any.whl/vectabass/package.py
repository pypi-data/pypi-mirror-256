...
"""
    ...
    def create_endpoint(
        ...
        if any(issubclass(param.annotation, BaseModel) for param in params.values()):
            ...
            async def model_wrapper(*args, body: model_type = Body(...), **kwargs):
                return await func(body, *args, **kwargs)
            ...
            async def model_wrapper(*args, body: RequestModel = Body(...), **kwargs):
                # Convert Pydantic model to dict and unpack as function arguments
                model_dict = body.dict()
                return await func(*args, **model_dict, **kwargs)

        @wraps(func)
        async def wrapped_func(*args, **kwargs):
            return await model_wrapper(*args, **kwargs)

        # Register the endpoint with FastAPI
        for method in methods:
            route_decorator = getattr(self.app, method.lower())
            route_decorator(endpoint_route, response_model=BaseModel)(wrapped_func)
