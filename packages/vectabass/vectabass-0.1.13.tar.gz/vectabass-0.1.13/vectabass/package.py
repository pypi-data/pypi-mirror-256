...
"""
    ...
    def create_endpoint(
        self, func: Callable, methods: List[str] = ["POST"], route: str = None
    ):
        # Extract function parameters to create a Pydantic model with descriptions
        ...
        async def wrapper(model: request_model = Depends()):
            # Convert Pydantic model to dict and unpack as function arguments
            result = func(**model.dict())
            return GenericResponseModel(data=result, message="Success")

        # Register the wrapper function with FastAPI
