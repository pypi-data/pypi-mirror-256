...
class CloudDelivery:
    ...
    def image(img_path):
        if CloudDelivery.is_running_in_cloud():
            # Determine the file type based on the image file extension
            file_extension = img_path.split(".")[-1]
            # Defaulting to dynamic and public for CDN purposes
            dynamic = True
            make_public = True

            # Adjust the file_type based on common image file extensions
            if file_extension.lower() in ["jpg", "jpeg"]:
                file_type = "jpg"
            elif file_extension.lower() == "png":
                file_type = "png"
            else:
                # If you support more types, add them here. Otherwise, default to jpg.
                file_type = "jpg"

            url = CloudDelivery.upload_file_to_gcp(
                img_path, file_type, dynamic, make_public
            )
            return url
        else:
    ...
    def download_file_from_gcp(
        ...
        """
        Downloads a file from GCP Cloud Storage.

        :param blob_name: The name of the blob in the GCP bucket.
        :param destination_file_path: Local path to save the downloaded file.
        :param bucket_name: The name of the GCP bucket.
        """
        try:
            # Assumes you have set GOOGLE_APPLICATION_CREDENTIALS for your GCP service account
            client = storage.Client()
            bucket = client.bucket(bucket_name)
            blob = bucket.blob(blob_name)
            blob.download_to_filename(destination_file_path)
            logging.info(f"Downloaded {blob_name} to {destination_file_path}")
        except Exception as e:
    ...
    def upload_file_to_gcp(
        file_path: str, file_type: str, dynamic: bool = False, make_public: bool = False
    ) -> str:
        """
    ...
    @staticmethod
    def download_file_from_gcp(file_identifier: str, local_save_path: str) -> bool:
        """
        Requests the Intermediary Server to download a file from GCP and saves it locally.

        :param file_identifier: A unique identifier for the file in GCP, could be the file name or path.
        :param local_save_path: Local path where the file should be saved.
        ...
        response = requests.get(download_endpoint, params=payload)

        if response.status_code == 200:
            # Assuming the file is sent back as a base64 encoded string in the response
            file_data = base64.b64decode(response.json().get("file_data"))
