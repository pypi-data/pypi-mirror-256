...
"""
    ...
    def endpoint(self, methods: List[str] = ["GET"]):
        def decorator(func: Callable[..., T]) -> Callable[..., T]:
            model = self.create_model_for_func(func)

            if "POST" in methods:

                async def endpoint_wrapper(body: model = Body(...)):
                    return func(**body.dict())

                self.app.post(f"/{func.__name__}", response_model=model)(
                    endpoint_wrapper
                )
            ...
            if "GET" in methods:
                ...
                self.app.get(f"/{func.__name__}", response_model=model)(
                    endpoint_wrapper
                )
    ...
    def create_model_for_func(self, func: Callable[..., T]) -> Type[BaseModel]:
        params = inspect.signature(func).parameters
        annotations = {name: (param.annotation, ...) for name, param in params.items()}
        return create_model(f"{func.__name__}Model", **annotations)
