...
"""
    ...
    def create_endpoint(
        self, func: Callable, methods: List[str] = ["GET"], route: str = None
    ):
        ...
        for name, param in params.items():
            ...
            # Check if the annotation is a list for query parameters
            if param.annotation == List[str]:
                # Use FastAPI's Query to handle list parameters
                default_value = (
                    Query(default=[])
                    if param.default is inspect.Parameter.empty
                    else Query(default=param.default)
                )
                fields[name] = (param.annotation, default_value)
            else:
                default_value = (
                    Field(default=...)
                    if param.default is inspect.Parameter.empty
        ...
        async def wrapper(*args, **kwargs):
            ...
            if "GET" in methods:
                # For GET, extract parameters from query
                request_params = {k: v for k, v in kwargs.items() if v is not None}
            else:
                # For POST, parse the body to model and then to dict
                model_instance = request_model(**kwargs)
                request_params = model_instance.dict()

            try:
                # Execute the original function
                result = await func(**request_params)
        ...
        # Register the endpoint with FastAPI using the appropriate method
        endpoint_route = route or f"/{func.__name__}"
        for method in methods:
            operation_id = f"{func.__name__}_{method.lower()}"
            summary = f"Execute the '{func.__name__}' function"
    ...
    def endpoint(self, methods: List[str] = ["GET"], route: str = None):
        def decorator(func: Callable):
            @wraps(func)
            async def wrapper(*args, **kwargs):
                return await func(*args, **kwargs)

            route_path = route or f"/{func.__name__}"
            for method in methods:
                if method.upper() == "GET":
                    self._register_get_endpoint(route_path, func, wrapper)
                elif method.upper() == "POST":
                    self._register_post_endpoint(route_path, func, wrapper)

            return wrapper

        return decorator

    def _register_get_endpoint(
        self, route_path: str, func: Callable, wrapper: Callable
    ):
        # Extract query parameters directly from function signature
        ...
        async def get_wrapper(*args, **kwargs):
            # Convert query parameters to function arguments
            func_params = {
        ...
        # Use FastAPI to add a GET endpoint with the extracted parameters
        self.app.get(route_path)(get_wrapper)

    def _register_post_endpoint(
        self, route_path: str, func: Callable, wrapper: Callable
    ):
        # Use FastAPI to add a POST endpoint, expecting a JSON body
        self.app.post(route_path)(wrapper)
