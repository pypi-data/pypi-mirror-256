...
class APIWrapper:
    def __init__(self, api_key: str, upload_url: str):
        self.app = FastAPI()
    ...
    def create_endpoint(
        ...
        if "POST" in methods:
            request_model = create_model(
                f"{func.__name__}RequestModel",
                **{name: (param.annotation, ...) for name, param in params.items()},
            )

            @self.app.post(endpoint_route)
            async def post_wrapper(
                *args: Any, model: request_model = Depends(), **kwargs: Any
            ) -> T:
                return await func(*args, **model.dict(), **kwargs)
