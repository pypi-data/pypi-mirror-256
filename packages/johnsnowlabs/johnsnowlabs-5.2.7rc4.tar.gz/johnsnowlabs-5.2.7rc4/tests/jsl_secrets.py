...
class JslSecretsTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.valid_healthcare_secret = (
            settings.raw_version_secret_medical + "-healthcare-secret"
        )
    ...
    @patch("johnsnowlabs.py_models.jsl_secrets.get_secrets")
    def test_should_return_existing_secret_if_no_matching_secret_found(
        self, mock_get_secrets
    ):
        existing_secrets = JslSecrets(
            HC_LICENSE=self.license,
            OCR_SECRET="ocr-invalid-secret",
            HC_SECRET="healthcare-invalid-secret",
        )
        mock_get_secrets.return_value = [
            LibSecretResponse(
                product="Healthcare NLP",
