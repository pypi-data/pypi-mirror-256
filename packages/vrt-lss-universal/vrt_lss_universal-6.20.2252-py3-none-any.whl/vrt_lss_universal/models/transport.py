...
class Transport(BaseModel):
    """
    Transport. Used by performer for relocating and cargo transfer.   # noqa: E501
    """
    ...
    boxes: Optional[Annotated[List[Box], Field(min_length=0, max_length=100)]] = Field(default=None, description="A list of transport compartments that can accommodate the cargo.")
    ...
    __properties = ["key", "shifts", "transport_type", "boxes", "compatibilities", "limits", "attributes"]
    ...
    def from_dict(cls, obj: dict) -> Transport:
        ...
        _obj = Transport.parse_obj({
            "key": obj.get("key"),
            "shifts": [TransportShift.from_dict(_item) for _item in obj.get("shifts")] if obj.get("shifts") is not None else None,
            "transport_type": obj.get("transport_type"),
            "boxes": [Box.from_dict(_item) for _item in obj.get("boxes")] if obj.get("boxes") is not None else None,
            "compatibilities": TransportCompatibilities.from_dict(obj.get("compatibilities")) if obj.get("compatibilities") is not None else None,
            "limits": TransportLimits.from_dict(obj.get("limits")) if obj.get("limits") is not None else None,
