...
class ActualizeTask(BaseModel):
    """
    Task for the trips actualization.   # noqa: E501
    """
    ...
    orders: Annotated[List[Order], Field(min_length=1, max_length=15001)] = Field(description="Orders list.")
    performers: Annotated[List[Performer], Field(min_length=1, max_length=15001)] = Field(description="Available performers list.")
    ...
    trips: Annotated[List[Trip], Field(min_length=0, max_length=15001)] = Field(description="Existing routes.")
    ...
    __properties = ["locations", "orders", "performers", "transports", "hardlinks", "trips", "facts", "external_routing", "actualize_settings", "dataset_name"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ActualizeTask:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ActualizeTask:
        """Create an instance of ActualizeTask from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ActualizeTask.parse_obj(obj)

        _obj = ActualizeTask.parse_obj({
            ...
            "facts": Facts.from_dict(obj.get("facts")) if obj.get("facts") is not None else None,
