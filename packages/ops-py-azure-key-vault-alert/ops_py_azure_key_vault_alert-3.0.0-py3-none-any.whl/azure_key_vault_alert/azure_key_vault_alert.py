...
def slack_alert(title, report, slack_app=False):
    ...
    if slack_app:
        ...
        if len(str(payload)) > max_chars_app:
            title = f"{title} - TRUNCATED"
            report = f"{report[:max_chars_app]}\n...\nTRUNCATED"
            payload = {"text": f"*{title}*\n```{report}```"}
        a.set_payload(payload)
    else:
        if len(report) > max_chars_workflow:
            warning_msg = f"The {title} length is above the character limit count of {max_chars_workflow}"
            logging.warning(warning_msg)
...
def main():
    ...
    if teams_output:
        ...
        if stdout_only:
            ...
            success = slack_alert(title, report, slack_app=slack_app)
