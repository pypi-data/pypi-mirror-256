...
from credici.async_.user_cycle import AsyncUserCycle
...
class AsyncCrediciClient:
    def __init__(self, storage: AsyncStorage):
        ...
        self.user_balance = AsyncUserCycle(storage=self.storage)
        self.ledger = AsyncLedger(storage=self.storage)

    async def add(self, user_id: UUID, amount: int, expire_at: datetime = None) -> None:
        """Add credits to a user asynchronously.

        Args:
            user_id (UUID): The unique identifier of the user.
            amount (int): The absolute amount of credits to add.
            expire_at (datetime): The time at which the credits will expire.
        """
        await self.ledger.add(user_id, amount, expire_at)
    ...
    async def get_balance(self, user_id: UUID) -> AsyncUserCycle:
    ...
    async def start_cycle(
        ...
        """
        await self.user_balance.start_cycle(amount, time_delta, user_id)
