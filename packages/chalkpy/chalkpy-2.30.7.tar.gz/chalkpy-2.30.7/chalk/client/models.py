...
class ChalkError(BaseModel, frozen=True):
    ...
    def is_resolver_runtime_error(self) -> bool:
        ...
        """
        return self.code in [ErrorCode.RESOLVER_FAILED, ErrorCode.RESOLVER_TIMED_OUT]
...
class CreateOfflineQueryJobRequest(BaseModel):
    ...
    input: Union[OfflineQueryInput, Tuple[OfflineQueryInput, ...], None] = Field(default=None, description="Any givens")
    ...
    def _validate_multiple_computers(cls, values: Dict[str, Any]):
        if values["input"] is not None and values["use_multiple_computers"] != isinstance(values["input"], tuple):
            raise ValueError("input should be tuple exactly when use_multiple_computers is True")
        return values
