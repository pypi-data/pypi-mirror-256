...
from typing import TYPE_CHECKING, Any, Callable, Deque, Dict, List, Mapping, Optional, Sequence, Union, cast
...
def _safe_add_metrics(metrics: Mapping[str, Union[int, float]]):
    try:
        ...
        if span:
            span.set_metrics(dict(metrics))
    except ImportError:
        ...


def _safe_add_tags(tags: Mapping[str, str]):
    try:
        import ddtrace
    ...
    def _get_result_csv_buffer(
        ...
        with _safe_trace("_get_result_csv_buffer"):
            ...
            if (
                ...
                with _safe_trace("skip_datetime_timezone_cast"):
            ...
            with self.get_engine().connect() if connection is None else contextlib.nullcontext(connection) as cnx:
                ...
                with dbapi.cursor() as cursor:
                    ...
                    with _safe_trace("copy_expert"):
                        _safe_add_tags({"query": formatted_stmt.as_string(cursor)[0:2000]})
    ...
    def execute_query_efficient(
        ...
        with _safe_trace("postgres.execute_query_efficient"):
            ...
            if env_var_bool("CHALK_FORCE_SQLALCHEMY_QUERY_EXECUTION_WITHOUT_EXCEPTION"):
                """Leaving this is a hard fallback"""
            ...
            if query_execution_parameters.postgres.polars_read_csv or env_var_bool("CHALK_FORCE_POLARS_READ_CSV"):
                _logger.debug("postgres: doing force polars CSV reader")
                try:
                    return self._get_parsed_table_via_polars(
                        finalized_query, connection, columns_to_converters, query_execution_parameters
                    )
                except Exception as e:
                    _logger.error("Failed to parse CSV with Polars, falling back to PyArrow CSV reader", exc_info=e)

            _logger.debug("postgres: doing PyArrow CSV reader")
            buffer = self._get_result_csv_buffer(
                finalized_query,
                connection=connection,
                escape_char="\\",
            ...
            with _safe_trace("non_polars_csv_parse"):
                ...
                if query_execution_parameters.postgres.csv_read_then_cast or env_var_bool("CSV_READ_THEN_CAST"):
