...
from chalk.utils.df_utils import pa_array_to_pl_series, pa_cast, recursive_convert_map_primitive
    ...
    def __getitem__(cls, item: Any) -> Type[DataFrame]:
        ...
        for a in item:
            ...
            if isinstance(a, Filter):
                ...
                new_columns.append(unwrap_feature(a))
...
class DataFrame(metaclass=DataFrameMeta):
    ...
    def __getitem__(self, item: Any) -> Union[Features, DataFrame]:
        ...
        item = tuple(self._parse_underscore(exp=x) if isinstance(x, Underscore) else x for x in ensure_tuple(item))
    ...
    def all(self) -> bool:
        ...
        if any(
            (not isinstance(x, type) or not issubclass(x, pl.Boolean)) and x != pl.Boolean
