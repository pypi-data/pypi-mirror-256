        ...
        if short_name in self._short_name_to_resolver:
            if not override and not notebook.is_notebook():
                if resolver.fqn == self._short_name_to_resolver[short_name]:
                    ...
                    resolver.lsp_builder.add_diagnostic(
                        message=f"Another resolver with the same function name '{resolver.name}' exists. Resolver function names must be unique. Please rename this resolver.",
...
class Resolver(ResolverProtocol[P, T], abc.ABC):
    def __init__(
        ...
        self.name = fqn.split(".")[-1] if name is None else name
        super().__init__()
...
def sink(
    ...
    *,
    ...
    def decorator(fn: Callable[P, T]):
        ...
        error_builder = get_resolver_error_builder(fn)
        parsed = parse_sink_function(fn, caller_globals, caller_locals, error_builder)
...
def parse_and_register_stream_resolver(
    *,
    ...
    if keys is not None:
        keys = _validate_keys(stream_fn=fn, keys=keys, params=params, error_builder=error_builder)
    ...
    parsed = parse_function(
        fn, caller_globals, caller_locals, error_builder, allow_custom_args=True, is_streaming_resolver=True
    )

    if timestamp:
        _validate_timestamp(stream_fn=fn, timestamp=timestamp, params=params, error_builder=error_builder)
