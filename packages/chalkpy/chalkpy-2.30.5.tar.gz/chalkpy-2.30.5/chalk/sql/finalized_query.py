...
class FinalizedChalkQuery:
    ...
    def __init__(
        ...
        temp_tables: Mapping[
            str, Tuple[Mapping[str, sqlalchemy.types.TypeEngine], pa.Table, CreateTable, Table, DropTable]
        ]
        | None = None,
        is_empty: bool = False,
    ) -> None:
    ...
    async def async_execute(
        ...
        pa_table = await self.async_execute_to_pyarrow(
            expected_features,
            connection,
    ...
    def execute_to_pyarrow(
        ...
        pa_table = self.source.execute_query(
            finalized_query=self,
            columns_to_converters=col_to_converters,
    ...
    async def async_execute_to_pyarrow(
        ...
        pa_table = await self.source.async_execute_query(
            self, col_to_converters, connection, query_execution_parameters
        )
    ...
    def _postprocess_table(self, pa_table: pa.Table, expected_features: Optional[Sequence[Feature]]):
        ...
        if len(pa_table.column_names) == 0:
            if expected_features is None:
                raise ValueError("Received empty result (no columns) and unable to infer schema.")
            pa_table = pa.Table.from_pydict(
                {f.root_fqn: [] for f in expected_features},
                schema=pa.schema([(f.root_fqn, f.converter.pyarrow_dtype) for f in expected_features]),
            )
    ...
    def execute_to_pyarrow_batches(
        ...
        if not hasattr(self.source, "execute_to_batches"):
            yield self.execute_to_pyarrow(expected_features, connection, query_execution_parameters)
        ...
        for pa_table in self.source.execute_to_batches(self, converter_getters, connection):  # pyright: ignore
            col_name_mapping = self.get_col_name_mapping(tuple(pa_table.column_names), expected_feature_root_fqns)
            pa_table = pa_table.select(list(col_name_mapping.keys()))
            pa_table = pa_table.rename_columns([col_name_mapping[x] for x in pa_table.column_names])
            # if self._finalizer == Finalizer.ONE:
    ...
    def execute_to_result_handles(
        ...
        return self.source.execute_to_result_handles(self, converter_getters, connection)  # pyright: ignore
    ...
    def get_col_name_mapping(
        ...
        if expected_features is not None:
            ...
            if len(unexpected_fields) > 0:
                raise ValueError(
                    (
                        f"Fields {unexpected_fields} were in the field mapping but "
                        "are not included in the resolver output signature."
                    )
                )
