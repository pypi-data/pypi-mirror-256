...
class DatasetRevisionImpl(DatasetRevision):
    ...
    def get_data_as_polars(
        ...
        caller_name = None if current_frame is None else current_frame.f_code.co_name
        self._hydrate(caller_method=caller_name)
        return self._client.await_offline_query_job(
    ...
    def to_polars(
        ...
        return self.get_data_as_polars(output_id=output_id, output_ts=output_ts).collect()
    ...
    def to_polars_lazyframe(
        ...
        return self.get_data_as_polars(output_id=output_id, output_ts=output_ts)
    ...
    def to_pandas(
        ...
        return self.get_data_as_pandas(output_id=output_id, output_ts=output_ts)
    ...
    def get_data_as_pandas(
        ...
        self._hydrate(caller_method=caller_name)
        return (
            self._client.await_offline_query_job(
                job_id=self.revision_id,
                outputs=self.outputs,
                lazy=True,
                context=context,
    ...
    def get_data_as_dataframe(
        ...
        self._hydrate(caller_method=caller_name)
        return DataFrame(
            data=self._client.await_offline_query_job(
                job_id=self.revision_id,
                outputs=self.outputs,
                lazy=True,
                context=context,
    ...
    def download_data(
        ...
        self._hydrate(caller_method=caller_name)
        context = OfflineQueryContext(environment=self.environment)
        urls = self._client.await_offline_query_job(
            job_id=self.revision_id,
            outputs=self.outputs,
            lazy=True,
            context=context,
    ...
    def get_input_dataframe(self) -> pl.LazyFrame:
        ...
        return self._client.await_offline_query_input(
            job_id=self.revision_id,
            lazy=True,
            context=context,
            branch=self._branch,
    ...
    def resolver_replay(
        ...
        if response.error:
            # TODO<BILL> Not sure what kind of error, Chalk or otherwise, to throw here
        ...
        if len(filtered_urls) == 1:
            no_input_df = _parallel_download(filtered_urls, _DEFAULT_EXECUTOR, lazy=False)
        ...
        for url in filtered_urls:
            ...
            if operator_id is not None:
                no_input_df = _parallel_download([url], _DEFAULT_EXECUTOR, lazy=False)
                if isinstance(no_input_df, pl.LazyFrame):
                    no_input_df = no_input_df.collect()
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!""",
    ...
    def _hydrate(self, show_progress: Optional[bool] = None, caller_method: Optional[str] = None) -> None:
        ...
        should_show_progress = self.show_progress
        ...
        if caller_method != "resolver_replay":
...
class DatasetImpl(Dataset):
    ...
    def to_polars(self, output_id: bool = False, output_ts: bool = False) -> pl.DataFrame:
        return self.get_data_as_polars(output_id=output_id, output_ts=output_ts).collect()

    def to_polars_lazyframe(self, output_id: bool = False, output_ts: bool = False) -> pl.LazyFrame:
        return self.get_data_as_polars(output_id=output_id, output_ts=output_ts)
    ...
    def download_uris(
        ...
        return self.revisions[-1].download_uris(output_id=output_id, output_ts=output_ts)
    ...
    def recompute(
        ...
        if len(self.revisions) == 0:
            raise IndexError("No revisions exist for dataset")
        branch = branch or self._client.config.branch
        if branch is None:
        ...
        new_revision.show_progress = show_progress
    ...
    def resolver_replay(self, resolver: ResolverProtocol):
        if len(self.revisions) == 0:
            raise IndexError("No revisions exist for dataset")
        return self.revisions[-1].resolver_replay(resolver=resolver)
