            ...
            for file in files:
                ...
                if len(items) > chunkSize:
                    ...
                    for i in range(0, len(items), chunkSize):
                        result = client.addItems(inventoryName, items[i : i + chunkSize])
                        logger.info(f"{len(result)+lenResult} items of {len(items)} imported.")
                        sleep(pause)
                else:
                    result = client.addItems(inventoryName, items)
        ...
        with logger.contextualize(correlation_id=correlationId):
            ...
            for file in files:
                ...
                if len(validityItems) > 0:
                    if len(validityItems) > chunkSize:
                        lenResult = 0
                        for i in range(0, len(validityItems), chunkSize):
                            result = client.addItems(inventoryName, validityItems[i : i + chunkSize])
                            logger.info(f"{len(result) + lenResult} items of {len(validityItems)} imported.")
                            sleep(pause)
                    else:
                        result = client.addValidityItemsToParents(inventoryName, validityItems)
                        logger.info(f"{len(result)} validity items of file '{filePath}' imported.")

                # Import new items
                if len(newItems) > 0:
                    if len(newItems) > chunkSize:
                        lenResult = 0
                        for i in range(0, len(newItems), chunkSize):
                            result = client.addItems(inventoryName, newItems[i : i + chunkSize])
                            logger.info(f"{len(result) + lenResult} items of {len(newItems)} imported.")
                            sleep(pause)
                    else:
                        result = client.addItems(inventoryName, newItems)
                        logger.info(f"{len(result)} items of file '{filePath}' imported.")
            ...
            for file in files:
                ...
                if len(timeSeriesItems) > chunkSize:
                    ...
                    for i in range(0, len(timeSeriesItems), chunkSize):
                        result = client.TimeSeries.addTimeSeriesItems(inventoryName, timeSeriesItems[i : i + chunkSize])
                        logger.info(f"{len(result)+lenResult} items of {len(timeSeriesItems)} imported. Waiting {pause} second(s) before continuing...")
                        sleep(pause)
                else:
                    result = client.TimeSeries.addTimeSeriesItems(inventoryName, timeSeriesItems)
        ...
        with logger.contextualize(correlation_id=correlationId):
            ...
            for file in files:
                ...
                if len(items) > chunkSize:
                    lenResult = 0
                    for i in range(0, len(items), chunkSize):
                        result = client.TimeSeries.addTimeSeriesItemsToGroups(groupInventoryName, items[i : i + chunkSize])
                        logger.info(f"{len(result)+lenResult} items of {len(items)} imported.")
                        sleep(pause)
                else:
                    result = client.TimeSeries.addTimeSeriesItemsToGroups(groupInventoryName, items)
