...
parser.add_argument('--nezu-color', action='store_true', help='Enable nezu debuger in color.')
parser.add_argument('--nezu-seek', type=int, choices=range(1, 11), help='Seek for hidden debug messages.')
...
def __lgu(loc, glob, bins, long_str):
        ...
        if key in loc:
            ...
            try:
                ...
                _src = 'b'
                _type = type(_val).__name__ if type(_val).__name__ != "builtin_function_or_method" else "function"
                _sep1 = ':'
                _sep2 = '  =>  '
            except Exception:
                is_undefined = True
        ...
        if _type == "function" and _val.__doc__:
            _desc = f"{_val.__doc__}"[:40] +"..." if len(_val.__doc__) > 43 else _val.__doc__
        elif _type == "function":
            _desc = 'No docstring'
        else:
            _desc = _val
...
def say(*keys, note = ..., hide=1):
    ...
    if seek >= hide:
        ...
        if len(keys) <2:
            prfx = f'@{LINE}\t'
            desc = '  |  '.join([__lgu(LOCAL, GLOBAL, BINS, key) for key in keys])
            sufx = f'  |  << {note} >>' if note != ... else ''
        else:
            prfx = f'@{LINE}\t{"-"*70}\n\t'
            desc = '\n\t'.join([__lgu(LOCAL, GLOBAL, BINS, key) for key in keys])
            sufx = f'\n\t<< {note} >>' if note != ... else ''
            sufx += f'\n\t{"-"*70}'
