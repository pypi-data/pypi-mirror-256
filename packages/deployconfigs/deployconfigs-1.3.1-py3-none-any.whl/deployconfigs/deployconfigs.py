...
class DeployConfigs:
    ...
    def _get(self, option, default: Optional[str] = REQUIRED, _convert_func=lambda x: x):
        ...
        if val is Undefined and option in self.defaults:
    ...
    def get_path(self, option: str, default: Union[Path, str] = REQUIRED) -> Optional[Path]:
        value: str = self._get(option, default)
        return self._to_path(value)

    def get_list(self, option: str, default: list = REQUIRED, sep=',') -> list:
        value = self._get(option, self.REQUIRED if default == self.REQUIRED else None)
        if not value:
            return default or []
        return list(filter(None, map(lambda x: x.strip(), value.split(sep))))

    def get_log_level(self, option: str = 'LOG_LEVEL', default=REQUIRED) -> int:
        ...
        value = self._get(option, str(default)).upper()
        try:
            value = int(value)
        except ValueError:
            pass
