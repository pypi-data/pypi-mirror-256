...
from arraylake.chunkstore.base_chunkstore import (
    MAX_INLINE_THRESHOLD_BYTES,
    BaseChunkstore,
)
from arraylake.chunkstore.fsspec_chunkstore import (
    FSSpecObjectStore,
    GCSFSObjectStore,
    S3FSObjectStore,
)
from arraylake.chunkstore.s3chunkstore import S3ObjectStore
from arraylake.log_util import get_logger
from arraylake.types import DBID, Bucket, ChunkstoreSchemaVersion, Platform
...
def mk_chunkstore_from_uri(
    ...
    return BaseChunkstore(
        object_store=object_store_type(bucket_name=bucket_name, client_kws=kwargs),
        prefix=prefix,
...
def mk_chunkstore_from_bucket_config(bucket: Bucket, repo_id: DBID, inline_threshold_bytes: int = 0, **kwargs) -> BaseChunkstore:
    ...
    return BaseChunkstore(
        object_store=object_store_type(bucket_name=bucket_name, client_kws=kws),
        prefix=prefix,
