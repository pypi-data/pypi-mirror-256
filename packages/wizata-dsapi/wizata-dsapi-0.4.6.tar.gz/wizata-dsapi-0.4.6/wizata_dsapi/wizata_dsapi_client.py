...
class WizataDSAPIClient(ApiInterface):
    ...
    def _get_class(self,
        ...
        if isinstance(entity, str):
            if entity in self.__dto_registry.keys():
                if operation is not None and operation not in self.__dto_registry[entity].operations():
                    raise NotImplementedError(f"Operation {operation} not supported for {entity} entity.")
                return self.__dto_registry[entity]
            else:
                raise TypeError(f"Api entity {entity} is not supported.")
        elif entity in self.__dto_registry.values():
            if operation is not None and operation not in entity.operations():
                raise NotImplementedError(f"Operation {operation} not supported for {entity} entity.")
            return entity
        else:
            raise TypeError(f"Api entity {entity} is not supported.")
    ...
    def info(self):
        ...
        print(f'operations: ')
        for class_name in self.__dto_registry:
            print(f'    {class_name} - {self.__dto_registry[class_name].operations()} - {self.__dto_registry[class_name].get_type()}')
        print(f"")
    ...
    def delete(self, obj):
        ...
        self._process_request(
            method="DELETE",
            route=dto_class.route() + "/" + str(uuid.UUID(obj.api_id())) + "/"
        )
