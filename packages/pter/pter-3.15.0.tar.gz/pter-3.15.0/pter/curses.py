...
class CursesApplication(Application):
    def __init__(self, sources, conf, initial_search):
        ...
        # searching
        try:
            case_sensitive = common.SearchCaseBehaviour(
    ...
    def load_key_configuration(self):
        ...
        for item in self.conf[common.SETTING_GROUP_EDITORKEYS]:
            ...
            target = None
            if fnc in self.editor_key_mapping.values():
                target = self.editor_key_mapping
            elif fnc in self.completion_key_mapping.values():
                target = self.completion_key_mapping
            else:
            ...
            if len(basekseq) > 1:
                logging.warning(f"Can not use key sequences for {fnc}")
            kseq = parse_base_key_sequence(basekseq)
            if kseq is None:
                continue
            target[kseq[0]] = fnc
