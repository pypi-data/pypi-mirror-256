...
class PushConfig(BaseStreamConfig):
    ...
    def __init__(
        ...
        post_processor: Optional[
            Union[Callable[[Dict], Optional[Dict]], Callable[[pd.DataFrame], Optional[pd.DataFrame]]]
        ] = None,
        ...
        :param: post_processor: An optional user-defined function that pre-processes records in the source.
...
class PandasBatchConfig(BaseBatchConfig):
    ...
    **Do not instantiate this class directly.** Use :py:func:`tecton.pandas_batch_config` instead.
    ...
    class PandasDataSourceFunctionType3(Protocol):
        def __call__(self, secrets_context: SecretContext) -> DataFrame:
            ...

    class PandasDataSourceFunctionType4(Protocol):
        def __call__(self, filter_context: FilterContext, secrets_context: SecretContext) -> DataFrame:
            ...
    ...
    def __init__(
        ...
        :param supports_time_filtering: When set to ``True``, the Data Source Function must take the ``filter_context``
                                    ...
                                    the Data Source Function with the :class:`tecton.FilterContext`, which has the ``start_time`` and
                                    ``end_time`` set.
        :param secrets: A list of :class:`tecton.Secret` that will be used in the Data Source Function.
        ...
        self._args.supports_time_filtering = supports_time_filtering
        if secrets:
            self._args.secrets.extend([s.to_proto() for s in secrets])
        self._data_delay = data_delay
        self.data_source_function = data_source_function
