...
def load_and_push_jira_to_s3(ingest_config: IngestionConfig):
    ...
    logging_level_for_debug_statements = (
        logging.DEBUG if ingest_config.ingest_type == IngestionType.AGENT else logging.INFO
    )
    ...
    logger.log(
        level=logging_level_for_debug_statements, msg=f"Feature flags: {jira_config.feature_flags}"
    )
    ...
    if jira_config.feature_flags.get("lusca-auth-always-use-connect-for-atlassian-apis-Q423"):
        logger.log(
            level=logging_level_for_debug_statements, msg="Will use connect for most API calls"
        )
        jira_connect_or_fallback_connection = jira_atlas_connect_connection
    else:
        logger.log(
            level=logging_level_for_debug_statements, msg="Will use basic auth for most API calls"
        )
    ...
    if not jira_config.skip_issues:
        ...
        logger.log(
            level=logging_level_for_debug_statements,
            msg=f"All issue operations will use a batch_size of {jira_issues_batch_size}",
        )
        ...
        for batch_number, batch_issues in enumerate(
            ...
            logger.log(
                level=logging_level_for_debug_statements,
                msg=f'Saving {len(batch_issues)} issues as batch number {batch_number}',
            )
        ...
        logger.info(
            f"Successfully saved {len(all_downloaded_issue_ids)} Jira Issues in {total_issue_batches} separate batches, with each batch limited to {round(jira_issues_uncompressed_file_size / MB_SIZE_IN_BYTES)}MB per batch"
        )
