...
def get_jira_search_batch_size(
    jira_connection: JIRA, optimistic_batch_size: int = MAX_ISSUE_API_BATCH_SIZE
) -> int:
    ...
    1000, so we've limited the batch_size to be {MAX_ISSUE_API_BATCH_SIZE}

    Args:
        jira_connection (JIRA): A Jira Connection Object
        optimistic_batch_size (int, optional): An optimistic batch size. Defaults to {MAX_ISSUE_API_BATCH_SIZE}.
...
def _download_issue_page(
    ...
    while True:
        try:
            # Get Issues
            issues: list[dict] = retry_for_429s(
                jira_connection.search_issues,
                jql_query,
                ...
                startAt=start_at,
                maxResults=changeable_batch_size,
...
def generate_jql_for_batch_of_ids(id_batch: list[str]) -> str:
    ...
    """
    return f'id in ({",".join(sorted(id_batch, key=int))})'
...
def pull_jira_issues_by_jira_ids(
    ...
    with ThreadPoolWithTqdm(
        ...
        for issue_batch in batch_iterable(sorted(jira_ids, key=int), batch_size=batch_size):
            jql_query = generate_jql_for_batch_of_ids(issue_batch)
            all_futures.append(
                pool.submit_with_custom_callbacks(
                    _download_issue_page,
                    custom_call_backs=[lambda future: issues_queue.put(future.result())],
                    jira_connection=jira_connection,
                    jql_query=jql_query,
                    start_at=0,
        ...
        completed_batches = 0
        while completed_batches < len(all_futures):
            # TODO: How to handle error encounters?
            issue_batch = issues_queue.get()
            ...
            # For each issue batch that we complete
            completed_batches += 1
...
def pull_all_jira_issues_by_date(
    ...
    # Generate a list of tasks to submit to a threadpool
    all_futures: list[Future] = []
    with ThreadPoolWithTqdm(
    ...
    for project_key in project_issue_count_map.keys():
        ...
        if expected_issue_count != received_issue_count:
            ...
            logger.debug(f"Successfully found {expected_issue_count} for project {project_key}")

    logger.debug(f"Found {len(all_issues)}/{total_expected_issues} total issues")
    return all_issues
