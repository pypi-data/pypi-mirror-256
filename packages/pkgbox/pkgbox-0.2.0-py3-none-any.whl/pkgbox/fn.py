...
def fn_execute_notebook(notebook, params={}, output_notebook=None, channel_id=None) -> str:
    ...
    if (cwd is None) and (notebook=='esto_base_table_update.ipynb' or notebook=='esto_v3_table_update.ipynb' or notebook=='esto_sheet_update.ipynb'):
        cwd = "/usr/local/airflow/dags/repo/dags/data_analytics/wastage/etl/inventory_flow"

    if cwd is not None:
        notebook = f"{cwd}/{notebook}"
        output_notebook = f"{cwd}/{output_notebook}"
