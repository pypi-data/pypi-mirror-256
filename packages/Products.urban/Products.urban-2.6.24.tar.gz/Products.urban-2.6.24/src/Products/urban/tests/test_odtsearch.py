...
class TestODTSearchScript(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.odt_files = [self.templates_folder_path + filename for filename in odt_names]
    ...
    def test_recursive_folder_search(self):
        ...
        to_find = ['get']
        odtsearch = SearchPODTemplates(to_find, urban.__path__[0] + '/' , silent=True, recursive=True)
        result = odtsearch.run()
    ...
    def test_case_sensitive_search_with_existing_pattern(self):
        to_find = ['GeT']
        odtsearch = SearchPODTemplates(to_find, self.odt_files, silent=True, ignorecase=False)
        result = odtsearch.run()
    ...
    def test_case_insensitive_search_with_existing_pattern(self):
        to_find = ['GeT']
        odtsearch = SearchPODTemplates(to_find, self.odt_files, silent=True, ignorecase=True)
        result = odtsearch.run()
