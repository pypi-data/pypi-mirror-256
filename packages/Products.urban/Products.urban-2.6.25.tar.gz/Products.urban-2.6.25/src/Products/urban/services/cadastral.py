...
class CadastreService(SQLService):
    ...
    def __init__(self, dialect='postgresql+psycopg2', user='urb_xxx', host='', port='', db_name='urb_xxx', password='', timeout='0'):
...
class CadastreSession(SQLSession):
    ...
    def query_parcels_wkt(self, parcels):
        ...
        query_geom = self.session.query(func.ST_AsText(func.ST_Union(capa.the_geom).label('geo_union')))
