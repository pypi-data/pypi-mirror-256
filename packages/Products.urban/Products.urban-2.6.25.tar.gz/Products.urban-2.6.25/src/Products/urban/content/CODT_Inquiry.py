...
class CODT_Inquiry(BaseContent, Inquiry, BrowserDefaultMixin):
    ...
    def get_inquiry_fields_to_display(self, exclude=[]):
        ...
        all_fields = utils.getSchemataFields(self, displayed_fields, 'urban_inquiry', exclude)
        inquiry_type = self.getInquiry_type()
        fields_to_hide = [slave['name'] for slave in full_inquiry_slave_fields if slave['action'] == 'hide' and inquiry_type in slave['hide_values']]
