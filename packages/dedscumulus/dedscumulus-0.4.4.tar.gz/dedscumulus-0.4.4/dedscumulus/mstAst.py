...
def insertSLALog(CREDS,service,function,factor,ex_time,logger):
    try:
        errors=0
        while errors<3:
            try:
                with pyodbc.connect('Driver='+CREDS['driver']+';Server=tcp:'+CREDS['db_server']+',1433;Database='+CREDS['database']+';Uid='+CREDS['db_user']+';Pwd={'+CREDS['db_password']+'};Encrypt=yes;TrustServerCertificate=no;Connection Timeout=30') as conn:
                    with conn.cursor() as cursor:
                        if ex_time<1:#if a quick function, we set it to 1 second anyway
                            ex_time=1
                        if isinstance(service, int):#in case of use a hard-coded id, like in ghppun
                ...
                errors+=1
                time.sleep(0.05)
        logger.error("mstAst.executeCommand Failed 3 times, stopping execution, last command: "+sql)
        return -1
    except Exception as e1:
        logger.error(f"mstAst.executeCommand Failed, error: {e1.args[1]}, service: {service}")
        return -1
    
def writeFileDBX(path, file_name, data, logger, useJson=False):
    try:
        bytes_written=-1
        ...
        with open(full_path, mode='w', encoding="utf-8") as f:
            ...
            if bytes_written>10000:
                bytes_written/=1024
                k="kB"
            logger.debug(f"File written ({bytes_written} {k})")
