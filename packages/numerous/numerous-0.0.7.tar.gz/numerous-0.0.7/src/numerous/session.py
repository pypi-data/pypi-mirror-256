...
from numerous.tools import HTML, Slider
...
def _add_data_models(
    instance: object,
    data_model: Union[ToolDataModel, ContainerDataModel],
) -> None:
...
def _get_element_id(instance: ToolT, name: str) -> Optional[str]:
...
def _get_data_model(instance: ToolT, name: str) -> Optional[ElementDataModel]:
...
def _validate_element(elem: ElementDataModel, sess_elem: ToolSessionElement) -> None:
    ...
    valid_html_element = isinstance(elem, HTMLElementDataModel) and isinstance(
        sess_elem,
        AllElementsSessionAllHTMLElement,
    )
...
def _get_kwargs(cls: Type[ToolT], session: AllElementsSession) -> dict[str, Any]:
    kwargs: dict[str, Any] = {}
    annotations = _get_annotations(cls)
...
def _get_kwarg_value(
    ...
    if isinstance(element, AllElementsSessionAllHTMLElement) and annotation is str:
        return element.html
