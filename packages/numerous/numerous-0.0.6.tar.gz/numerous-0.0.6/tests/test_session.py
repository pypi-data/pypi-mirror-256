...
from numerous.tools import action, container, html, slider, tool
...
async def test_initialize_requests_element_information() -> None:
    ...
    gql.all_elements.return_value = AllElements(
        session=AllElementsSession(id=DEFAULT_SESSION_ID, all=[TEXT_ELEMENT]),
    )
    ...
    await Session.initialize(DEFAULT_SESSION_ID, DEFAULT_CLIENT_ID, gql, TestTool)
...
async def test_html_element_update_updates_value_in_instance() -> None:
    gql = Mock(Client)
    gql.all_elements.return_value = AllElements(
        session=AllElementsSession(id=DEFAULT_SESSION_ID, all=[HTML_ELEMENT]),
    )
...
async def test_number_element_update_updates_value_in_instance() -> None:
    ...
    gql = Mock(Client)
    gql.all_elements.return_value = AllElements(
        session=AllElementsSession(id=DEFAULT_SESSION_ID, all=[NUMBER_ELEMENT]),
    )
...
async def test_slider_element_update_updates_value_in_instance() -> None:
    ...
    gql = Mock(Client)
    gql.all_elements.return_value = AllElements(
        session=AllElementsSession(id=DEFAULT_SESSION_ID, all=[SLIDER_ELEMENT]),
    )
    ...
    class TestTool:
        slider_param: float = slider(
            min_value=SLIDER_ELEMENT_MIN_VALUE, max_value=SLIDER_ELEMENT_MAX_VALUE,
        )
...
async def test_action_triggered_event_calls_action_method() -> None:
    gql = Mock(Client)
    gql.all_elements.return_value = AllElements(
        session=AllElementsSession(id=DEFAULT_SESSION_ID, all=[ACTION_ELEMENT]),
    )
