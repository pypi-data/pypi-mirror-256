...
class SpecsHTTPSClient():
    ...
    def _jsonArrayToPythonArray(self, arr : dict) -> list:
        ...
        # Sometimes there might not be a time_billed
        # like in terminated jobs and streaming jobs
        ...
        while str(ix) in arr.keys():
            solution = arr[str(ix)]
            try:
                solution['time_billed'] = arr['time_billed']
    ...
    def _submitJob(self, problem_files: list, specs: list, tag : str = "") -> uuid:
        ...
        start_job = JobDto(jobId=jobId, problemFiles=problems, specFiles=spec_files, tag=tag)
    ...
    def _checkJob(self, jobid: uuid) -> str:
        ...
        response = requests.get(self.server + SpecsEndpoints.checkjob, params=params,
    ...
    ### blocking interface
    def submitJob(self, problem_files: list, specs: list, tag : str = "") -> uuid:
        return self._submitJob(problem_files, specs, tag)
    ...
    async def submitJobAsync(self, problem_files: list, specs: list, tag : str = "") -> uuid:
        return self._submitJob(problem_files, specs, tag)
