...
from orbit.environment import StrictEnvironment
...
from orbit.find import find_template
from orbit.utils import make_sure_path_exists, rmtree, work_in
...
def is_copy_only_path(path, context):
    ...
    :param context: orbit context.
    """
    try:
        # Check allowed list
        allowed = context["orbit"].get("_copy_and_render_only", [])
        if allowed and os.path.isfile(path):
            matched = False
            for render in allowed:
                if fnmatch.fnmatch(path, render):
                    matched = True
                    break
            if not matched:
                return True
        ...
        # Check ignore list
        for dont_render in context["orbit"]["_copy_without_render"]:
    ...
    for variable, overwrite in overwrite_context.items():
        ...
        if isinstance(context_value, list):
            ...
            if overwrite in context_value:
                ...
                # see ``orbit.prompt.prompt_choice_for_config``
...
def generate_context(context_file="orbit.json", default_context=None, extra_context=None):
    """Generate the context for a orbit project template.

    Loads the JSON file as a Python object, with key being the JSON filename.

    :param context_file: JSON file containing key/value pairs for populating
        the orbit's variables.
...
def generate_file(project_dir, infile, context, env, skip_if_file_exists=False):
    ...
    :param infile: Input file to generate the file from. Relative to the root
        template dir.
    :param context: Dict for populating the orbit's variables.
    ...
    if is_binary(infile):
        ...
        with open(infile, encoding="utf-8", newline="") as rd:
            ...
            newline = rd.newlines
            if context["orbit"].get("_new_lines", False):
                newline = context["orbit"]["_new_lines"]
...
def generate_files(
    ...
    envvars = context.get("orbit", {}).get("_jinja2_env_vars", {})
    ...
    # Join with project dir
    ignored_files = [os.path.join(project_dir, f) for f in ignored_files] if ignored_files else []
    allowed_files = [os.path.join(project_dir, f) for f in allowed_files] if allowed_files else []
