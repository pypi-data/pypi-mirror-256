...
class ERRJupiterIE(LazyLoadExtractor):
    ...
    IE_NAME = 'ERRJupiter'
    _VALID_URL = 'https?://jupiter(?:pluss)?\\.err\\.ee/(?P<id>\\d+)'
    _RETURN_TYPE = 'video'
...
class NebulaClassIE(NebulaBaseIE):
    ...
    _VALID_URL = 'https?://(?:www\\.|beta\\.)?(?:watchnebula\\.com|nebula\\.app|nebula\\.tv)/(?P<id>[-\\w]+)/(?P<ep>\\d+)'
    _NETRC_MACHINE = 'watchnebula'
    _RETURN_TYPE = 'video'
...
class UtreonIE(LazyLoadExtractor):
    ...
    IE_NAME = 'Utreon'
    _VALID_URL = 'https?://(?:www\\.)?utreon\\.com/v/(?P<id>[\\w-]+)'
    _RETURN_TYPE = 'video'
