...
class CalculationAsyncResult(BaseModel):
    """
    The result of starting an asynchronous calculation.  # noqa: E501
    """
    tracedata: Tracedata
    id: StrictStr = Field(description="Calculation identifier.")
    __properties = ["tracedata", "id"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CalculationAsyncResult:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CalculationAsyncResult:
        """Create an instance of CalculationAsyncResult from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CalculationAsyncResult.parse_obj(obj)

        _obj = CalculationAsyncResult.parse_obj({
