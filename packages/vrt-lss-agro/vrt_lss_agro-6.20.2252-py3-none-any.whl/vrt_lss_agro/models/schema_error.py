...
class SchemaError(BaseModel):
    """
    Data error by schema.  # noqa: E501
    """
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SchemaError:
    ...
    def to_dict(self):
        ...
        if self.entity is None and "entity" in self.__fields_set__:
            _dict['entity'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> SchemaError:
        """Create an instance of SchemaError from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SchemaError.parse_obj(obj)

        _obj = SchemaError.parse_obj({
