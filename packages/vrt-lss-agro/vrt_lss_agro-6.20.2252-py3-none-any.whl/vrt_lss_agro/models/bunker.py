...
class Bunker(BaseModel):
    ...
    __properties = ["key", "capacity_forecast", "cost_forecast", "attributes"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Bunker:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> Bunker:
        """Create an instance of Bunker from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Bunker.parse_obj(obj)

        _obj = Bunker.parse_obj({
            "key": obj.get("key"),
