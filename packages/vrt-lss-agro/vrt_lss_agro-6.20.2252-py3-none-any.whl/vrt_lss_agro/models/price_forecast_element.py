...
class PriceForecastElement(BaseModel):
    """
    Grain price forecast for the specified date.  # noqa: E501
    """
    date_window: DateWindow
    price: Annotated[int, Field(le=10000000, strict=True, ge=0)] = Field(description="Value.")
    __properties = ["date_window", "price"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PriceForecastElement:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> PriceForecastElement:
        """Create an instance of PriceForecastElement from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PriceForecastElement.parse_obj(obj)

        _obj = PriceForecastElement.parse_obj({
