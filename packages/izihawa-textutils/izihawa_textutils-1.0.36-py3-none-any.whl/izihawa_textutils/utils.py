...
from .regex import (
    EMAIL_REGEX, HASHTAG_REGEX, MULTIWHITESPACE_REGEX, NON_ALNUMWHITESPACE_REGEX,
    TELEGRAM_LINK_REGEX, URL_REGEX,
)
...
def remove_mm_commands(text: str, remove_tables: bool = True):
    text = re.sub(r'\\+(?:usepackage|nonumber|prod|alpha|beta|left|right|texttt|documentclass|setlength|varvec)(?:[{\[][A-Za-z\d\-.\\]+[}\]])*', '', text)
    if remove_tables:
        text = re.sub(r'\\+(?:begin|end)[{\[](?!tabular)[A-Za-z\d\-.\\]+[}\]]', '', text)
    text = re.sub('[ \t]+', " ", text).strip()
    return text


def remove_markdown(text: str, remove_tables: bool = True):
    ...
    text = re.sub(r"(^|[^\\])_{2,}", r"\g<1>", text)
    text = re.sub(r"(^|[^\\])\*{2,}", r"\g<1>", text)
    text = re.sub(r"\\\*+", "\\*", text)
    text = re.sub(r"\\_+", "\\_", text)
    if remove_tables:
        text = re.sub(r"\\begin\{tabular}.*?\\end\{tabular}", "", text, flags=re.DOTALL | re.MULTILINE)
        text = re.sub(r"(^\|[^\n]+\|(?:\n|$))+", "", text, flags=re.MULTILINE)
    text = remove_mm_commands(text, remove_tables=remove_tables)
    return text
