...
def remove_mm_commands(text: str):
    text = re.sub(r'\\+(?:usepackage|nonumber|prod|end|alpha|beta|left|right|texttt|documentclass|setlength|begin|varvec)(?:[{\[][A-Za-z\d\-.\\]+[}\]])*', '', text)
...
def remove_markdown(text: str, remove_tables: bool = True):
    ...
    if remove_tables:
        ...
        text = re.sub(r"\\begin\{tabular}(.*?)\\end\{tabular}", "\\starttable\n\\g<1>\n\\endtable", text, flags=re.DOTALL | re.MULTILINE)
