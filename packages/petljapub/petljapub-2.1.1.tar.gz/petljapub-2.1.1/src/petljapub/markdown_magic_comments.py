...
def format_divs(md, target, div_specs=None, skip_div_tags=False):
    ...
    for line in md.split('\n'):
        ...
        if is_magic_comment_start('div', line):
            ...
            if not skip_div_tags:
                if "title" in comment:
                    result.append("<div class=\"{}\" title=\"{}\">".format(div_type, comment["title"]))
                else:
                    result.append("<div class=\"{}\">".format(div_type))

            label = comment.get("label", None)
            
            if label != None:
                result.append(md_util.label(div_type, label))
            ...
            if div_start != None:
                if "counter" in specs:
                    if label == None:
                        ...
                        label = "__{}__{}__".format(div_type, format_divs.label_num[div_type])
                        result.append(md_util.label(div_type, label))
                    div_start = div_start.replace("#", md_util.reference(div_type, label))
                else:
        ...
        # end of a div
            ...
            if div_end_stack[-1] != None:
                if result[-1].strip().endswith("$$"):
                    result.append("\n")
                result.append(div_end_stack[-1])
            div_end_stack.pop()
            if not skip_div_tags:
                if result[-1].strip().endswith("$$"):
                    result.append("\n")
