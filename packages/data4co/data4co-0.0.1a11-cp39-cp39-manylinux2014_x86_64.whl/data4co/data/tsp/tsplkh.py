...
class TSPLKHDataset:
    def __init__(self):
        ...
        if not os.path.exists('dataset'):
            os.mkdir('dataset')
        if not os.path.exists('dataset/tsplkh'):
            download(filename="dataset/tsplkh.tar.gz", url=self.url, md5=self.md5)
            extract_archive(archive_path="dataset/tsplkh.tar.gz", extract_path=self.dir)
