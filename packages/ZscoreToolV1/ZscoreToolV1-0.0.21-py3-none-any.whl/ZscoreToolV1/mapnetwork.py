...
def MakeImages(path_to_spatial, spatial_sample_name, method, mode, path_to_network,network_name,
    ...
    # make output folders
    if not os.path.isdir('./Results/02. Images/'):
        os.mkdir('./Results/02. Images/')
    if not os.path.isdir('./Results/02. Images/' + spatial_sample_name):
        os.mkdir('./Results/02. Images/' + spatial_sample_name)
    if not os.path.isdir('./Results/02. Images/' + spatial_sample_name + '/clusters/'):
        os.mkdir('./Results/02. Images/' + spatial_sample_name + '/clusters/')
    if not os.path.isdir('./Results/02. Images/' + spatial_sample_name + '/genes/'):
        os.mkdir('./Results/02. Images/' + spatial_sample_name + '/genes/')
    ...
    if mode=='cluster':
        ...
        zf = pd.read_csv(path_to_network + str(inputvalue) + '.csv')
        zf.columns = ['geneID', 'mean', 'sum']
        
    if mode=='gene':
        # Load the single nuc z-score file (output from script 01) into a Pandas DataFrame
        print("Load z-scores for gene " + inputvalue)
        zf = pd.read_csv(path_to_network + '/res_zscore.csv')
    ...
    # create a new image 
        ...
        if mode=='cluster':
            tiff.imsave('./Results/02. Images/' + spatial_sample_name + '/clusters/cluster' + inputvalue + '_from_' + network_name + '_r' + str(r) + '.tif', new_image)
        if mode=='gene':
            tiff.imsave('./Results/02. Images/' + spatial_sample_name + '/genes/' + inputvalue + '_from_' + network_name + '_r' + str(r) + '.tif', new_image)
        ...
        if fill==True:
            ...
            # Save or display the image
            if mode=='cluster':
                tiff.imsave('./Results/02. Images/' + spatial_sample_name + '/clusters/cluster' + inputvalue + '_from_' + network_name + '_fill_r' + str(r) + '.tif', new_image)
            if mode=='gene':
                tiff.imsave('./Results/02. Images/' + spatial_sample_name + '/genes/' + inputvalue + '_from_' + network_name + '_fill_r' + str(r) + '.tif', new_image)
    ...
    if mode=='cluster':
        plt.savefig('./Results/02. Images/' + spatial_sample_name + '/clusters/cluster' + inputvalue + '_from_' + network_name + '_mean_r' + str(r) + '_thres0_sumblocksums.pdf', format = 'pdf')
    if mode=='gene':
        plt.savefig('./Results/02. Images/' + spatial_sample_name + '/genes/' + inputvalue + '_from_' + network_name + '_mean_r' + str(r) + '_thres0_sumblocksums.pdf', format = 'pdf')
    ...
    if mode=='cluster':
        res.to_csv('./Results/02. Images/' + spatial_sample_name + '/clusters/cluster' + inputvalue + '_from_' + network_name + '_mean_r' + str(r) + '_countsundermask.csv', index=False)
    if mode=='gene':
        res.to_csv('./Results/02. Images/' + spatial_sample_name + '/genes/' + inputvalue + '_from_' + network_name + '_mean_r' + str(r) + '_countsundermask.csv', index=False)


    # result_df_sub
    if mode=='cluster':
        result_df_sub.to_csv('./Results/02. Images/' + spatial_sample_name + '/clusters/cluster' + inputvalue + '_from_' + network_name + '_mean_r' + str(r) + '_result_df_sub.csv', index=False)
    if mode=='gene':
        result_df_sub.to_csv('./Results/02. Images/' + spatial_sample_name + '/genes/' + inputvalue + '_from_' + network_name + '_mean_r' + str(r) + '_result_df_sub.csv', index=False)
