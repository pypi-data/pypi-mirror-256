...
from MarkService.const import SD, MODULES
    ...
    if A.U.for_service(SD, MODULES, as_standalone):
        ...
        def time_tracking_log_handler() -> None:
            def time_tracking_logger() -> None:
                ...
                print(
                    now.replace(minute=max(0, now.minute - 1)),
                    now.replace(minute=min(59, now.minute + 1)),
                )
        ...
        @staticmethod
        def door_operation(door_name: str, operation_name: str) -> None:
            A.EXC.execute(
