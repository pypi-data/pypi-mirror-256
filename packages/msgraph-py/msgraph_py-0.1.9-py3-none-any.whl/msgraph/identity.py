...
def get_user(
    ...
    while True:
        ...
        if next_link and (all and top is None):
            if response.json().get("@odata.count"):
...
def get_signin(
    ...
    The parameters filter, search, and orderby use OData queries:
    https://learn.microsoft.com/en-us/graph/query-parameters
    ...
    if signin_id and (filter or search):
        raise ValueError(
            "Parameters signin_id and filter|search are mutually exclusive."
        )
