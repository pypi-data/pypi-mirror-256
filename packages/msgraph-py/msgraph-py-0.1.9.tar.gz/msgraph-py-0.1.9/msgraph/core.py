...
def get_token2() -> str:
    ...
    BASE_URL = "https://login.microsoft.com/{}"
    SCOPES = ["https://graph.microsoft.com/.default"]

    if not get_token.__dict__.get("client_app"):
        print("setting up new app..")
        (tenant_id, client_id, client_secret) = _get_config()
        get_token.client_app = ConfidentialClientApplication(
            client_id=client_id,
            authority=BASE_URL.format(quote_plus(tenant_id)),
            client_credential=client_secret,
        )
    else:
        print("app already cached")
    print("getting token ..")
    return get_token.client_app.acquire_token_for_client(SCOPES)["access_token"]
