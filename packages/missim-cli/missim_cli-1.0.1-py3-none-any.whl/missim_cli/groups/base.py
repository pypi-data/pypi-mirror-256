...
DOCKER = docker_compose_path("./sim/docker-compose.yaml")
DOCKER_NETWORK_SHARED = docker_compose_path("./sim/docker-compose.network-shared.yaml")
DOCKER_NETWORK_HOST = docker_compose_path("./sim/docker-compose.network-host.yaml")
DOCKER_UE_DEV = docker_compose_path("./sim/docker-compose.ue_editor.yaml")
DOCKER_UE_DEV_SHARED = docker_compose_path("./sim/docker-compose.ue_editor.shared.yaml")
DOCKER_UE_DEV_HOST = docker_compose_path("./sim/docker-compose.ue_editor.host.yaml")
DOCKER_UE_STANDALONE = docker_compose_path("./sim/docker-compose.ue_standalone.yaml")
DOCKER_UE_STANDALONE_SHARED = docker_compose_path("./sim/docker-compose.ue_standalone.shared.yaml")
DOCKER_UE_STANDALONE_HOST = docker_compose_path("./sim/docker-compose.ue_standalone.host.yaml")
...
def _get_compose_files(mode: Mode, network: Network = Network.HOST) -> List[ValidPath]:
    ...
    if mode == Mode.UE_EDITOR:
        compose_files.append(DOCKER_UE_DEV)
        if network == Network.HOST:
            compose_files.append(DOCKER_UE_DEV_HOST)
        if network == Network.SHARED:
            compose_files.append(DOCKER_UE_DEV_SHARED)
...
def up(spin: bool, build: bool, services: List[str]):
    ...
    docker = DockerClient(
        compose_files=_get_compose_files(mode, config.network),
        compose_project_directory=get_project_root(),
    )
    if spin:
        if mode == "standalone":
