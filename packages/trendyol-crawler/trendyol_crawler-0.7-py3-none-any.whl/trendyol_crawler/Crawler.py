...
class TrendyolAPI:
    def __init__(self, supplier_id: str, api_key: str, api_secret: str):
        load_dotenv()
        self.url = f"https://api.trendyol.com/mealgw/suppliers/{supplier_id}/"
    ...
    def get_orders(self, page: int = 0, size: int = 50, status: OrderStatus = OrderStatus.Created) -> OrderResponse:
        ...
        return OrderResponse(**self.session.get(self.url + url_path, headers=_headers, params=params).json(), current_page=page)
