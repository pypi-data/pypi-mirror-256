...
from EVMVerifier.certoraBuild import InputConfig, CertoraBuildGenerator, CertoraVerifyGenerator, FunctionSig, VY
...
OPTIMISTIC_FALLBACK_SETTING = "-optimisticFallback true"
...
class EquivalenceChecker:
    ...
    def build_fun_signatures(self) -> None:
        ...
        for i in range(2):
            ...
            context, _ = get_args([str(tmp_conf)])
            config = InputConfig(context)
            cfb = CertoraBuildGenerator(config, context)
            cfb.certora_verify_generator = CertoraVerifyGenerator(cfb, context)
    ...
    def generate_conf_file(self, spec: Path) -> Path:
        ...
        contents.setdefault(PROVER_ARGS, []).append(OPTIMISTIC_FALLBACK_SETTING)
        if self.is_multicontract:
