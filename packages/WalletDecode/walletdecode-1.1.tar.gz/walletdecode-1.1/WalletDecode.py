...
class extensionWalletDecrypt:
	...
	def __keyfrom_password(self, password, salt):
		saltBuffer = base64.b64decode(salt)
		passwordBuffer = password.encode('utf-8')
		key = hashlib.pbkdf2_hmac('sha256', passwordBuffer, saltBuffer, 10000, dklen=32)
		return key
	...
	def decryptSingle(self, password, data):
		try:
			...
			if res['s']:
				...
				salt = payload['salt']
				btyes = algorithmb()
				key = self.__keyfrom_password(password, salt)
	...
	def decryptList(self, passwords, data):
		res = self.toDoinput(data)
		btyes = algorithmb()
		if res['s']:
			payload = res['d']
