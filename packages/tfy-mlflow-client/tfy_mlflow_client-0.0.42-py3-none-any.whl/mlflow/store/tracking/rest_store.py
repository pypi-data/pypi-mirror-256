...
from mlflow.utils.proto_json_utils import get_field_if_set, message_to_json
...
class RestStore(AbstractStore):
    ...
    def list_experiments(
        ...
        next_page_token = get_field_if_set(response_proto, "next_page_token", default=None)
    ...
    def get_run_by_name(
        ...
        request_body_proto = GetRunByName(
            run_name=run_name, experiment_id=experiment_id, experiment_name=experiment_name
        )
        request_body = message_to_json(request_body_proto)
        response_proto = self._call_endpoint(GetRunByName, request_body)
        return Run.from_proto(response_proto.run)
    ...
    def list_files_for_artifact_version(
        ...
        message = mlfa_pb2.ListFilesForArtifactVersion(
            id=str(version_id),
            path=path,
            max_results=max_results,
            page_token=page_token,
        ...
        next_page_token = get_field_if_set(response_proto, "next_page_token", default=None)
        return PagedList(
            [FileInfo.from_proto(f) for f in response_proto.files],
    ...
    def get_artifact_version(
        ...
        kwargs = dict(
            experiment_id=experiment_id, artifact_name=artifact_name, artifact_type=artifact_type
        )
        if isinstance(artifact_type, ArtifactType):
            kwargs["artifact_type"] = artifact_type.to_proto()
        if version:  # if version is not passed, latest version will be fetched
            kwargs["version"] = version
        message = mlfa_pb2.GetArtifactVersionByName(**kwargs)
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.GetArtifactVersionByName, request_body)
        return ArtifactVersion.from_proto(response_proto.artifact_version)
    ...
    def get_model_version(
        ...
        kwargs = dict(experiment_id=experiment_id, model_name=model_name)
        if version:  # if version is not passed, latest version will be fetched
            kwargs["version"] = version
        message = mlfa_pb2.GetModelVersionByName(**kwargs)
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.GetModelVersionByName, request_body)
        return ModelVersion.from_proto(response_proto.model_version)
    ...
    def list_artifacts(
        ...
        total = get_field_if_set(response_proto, "total", default=None)
        return PagedList(
    ...
    def create_multipart_upload(
        ...
        message = mlfa_pb2.CreateMultiPartUpload(
            artifact_version_id=str(artifact_version_id), path=path, num_parts=num_parts
        )
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.CreateMultiPartUpload, request_body)
        return MultiPartUpload.from_proto(response_proto)
    ...
    def authorize_user_for_model(
        ...
        message = mlfa_pb2.AuthorizeUserForModel(id=str(model_id), role=role)
        request_body = message_to_json(message)
        self._call_endpoint(mlfa_pb2.AuthorizeUserForModel, request_body)

    def authorize_user_for_model_version(self, version_id: uuid.UUID, role: typing.Text) -> None:
        message = mlfa_pb2.AuthorizeUserForModelVersion(id=str(version_id), role=role)
        request_body = message_to_json(message)
        self._call_endpoint(mlfa_pb2.AuthorizeUserForModelVersion, request_body)
    ...
    def create_artifact(
        ...
        message = mlfa_pb2.CreateArtifact(
            experiment_id=str(experiment_id), name=name, artifact_type=artifact_type
        )
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.CreateArtifact, request_body)
        return uuid.UUID(response_proto.id)
    ...
    def create_dataset(
        ...
        message = mlfa_pb2.CreateDataset(
            experiment_id=str(experiment_id),
            name=name,
            description=description,
        ...
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.CreateDataset, request_body)
        return response_proto.dataset

    def get_dataset(
        self,
        id: str,
    ) -> Dataset:
        message = mlfa_pb2.GetDataset(
            id=id,
        )
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.GetDataset, request_body)
        return response_proto.dataset
    ...
    def get_dataset_by_fqn(
        ...
        message = mlfa_pb2.GetDatasetByFqn(
            fqn=fqn,
        )
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.GetDatasetByFqn, request_body)
        return response_proto.dataset

    def get_signed_urls_for_dataset_read(
        self, fqn: str, paths: typing.List[str]
    ) -> typing.List[SignedURL]:
        message = mlfa_pb2.GetSignedURLsForDatasetRead(dataset_fqn=fqn, paths=paths)
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.GetSignedURLsForDatasetRead, request_body)
        return [SignedURL.from_proto(s) for s in response_proto.signed_urls]

    def get_signed_urls_for_dataset_write(
        self, fqn: str, paths: typing.List[str]
    ) -> typing.List[SignedURL]:
        message = mlfa_pb2.GetSignedURLsForDatasetWrite(dataset_fqn=fqn, paths=paths)
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.GetSignedURLsForDatasetWrite, request_body)
        return [SignedURL.from_proto(s) for s in response_proto.signed_urls]
    ...
    def list_files_for_dataset(
        ...
        message = mlfa_pb2.ListFilesForDataset(
            dataset_fqn=fqn, path=path, max_results=max_results, page_token=page_token
        )
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.ListFilesForDataset, request_body)
        next_page_token = get_field_if_set(response_proto, "next_page_token", default=None)
        return PagedList(
            [FileInfo.from_proto(f) for f in response_proto.files],
            token=next_page_token,
    ...
    def create_multipart_upload_for_dataset(
        ...
        message = mlfa_pb2.CreateMultiPartUploadForDataset(
            dataset_fqn=fqn, path=path, num_parts=num_parts
        )
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.CreateMultiPartUploadForDataset, request_body)
        return MultiPartUpload.from_proto(response_proto)
    ...
    def list_datasets(
        ...
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.ListDatasets, request_body)
        next_page_token = get_field_if_set(response_proto, "next_page_token", default=None)
        total = get_field_if_set(response_proto, "total", default=None)
        return PagedList(
            [Dataset.from_proto(av) for av in response_proto.datasets],
            token=next_page_token,
            total=total,
        )
    ...
    def update_dataset(
        ...
        kwargs = dict(description=description)
        if dataset_metadata is not SENTINEL:
            kwargs["dataset_metadata"] = dict_to_proto(dataset_metadata)
        kwargs = {k: v for k, v in kwargs.items() if v is not SENTINEL}
        message = mlfa_pb2.UpdateDataset(fqn=fqn, **kwargs)
        request_body = message_to_json(message)
        response_proto = self._call_endpoint(mlfa_pb2.UpdateDataset, request_body)
        return Dataset.from_proto(response_proto.dataset)
