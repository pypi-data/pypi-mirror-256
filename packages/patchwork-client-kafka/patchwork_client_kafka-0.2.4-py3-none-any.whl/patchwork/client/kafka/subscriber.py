...
class AsyncKafkaSubscriber(AsyncSubscriber):
    ...
    async def _fetch_messages_task(self):
        ...
        while not self.is_stopping:
            ...
            # prioritize TP, not in process firstly
            partitions = sorted(self.consumer.assignment(), key=lambda tp: tp in self._in_process)
            with kafka_get_time.time():
                msg: ConsumerRecord = await self.consumer.getone(*partitions)

            tp = TopicPartition(topic=msg.topic, partition=msg.partition)
            kafka_in_count.inc()
            kafka_in_size.inc(amount=len(msg.value))

            if tp in self._in_process:
