...
def test_extract_maybe() -> None:
    item = Unfolder.extract_maybe(
        JsonValue.from_primitive(JsonPrimitive.empty())
    )
    ...
    assert item2 == Maybe.from_value(json_value)


def test_to_optional() -> None:
    json_value_empty = JsonValue.from_primitive(JsonPrimitive.empty())
    fail = Exception("Fail")
    item = Unfolder.to_optional(
        json_value_empty, lambda _: Result.failure(fail)
    )
...
def test_optional_missing_key() -> None:
    ...
    result = JsonUnfolder.optional(
        json_obj, "missing_key", lambda _: Result.success(77)
    )
    assert result.value_or(None) == Maybe.empty()
    json_obj_2: JsonObj = FrozenDict(
        {"empty_key": JsonValue.from_primitive(JsonPrimitive.empty())}
    )
    result_2 = JsonUnfolder.optional(
        json_obj_2, "empty_key", lambda _: Result.success(77)
    )
    ...
    result_3 = JsonUnfolder.optional(
        json_obj_3, "key1", lambda _: Result.success(77)
    )
    assert result_3 == Result.success(Maybe.from_value(77))
    result_4 = JsonUnfolder.optional(
        json_obj_3, "key1", lambda _: Result.failure(fail, int)
    )
    assert result_4.map(lambda _: True).value_or(False) == False
