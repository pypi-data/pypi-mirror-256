...
class MomentumTimeline(Timeline):
    ...
    def _find_earliest_time_slot(state: SortedList[ScheduleEvent],
        ...
        while current_start_time < last_time:
            ...
            # of the execution interval, i.e., current_start_idx
            ...
            for idx in range(end_idx - 1, current_start_idx - 1, -1):
                if state[idx].available_workers_count < required_worker_count:
                    ...
                    # the last index passing the quantity check
    ...
    def _schedule_with_inseparables(self,
        ...
        for i, chain_node in enumerate(inseparable_chain):
            ...
            assert self.can_schedule_at_the_moment(chain_node, worker_team, spec, node2swork, start_time, node_time)
            deliveries, mat_del_time = self._material_timeline.deliver_resources(chain_node,
                                                                                 start_work,
                                                                                 chain_node.work_unit.need_materials(),
                                                                                 True)
