...
#%%
def cal_pct_dirt(v0, v1, rev, *args, **kwargs):
    if rev:
        return cal_pct(v1, v0, *args, **kwargs)
    return cal_pct(v0, v1, *args, **kwargs)

#%%
@beartype
def find_turns(series: Union[pd.Series, np.ndarray, List[Union[float, int]]],
               t_min: int = 0,
               t_max: int = np.inf,
               min_dif_pct: Union[float, None] = None,
               min_dif_val: Union[float, int, None] = None,
               max_dif_pct: Union[float, None] = None,
               max_dif_val:  Union[float, int, None] = None,
               pct_v00: Union[int, float] = 1,
               skip_nan: bool = True,
               logger: logging.Logger = None,
               plot_process: bool = False,
               plot_process_detail: bool = False) -> pd.Series:
    '''
    标注序列中的极值点
    ...
    min_dif_val, min_dif_pct : float
        ...
        min_dif_val/min_dif_pct，则删除该极值点对（两个参数二选一）
    ...
    assert t_min < t_max, '`t_max`必须大于`t_min`'
    ...
    if (series == np.inf).sum() > 0 or \
       (series == -np.inf).sum() > 0:
        raise ValueError('发现无穷值，请先处理！')
    ...
    if t_min:
        def __max_dif_ok(v2, v3, rev):
            '''判断v2和v3差值是否大于max_dif_pct/max_dif_val'''
            if not isnull(max_dif_pct) and \
                                abs(cal_pct_dirt(v2, v3, rev, pct_v00)) >= max_dif_pct:
                return True
        ...
        def __min_dif_ok(v2, v3, rev):
            '''判断v2和v3差值是否小于min_dif_pct/min_dif_val'''
            if not isnull(min_dif_pct) and \
                                abs(cal_pct_dirt(v2, v3, rev, pct_v00)) <= min_dif_pct:
                return True
        ...
        def __to_del(df, k1, k2, k3, k4, rev):
            '''
            判断是否需要删除(k2, k3)位置的极值点对，判断条件见_del_t_min函数
            
            TODO
            ----
            确定v2和v4、v1和v3比较以判断是否影响前后趋势时，是否需要分正向和逆向情况考虑（通过rev参数来控制是否取等号）
            '''
            if k4 < df.shape[0] and k1 < 0: # 开头位置特殊处理
                v2, v3, v4 = df.loc[k2, col], df.loc[k3, col], df.loc[k4, col]
                if k3-k2 >= t_max+1: # 间隔大于t_max
                    return False, None
                elif k3-k2 < t_min+1: # 间隔小于t_min
                    if df.loc[k2, 'label'] == 1:
                        if v2 >= v4: # 若影响前后趋势，保留
                            return False, None
                        elif __max_dif_ok(v2, v3, rev): # 差值满足条件，保留
                            return False, None
                        else:
                            return True, k2
                    ...
                    if df.loc[k2, 'label'] == 1:
                        if v2 >= v4: # 若影响前后趋势，保留
                            return False, None
                        elif __min_dif_ok(v2, v3, rev): # 不满足差值条件，删除
                            return True, k2
                        else:
                            return False, None
                    elif df.loc[k2, 'label'] == -1:
        ...
        def __del_t_min(df, rev):
            ...
            while k2 < df.shape[0]:
                if df.loc[k2, 'label'] == 0:
                    ...
                    to_del, idxs = __to_del(df, k1, k2, k3, k4, rev)
                    if to_del:
                        df.loc[idxs, 'label'] = 0
                        if plot_process and plot_process_detail:
                            plot_maxmins(df, col, 'label',
                                         title='%s筛除不满足条件的极值对(%s)'%('(反向)' if rev else '', idxs),
                                         grid=True)
        ...
        def __check2point(df):
            df.index = range(df.shape[0])
            k2 = df[df['label'] != 0].index[0]
            k3 = df[df['label'] != 0].index[1]
            v2, v3 = df.loc[k2, col], df.loc[k3, col]
            _del = False
            if k3-k2 < t_min+1: # 间隔小于t_min
                if not __max_dif_ok(v2, v3, False): # 差值不满足条件，删除
                    df['label'] = 0
                    _del = True
            if not (k3-k2 >= t_max+1): # 间隔大于等于t_min小于t_max
                if __min_dif_ok(v2, v3, False): # 不满足差值条件，删除
                    df['label'] = 0
                    _del = True
            if plot_process:
                plot_maxmins(df, col, 'label', title='剩余两个拐点特殊筛除')
            return _del, df
        
        # 总共只有两个极值点
        if (df['label'] != 0).sum() == 2:
            _, df = __check2point(df)
            df = df.set_index(series.index.name)
            return df['label']

        df = __del_t_min(df, False)
        if plot_process:
            plot_maxmins(df, col, 'label', title='第1次正向筛除')

        df.index = range(df.shape[0]-1, -1, -1)
        df = __del_t_min(df, True)
        
        def __check_t_min(df, t_min):
            '''检查是否所有极大值和极小值之间的间隔均大于等于t_min'''
            ...
            if len(tmp) <= 3:
                ...
                if all([x >= t_min for x in tmp]):
                    ...
                    return False, tmp
        t_min_ok, tmp = __check_t_min(df, t_min)
        tmp_new = []
        if plot_process:
        ...
        while not t_min_ok and not tmp == tmp_new:
            ...
            df.index = range(df.shape[0]-1, -1, -1)
            df = __del_t_min(df, True)
            t_min_ok, tmp_new = __check_t_min(df, t_min)
            if plot_process:
                plot_maxmins(df, col, 'label', title='第%s次反向筛除，t_min check: %s'%(k, t_min_ok))
            k += 1
            
        def __del_begin(df, rev):
            can_del, k = True, 0
            while can_del and k < df.shape[0]:
                if (df['label'] != 0).sum() == 2:
                    _, df = __check2point(df)
                    can_del = False
                if df.loc[k, 'label'] != 0:
                    k2, k3 = k, k+1
                    while df.loc[k3, 'label'] == 0:
                        k3 += 1
                    v2, v3 = df.loc[k2, col], df.loc[k3, col]
                    ...
                    if k3-k2 >= t_max+1:
                        ...
                        if not __max_dif_ok(v2, v3, rev): # 差值不满足条件，删除
                            df.loc[k2, 'label'] = 0
                        else:
                            can_del = False
                    else: # 间隔大于等于t_min小于t_max
                        if __min_dif_ok(v2, v3, rev): # 不满足差值条件，删除
                            df.loc[k2, 'label'] = 0
                        else:
                            can_del = False
                    if plot_process:
                        plot_maxmins(df, col, 'label', title='两端特殊筛除'+('' if not rev else '(反向)'))
        ...
        # 起止位置单点循环筛除
        df.index = range(df.shape[0])
        df = __del_begin(df, False)
        df.index = range(df.shape[0]-1, -1, -1)
        df = __del_begin(df, True)
...
#%%
def check_turns(df, col, col_label, max_lbl=1, min_lbl=-1):
    '''
...
#%%
def find_turns_rolling(series, window, cal_n=None, **kwargs):
    '''
    滚动寻找极值点
    ...
    >>> window, t_min = 100, 3
    >>> col = 'close'
    >>> df['label'] = find_turns_rolling(df[col],
    ...                                  window=window,
    ...                                  cal_n=50,
    ...                                  t_min=t_min)
    ...
    tqdm.write('find turns rolling...')
    time.sleep(0.5)
    start = window if isnull(cal_n) else max(window, n-cal_n)
    for k in tqdm(range(start, n+1)):
        subseries = series.iloc[k-window:k]
        res.iloc[k-1] = find_turns(subseries, **kwargs).iloc[-1]
    return res


def find_turns_cum(series, window_min=None, cal_n=None, **kwargs):
    '''
    滚动寻找极值点（用累计历史数据）
    ...
    >>> col = 'close'
    >>> df['label'] = find_turns_cum(df[col],
...
#%%
def get_his_turns_info(df, col, col_label, max_lbl=1, min_lbl=-1):
    '''获取历史极值点序列信息'''
...
#%%
def get_turns_neighbor_label(df, label_col, base_col, labels=[-1, 1],
...
#%%
def del_tooclose_turns(df, label_col, base_col, min_pct=5.0/100):
    '''
    删除 :func:`find_turns` 结果中极大值和极小值差别太小的极值点
    '''
    df = df.reindex(columns=[base_col, label_col])
    k, k1 = 0, 0
...
#%%
def get_turn_records(df, turn_col='turn'):
    '''获取所有拐点记录'''
    turn_records = df.copy()
    turn_records['idx'] = range(turn_records.shape[0])
    turn_records = turn_records[turn_records[turn_col] != 0].copy()
    return turn_records


def get_last_sure_turn_info(df, turn_col='turn', price_col='ma',
                            sure_gap_min=5, sure_pct_min=0.1/100):
    '''获取最后一个确定的turn标签信息'''
    turn_records = get_turn_records(df, turn_col=turn_col)
    last_is_turn = False # 最后一条记录turn是否不为0
    if turn_records['idx'].iloc[-1] == df.shape[0]-1:
        turn_records = turn_records.iloc[:-1, :]
        last_is_turn = True
    sure_loc = turn_records['idx'].iloc[-1] # turn位置
    sure_turn = turn_records[turn_col].iloc[-1] # turn标签值
    sure_gap = df.shape[0]-1 - sure_loc # 时间间隔
    sure_price = turn_records[price_col].iloc[-1] # turn价位
    sure_pct = df[price_col].iloc[-1] / sure_price - 1 # 价差百分比
    if sure_gap > sure_gap_min and abs(sure_pct) > sure_pct_min:
        sure = True
    else:
        sure = False
    other_info = (sure_turn, sure_loc, sure_gap, sure_price, sure_pct)
    return sure, last_is_turn, other_info
            


def get_continue_turn_info(turns_his, turn_col='turn'):
    '''获取连续turn标签的连续期数和信号类型'''
    idx_turn = [(x['idx'].iloc[-1], x[turn_col].iloc[-1]) \
                  for x in turns_his]
    idx_last, turn_last = idx_turn[-1][0], idx_turn[-1][1]
    itmp = len(idx_turn)-2
    n_con = 1
    while itmp > 0:
        idx, turn = idx_turn[itmp][0], idx_turn[itmp][1]
        if idx == idx_last - 1 and turn == turn_last:
            n_con += 1
            idx_last = idx
            itmp -= 1
        else:
            break
    return n_con, turn_last


def disaper(turns_his, loc, col_iloc='idx', tol_gap=10):
    '''
    | 判断turn信号消失
    ...
    if loc == 0:
        return False
    idxs_his = [list(x[col_iloc]) for x in turns_his]
...
#%%
def find_turns_dy(df, col, t_min, his_lag=None, use_all=True,
    ...
    for it in range(his_lag, N):
        ...
        if n_log and it % n_log == 0:
            logger_show('{} / {}, {} find_turns_dy ...'.format(it, N, it_idx),
                        logger, 'info')
        ...
        # 极值点
        df['mm'] = find_turns(df[col], t_min=t_min, **kwargs_findMaxMin)
        turns = get_turn_records(df, turn_col='mm')
        turns_his.append(turns)
        ...
        if '正常' in str(last_sig_type):
            if disaper(turns_his, last_sig_mm_loc,
...
if __name__ == '__main__':
    ...
    df['label'] = find_turns(df['value'],
                             t_min=3,
                                ...
                                # max_dif_val=1,
                               # plot_process=True,
                              # plot_process_detail=True
                             )
    plot_maxmins(df, 'value', 'label',
                 title='标注极大极小值test')
    ...
    df1['label'] = find_turns(df1['col'])
    plot_maxmins(df1, 'col', 'label',
                 title='标注极大极小值test1')
    OK, e = check_turns(df1, df1.columns[0], df1.columns[1])
    if OK:
        print('极值点排列正确！')
    else:
        print('极值点排列错误:', e)
    ...
    df2['label'] = find_turns(df2['col'])
    plot_maxmins(df2, 'col', 'label',
                 title='标注极大极小值test2')
    OK, e = check_turns(df2, df2.columns[0], df2.columns[1])
    if OK:
        print('极值点排列正确！')
    else:
        print('极值点排列错误:', e)
    ...
    # t_min = None
    t_min = 5
    df['label'] = find_turns(df['test'], t_min=t_min, min_dif_val=4, t_max=10)
    ...
    OK, e = check_turns(df, 'series', 'label')
    if OK:
        print('极值点排列正确！')
    else:
        print('极值点排列错误:', e)
    ...
    series_ = df[col]
    t_min = 5
    df['label'] = find_turns(series_, t_min=t_min, min_dif_pct=0.15/100, t_max=10)
    ...
    his_cycle_info, df_his = get_his_turns_info(df, col, 'label')
    last_pos_info = get_last_pos_info(df.iloc[:-1, :], col, 'label', min_gap=0)
    # '''

    #%%
    # 动态标签
    # df['signal'], df['sig_type'] = find_turns_dy(df, col, t_min,
    #                                 his_lag=50,
    #                                 use_all=False,
    #                                 logger=None,
    #                                 n_log=10,
    #                                 # fig_save_dir='./_test/find_turns_dy/50ETF',
    #                                 fig_save_dir=None,
    #                                 plot_all=True,
    #                                 plot_lag='all',
    #                                 kwargs_findMaxMin={},
