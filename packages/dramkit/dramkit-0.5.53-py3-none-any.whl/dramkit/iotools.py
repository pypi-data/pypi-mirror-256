...
from dramkit.const import WIN_NOT_ALLOW_FILE_STR
...
def _process_df_pd_big(fpath,
                       ...
                       max_iter=10000):
    if isnull(read_func):
        read_func = _pd_load_df_check(fpath, None, None)
        if isnull(read_func):
            return None
    ...
    if random_choice:
        assert max_chunk < np.inf, '随机选择时`max_chunk`不能为无穷大！'
        assert max_iter < np.inf, '随机选择时`max_iter`不能为无穷大！'
    ...
    if not random_choice:
        ...
        idxs = np.random.choice(range(max_iter), size=max_chunk, replace=False)
        idxs.sort()
        k = 0
        for tmp in df_iter:
            if k > idxs[-1]:
...
def process_df_pd_big(fpath: str,
    ...
    TODO
    ----
    增加random_seed设置（或将random_seed与random_choice合并为一个参数）
    
    Note
    ----
    若只随机读取一部分数据，须注意 `chunksize` , `max_chunk` , `max_iter` 三个参数的配合，
    `max_iter` 值太大（远远大于max_chunk）可能导致随机循环中取不到数据而报错，或者费时很长
    """
    read_func = _pd_load_df_check(fpath, ftype, logger)
    if isnull(read_func):
        return None
    res = _process_df_pd_big(fpath, read_func=read_func,
                             chunksize=chunksize,
...
def load_df_pd(fpath: str,
    ...
    if 'chunksize' in kwargs and not isnull(kwargs['chunksize']):
        ...
        chunksize, kwargs = get_update_kwargs(
            'chunksize', None, kwargs, 100000, func_update=False)

    max_chunk, kwargs = get_update_kwargs('max_chunk', np.inf, kwargs, np.inf, func_update=False)
    random_choice, kwargs = get_update_kwargs('random_choice', False, kwargs, False, func_update=False)
    max_iter, kwargs = get_update_kwargs('max_iter', 10000, kwargs, 10000, func_update=False)
    ...
    for en in ens:
        try:
            ...
            pass
            # if isnull(data):
            #     logger_show('读取出错！', logger, level='err', show_pos=True)
            #     # raise_error('FielReadError', '读取出错！', logger=logger)
    if isnull(data):
        if not _big:
...
def load_dfs_pd(fpaths,
    ...
    **kwargs_loaddf :
    ...
    if len(kwargs_sort) > 0:
        _, kwargs_sort = get_update_kwargs('inplace', True, kwargs_sort, True)
        data.sort_values(**kwargs_sort, inplace=True)
    if len(kwargs_drop_dup) > 0:
        _, kwargs_drop_dup = get_update_kwargs('inplace', True, kwargs_drop_dup, True)
...
def get_all_paths(dir_path, ext=None, start=None,
    ...
    if abspath:
        ...
        fpaths = [x.replace('\\', '/') for x in fpaths]
...
def handle_win_fname(fname: str) -> str:
    '''将windows不支持的文件名字符进行填充'''
    for k, v in WIN_NOT_ALLOW_FILE_STR.items():
...
if __name__ == '__main__':
    ...
    data1 = load_text(fpath, encoding='gbk', logger=logger)
    data2 = load_csv(fpath, encoding='utf-8', logger=logger)
