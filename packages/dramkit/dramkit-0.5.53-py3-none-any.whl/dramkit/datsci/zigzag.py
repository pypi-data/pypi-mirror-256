...
def zigzag(df: pd.DataFrame,
    ...
    def zigzag_from_k(df, k, ktype):
        ...
        while k < df.shape[0]:
            ...
            if ok_mid and ok_right:
                df.loc[k, zzcol] = -ktype
                ktype = -ktype
            elif ok_mid and not ok_right:
                df.loc[k, zzcol] = -ktype * 0.5
                break
    ...
    if zzcol in df.columns:
        cols = cols + [zzcol]
        df = df[cols].copy()
        k = df.shape[0] - 1
        while k > 0 and df.loc[k, zzcol] in [0, 0.5, -0.5]:
            k -= 1
        ktype = df.loc[k, zzcol]
        if ktype in [1, -1]:
            df[zzcol] = zigzag_from_k(df, k, ktype)
            
    # 若data中没有zigzag列或已有zigzag列不能确定有效的转折点，则需要全部计算
    if (not zzcol in df.columns) or (ktype in [0, 0.5, -0.5]):
        df = df[cols].copy()
        df[zzcol] = 0.0
        ...
        df0[zzcol] = zigzag_from_k(df0, 0, ktype0)
        df0 = df0[::-1].reset_index(drop=True)
        
        # 方式一：先确定第一个高/低点标签，再往后计算
        k, ktype = get_first_appear(df0[zzcol], lambda x: x in [-1, 1])
        if not isnull(k):
            df[zzcol] = df0[zzcol].iloc[:k+1]
            df[zzcol] = df[zzcol].fillna(0)
            df[zzcol] = zigzag_from_k(df, k, ktype)
        # # TODO: 确定方式二是否完全准确
        # # 方式二：直接使用df0中的结果
        # df[zzcol] = df0[zzcol]
        # df[zzcol] = df[zzcol].fillna(0)
        # k, ktype = get_first_appear(df0[zzcol],
        #                             lambda x: x in [-1, 1],
        #                             reverse=True)
        # if not isnull(k):
        #     df[zzcol] = zigzag_from_k(df, k, ktype)
...
#%%
# def zigzag_cum(df, window_min=None, cal_n=None, **kwargs):
#     '''
#     滚动ZigZag标注（用累计历史数据）
...
#     >>> df = load_csv(fpath)
#     >>> df = df.set_index('date', drop=False).iloc[-5000:, :]
#     >>> window_min, t_min = 100, 3
#     >>> df['label'] = zigzag_cum(df,
