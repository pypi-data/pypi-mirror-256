...
def __call_utility_details(main_globals) -> None:
    ...
    if num_params != 0:
        raise ValueError(f"Expecting 0 arguments for details function, found {num_params}")
...
def __call_utility_run(args, data, main_globals):
    ...
    if args.app_key:
        # Note: Using class variable method for app key, since this allows user creating a utility to create other model connections
        # without requiring app key to be supplied for each
