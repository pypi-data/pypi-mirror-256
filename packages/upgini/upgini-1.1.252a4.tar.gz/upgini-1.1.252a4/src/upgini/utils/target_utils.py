...
def balance_undersample(
    ...
    if task_type == ModelTaskType.MULTICLASS:
        ...
        if max_class_count > (quantile25_class_cnt * 2):
            ...
            for class_idx in range(quantile25_idx):
                ...
                sample_strategy[class_value] = min(class_count, quantile25_class_cnt * 2)
        ...
        sampler = RandomUnderSampler(
            sampling_strategy={max_class_value: 5 * min_class_count}, random_state=random_state
        )
        X = df[SYSTEM_RECORD_ID]
        X = X.to_frame(SYSTEM_RECORD_ID)
        new_x, _ = sampler.fit_resample(X, target)  # type: ignore
