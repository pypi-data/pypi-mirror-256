...
class FeaturesEnricher(TransformerMixin):
    ...
    def __sample_imbalanced(
        ...
        if eval_set is not None:
            ...
            x_columns = [
                c
                for c in (validated_X.columns + self.fit_generated_features + [SYSTEM_RECORD_ID])
        ...
        self.__cached_sampled_datasets = (X_sampled, y_sampled, enriched_X, eval_set_sampled_dict, self.fit_search_keys)

        return self.__mk_sampled_data_tuple(X_sampled, y_sampled, enriched_X, eval_set_sampled_dict, self.fit_search_keys)
