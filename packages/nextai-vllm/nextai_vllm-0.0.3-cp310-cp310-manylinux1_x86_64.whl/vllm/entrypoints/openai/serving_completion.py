...
def create_usage_record(settings: AppSettings, usage: UsageInfo, metadata: Union[ChatCompletionResponse, CompletionResponse], token: str):
    if settings.cluster_id:
        ...
        requests.post('{}/public/usage-records/'.format(settings.auth_server, settings.cluster_id), json={
            "usage": usage.dict(),
            "metadata": metadata.dict()
        }, headers={
            "x-api-key": token
    ...
    async for prompt_idx, res in result_generator:
        ...
        for output in res.outputs:
            ...
            if output.finish_reason is not None:  # return final usage
                ...
                create_background_tasks(background_tasks=background_tasks, settings=settings, usage=final_usage, metadata=CompletionResponse(
                            id=request_id,
                            model=model_name,
                            choices=[],
                            usage=final_usage
...
def request_output_to_completion_response(
    ...
    create_background_tasks(background_tasks=background_tasks, settings=settings, usage=usage, metadata=CompletionResponse(
        id=request_id,
        created=created_time,
        model=model_name,
        choices=choices,
...
class OpenAIServingCompletion(OpenAIServing):
    ...
    async def create_completion(self, request: CompletionRequest,
        ...
        response = request_output_to_completion_response(
            ...
            created_time, model_name, settings=settings, token=token, background_tasks=background_tasks)
