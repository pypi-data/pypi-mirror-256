...
class WinStandardTrosnothGame(Goal):
    ...
    def reevaluate(self, *args, **kwargs):
        ...
        if my_zone and my_zone.owner == player.team and my_zone.isBorderline():
        ...
        if (
                ...
                and my_zone.isCapturableBy(player.team)):
    ...
    def getMostLikelyUrgentZone(self, myZone):
        ...
        while pending:
            ...
            if not any(r.owner == room.owner for r in room.all_neighbours):
                # This is the last remaining zone
                awesomeness = 5
            else:
                awesomeness = room.consequenceOfCapture()
...
class CaptureZonesButDoNotDie(Goal):
    ...
    def reevaluate(self):
        ...
        elif my_zone.isCapturableBy(self.bot.player.team):
            self.setSubGoal(None)
...
class DefendZone(ZoneMixin, Goal):
    ...
    def reevaluate(self):
        ...
        player_zone = player.getZone()
        if player_zone == self.zone:
            teams_who_can_tag = self.zone.teamsAbleToTag() - {player.team}
