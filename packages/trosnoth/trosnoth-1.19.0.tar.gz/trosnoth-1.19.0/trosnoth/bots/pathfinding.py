...
def buildWorldAndBlockDefForSingleLayout(
    ...
    map_layout = MapLayout(3, 4, False)

    if layout.kind in ('fwd', 'bck'):
        i, j = 2, 1
    else:
        map_layout.addZone(1, 1, 0)
        if layout.kind == 'top':
            i, j = 1, 1
        else:
            i, j = 1, 2

    block = map_layout.blocks[j][i]
    layout.applyTo(block)
    map_layout.setupComplete(pathFinderFactory=pathFinderFactory)
    world.setLayout(map_layout)
...
def shiftIndices(indices, direction):
    ...
    :param indices: the map block indices in the same order that they're
        used to index map layout.blocks (i.e., (row index, column index))
...
class PathFindingStoreBuilder(object):
    ...
    def buildPathFinder(self, map_layout):
        return BuildTimePathFinder(self, map_layout)
...
class OneBlockActionProcessor(ActionQueueProcessor):
    ...
    def simulationAdvanced(self, path, player, action, ticks, pointData):
        ...
        map_layout = player.world.map.layout
        blockDef, offset = getRegionAndRelativePos(map_layout, player.pos)
...
class PathFindingRoute(object):
    ...
    def extendTo(self, pos, sprint=False):
        ...
        if zone and distance(zone.defn.pos, pos) < ZONE_CAP_DISTANCE:
            if self.attemptPathToOrb(zone, sprint=sprint):
                return
    ...
    def attemptPathToOrb(self, room, *, sprint=False):
        zoneDef = room.defn
        dests = []
...
def getRegionAndRelativePos(map_layout, pos):
    i, j = MapLayout.getMapBlockIndices(*pos)
    blockDef = map_layout.blocks[i][j]
...
class RunTimePathFinder:
    def __init__(self, map_layout, regionFactory=None, ddb=None, pfdb=None):
        if ddb is None:
            ddb = DiscretisationDatabase.get()
        self.ddb = ddb
        self.pfdb = pfdb
...
class StationaryPoint:
    ...
    def fromPos(cls, pathFinder, pos):
        map_layout = pathFinder.layout
        blockDef, relPos = getRegionAndRelativePos(map_layout, pos)
        locKey = DiscretisationDatabase.getLocKey(relPos)
...
class BuildTimePathFinder(RunTimePathFinder):
    def __init__(self, builder, map_layout):
        self.builder = builder
        super().__init__(map_layout)
