...
class MessAroundInZone(Goal):
    ...
    def reevaluate(self):
        ...
        pathFinder = self.bot.world.path_finder
        ...
        okRoutes = [
            r for r in routes
            if r.finishPoint is None or self.bot.world.rooms.get_at(r.finishPoint) == self.zone]
        if okRoutes:
            routes = okRoutes
    ...
    def reevaluate(self):
        ...
        while zones:
            ...
            if player.isZoneRespawnable(zone):
                best_zone = zone
                break
            adjacent = [z for z in zone.open_neighbours if z not in seen]
