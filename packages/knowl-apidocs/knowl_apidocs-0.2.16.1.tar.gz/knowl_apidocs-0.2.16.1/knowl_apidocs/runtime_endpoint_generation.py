...
def get_endpoints(manage_py_path, url_conf):
    ...
    urlconf = import_module(url_conf)
...
def main(input_dir, result_dir, url_conf, manage_py_path, settings_conf = None):
    ...
    for file_path in files_list:
        if file_path.endswith("manage.py"):
            if manage_py_path is None or len(manage_py_path) > len(file_path):
                manage_py_path = file_path
    endpoints = get_endpoints(manage_py_path, url_conf)
