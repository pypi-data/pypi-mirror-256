...
def check_proxie_reachable(urls, working_proxies):
    for full_url in tqdm(urls, desc="Checking Proxies", unit="URL"):
        try:
            ...
            tqdm.write(f"Progress: {tqdm.format_meter(len(working_proxies), len(urls), elapsed=0, suffix='')} ", end='\r')
...
def get_proxies():
    ...
    if result:
        working_proxies = []
        num_threads = len(result)  # You can adjust the number of threads based on your system capabilities

        # Split the list of proxies into chunks for parallel processing
        chunks = [result[i:i + len(result) // num_threads] for i in range(0, len(result), len(result) // num_threads)]

        threads = []

        for chunk in chunks:
            thread = threading.Thread(target=check_proxie_reachable, args=(chunk, working_proxies))
            thread.start()
