...
def check_proxies(urls):
    total_urls = len(urls)
    working = []

    with ThreadPoolExecutor(max_workers=50) as executor:  # You can adjust max_workers as needed
        futures = [executor.submit(check_proxy_reachable, url) for url in urls]

        for future in tqdm(as_completed(futures), total=total_urls, desc="Checking Proxies", unit="URL"):
...
def get_proxies():
    ...
    if result:
        working_proxies = check_proxy_reachable(result)
