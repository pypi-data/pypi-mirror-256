...
setup.gse.aeu.crio.calibration.n_cam_ocp_s_t = [600.0, 600.0, 600.0, 600.0, 600.0, 600.0]

setup = submit_setup(setup, "increased the ocp start time to 600ms")
setup.get_id()  # Setup 59
...
crio.reset()  # this didn't work to clear the error

crio.get_num_errors()
crio.get_error_info()
crio.clear_error_queue()
...
for idx in range(1, 7):
    psu[idx].reset()  # this didn't work to clear the error

# This is the only thing that seems to work to get back into proper standby mode
crio.set_operating_mode(OperatingMode.FC_TVAC)

execute(aeu.n_cam_swoff)

print(setup.gse.aeu.crio.calibration)
setup.gse.aeu.crio.calibration.n_cam_ocp_s_t = [750.0, 750.0, 750.0, 750.0, 750.0, 750.0]

setup = submit_setup(setup, "increased the ocp start time to 750ms")
setup.get_id()  # Setup 60
...
execute(aeu.n_cam_swoff)  # first time didn't work
execute(aeu.n_cam_swoff)

print(setup.gse.aeu.crio.calibration)
setup.gse.aeu.crio.calibration.n_cam_uvp = [34, 15.7, 6.2, 6.2, -6.2, 4.45]

setup = submit_setup(setup, "set n_cam_uvp to [34, 15.7, 6.2, 6.2, -6.2, 4.45]")
setup.get_id()  # Setup 61
...
execute(aeu.n_cam_swoff)  # needed to try this three times!
