...
def get_random_picture(width=400, height=300, save_path=None):
    image = download_image(f"https://picsum.photos/{width}/{height}", save_path)
    return image
...
def rotate_image(image, degree, expand=False):
    ...
    rotation_matrix = cv2.getRotationMatrix2D(center_point, degree, 1.)
...
def shift_image(image, dx, dy, new_height=None, new_width=None):
    ...
    if new_height and new_width:
        padded_image = resize_with_black_padding(padded_image, new_height, new_width)

    translation_matrix = np.float32([[1, 0, dx], [0, 1, dy]])
    shifted_image = cv2.warpAffine(padded_image, translation_matrix, (padded_image.shape[1], padded_image.shape[0]))
