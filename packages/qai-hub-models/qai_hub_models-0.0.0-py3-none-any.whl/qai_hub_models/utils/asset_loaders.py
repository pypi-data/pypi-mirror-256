...
class ModelZooAssetConfig:
    ...
    def get_local_store_model_path(
        self, model_name: str, version: str | int, filename: str
    ) -> str:
    ...
    def get_local_store_dataset_path(
        self, dataset_name: str, version: str | int, filename: str
    ) -> str:
        model_dir = os.path.join(
            self.local_store_path,
    ...
    def get_qaihm_repo(self, model_id: str, relative=True):
        ...
        if not relative:
            if self.repo_url.startswith("http"):
                return urljoin(self.repo_url, relative_path)
            elif self.repo_url.endswith("/"):
            ...
            for key, value in data.items():
                ...
                if isinstance(value, str) and value.startswith("env:"):
                    ...
                    if env_var_name in os.environ:
                        data[key] = env_var_name
                    elif verify_env_has_all_variables:
                        raise ValueError(
                            f"Environment variable '{value[4:]}' was specified in "
...
def load_image(image: str | CachedWebAsset) -> Image.Image:
    return _load_file(image, Image.open)


def load_numpy(file: str | CachedWebAsset) -> Any:
    return _load_file(file, np.load)


def load_torch(pt: str | CachedWebAsset) -> Any:
    def _load_torch(path):
        return torch.load(path, map_location="cpu")

    return _load_file(pt, _load_torch)


def load_json(json_file: str | CachedWebAsset) -> Dict:
...
def load_path(
    file: str | CachedWebAsset, tmpdir: tempfile.TemporaryDirectory | str
) -> str | Path:
    """
...
class SourceAsRoot:
    ...
    def __init__(
        ...
        self.source_repo_version = source_repo_version
        self.keep_sys_path = keep_sys_path
        self.source_repo_patches = source_repo_patches

    def __enter__(self):
        SourceAsRoot.THREAD_LOCK.acquire()
        self.repository_path = maybe_clone_git_repo(
            self.source_repo_url,
            self.source_repo_commit_hash,
            self.source_repo_name,
            self.source_repo_version,
            patches=self.source_repo_patches,
        )
        self.cwd = os.getcwd()
        ...
        sys.path.insert(0, self.repository_path)
