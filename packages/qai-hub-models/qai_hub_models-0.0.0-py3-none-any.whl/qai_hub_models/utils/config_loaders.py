...
from qai_hub_models.utils.args import TargetRuntime
    ...
    def get_perf_details(
        ...
        for model in self.perf_details["models"]:
            ...
            for device_metrics in metrics:
                ...
                compute_unit_counts = {}
                for name, count in layer_info.items():
                    if "layers_on" in name:
                        if count > 0:
                            compute_unit_counts[name[-3:].upper()] = count
...
class QAIHMModelInfo:
    ...
    def validate(self) -> Tuple[bool, Optional[str]]:
        ...
        if ("Quantized" in self.name) and (MODEL_TAG.QUANTIZED not in self.tags):
            return False, f"Quantized models must have quantized tag. tags: {self.tags}"
        if ("Quantized" not in self.name) and (MODEL_TAG.QUANTIZED in self.tags):
            return (
                False,
                f"Models with a quantized tag must have 'Quantized' in the name. tags: {self.tags}",
            )
