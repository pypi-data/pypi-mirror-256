# THIS FILE WAS AUTO-GENERATED by `.../qai_hub_models/scripts/generate_export_files.py`
...
from qai_hub_models.models.stable_diffusion import Model
...
DEFAULT_DEVICE = "Samsung Galaxy S23"

ALL_COMPONENTS = [
    "StableDiffusionTextEncoder",
    "StableDiffusionVAEDecoder",
    "StableDiffusionUNet",
    "StableDiffusonVAEEncoder",
]
DEFAULT_COMPONENTS = ["text_encoder", "vae_decoder", "unet"]
...
def export_model(
    ...
    **additional_model_kwargs,
    ...
    This function accomplishes 6 main tasks:
        ...
        4. Inferences the model on sample inputs and returns the outputs.
    ...
    Parameters:
        ...
        skip_summary: If set, skips waiting for and summarizing results
            from profiling and inferencing.
    ...
    components_dict = {}
    if "StableDiffusionTextEncoder" in components:
        components_dict["StableDiffusionTextEncoder"] = model.text_encoder
    if "StableDiffusionVAEDecoder" in components:
        components_dict["StableDiffusionVAEDecoder"] = model.vae_decoder
    if "StableDiffusionUNet" in components:
        components_dict["StableDiffusionUNet"] = model.unet
    if "StableDiffusonVAEEncoder" in components:
        components_dict["StableDiffusonVAEEncoder"] = model.vae_encoder
    ...
    for component_name, component in components_dict.items():
        ...
        model_compile_options = component.get_hub_compile_options(target_runtime)
        print(
            f"Launching compilation job for {component_name} to run the target model on-device."
        )
        compile_jobs[component_name] = hub.submit_compile_job(
            ...
            options=model_compile_options + " " + compile_options,
        )
    ...
    if not skip_profiling:
        for component_name in components:
            print(
                f"Compilation job was successful. Now, launching profile job for {component_name} to get performance numbers on-device."
            )
    ...
    if not skip_inferencing:
        for component_name in components:
            print(
                f"Launching inference job for {component_name} to get on-device output."
            )
    ...
    # 6. Summarize the results from profile and inference
