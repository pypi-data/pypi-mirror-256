    ...
    for component_name, result in results.items():
        ...
        if os.environ.get("TEST_HUB_ASYNC", 0):
            with open(os.environ["COMPILE_JOBS_FILE"], "a") as f:
                f.write(
                    f"mediapipe_hand_TFLITE_{component_name}: {compile_job.job_id}\n"
                )
        else:
...
def test_compile_qnn():
    ...
    for component_name, result in results.items():
        ...
        if os.environ.get("TEST_HUB_ASYNC", 0):
            with open(os.environ["COMPILE_JOBS_FILE"], "a") as f:
                f.write(f"mediapipe_hand_QNN_{component_name}: {compile_job.job_id}\n")
        else:
            result = compile_job.wait()
            assert result.success
    ...
    if os.environ.get("TEST_HUB_ASYNC", 0):
        with open(os.environ["COMPILE_JOBS_FILE"], "r") as f:
            ...
            for i in job_ids.keys():
                if i.startswith("mediapipe_hand_TFLITE"):
    ...
    for component_name, result in results.items():
        ...
        if os.environ.get("TEST_HUB_ASYNC", 0):
            with open(os.environ["PROFILE_JOBS_FILE"], "a") as f:
                f.write(
                    f"mediapipe_hand_TFLITE_{component_name}: {profile_job.job_id}\n"
                )
        else:
