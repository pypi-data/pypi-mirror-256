...
from qai_hub_models.models.mobilenet_v2_tv import Model
...
def export_model(
    ...
    **additional_model_kwargs,
    ...
    model_compile_options = model.get_hub_compile_options(target_runtime)
    print(
        f"Launching compilation job for {model_name} to run the target model on-device."
    )
    ...
    if not skip_profiling:
        print(
            f"Compilation job was successful. Now, launching profile job for {model_name} to get performance numbers on-device."
        )
    ...
    if not skip_inferencing:
        print(f"Launching inference job for {model_name} to get on-device output.")
