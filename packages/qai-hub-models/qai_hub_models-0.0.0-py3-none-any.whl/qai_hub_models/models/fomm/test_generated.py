    ...
    for component_name, result in results.items():
        ...
        if os.environ.get("TEST_HUB_ASYNC", 0):
            with open(os.environ["COMPILE_JOBS_FILE"], "a") as f:
                f.write(f"fomm_TFLITE_{component_name}: {compile_job.job_id}\n")
        else:
...
@pytest.mark.skip(
    reason="Compilation fails https://dev.qaihub.qualcomm.com/jobs/j0pxxnx9p"
)
@pytest.mark.compile
@skip_clone_repo_check
def test_compile_qnn():
    ...
    for component_name, result in results.items():
        ...
        if os.environ.get("TEST_HUB_ASYNC", 0):
            with open(os.environ["COMPILE_JOBS_FILE"], "a") as f:
                f.write(f"fomm_QNN_{component_name}: {compile_job.job_id}\n")
        else:
            result = compile_job.wait()
            assert result.success
    ...
    for component_name, result in results.items():
        ...
        if os.environ.get("TEST_HUB_ASYNC", 0):
            with open(os.environ["PROFILE_JOBS_FILE"], "a") as f:
                f.write(f"fomm_TFLITE_{component_name}: {profile_job.job_id}\n")
        else:
