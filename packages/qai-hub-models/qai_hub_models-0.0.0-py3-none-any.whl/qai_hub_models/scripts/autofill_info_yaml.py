...
def get_target_model_size(model, input_spec):
...
def get_model_size_and_trainable_parameters(model):
    input_spec = model.get_input_spec()
    traced_model = trace_model(model, input_spec)
    trainable_parameters = get_num_trainable_parameters(traced_model)
    size = get_target_model_size(traced_model, input_spec)
    return (size, trainable_parameters)


def add_details_to_info_yaml(info, details):
    for name, (size, trainable_parameters) in details.items():
        if "technical_details" in info:
            ...
            info["technical_details"][param_key] = trainable_parameters
        ...
        try:
            ...
            if "components" in export_options:
                ...
                for submodel_name, submodel in export_options["components"].items():
                    ...
                    details[submodel_name] = get_model_size_and_trainable_parameters(
                        comp_model
                    )

            else:
                model = model_module.Model.from_pretrained()
                details["model"] = get_model_size_and_trainable_parameters(model)
