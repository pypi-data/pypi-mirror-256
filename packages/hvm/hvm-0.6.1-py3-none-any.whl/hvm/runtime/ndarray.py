...
from .._ffi.runtime_ctypes import DataType, HerculesDevice, HVMArray, HVMArrayHandle
from .._ffi.runtime_ctypes import DataTypeCode, hvm_shape_index_t
...
class NDArray(Object):
    ...
    """Lightweight NDArray implementation for hvm runtime
    ...
    Construction method 2
        ...
        Examples:
            >>> import hvm
            >>> nd = hvm.NDArray([[1, 2, 3,], [4, 5, 6]], [], "int32")
            >>> nd
            [
             [ 1 2 3 ]
             [ 4 5 6 ]
    ...
    def __init_self__(self):
        tensor_handle = HVMArrayHandle()
        handle = self.handle
        if not isinstance(handle, ctypes.c_void_p):
            handle = ctypes.c_void_p(handle)
        check_call(_LIB.HerculesGetDLTensor(handle, ctypes.byref(tensor_handle)))
    ...
    def __setstate__(self, state):
        ...
        handle, code = _ffi.hvm_script_api.steal_object_handle(arr)
        self.handle = handle
        self.type_code = code
        self.__init_self__()
    ...
    def to_list(self):
        """Convert a NDArray to a hvm.List corresponding to the shape
    ...
    def device(self):
        ...
        if self.tensor_handle.contents.device.device_type == 1:
            return "cpu"
        return "%s:%s" % (
            HerculesDevice.MASK2STR[self.tensor_handle.contents.device.device_type],
    ...
    def from_numpy(self, source_array):
        """Copy data from a numpy.ndarray to the current NDArray, requiring both to have the same size.
           Note! This method cannot be compiled for use in hvm.script
        ...
        Examples:
            ...
            >>> nd = hvm.NDArray([], arr.shape, str(arr.dtype))
            >>> nd.from_numpy(arr)
            [
        ...
        check_call(_LIB.HerculesArrayCopyFromBytes(self.tensor_handle, data, nbytes))
        return self
    ...
    def asnumpy(self):
        ...
        check_call(_LIB.HerculesArrayCopyToBytes(self.tensor_handle, data, nbytes))
        return np_arr
    ...
    #    check_call(
    #        _LIB.HerculesArrayCopyFromTo(self.tensor_handle, target_nd.tensor_handle, None)
    #    )
    #    return target_nd
...
#        if isinstance(target, HerculesDevice):
...
#            raise TypeError('type %s not supported' % str(type(value)))
    ...
    def release(self):
        _ffi.hvm_script_api.release_object_handle(self)

    def to_dlpack(self):
        return _ffi.hvm_script_api._to_dlpack(self)
...
def add(lhs, rhs):
    ...
    Specific use of the interface: hvm.array.add

    Args:
        lhs (hvm.NDArray or number): Left operand
        rhs (hvm.NDArray or number): Right operand
    ...
    Examples:
        >>> import hvm
        >>> nd1 = hvm.array.rand([2, 3]) # Analogous to numpy.random.rand(2, 3)
        >>> nd1
        [
...
def sub(lhs, rhs):
    ...
    Specific use of the interface: hvm.array.sub
...
def div(lhs, rhs):
    ...
    Specific use of the interface: hvm.array.div
...
def mul(lhs, rhs):
    ...
    Specific use of the interface: hvm.array.mul
...
def concatenate(seq, axes=0):
    ...
    Specific use of the interface: hvm.array.concatenate

    Args:
        seq (List[hvm.NDArray] or Tuple[hvm.NDArray])
    ...
    Examples:
        >>> import hvm
        >>> nd = hvm.NDArray([0, 1, 2, 3, 4, 5, 6, 7], [2, 2, 2], "int32")
        >>> hvm.array.concatenate([nd, nd], 0)
        [
         [
          [ 0 1 ]
          [ 2 3 ]
...
def stack(seq, axes=0):
    ...
    Specific use of the interface: hvm.array.stack
...
#    """Constructs the HVM context with the given dev_type dev_id.
#       Note! This method cannot be compiled for use in hvm.script
#
#    Args
...
#    Examples
#        >>> import hvm
#        >>> assert hvm.context("cpu", 1) == hvm.cpu(1)
#        >>> assert hvm.context("gpu", 0) == hvm.gpu(0)
#        >>> assert hvm.context("cuda", 0) == hvm.gpu(0)
#    """
#    if isinstance(dev_type, string_types):
#        if '-device=micro_dev' in dev_type:
#            dev_type = HerculesDevice.STR2MASK['micro_dev']
#        else:
#            dev_type = dev_type.split()[0]
#            if dev_type not in HerculesDevice.STR2MASK:
#                raise ValueError("Unknown device type %s" % dev_type)
#            dev_type = HerculesDevice.STR2MASK[dev_type]
#    return HerculesDevice(dev_type, dev_id)
#
#
# def cpu(dev_id=0):
...
#        HerculesDevice
#    """
#    return HerculesDevice(1, dev_id)
#
#
# def gpu(dev_id=0):
...
#        [type]: [description]
#    """
#    return HerculesDevice(2, dev_id)
#
#
# def ndarray(arr: List, shape: List, dtype: str):
#    """Construct the module method of hvm.NDArray
#       Note! This method cannot be compiled for use in hvm.script
#
#    Args:
#        arr (List)
...
def from_numpy(arr, device="cpu"):
    """Construct a module method for hvm.NDArray from numpy.ndarray.
       Note! This method cannot be compiled for use in hvm.script

    Args:
        arr (numpy.ndarray)
        device (HerculesDevice, optional
...
#    """Return a HVMArray representation of a numpy array.
#    """
#    data = np_data
#    assert data.flags['C_CONTIGUOUS']
#    arr = HVMArray()
#    shape = c_array(hvm_shape_index_t, data.shape)
...
#
#    device : HerculesDevice
#        The context of the array
#
#    Returns
#    -------
#    arr : nd.NDArray
#        The array hvm supported.
#    """
#    shape = c_array(hvm_shape_index_t, shape)
...
def from_dlpack(dltensor):
    ...
    """
    return _ffi.hvm_script_api._from_dlpack(dltensor)
