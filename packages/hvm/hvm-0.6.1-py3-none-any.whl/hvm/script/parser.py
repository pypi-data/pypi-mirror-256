...
from .reporter.script_error import HerculesError
...
class HerculesParser(ast.NodeVisitor):
    """Python AST visitor pass which finally lowers it to HVM's IR
    Notes for extension:
    ...
    def visit_JoinedStr(self, node: ast.JoinedStr):
        ...
        curr = visited_node[0]
        span = self.build_span(node)
        string_add = HerculesParser._binop_maker[ast.Add]
    ...
    def visit_Return(self, node):
        ...
        if isinstance(rt_expr, tuple):
            rt_expr = Builtin2Op.lookup("hvm.runtime.container.Tuple")(*rt_expr)
    ...
    def visit_Assign(self, node: ast.Assign):
        ...
        By now 4 patterns of Assign is supported:
            1. special stmts with return value
                1.1 tmp = hvm.List() # hvm.Dict() hvm.Set()...
                1.2 tmp = 3 # scalar type
                1.3 tmp = var
            2. (container ops) List[Expr] = Expr
                2.1 tmp["hi"] = 30 # tmp is hvm.Dict
            ...
            if isinstance(n.func, ast.Attribute):
                if n.func.attr == 'script':
                    if isinstance(n.func.value, ast.Name):
                        ...
                        if mod is HVM_MODULE and n.args:
            ...
            if isinstance(n.func, ast.Attribute):
                if n.func.attr in ('pmap', 'pstarmap', 'apply_async'):
                    if isinstance(n.func.value, ast.Name):
                        ...
                        if mod is HVM_MODULE:
                            return True
            return False

        # ignore nested hvm.script
    ...
    def visit_BinOp(self, node):
        ...
        if not isinstance(node.op, tuple(HerculesParser._binop_maker.keys())):
            self.report_error('This operator is not available', NotImplementedError)
        op = HerculesParser._binop_maker[type(node.op)]
        span = self.build_span(node)
    ...
    def visit_Compare(self, node):
        ...
        for i in range(len(node.ops)):
            ...
            my_binop = HerculesParser._binop_maker.get(ast_op_node, None)
            if my_binop is None:
    ...
    def visit_BoolOp(self, node):
        ...
        span = self.build_span(node)
        return HerculesParser._boolop_marker[type(node.op)](span, *values)
    ...
    def visit_UnaryOp(self, node):
        ...
        if not isinstance(node.op, tuple(HerculesParser._unaryop_maker.keys())):
            self.report_error('This operator is not supported', NotImplementedError)
        span = self.build_span(node)
        return HerculesParser._unaryop_maker[type(node.op)](operand, span)
    ...
    def visit_Attribute(self, node: ast.Attribute):
        ...
        if inspect.ismodule(symbol):
            # lookup builtin module: hvm unicodedata and so on...
            ...
            if inspect.isclass(module_attr) and issubclass(module_attr, NativeClass):
                return op_wrapper(_ir.op.hvm_make_native_object, node.attr)
            if isinstance(module_attr, NativeFunction):
                return op_wrapper(_ir.op.hvm_call_native_function, node.attr)
            if inspect.isclass(module_attr) and issubclass(module_attr, OpKernel):
                ...
                def wrapped_native_op(span, *args, **kwargs):
                    return _ir.op.hvm_make_native_op(span, module_attr, *args, **kwargs)
    ...
    def visit_Name(self, node: ast.Name):
        ...
        if global_dep is not NAME_NOT_FOUND:
            ...
            if isinstance(global_dep, OpKernel):
                ...
                def wrapped_native_op(span, *args, **kwargs):
                    return _ir.op.hvm_make_native_op(span, global_dep, *args, **kwargs)
...
def _make_user_function(value, resource_handle, span):
    func_addr = "(HerculesBackendPackedCFunc)" + value.context.unbound_name + "__c_api"
...
def _make_user_class_creator(value, resource_handle, span):
    ...
    func_addr = "(HerculesBackendPackedCFunc)" + unbound_name + "__c_api"
    value = _ir.call_extern(_ir.UserDataType(),
                            "MakeUserFunction",
                            span,
