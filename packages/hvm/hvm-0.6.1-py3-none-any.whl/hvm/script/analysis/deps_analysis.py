...
class DepsAnalysis(ast.NodeVisitor):
    ...
    def try_to_add_dependency(self, dep, dep_node) -> bool:
        ...
        if (root_mod in self.SKIP_MODULES
                and not belong_to_module(dep, self.HVM_MODULE.tools)):
            return False
        if dep in self.SKIP_OBJECTS:
            return False
        if not isinstance(dep, (type, types.FunctionType)):
            return False
        if getattr(dep, "__HVM_NATIVE_FUNCTION__", None):
            return False
        if getattr(dep, "__HVM_NATIVE_OBJECT__", None):
            return False
        if getattr(dep, "__HVM_NATIVE_OP__", None):
            return False
        if dep is abc.ABC:
            return False
