...
hvm_shape_index_t = ctypes.c_int64


class TypeIndex(object):
    """global type index"""
    kRuntimeNullptr = -1
    kRuntimeOpaqueHandle = -2
    kRuntimeInteger = -3
    kRuntimeFloat = -4
    ...
    kRuntimeDataType = -6
    kRuntimeContext = -7
    kRuntimeDLTensorHandle = -8
    kRuntimePackedFuncHandle = -9
    kRuntimeObjectRValueRefArg = -10
    kRuntimeString = -11
    kRuntimeUnicode = -12
    ...
    kRuntimeSet = 9
    kRuntimeIterator = 10
    kRuntimeBoolGenerator = 11
    kRuntimeInt32Generator = 12
    kRuntimeInt64Generator = 13
    kRuntimeFloat32Generator = 14
    kRuntimeFloat64Generator = 15
    kRuntimeRTValueGenerator = 16
    kRuntimeFile = 17
    kRuntimeTrie = 18
    kRuntimeRegex = 19
    kRuntimeUserData = 20
...
class ArgTypeCode(object):
    ...
    INT = TypeIndex.kRuntimeInteger
    FLOAT = TypeIndex.kRuntimeFloat
    HANDLE = TypeIndex.kRuntimeOpaqueHandle
    NULL = TypeIndex.kRuntimeNullptr
    TVM_TYPE = TypeIndex.kRuntimeDataType
    TVM_CONTEXT = TypeIndex.kRuntimeContext
    DLTENSOR_HANDLE = TypeIndex.kRuntimeDLTensorHandle
    PACKED_FUNC_HANDLE = TypeIndex.kRuntimePackedFuncHandle

    OBJECT_HANDLE = TypeIndex.kRuntimeObject
    MODULE_HANDLE = TypeIndex.kRuntimeModule

    STR = TypeIndex.kRuntimeUnicode
    BYTES = TypeIndex.kRuntimeString
    NDARRAY_HANDLE = TypeIndex.kRuntimeNDArray
    OBJECT_RVALUE_REF_ARG = TypeIndex.kRuntimeObjectRValueRefArg


class HVMByteArray(ctypes.Structure):
...
class DataType(ctypes.Structure):
    ...
    def __init__(self, type_str):
        ...
        if head.startswith("int"):
            ...
            self.type_code = _ffi_api._datatype_get_type_code(type_name)
            head = head[high + 1:]
        else:
    ...
    def __repr__(self):
        ...
        if self.type_code in DataType.CODE2STR:
            ...
            from ..runtime import _ffi_api
            type_name = "custom[%s]" % \
                        _ffi_api._datatype_get_type_name(self.type_code)
...
class HerculesDevice(ctypes.Structure):
    """HerculesDevice strucure."""
    ...
    def __init__(self, device_type, device_id):
        super(HerculesDevice, self).__init__()
    ...
    def _GetDeviceAttr(self, device_type, device_id, attr_id):
        ...
        # pylint: disable=import-outside-toplevel
        import hvm.runtime._ffi_api
        return hvm.runtime._ffi_api.GetDeviceAttr(
    ...
    def sync(self):
        ...
        check_call(_LIB.HVMSynchronize(self.device_type, self.device_id, None))

    def __eq__(self, other):
        return (isinstance(other, HerculesDevice) and
    ...
    def __repr__(self):
        if self.device_type >= RPC_SESS_MASK:
            ...
            return "remote[%d]:%s(%d)" % (
                tbl_id, HerculesDevice.MASK2STR[dev_type], self.device_id)
        return "%s(%d)" % (
            HerculesDevice.MASK2STR[self.device_type], self.device_id)


class HVMArray(ctypes.Structure):
    """HVMValue in C API"""
    _fields_ = [("data", ctypes.c_void_p),
                ("device", HerculesDevice),
                ("ndim", ctypes.c_int),
                ("dtype", DataType),
                ("shape", ctypes.POINTER(hvm_shape_index_t)),
                ("strides", ctypes.POINTER(hvm_shape_index_t)),
...
HVMArrayHandle = ctypes.POINTER(HVMArray)
HVMTensorHandle = ctypes.POINTER(HVMArray)
