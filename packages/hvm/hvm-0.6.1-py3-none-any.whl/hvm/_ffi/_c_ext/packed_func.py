...
from ._loader import hvm_script_api
...
PackedFuncBase = hvm_script_api.PackedFuncBase
...
def _set_class_module(module_class):
    ...
    def _conv(mod: _CLASS_MODULE):
        ...
        return hvm_script_api.make_any(ArgTypeCode.MODULE_HANDLE, 0, handle, 0)
...
def _set_class_object_generic(object_generic_class, func_convert_to_object):
    ...
    hvm_script_api.register_input_callback(_CLASS_OBJECT_GENERIC, _FUNC_CONVERT_TO_OBJECT)
...
def _get_global_func(name, allow_missing=False):
    return hvm_script_api.get_global_func(name, allow_missing)
...
def to_packed_func(py_func):
    ...
    """
    return hvm_script_api.convert_to_packed_func(py_func)


def _register_input_callback(cls, callback):
    hvm_script_api.register_input_callback(cls, callback)


def _rvalue_ref_conv(rvalue: ObjectRValueRef):
    handle, code = hvm_script_api.steal_object_handle(rvalue.obj)
    return hvm_script_api.make_any(code, 0, handle, 1)


_register_input_callback(ObjectRValueRef, _rvalue_ref_conv)
hvm_script_api.set_packedfunc_creator(_handle_return_func)
hvm_script_api.register_object(ArgTypeCode.MODULE_HANDLE, _return_module)
