...
def _load_cuda_lib(base_lib):
    ...
    if sys.platform.startswith('win32'):
        cuda_lib_name = "libhvm_cuda.dll"
    elif sys.platform.startswith('darwin'):
        cuda_lib_name = "libhvm_cuda.dylib"
    else:
        cuda_lib_name = "libhvm_cuda.so"
...
def register_error(func_name=None, cls=None):
    ...
    .. code-block:: python
      ...
      @hvm.error.register_error
      class MyError(RuntimeError):
          pass

      err_inst = hvm.error.create_ffi_error("MyError: xyz")
...
def get_last_ffi_error():
    """Create error object given result of HVMAPIGetLastError.
    ...
    if err_type is not None and err_type.startswith("hvm.error."):
