...
def _hvm_codegen_demangle(fname):
    c_api = '__c_api'
...
def _trace_to_python(cc_stack):
    ...
    for cc_frame in reversed(cc_stack):
        ...
        func = _hvm_codegen_demangle(func)
...
def _trans_message(what):
    ...
    bt_pat_runtime = re.compile(r".*hercules/src.*")
    while i < len(lines):
        line = lines[i].strip()
...
def trans_exception(e: Exception, use_cc_stacktrace: bool):
    trans_state = getattr(e, "__IS_HVM_TRANS_EXCEPTION__", False)
    if trans_state:
    ...
    if ty_pos != -1:
        ...
        if issubclass(ex_ty, BaseException):
            ...
            py_exc.__IS_HVM_TRANS_EXCEPTION__ = True
...
def trans_exception_from_c_to_py(f):
    ...
    def wrap(*args, **kwargs):
        try:
            ...
            counter.set('hvm_runtime_error', str(ex))
            counter.set('hvm_runtime_error_counter', 1)
            counter.flush()
            raise ex from None
