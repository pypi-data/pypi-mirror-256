...
def register_object(type_key=None, callback=None):
    ...
    .. code-block:: python
      ...
      @hvm.register_object("test.MyObject")
    ...
    def register(cls):
        ...
        if hasattr(cls, "_type_index"):
            ...
            if not _RUNTIME_ONLY:
                check_call(_LIB.HerculesObjectTypeKey2Index(
                    c_str(object_name), ctypes.byref(tidx)))
            else:
                # directly skip unknown objects during runtime.
                ret = _LIB.HerculesObjectTypeKey2Index(
                    c_str(object_name), ctypes.byref(tidx))
                if ret != 0:
                    return cls
            tindex = tidx.value
...
def register_func(func_name, f=None, override=False):
    ...
    .. code-block:: python
      ...
      f = hvm.get_global_func("my_packed_func")
      assert isinstance(f, hvm.PackedFunc)
      y = f(*targs)
      assert y == 10
    """
    ...
    def register(myf):
        ...
        check_call(_LIB.HerculesFuncRegisterGlobal(
...
def list_global_func_names():
    ...
    check_call(_LIB.HerculesFuncListGlobalNames(ctypes.byref(size),
                                                  ctypes.byref(plist)))
    fnames = []
