...
def hvm_math_exp2(span, x):
    ...
    x = convert_type(_cast_to_prim_float,
                     ...
                     span,
                     x)
    return call_intrin(x.dtype, "ir.hvm_math_exp2", span, x)


def hvm_math_exp10(span, x):
    """Calculate 10**x
    ...
    x = convert_type(_cast_to_prim_float,
                     ...
                     span,
                     x)
    return call_intrin(x.dtype, "ir.hvm_math_exp10", span, x)
...
def hvm_math_sigmoid(span, x):
    ...
    y : PrimExpr
        The result.
    """
    return call_intrin(x.dtype, "ir.hvm_math_sigmoid", span, x)
...
def hvm_math_rsqrt(span, x):
    ...
    y : PrimExpr
        The result.
    """
    return call_intrin(x.dtype, "ir.hvm_math_rsqrt", span, x)
...
def hvm_math_nearbyint(span, x):
    ...
    y : PrimExpr
        The result.
    """
    return _ffi_api.hvm_math_nearbyint(x, span)
...
def hlo_call_intrin_template(ret_type, func_name, span, type_args, *args, **kwargs):
    ...
    assert op.num_inputs_max == -1 or num_op_arguments == op.num_inputs_max, \
        "op.arguments != op.num_inputs_max. Please report error to hercules developers."
    new_args = []
    i = 0
...
# hvm function
def hvm_make_native_object(span, native_cls_name, *args):
...
def hvm_make_native_op(span, op_cls, *args, **kwargs):
...
def hvm_make_native_function(span, native_func_name, *args):
...
def hvm_call_native_function(span, native_func_name, *args):
    if isinstance(native_func_name, str):
        native_func_name = native_func_name.encode()
    if isinstance(native_func_name, (bytes, bytearray)):
        native_func_name = StringImm(native_func_name)
...
def hvm_pmap(span, func, data, sess):
...
def hvm_pstarmap(span, func, data, sess):
    input_ty = data.checked_type
    if isinstance(input_ty, _type.TupleType):
        num_inputs = len(input_ty.fields)
        result_type = _type.TupleType([_type.ObjectType()] * num_inputs)
...
def hvm_apply_async(span, func, *args):
    func_ty = func.checked_type
    if not isinstance(func_ty, _type.UserDataType):
        func = smart_adapt_to(func, _type.UserDataType(), span)
    result_type = _type.ObjectType()
