...
namespace runtime {
// clang-format off
HVM_REGISTER_NATIVE_OP(TorchInferOp).SetThreadSafety(false);
// clang-format on
...
RTValue TorchInferOp::Process(PyArgs inputs) const {
  torch::NoGradGuard no_grad;
#ifdef HVMSCRIPT_PYTHON_MODE
  if (th_model_ && th_model_->example.is_nullptr()) {
    // for bundle example data
    th_model_->example = Tuple(inputs.begin(), inputs.end());
  }
#endif  // HVMSCRIPT_PYTHON_MODE
#ifdef HVM_ENABLE_TORCH_MODEL_AUTO_SYNCHRONIZATION_WITH_PREPROCESS
