...
CMAKE_DIR = os.path.join(HVM_DIR, "extension/cpp/tensorflow")
if not os.path.exists(os.path.join(HVM_DIR, 'include')):
    # for development
    HVM_DIR = os.path.abspath(os.path.join(BASE_DIR, "../../../../"))
    CMAKE_DIR = os.path.join(HVM_DIR, "python/hvm/extension/cpp/tensorflow")
if sys.platform.startswith('win32'):
    HVM_TF_LIB_NAME = 'libhvm_tensorflow+tensorflow{}.dll'.format(tf.__version__)
elif sys.platform.startswith('darwin'):
    HVM_TF_LIB_NAME = 'libhvm_tensorflow+tensorflow{}.dylib'.format(tf.__version__)
else:
    HVM_TF_LIB_NAME = 'libhvm_tensorflow+tensorflow{}.so'.format(tf.__version__)
...
def build_with_cmake():
    try:
        ...
        raise RuntimeError(
            "cmake not found by Hercules TensorFlow extension, please install it!!!"
        ) from None
    ...
    if TX_CXX11_ABI_FLAG != TF_CXX11_ABI_FLAG:
        raise RuntimeError(
            ...
            f"but Hercules define _GLIBCXX_USE_CXX11_ABI={TX_CXX11_ABI_FLAG}"
        ) from None
    ...
    curdir = os.getcwd()
    build_dir = tempfile.TemporaryDirectory(prefix="hvmscript_tensorflow_build")
    print(f"[BUILD DIRECTORY]: {build_dir}")
    os.chdir(build_dir.name)
    cmake_cmd = f'''
    cmake \
    -DCMAKE_HVM_TF_COMPILE_FLAGS="{all_compile_flags}" \
    -DCMAKE_HVM_TF_LIB_PATHS="{all_lib_paths}" \
    -DCMAKE_HVM_TF_LIB_NAMES="{all_lib_names}" \
    ...
    try:
        ...
        errmsg = 'internal error: build libhvm_tensorflow failed!!!'
        assert ret == 0 and os.path.exists(HVM_TF_LIB_NAME), errmsg
        cp_cmd = 'cp {} {}'.format(HVM_TF_LIB_NAME, HVM_USER_DIR)
