...
import numpy as np
import hvm
from hvm import pipeline
from hvm.pipeline import _ffi_api, make_op_creator_function
from hvm.pipeline.symbol import Symbol, Variable, Constant, BaseSymbol, make_symbol
from hvm.pipeline._tracing_state import tracing
...
class PytorchModule(object):
    ...
    def __init__(
        ...
        # init hvm cpp resource name and location
        if location is None:
            location = f"./torch_jit_{timestamp}.jit"

        # init hvm cpp pass op and resource
    ...
    def _trans_(self, data):
        ...
        elif isinstance(data, (tuple, hvm.Tuple)):
            raise NotImplementedError("Tuple is not supported as model input")
        elif isinstance(data, (list, hvm.List)):
            return self._trans_list_(data)
        elif isinstance(data, (dict, hvm.Dict)):
            return self._trans_dict_(data)
        elif isinstance(data, (set, hvm.Set)):
            return self._trans_set_(data)
        else:
            if isinstance(data, hvm.array.NDArray):
    ...
    def __call__(self, *args):
        with self._torch.no_grad():
            ...
            if "example" not in self._pass_r_options:
                ...
                example = hvm.to_runtime_object(tuple(model_args))
    ...
    def make_pipeline_op(self):
        py_callable = hvm.to_packed_func(self)
