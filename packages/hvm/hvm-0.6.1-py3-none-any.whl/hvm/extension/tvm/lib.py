...
def _load_lib():
    global _LOAD_HVM_TVM
    global _HVM_TVM_LIB
    if _LOAD_HVM_TVM:
        return
    from .build import HVM_TVM_LIB_NAME
    _HVM_TVM_LIB_PATH = os.path.join(HVM_USER_DIR, HVM_TVM_LIB_NAME)
    curdir = os.getcwd()
    libdir = os.path.abspath(os.path.dirname(__file__))
    os.chdir(libdir)
    try:
        _HVM_TVM_LIB = ctypes.CDLL(_HVM_TVM_LIB_PATH, ctypes.RTLD_LOCAL)
    finally:
        os.chdir(curdir)
    _LOAD_HVM_TVM = True
...
def compile_or_load_lib(silent=True):
    ...
    _HVM_TVM_LIB_PATH = os.path.join(HVM_USER_DIR, HVM_TVM_LIB_NAME)
    try:
        with contrib.util.filelock(_HVM_TVM_LIB_PATH, timeout=300):
            try:
                _load_lib()
            except:
