...
addLoggingLevel('HVM_INFO', logging.INFO - 5)

LIB_PATH = os.environ.get('HVM_DSO_DIR', 'dso')

USE_SO_CACHE = os.environ.get('HVM_USE_SO_CACHE', '').lower() != 'false'

DISABLE_SCRIPT = os.environ.get('HVM_DISABLE_SCRIPT', '').lower() == 'true'
DISABLE_GENERATE_CC = os.environ.get('HVM_DISABLE_GENERATE_CC', '').lower() == 'true'
        ...
        if is_function:
            ...
            if bundle_args is None:
                ...
                if not isinstance(bundle_args, (list, set, tuple)):
                    raise TypeError("hvm.script: bundle_args must be None or List")
            ...
            logging.hvm_info('need_bundle: %s', need_bundle)
            try:
...
def path_prefix(sc_ctx: context.ScriptContext):
    ...
    _mk_lib_dir()
    # code + sha1(libhercules.so) + commit_id(__version__)
...
def toolchain_path_prefix(sc_ctx: context.ScriptContext, toolchain_str: str):
    ...
    # mkdir LIB_PATH
    _mk_lib_dir()
    # code + sha1(libhercules.so) + commit_id(__version__) + toolchain_str
    dep_source_codes = "".join(dep_node.span.source_code for dep_node in sc_ctx.deps_node)
...
def toolchain_build(sc_ctx: context.ScriptContext, toolchain: ToolChain):
    ...
    with contrib.util.filelock(base_path):
        ...
        if not hit_cache(so_path):
            logging.hvm_info(
                "hvm compile function/class: [{}:{}]".format(main_node_name, so_path))
...
def build_dso(sc_ctx: context.ScriptContext, use_toolchain=False):
    ...
    with contrib.util.filelock(base_path):
        sopath = base_path + '.so'
        sopath_cxx00 = base_path + '_cxx00.so'
        ...
        if not hit_cache(sopath):
            logging.hvm_info("hvm compile function/class: [{}:{}]".format(main_node_name, sopath))
            rt_mod.export_library(sopath, options=cxx11_with_abi_options, cc=sys_cc_path)
        else:
            logging.hvm_info(
                "info matched, skip compiling: [{}:{}]".format(
        ...
        if not use_toolchain:
            ...
            if server_cc_path is not None:
                ...
                if not hit_cache(sopath_cxx00):
                    logging.hvm_info(
                        "hvm compile function/class: [{}:{}]".format(main_node_name, sopath_cxx00))
                    rt_mod.export_library(
                        sopath_cxx00,
                        options=cxx11_no_abi_options,
                        cc=server_cc_path)
                else:
                    logging.hvm_info(
                        "info matched, skip compiling: [{}:{}]".format(
                            main_node_name, sopath_cxx00))
            else:
                msg_t = "hvm compile \"{}\": server gcc not found, will disable build \"{}\""
                logging.warning(msg_t.format(main_node_name, os.path.basename(sopath_cxx00)))
                sopath_cxx00 = ""
        else:
            sopath_cxx00 = ""

        sc_ctx.dso_path = (sopath_cxx00, sopath)
...
def script(compiling_obj, *, share=False, toolchain=None, bundle_args=None):
    ...
    simple class, compile it to a hvm4 object which mostly
