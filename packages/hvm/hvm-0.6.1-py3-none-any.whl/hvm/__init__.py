...
def pmap(func, data):
    ...
    if tracing():
        ...
        if not isinstance(func, (pipeline.ops.OpKernel, runtime.object.ObjectBase)):
            raise TypeError(f"hvm.pmap: the first argument '{func}' is not a traceable op")
        if not isinstance(
                data, (pipeline.symbol.BaseSymbol, runtime.List, runtime.Tuple, list, tuple)
        ):
            raise TypeError(f"hvm.pmap: the second argument '{data}' is not supported")
...
def pstarmap(func, data):
    ...
    if tracing():
        ...
        if not isinstance(func, (pipeline.ops.OpKernel, runtime.object.ObjectBase)):
            raise TypeError(f"hvm.pstarmap: the first argument '{func}' is not a traceable op")
        if not isinstance(
                data, (pipeline.symbol.BaseSymbol, runtime.List, runtime.Tuple, list, tuple)
        ):
            raise TypeError(f"hvm.pstarmap: the second argument '{data}' is not supported")
...
def script(compiling_obj, *args, backend=None, **kwargs):
    ...
    simple class, compile it to a hvm object which mostly
