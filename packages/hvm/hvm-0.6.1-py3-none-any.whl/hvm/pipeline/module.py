...
class JITModule(Module):
    ...
    def trace(self, sym):
        ...
        if isinstance(sym, tuple) or isinstance(sym, list):
            ...
            raise RuntimeError(
                f"Type {type(sym)} cannot be traced. Only the values returned by hvm op can be traced. "
            )
...
def LoadModule(folder, name, device):
    warnings.warn("The function hvm.pipeline.LoadModule is deprecated.", DeprecationWarning)
