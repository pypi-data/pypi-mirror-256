...
from hvm.pipeline.symbol import Symbol, Variable as Var
from hvm.pipeline.symbol import Constant as Const
...
def TraceImpl(func, *args, **kwargs):
    ...
    ----------
    func : callable
        A Python function or a hvm Symbol(s) that will be run with `args`.
...
def trace(func, *args, **kwargs):
    ...
    try:
        ...
        # print(e)
        counter.set('hvm_trace_error_counter', 1)
        if HVM_DEV_MODE:
            counter.set('hvm_trace_error', str(e))
            counter.flush()
            raise
        else:
            ex, trans_state = trans_exception(e, use_cc_stacktrace=False)
            counter.set('hvm_trace_error', str(ex))
            counter.flush()
            if trans_state:
...
_HVM_GLOBAL_PLUGINS_ = list()


def load_ops(lib_paths):
    """Load extend ops from dynamic lib and register to hvm
    ...
    for op_dll in lib_paths:
        ...
        os.chdir(cwd)
        _HVM_GLOBAL_PLUGINS_.append(lib)
