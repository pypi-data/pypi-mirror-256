...
SPARK_CONF_DEPENDENCY_WARNING = "cdh_lava_demo.dbx_accelerator.dependency.warning"
SPARK_CONF_DEPRECATION_LOGGING = "cdh_lava_demo.dbx_accelerator.deprecation.logging"
...
def check_deprecation_logging_enabled():
    import cdh_lava_demo.dbx_accelerator.common
    from cdh_lava_demo.dbx_accelerator.dbgems.mock_dbutils_class import MockDBUtils

    if spark is None:
        return
    status = get_spark_config(SPARK_CONF_DEPRECATION_LOGGING, default=None)
    if status is None:
        cdh_lava_demo.dbx_accelerator.common.deprecation_log_level = "ignore"
    elif status.lower() == "enabled":
        cdh_lava_demo.dbx_accelerator.common.deprecation_log_level = "warn"
    else:
        cdh_lava_demo.dbx_accelerator.common.deprecation_log_level = status.lower()
...
def validate_dependencies(module: str, curriculum_workspaces_only=True) -> bool:
    ...
    from cdh_lava_demo.dbx_accelerator.clients import github
    from cdh_lava_demo.dbx_accelerator import common
...
def proof_of_life(
    ...
    import dbruntime
    import pyspark
    from cdh_lava_demo.dbx_accelerator.common.cloud import Cloud
