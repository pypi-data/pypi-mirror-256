...
class ApiContainer(object):
    ...
    def _register_api(self, client, **modules: str) -> None:
        ...
        >>> self._register_api("budgets", "cdh_lava_demo.dougrest.accounts.budgets:Budgets")
        >>> self.budgets
        """
        import importlib
...
class ApiClient(ApiContainer):
        ...
        *,
        ...
        if authorization_header:
            ...
            authorization_header = (
                "Basic " + base64.standard_b64encode(encoded_auth).decode()
            )
        elif client is not None:
            authorization_header = client.session.headers["Authorization"]
        else:
            raise ValueError(
                "Must specify one of token, password, or authorization_header"
            )
        ...
        if throttle_seconds > 0:
            ...
            print(
                f"** WARNING ** Requests are being throttled by {throttle_seconds} second{s} per request."
            )
        ...
        retry = Retry(
            connect=Retry.BACKOFF_MAX / backoff_factor, backoff_factor=backoff_factor
        )
    ...
    def api_raw(
        ...
        *,
        ...
        if endpoint_path.startswith(self.url):
            ...
            raise ValueError(
                f"endpoint_path must be relative url, not {endpoint_path!r}."
            )
        ...
        if http_method == "GET":
            ...
            response = self.session.request(
                http_method, url, params=params, timeout=timeout
            )
        else:
            # if self.verbose: print(json.dumps(data, indent=4))
            response = self.session.request(
                http_method, url, data=json.dumps(data), timeout=timeout
            )
    ...
    def _raise_for_status(
        response: Response, expected: Union[int, Container[int]] = None
    ) -> None:
        """
        ...
        http_error_msg = (
            f"{response.status_code} {error_type}: {reason} for url: {response.url}"
        )
...
class DatabricksApiException(Exception):
    def __init__(self, message=None, http_code=None, http_exception=None):
        ...
        if http_exception:
            ...
            try:
                ...
                self.message = self.body.get(
                    "message", self.body.get("error", http_exception.response.text)
                )
    ...
    def __repr__(self):
        return (
            f"DatabricksApiException(message={self.message!r}, "
