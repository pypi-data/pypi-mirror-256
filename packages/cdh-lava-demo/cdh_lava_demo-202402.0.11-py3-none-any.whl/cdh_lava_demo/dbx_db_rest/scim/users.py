...
class ScimUsersClient(ApiContainer):
    ...
    def list(self):
        response = self.client.execute_get_json(
            f"{self.client.endpoint}/api/2.0/preview/scim/v2/Users"
        )
        users = response.get("Resources", list())
        total_results = response.get("totalResults")
        assert len(users) == int(
            total_results
    ...
    def to_users_list(self, users):
        ...
        if users is None:
            ...
            assert (
                type(users) == list
            ), f'Expected the parameter "users" to be a list, found {type(users)}'
    ...
    def add_entitlement(self, user_id, entitlement):
        payload = {
            ...
            "Operations": [
                {"op": "add", "path": "entitlements", "value": [{"value": entitlement}]}
            ],
        }
        url = f"{self.client.endpoint}/api/2.0/preview/scim/v2/Users/{user_id}"
