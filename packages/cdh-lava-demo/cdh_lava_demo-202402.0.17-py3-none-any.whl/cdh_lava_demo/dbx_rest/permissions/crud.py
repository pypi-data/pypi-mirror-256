...
class PermissionsCrud(ApiContainer):
    ...
    def _validate_what(what: What):
        if what not in valid_whats:
            raise ValueError(
                f"Expected 'permission_level' to be one of {valid_whats}, found '{what}'"
            )

    def _validate_permission_level(self, permission_level: PermissionLevel):
        if permission_level not in self.valid_permissions:
            raise ValueError(
                f"Expected 'permission_level' to be one of {self.valid_permissions},"
    ...
    def update(self, id, what: What, value: str, permission_level: PermissionLevel):
        ...
        acl = [{what: value, "permission_level": permission_level}]
        return self.client.api_simple(
            "PATCH", f"{self.path}/{id}", access_control_list=acl
        )

    def replace(self, id, acl: ACL):
        return self.client.api_simple(
            "PUT", f"{self.path}/{id}", access_control_list=acl
        )
    ...
    def update_service_principal(self, id, service_principal_name, permission_level):
        return self.update(
            id, "service_principal_name", service_principal_name, permission_level
        )
