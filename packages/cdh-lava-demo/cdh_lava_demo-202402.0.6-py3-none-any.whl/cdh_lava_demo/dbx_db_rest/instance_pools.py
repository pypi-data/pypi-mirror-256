...
class InstancePoolsClient(ApiContainer):
    ...
    def get_by_id(self, instance_pool_id):
        return self.client.execute_get_json(
            f"{self.base_uri}/get?instance_pool_id={instance_pool_id}"
        )
    ...
    def list(self):
        ...
        return self.client.execute_get_json(f"{self.base_uri}/list").get(
            "instance_pools", []
        )
    ...
    def create(self, name: str, definition: dict, tags: list = None):
        from cdh_lava_demo.rest.dbgems_fix import dbgems
        ...
        for tag in [] if tags is None else tags:
            key_value = {"key": tag[0], "value": tag[1]}
        ...
        if dbgems.get_cloud() == "AWS":
            ...
            aws_attributes["availability"] = aws_attributes.get(
            ...
            definition["node_type_id"] = definition.get(
        ...
        pool = self.client.execute_post_json(
            f"{self.base_uri}/create", params=definition
        )
    ...
    def update_by_id(
        ...
        ), f"Expected id to be of type str, found {type(instance_pool_id)}"
        assert (
            instance_pool_name is None or type(instance_pool_name) == str
        ), f"Expected name to be of type str, found {type(instance_pool_name)}"
        assert (
            min_idle_instances is None or type(min_idle_instances) == int
        ), f"Expected min_idle_instances to be of type int, found {type(min_idle_instances)}"
        assert (
            max_capacity is None or type(max_capacity) == int
        ), f"Expected max_capacity to be of type int, found {type(max_capacity)}"
        assert (
            idle_instance_autotermination_minutes is None
            or type(idle_instance_autotermination_minutes) == int
        ), f"Expected idle_instance_autotermination_minutes to be of type int, found {type(idle_instance_autotermination_minutes)}"
