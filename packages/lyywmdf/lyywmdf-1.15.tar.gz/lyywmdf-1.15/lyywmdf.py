...
def calc_lastdate_kline_number(code, last_date_dict, debug=False):
    """
...
def mk_api_dcit(svlist, debug=False) -> list:
    api_dict = {}
    if debug:
...
def test_servers_speed(ip, debug=False):
    ...
    result = api.get_security_bars(9, 2, "430510", 0, 1)
    if len(result) < 1:
        return 9
...
def perfact_new_fast_server_list(cache_file=".", interval=24, nextfuntion=None, timeout=0.6, debug=False):
    """
    ...
    def read_cache(cache_file):
        with open(cache_file, "r", encoding="utf-8") as f:
            ...
            if debug:
                print("fater_server_list=", fater_server_list)
    ...
    if nextfuntion is None:
        print("next function is None,get old cache")
        return read_cache(cache_file)
    server_list = nextfuntion()
    if len(server_list) < 10:
        if debug:
            print("spare plane : server list is too short, try to get more")
        from pytdx.config.hosts import hq_hosts
...
def 通达信下载原始分钟K线(api, 股票数字代码, 要下载的K线数量, ktype="15min", debug=False) -> pd.DataFrame:
    """
    ...
    if debug:
        print("市场代码=", 市场代码, "，股票数字代码=", 股票数字代码, ",", k_type[ktype], ",要下载的K线数量=", 要下载的K线数量)
    # api = TdxHq_API()
    # tdxserverip = "120.79.210.76"
    # api.connect(tdxserverip, 7709)
    # 股票数字代码="873833"

    df_tdx = api.to_df(api.get_security_bars(k_type[ktype], 市场代码, 股票数字代码, 0, 要下载的K线数量))
