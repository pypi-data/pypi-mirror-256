...
class LayerNextClient:
    ...
    def upload_files_to_collection(
        ...
        if annotation_data is not None and annotation_data["json_data_file_path"] is not None and upload_details[
                "is_success"] == True:
            ...
            if annotation_data["operation_unique_id"] is None or not isinstance(annotation_data["is_normalized"],
                                                                                bool) or not isinstance(
                annotation_data["is_model_run"], bool) or annotation_data["operation_unique_id"] == "" or \
    ...
    def attach_model_run_to_project(self,
        ...
        assert isinstance(operation_id_array,
                          list), "operation_id_array must be an array"
        ...
        _studio_client = studio.StudioClient(
            self.encoded_key_secret, self.layernext_url)
        res = _studio_client.attach_model_run_to_project(
    ...
    def auto_tag_collection(
        ...
        res_obj = self.create_or_update_job(session_id, job_name, JobType.GENERATE_AUTO_TAGGING.value, 0,
        ...
        res = _automatic_analysis_client.model_tag_collection(
            collection_id, model_id, input_resolution, confidence_threshold, session_id, job_id, job_name, query,
            filters, inference_platform)
    ...
    def generate_auto_tags_to_metalake(
        ...
        res = _automatic_analysis_client.model_tag_population(
            item_type, model_id, input_resolution, confidence_threshold, session_id, job_id, job_name, query, filters,
            inference_platform)
    ...
    def auto_annotate_project(
        ...
        res_obj = self.create_or_update_job(session_id, job_name, JobType.GENERATE_AUTO_ANNOTATION.value, 0,
                                            JobStatus.QUEUED.value)
    ...
    def generate_embeddings_for_collection(
        ...
        res_obj = self.create_or_update_job(session_id, job_name, JobType.GENERATE_EMBEDDING.value, 0,
                                            JobStatus.QUEUED.value)
    ...
    def download_files_from_collection(self,
        ...
        assert isinstance(custom_download_path,
                          str), "custom_download_path must be a string"
    ...
    def insert_image_embeddings_batch(self, embedding_list: List[Dict[str, List[float]]], model_name: str,
                                      vector_dimension: List[int], session_id: str = ""):
    ...
    def insert_image_embeddings(
        ...
        self.create_or_update_job(session_uuid, job_name, JobType.UPLOAD_EMBEDDING.value, 0, JobStatus.QUEUED.value,
        ...
        for embedding in embedding_list:
            ...
            if count == len(embedding_list) - 1:
                ...
                if ('uniqueName' in embedding and isinstance(embedding['uniqueName'], str)):
                    ...
                    self.create_or_update_job(session_uuid, job_name, JobType.UPLOAD_EMBEDDING.value, 0,
                ...
                self.create_or_update_job(session_uuid, job_name, JobType.UPLOAD_EMBEDDING.value, progress, status,
    ...
    def create_or_update_job(
        ...
        if (not isinstance(session_id, str) or not isinstance(job_name, str) or not isinstance(progress, (
                int, float)) or not isinstance(JobStatus(status), JobStatus) or not isinstance(JobType(job_type),
                                                                                               JobType)):
    ...
    def find_documents(
        ...
        return self.get_item_list_from_datalake(
            item_type='other',
            filter={
                "keywords": keywords,
    ...
    def retrieve_documents(
        ...
        if doc_list and isinstance(doc_list, list) and len(
                doc_list) > 0:  # Check if the list is not empty and get the score of the first document
            if document_search_keys and isinstance(document_search_keys, list) and len(
                    document_search_keys) > 0:  # score exists only if document_search_keys available to do text search
                if page_index == 0:
                ...
                filtered_doc_list = [doc for doc in doc_list if doc[
                    'score'] > 0.25 * first_doc_score]
            else:
        ...
        chunk_dict = self.find_document_chunks(
            search_key_list=chunk_search_keys,
    ...
    def update_virtual_collection(
        ...
        if get_selection_id_success == True:
            ...
            print('v_collection_response: ', {
                "isSuccess": False
            })
            return {
                "isSuccess": False,
                "error": "Failed to get selection id"
            }
    ...
    def find_chunk(
        ...
        """
        print(
            f'find chunk - search_key_list: {search_key_list}, unique_name_list: {unique_name_list}, response_chunk_limit: {response_chunk_limit}, adjacent_chunk_limit: {adjacent_chunk_limit}')

        _datalake_client = datalake.DatalakeClient(
            self.encoded_key_secret, self.layernext_url)

        response = _datalake_client.find_chunk(
            search_key_list, unique_name_list, page_index, response_chunk_limit, adjacent_chunk_limit)
    ...
    def find_document_chunks(
        ...
        """
        print(
            f'find chunk - search_key_list: {search_key_list}, unique_name_list: {unique_name_list}')
    ...
    def execute_mongo_query(self, source_name: str, function, input_data, collection):
        """ 
        ...
        return _datalake_client.execute_mongo_query(source_name, function, input_data, collection)
    ...
    def run_mongodb_aggregation(self, source_name: str, pipeline_stages, collection):
        ...
        """
        return self.execute_mongo_query(source_name, 'aggregate', pipeline_stages, collection)
    ...
    def run_sql_query(self, source_name: str, query: str):
        ...
        return _datalake_client.run_sql_query(source_name, query)
