...
class DatalakeInterface:
    ...
    def upload_metadata_updates(self, meta_updates, operation_type, operation_mode, operation_id, is_normalized,
        ...
        try:
            response = requests.post(
                url=url, params=params, json=payload, headers=hed)
            return response.json()
        # Handle connection error
        except requests.exceptions.ConnectionError as e:
            print("Failed to connect with MataLake")
        ...
        with ThreadPool(5) as p:
            for res in p.imap(self.download_single_file, argument_list):
                ...
                if not is_success and "data" in res:
                    ...
                    print(
                        f"Retrying Download - {download_data[0]['fileName']}")
    ...
    def download_files_from_collection(self, required_info: dict, collection_id: str) -> dict:
        ...
        collection_list = self.get_item_list_from_collection(
    ...
    def update_virtual_collection(self, payload: dict) -> Union[List[dict], dict]:
        ...
        try:
            ...
            if status_code == 200:
                ...
                return {
                    "success": False,
                    "message": "update virtual collection failed"
                }
        # Handle connection error
        except requests.exceptions.ConnectionError as e:
    ...
    def get_data_dictionary(self):
        ...
        try:
            ...
            if status_code == 200:
                ...
                return {"isSuccess": False, "message": "No content return from this API"}
            else:
                return {"isSuccess": False, "message": f'Request return with status code: {status_code}'}
        # Handle connection error
        except requests.exceptions.ConnectionError as e:
            print("Failed to connect with MataLake")
            return {"isSuccess": False, "message": "Failed to connect to MataLake"}
    ...
    def execute_mongo_query(self, payload):
        hed = {'Authorization': 'Basic ' + self.auth_token}
        ...
        try:
            ...
            if status_code == 200:
                ...
                return {
                    "success": False,
                    "message": f"{res['error']['message']}"
                }
            else:
                return {
                    "success": False,
                    "message": f"Failed to execute mongo db query with status code: {status_code}"
                }
        # Handle connection error
        except requests.exceptions.ConnectionError as e:
            print("Connection error from MataLake connection")
    ...
    def run_sql_query(self, payload):
        hed = {'Authorization': 'Basic ' + self.auth_token}
