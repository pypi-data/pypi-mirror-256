...
from pih.tools import ParameterList, nl, j, b, js, e, ne, nn, one, lw
            ...
            if A.D_Ex.subscribtion_result(pl).result:
                ...
                if event in [
                    ...
                    if mailbox_address == A.CT_EML.ADD_EMAIL:
                        secret: int | None = A.D_Ex.decimal(subject) or A.D_Ex.decimal(
                            text
                        )
                        if nn(secret):
                            eventDS: EventDS | None = one(
                        ...
                        if e(event_success_ds):
                            ...
                            if e(event_not_success_ds):
                                ...
                                email_address = A.D.fill_data_from_source(
                                    EmailInformation(), event_not_success_ds.parameters
                                ).email
                                if lw(polibase_person.email) != lw(email_address):
                                    allow_for_notification = A.C.email(
                                        polibase_person.email, check_accesability=True
                                    )
                        else:
                            email_address = A.D.fill_data_from_source(
                                EmailInformation(), event_success_ds.parameters
                            ).email
