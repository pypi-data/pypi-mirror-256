...
def chunking_handler(pillar, iter: Iterable[ChatCompletionChunk], session_id, model):
    ...
    if function_name:
        pillar._write(session_id, f'[tool call] {function_name}({function_arguments})', 'assistant', 'assistant', model, SERVICE_NAME)

    for tool in new_tools.values():
        pillar._write(session_id, f'[tool call] {tool["name"]}({tool["arguments"]})', 'assistant', 'assistant', model, SERVICE_NAME)
    ...
    def hook(wrapped, instance, args, kwargs):
        try:
            ...
            if len(messages) > 0 and messages[0]['role'] == 'system':
                pillar._write(session_id, messages[0]['content'], messages[0]['role'], messages[0]['role'], model, SERVICE_NAME)
            ...
            if len(messages) > 0 and messages[-1]['role'] != 'system':                
                ...
                pillar._write(session_id, messages[-1]['content'], role, author, model, SERVICE_NAME)
            ...
            if message.function_call is not None:
                ...
                pillar._write(session_id, f'[function call] {function_call.name}({function_call.arguments})', message.role, message.role, model, SERVICE_NAME)
