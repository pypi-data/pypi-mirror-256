...
def chunking_handler(pillar, iter: Iterable[ChatCompletionChunk], conversation_id, model):
    message = ''
    function_name = ''
    function_arguments = ''
    new_tools = {}
    ...
    if message:
        pillar._write(conversation_id, message, 'assistant', 'assistant', model, SERVICE_NAME)
    if function_name:
        pillar._write(conversation_id, f'[tool call] {function_name}({function_arguments})', 'assistant', 'assistant', model, SERVICE_NAME)

    for tool in new_tools.values():
        pillar._write(conversation_id, f'[tool call] {tool["name"]}({tool["arguments"]})', 'assistant', 'assistant', model, SERVICE_NAME)
...
def hook_chat_completions(pillar: 'Pillar'):
    def hook(wrapped, instance, args, kwargs):
        try:
            ...
            conversation_id = context_vars.conversation_id.get()
            if conversation_id is None:
                conversation_id = str(uuid.uuid4())

            if len(messages) > 0 and messages[0]['role'] == 'system':
                pillar._write(conversation_id, messages[0]['content'], messages[0]['role'], messages[0]['role'], model, SERVICE_NAME)
            ...
            if len(messages) > 0 and messages[-1]['role'] != 'system':                
                ...
                author = user_id if messages[-1]['role'] == 'user' else role
                pillar._write(conversation_id, messages[-1]['content'], role, author, model, SERVICE_NAME)

            result: ChatCompletion = wrapped(*args, **kwargs)

            if kwargs.get('stream', False):
                return chunking_handler(pillar, result, conversation_id, model)

            message = result.choices[0].message
            if message.content is not None:
                pillar._write(conversation_id, message.content, message.role, message.role, model, SERVICE_NAME)
            
            if message.function_call is not None:
                function_call = message.function_call
                pillar._write(conversation_id, f'[function call] {function_call.name}({function_call.arguments})', message.role, message.role, model, SERVICE_NAME)
