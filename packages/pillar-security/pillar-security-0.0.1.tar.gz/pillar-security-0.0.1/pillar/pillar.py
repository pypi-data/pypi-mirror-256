...
class Pillar:
    ...
    def _check_prompt(self, prompt: str, policy: str = 'default', role: str = 'user'):
        ...
        response = self.requests.post(url, json={            
            'conversation_id': context_vars.conversation_id.get() or str(uuid.uuid4()),
            'message': prompt,
            'policy': policy,
            'author': context_vars.user_id.get() or '',
            'role': role,
    ...
    def _write(self, conversation_id: str, message: str, role: str, author: str, model: str, service: str):
    ...
    @contextmanager
    def session(self, user_id: Optional[str] = None, conversation_id: Optional[str] = None):
        if conversation_id is None:
            conversation_id = str(uuid.uuid4())

        conversation_id_token = context_vars.conversation_id.set(conversation_id)
        token_user_id = context_vars.user_id.set(user_id)
        try:
            yield
        finally:
            context_vars.conversation_id.reset(conversation_id_token)
    ...
    def _initialize_hooks_openai(self):
        ...
        if openai_version < version.parse('1.0.0'):
            self.logger.warn(f'using deperecated openai version {openai_version}, skipping...')
            return
