...
class SchemaTracker:
    ...
    def change_log(self, developmental_release=False, force_release=False):
        new_version = self.generate_new_version(
            developmental_release=developmental_release,
    ...
    def micro_changes_count(self):
        ...
        if not count and (
            self.code_changes() or not self.previous_schema()
        ):
            count = 1

        return count

    def generate_new_version(self, developmental_release=False, force_release=False):
        new_version = None
        ...
        if developmental_release and "dev" not in new_version:
            split_nv = new_version.split(".")

            new_version = f"{split_nv[0]}.{split_nv[1]}.dev{split_nv[2]}"

        return new_version

    def release(self, developmental_release=False, force_release=False):
        new_version = self.generate_new_version(
            developmental_release=developmental_release,
            force_release=force_release
        )
