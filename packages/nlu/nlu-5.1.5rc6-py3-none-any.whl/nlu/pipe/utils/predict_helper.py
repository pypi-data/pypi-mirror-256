...
def __predict__(pipe, data, output_level, positions, keep_stranger_features, metadata, multithread,
    ...
    :return:
    '''
    print('DEBUG DB ENV: PREDICIOTN ON DB ENDPOINT','DB_ENDPOINT_ENV' in os.environ)
    print(data)
    print(type(data))
    import logging
    _logger = logging.getLogger(__name__)
    _logger.warning(['LOGGGER:','DEBUG DB ENV: PREDICIOTN ON DB ENDPOINT','DB_ENDPOINT_ENV' in os.environ])
    _logger.warning(['LOGGGER:',data])
    _logger.warning(['LOGGGER:',type(data)])
    if embed_only:
        pipe.fit()
