...
def __predict__(pipe, data, output_level, positions, keep_stranger_features, metadata, multithread,
    ...
    _logger.log(msg=['LOGGGER:','DEBUG DB ENV: PREDICIOTN ON DB ENDPOINT','DB_ENDPOINT_ENV' in os.environ])
    _logger.log(msg=['LOGGGER:',data])
    _logger.log(msg=['LOGGGER:',type(data)])
    if embed_only:
        pipe.fit()
