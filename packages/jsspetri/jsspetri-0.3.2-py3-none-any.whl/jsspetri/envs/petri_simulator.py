...
class JSSPSimulator(Petri_build):
    ...
    def fire_colored(self, action):
        ...
        standby_id = self.max_nmachines if self.dynamic else self.n_machines
    ...
    def is_terminal(self, dynamic):
        """
        Checks if the simulation has reached a terminal state.
        if dynamicthe state is terminal if all the machines and queues are empty 
        ...
        if dynamic: 
            ...
            to_deliver = [len (job) for job in self.instance]
            delivered = [len(out.token_container) for out in output]
            terminal= all(to_deliver[i] <= delivered[i] for i in range(len(to_deliver)))
    ...
    def interact(self, gui, action):
        ...
        while sum(self.enabled_allocations()) == 1:
            self.fire_timed()
            self.time_tick(gui, action)
            if self.is_terminal(self.dynamic):
                break
