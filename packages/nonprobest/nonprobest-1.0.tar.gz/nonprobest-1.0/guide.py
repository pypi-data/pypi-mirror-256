...
psa_weights = npe.psa_weights(np_sample, p_sample, population_size, covariates = psa_covariates)
kw_weights = npe.kw_weights(np_sample, p_sample, population_size, covariates = psa_covariates)

psa_weights2 = npe.psa_weights(np_sample, p_sample, population_size, covariates = psa_covariates, model = npe.make_preprocess_estimator(BernoulliNB()))
kw_weights2 = npe.kw_weights(np_sample, p_sample, population_size, covariates = psa_covariates, model = XGBClassifier())
...
psa_calibration_weights = npe.calibration_weights(np_sample, population_totals, weights_column = "weights")
...
matching_values = npe.matching_values(np_sample, p_sample, "target_num", covariates = covariates, model = npe.make_preprocess_estimator(MLPRegressor()))
training_values = npe.training_values(np_sample, p_sample, "target_num", covariates = covariates, psa_model = XGBClassifier(enable_categorical = True, tree_method = "hist"), matching_model = XGBRegressor(enable_categorical = True, tree_method = "hist"))
...
dr_estimation = npe.doubly_robust_estimation(np_sample, p_sample, "target_num", covariates = covariates, matching_model = npe.make_preprocess_estimator(KNeighborsRegressor()))

print(naive_estimation, matching_estimation, training_estimation, dr_estimation)

np_sample['target_cat'] = np_sample['target_num'] > 3
training_values = npe.training_values(np_sample, p_sample, "target_cat", True, covariates = covariates)
...
imputed_values = npe.training_values(np_sample, population, target_var, target_category, covariates = covariates)
imputed_values2 = npe.matching_values(np_sample, population, target_var, target_category, covariates = covariates, model = npe.make_preprocess_estimator(KNeighborsClassifier()))
