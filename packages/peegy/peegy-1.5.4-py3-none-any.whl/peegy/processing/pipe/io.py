...
from peegy.io.elipse_tools import parse_eclipse_data
...
class GenericInputData(InputOutputProcess):
    ...
    def get_events(self, events):
        _events = get_event_times(event_channel=events, fs=self.fs)
        return _events


class EclipseReader(InputOutputProcess):
    def __init__(self,
                 file_path: np.array = None,
                 buffer: str = 'A_&_B',
                 figures_subset_folder: str = '') -> InputOutputProcess:
        """
    ...
    def run(self):
        ...
        if self.buffer == 'A':
            ...
            _data = data_buffer_b
            _ch = [ChannelItem(label='Buffer_B')]
            layout = np.array(_ch)
        elif self.buffer == 'A_&_B' or None:
            _data = data
            _ch = [ChannelItem(label='Buffers_A_and_B')]
            layout = np.array(_ch)
            self.buffer == 'A_&_B'
