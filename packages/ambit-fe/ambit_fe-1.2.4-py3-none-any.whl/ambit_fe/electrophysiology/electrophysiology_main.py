...
class ElectrophysiologyProblem(problem_base):
    ...
    def __init__(self, io_params, time_params, fem_params, constitutive_models, bc_dict, time_curves, ioe, mor_params={}, comm=None, solidvar={}):
        ...
        self.numdof = self.phi.vector.getSize()

        # initialize fluid time-integration class
        self.ti = timeintegration.timeintegration_electrophysiology(time_params, self.dt, self.numstep, time_curves=time_curves, t_init=self.t_init, dim=self.dim, comm=self.comm)
        ...
        if not bool(self.solidvar):
            # standard Eulerian fluid
            self.solidvar = {'Fale' : None, 'Fale_old' : None, 'Fale_mid' : None, 'w' : None, 'w_old' : None, 'w_mid' : None}
            self.vf = electrophysiology_variationalform.variationalform(self.var_v, var_p=self.var_p_, n0=self.io.n0, formulation=self.fluid_formulation)

        else:
            # mid-point representation of ALE velocity
            self.solidvar['w_mid']    = self.timefac * self.solidvar['w']    + (1.-self.timefac) * self.solidvar['w_old']
            # mid-point representation of ALE deformation gradient - linear in ALE displacement, hence we can combine it like this
            self.solidvar['Fale_mid'] = self.timefac * self.solidvar['Fale'] + (1.-self.timefac) * self.solidvar['Fale_old']

            # ALE formulation of EP
            self.vf = electrophysiology_variationalform.variationalform_ale(self.var_v, var_p=self.var_p_, n0=self.io.n0, formulation=self.fluid_formulation)

        # initialize boundary condition class
        self.bc = boundaryconditions.boundary_cond_electrophysiology(self.io, fem_params=fem_params, vf=self.vf, ti=self.ti, ki=self.ki)
    ...
    def get_problem_var_list(self):
        ...
        is_ghosted = [1]
        return [self.phi.vector], is_ghosted
    ...
    def set_variational_forms(self):
        ...
        # set mid-point representation (if needed...)
        ...
        for n, M in enumerate(self.domain_ids):
            ...
            self.deltaW_kin     += self.vf.deltaW_kin_navierstokes_transient(self.acc, self.v, self.rho[n], self.io.dx(M), w=self.solidvar['w'], F=self.solidvar['Fale'])
            self.deltaW_kin_old += self.vf.deltaW_kin_navierstokes_transient(self.a_old, self.v_old, self.rho[n], self.io.dx(M), w=self.solidvar['w_old'], F=self.solidvar['Fale_old'])
            self.deltaW_kin_mid += self.vf.deltaW_kin_navierstokes_transient(self.acc_mid, self.vel_mid, self.rho[n], self.io.dx(M), w=self.solidvar['w_mid'], F=self.solidvar['Fale_mid'])


            # internal virtual power
            self.deltaW_int     += self.vf.deltaW_int(self.ma[n].sigma(self.v, self.p_[j], F=self.solidvar['Fale']), self.io.dx(M), F=self.solidvar['Fale'])
            self.deltaW_int_old += self.vf.deltaW_int(self.ma[n].sigma(self.v_old, self.p_old_[j], F=self.solidvar['Fale_old']), self.io.dx(M), F=self.solidvar['Fale_old'])
            self.deltaW_int_mid += self.vf.deltaW_int(self.ma[n].sigma(self.vel_mid, self.pf_mid_[j], F=self.solidvar['Fale_mid']), self.io.dx(M), F=self.solidvar['Fale_mid'])

        # external virtual power (from Neumann or Robin boundary conditions, body forces, ...)
        w_neumann, w_source, w_robin, w_stabneumann, w_robin_valve, w_membrane = ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0)
        w_neumann_old, w_source_old, w_robin_old, w_stabneumann_old, w_robin_valve_old, w_membrane_old = ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0)
        w_neumann_mid, w_source_mid, w_robin_mid, w_stabneumann_mid, w_robin_valve_mid, w_membrane_mid = ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0), ufl.as_ufl(0)
        ...
        self.deltaW_ext     = w_neumann + w_source + w_robin + w_stabneumann + w_membrane + w_robin_valve
        self.deltaW_ext_old = w_neumann_old + w_source_old + w_robin_old + w_stabneumann_old + w_membrane_old + w_robin_valve_old
        self.deltaW_ext_mid = w_neumann_mid + w_source_mid + w_robin_mid + w_stabneumann_mid + w_membrane_mid + w_robin_valve_mid
        ...
        if self.ti.eval_nonlin_terms=='trapezoidal':
            ...
            self.weakform_phi = self.timefac_m * self.deltaW_kin + (1.-self.timefac_m) * self.deltaW_kin_old + \
        ...
        # evaluate nonlinear terms midpoint-like: f(a*u_{n+1} + (1-a)*u_{n})
            ...
            self.weakform_phi = self.deltaW_kin_mid + self.deltaW_int_mid - self.deltaW_ext_mid
