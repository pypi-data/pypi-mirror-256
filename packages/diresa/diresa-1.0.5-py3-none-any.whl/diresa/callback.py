...
class LossWeightAnnealing(Callback):
    ...
    """
    def __init__(self, weight, loss_name, target_loss=0.000003, anneal_step=0.1, start_epoch=5):
        """
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        """
        weight = float(tf.keras.backend.get_value(self.weight))
        if weight > 0.00001:
        ...
        if (epoch + 2) >= self.start_epoch and self.stop_annealing is None:
            tf.keras.backend.set_value(self.weight, weight + self.anneal_step)
        print("Cov/KL Loss weight: {:g}, Validation Reconstruction loss: {:g}, Validation Cov/KL loss: {:g}."
              .format(weight, logs["val_Decoder_loss"], unweighted_loss))
