...
def siamese_twin_model(x, x_twin, encoder, decoder):
    """
    ...
    dist = DistLayer()(x, x_twin, latent, latent_twin)
    output = decoder(latent)
    return Model((x, x_twin), (output, dist))


def diresa_model(x, x_twin, encoder, decoder):
    """
    ...
    latent_twin = encoder(x_twin)
    dist = DistLayer()(x, x_twin, latent_orig, latent_twin)
...
def build_diresa(input_shape=(),
    ...
    return diresa_model(input_orig, input_twin, encoder, decoder)
