...
class joinmeta(type):
    ...
    def new(cls: Type[Self], char: str = NIL, *args, nils: bool = False, **kwargs) -> Type[Self]:        
        def decorator(kls):
            dct = dict(char = char, nils = nils, __doc__ = cls.docs())
    ...
    def __call__(cls: Self, *args, **kwargs):
        if len(args) == 1 and isinstance(args[0], (type, str)): 
...
class justmeta(type):
    ...
    def new(cls: Type[Self], flag: KeepStrs = str.isascii, *args, flip: bool = False, **kwargs) -> Type[Self]:
        def decorator(kls):
            dct = dict(flag = flag, flip = flip, __doc__ = cls.docs())
            new = cls.__class__(kls.__name__, (cls, ), dct, **kwargs)
            return new
        return decorator
    ...
    def __call__(cls: Self, *args, **kwargs):
        if len(args) == 1 and isinstance(args[0], (type, str, Callable)) and cls.flag is None: 
            return cls.new(*args, **kwargs)
