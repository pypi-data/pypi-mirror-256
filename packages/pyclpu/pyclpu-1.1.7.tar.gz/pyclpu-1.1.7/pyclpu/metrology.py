...
class ToF():
    ...
    def __run__(self):
        ...
        islands = np.where(moving_average > self.signaltonoise * noise)[0]
        tips = []
        for l,land in enumerate(list(islands)):
            if l + 1 == islands.size:
                break
            if land + 1 != islands[l + 1]:
                tips.append(islands[l + 1])
        ...
        try:
            ...
            if self.warnings: warning(self.__class__.__name__,"Detect no gap between X-ray peak and peak of massive projectiles. See plot and tune, e.g. signal to noise ratio. Exits after pop-up window is closed.")
