...
class Win(dict):
    ...
    def read(self, file_name="cwannier.win"):
        ...
        Returns:
            dict: dictionary form of seedname.win.
                ...
                - projections*    : projections, {X: [atomic orbitals]}.
        ...
        for i, line in enumerate(win_data_lower):
            ...
            if "begin projections" in line:
                proj_data = win_data[i + 1 : win_data_lower.index("end projections")]
                d["projections"] = self._parse_projections(proj_data)
    ...
    # ==================================================
    def _parse_projections(self, lines):
        lines = [[vi for vi in line.split() if vi != ""] for line in lines]
        ...
        # V:l=2,mr=3,2,5 ! dyz, dzx, dxy
        # O:p;d

        # Mo: dz2; dx2-y2; dxy; dyz; dxz
        # S: pz; px; py
