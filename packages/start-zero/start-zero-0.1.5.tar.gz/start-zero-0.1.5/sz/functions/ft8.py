...
from sz.functions import pair, get_deconv_outsize, get_conv_outsize
from sz.functions.bf import Function
from sz.functions.ft0 import broadcast_to
...
def _im2col_gpu(img, kernel_size, stride, pad):
    ...
    dy, dx = 1, 1
    col = CUDA.to_gpu().cupy.empty((n, c, kh, kw, out_h, out_w), dtype=img.dtype)

    CUDA.to_gpu().cupy.ElementwiseKernel(
        ...
        ''',
        'im2col')(img.reduced_view(), h, w, out_h, out_w, kh, kw, sy, sx, ph, pw, dy, dx, col)
...
def _col2im_gpu(col, sy, sx, ph, pw, h, w):
    ...
    dx, dy = 1, 1
    img = CUDA.to_gpu().cupy.empty((n, c, h, w), dtype=col.dtype)
    ...
    CUDA.to_gpu().cupy.ElementwiseKernel(
        ...
        '''
           ...
           img = val;
        ''',
        'col2im')(col.reduced_view(), h, w, out_h, out_w, kh, kw, sy, sx, ph, pw, dx, dy, img)
    return img
