...
def interface_multi_prediction(coordinates_array, model_type, email, api_key, random_state=-1):
    ...
    for pair in coordinates_array:
        if ( (isinstance(pair[0],float)) & (isinstance(pair[1],float)) ):
            ...
            # need to change how my program indexes the thing
            df.to_csv("temporary.csv")  #remove when doen 
            df=df.iloc[2:]
    ...
    if(mode_wind==True):
        return get_wind_model_predictions(true_frame,random_state)
    elif(mode_solar==True):
        return get_solar_model_predictions(true_frame,random_state)
