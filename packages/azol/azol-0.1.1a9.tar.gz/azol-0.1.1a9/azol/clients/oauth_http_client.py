...
import logging
import requests
from azol.utils import is_token_expired, get_tenant_id
...
class OAuthHTTPClient:
    ...
    def new_client_from_foci(self, oauth_http_client_class, client_id=None):
        """
            Creat a new client from this client using a FOCI refresh

            Vars:
                - oauth_http_client_class - subclass of OAuthHTTPObject that will be used to
                                            initialize the new client. For example, ArmClient.
        ...
        if client_id is None:
            ...
            user = User(refresh_token=rt, client_id=client_id)
        client = oauth_http_client_class(tenant=self._t enant, cred=user)
        return client
