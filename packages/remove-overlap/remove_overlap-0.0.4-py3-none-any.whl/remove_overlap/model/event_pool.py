...
from remove_overlap.model.event import Event
from remove_overlap.model.event_type import EventType
from remove_overlap.model.hrectangle import HRectangle
...
class EventPool:
    ...
    def event_key(self, box_id: str, type: EventType):
        """
            {{docstring}}
        """
        return f"{box_id}{type.value}"

    def get_event(self, axis_index: int, box_id: str, type: EventType) -> Event:
        """
            {{docstring}}
        """
        index = self.get_event_index(axis_index, box_id, type)
        return self.events[axis_index][index]

    def get_event_index(self, axis_index: int, box_id: str, type: EventType) -> Event:
        """
            {{docstring}}
        """
        event_key = self.event_key(box_id, type)
