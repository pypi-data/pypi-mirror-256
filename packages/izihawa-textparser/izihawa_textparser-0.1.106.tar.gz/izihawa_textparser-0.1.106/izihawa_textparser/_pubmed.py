...
from .utils import BANNED_SECTIONS, SECTIONS_MAPS, md


def _process_single_pubmed_central_part(soup: BeautifulSoup):
    secs = []
    for rep in reversed(range(1, 5)):
        for h in soup.select('section > ' * rep + 'title'):
            if not h.text.strip():
                ...
                h.name = f'h{rep + 1}'
            title = SECTIONS_MAPS.get(h.text) or SECTIONS_MAPS.get(h.text.lower()) or h.text
...
def process_pubmed_central(text: str):
    ...
    if doi_tag := soup.select_one('article-meta > article-id[pub-id-type="doi"]'):
        document['id']['dois'] = [doi_tag.text.lower()]
    if pubmed_id_tag := soup.select_one('article-meta > article-id[pub-id-type="pmid"]'):
    ...
    for part in list(body.select('abstract, back, front, journal-id, journal-meta,'
...
def process_single_record(item):
    ...
    if isinstance(article["Language"], list):
        document["languages"] = [iso639.Language.match(l).part1 for l in article["Language"]]
    else:
