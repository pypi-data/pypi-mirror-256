...
class DocumentChunker:
    ...
    def _splits_to_chunks(
        ...
        for chunk_id, split in enumerate(splits):
            ...
            if chunks and (
                (_is_table(chunk.text) and _is_figure_header(chunks[-1].text)) or
                (_is_figure_header(chunk.text) and _is_table(chunks[-1].text))
            ):
                chunks[-1] = merge_chunks(chunks[-1], chunk)
                continue
            chunks.append(chunk)
