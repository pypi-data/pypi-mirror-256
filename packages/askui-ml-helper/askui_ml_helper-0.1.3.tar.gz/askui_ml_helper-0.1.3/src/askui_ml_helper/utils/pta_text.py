...
class PtaTextInference:
    ...
    def prepare_image(self, image, prompt, processor):
        resized_image = image.resize((1920, 1080)) # manual for pta-text-v0.1.pt
        ...
        return resized_image, encoding, render_variables
    ...
    def process_image(self, image, prompt):
        resized_image, encoding, render_variables = self.prepare_image(image, prompt, self.processor)
        pred_coordinates = self.predict_coordinates(encoding.to(self.device) , self.model, render_variables)
        return pred_coordinates, resized_image

    def process_image_and_draw_circle(self, image, prompt, radius = 5):
        pred_coordinates, resized_image = self.process_image(image, prompt)
        result_image = self.draw_circle_on_image(resized_image, pred_coordinates, radius=radius)
        return result_image
