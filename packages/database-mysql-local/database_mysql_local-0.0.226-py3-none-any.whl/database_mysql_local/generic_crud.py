...
class GenericCRUD(metaclass=MetaLogger, object=LOGGER_CRUD_CODE_OBJECT):
    ...
    def upsert(self, *, schema_name: str = None, table_name: str = None, data_json: dict = None) -> int:
        """Inserts a new row into the table, or updates an existing row if it already exists,
          and returns the id of the new row or -1 if an error occurred."""
        ...
        insert_query = "INSERT " + \
                       f"INTO `{schema_name}`.`{table_name}` ({columns}) " \
                       f"VALUES ({values}) " \
                       f"ON DUPLICATE KEY UPDATE {set_values} updated_timestamp=CURRENT_TIMESTAMP();"
        self.cursor.execute(insert_query, tuple(data_json.values()) * 2)
        self.connection.commit()
        inserted_id = self.cursor.lastrowid()
