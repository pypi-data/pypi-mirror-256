...
class ClusterDatabaseFactory(ClusterFactory):
    ...
    postgresql_backup_local_retention = factory.Faker(
        "random_int", min=1, max=24
    )
    meilisearch_backup_local_retention = factory.Faker(
        "random_int", min=1, max=24
    )
    ...
    rabbitmq_admin_password = factory.Faker(
        "password", special_chars=False, length=24
    )
