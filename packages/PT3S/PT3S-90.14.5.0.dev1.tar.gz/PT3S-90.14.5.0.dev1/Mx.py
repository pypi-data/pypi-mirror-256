...
class Mx():
    ...
    Attributes:
        ...
        * dataFrames
            ...
            * .dfVecAggs
                ...
                * MultiIndex:
                    * TYPE: SNAPSHOTTYPEs: STAT,TIME,TMIN,TMAX (from _readMxsFile) or Aggregates from 2 Times: MIN,MAX,... (from getVecAggs)
                    * Sir3sID
                    * TIMESTAMPL
                    * TIMESTAMPR
                * Cols: mx2Idx
    ...
    def _readMxsFile(self,mxsFilePtr,mxsVecsH5StorePtr,firstTime=None,maxRecords=None,StatTimeTminTmaxInVecsOnly=False):
        """
        Args:
            * mxsFilePtr: .MXS-File
            ...
            * StatTimeTminTmaxInVecsOnly: nur diese "Zeiten" werden in das h5FileVecs geschrieben; hat keinen Einfluss auf die Befuellung von dfVecAggs
        ...
        try:   
            df = None # the returned mxsFile (non Vectordata only) as DataFrame
            ...
            mxsFilePtr.seek(0,2) # ans Ende
            file_size = mxsFilePtr.tell()
            mxsFilePtr.seek(0) # an den Anfang
            ...
            while fPos < file_size:   
                ...
                try:
                    ...
                    # read record                         
                    cVerso, time, cSNAPSHOTTYPE, values, valuesVecs = self._readMxsFileSingleRecord(mxsFilePtr,MxRecordLength,fPos)
                    ...
                    # store vecs in H5
                    if mxsVecsH5StorePtr != None:                                                        
                                h5DumpLogSST="Actual SNAPSHOTTYPE: {:s}.".format(cSNAPSHOTTYPE)
                                h5Key=getMicrosecondsFromRefTime(refTime=firstTime,time=time)   
                                
                                h5Dump=True
                                if '/'+str(h5Key) in keysInH5Store:    
                                    h5DumpLog="{:s} - H5Dump:  NO: key=/{!s:>20s}     in keys".format(h5DumpLogSST,h5Key)  
                                    h5Dump=False
                                else: 
                                    h5DumpLog="{:s} - H5Dump: YES: key=/{!s:>20s} not in keys".format(h5DumpLogSST,h5Key)  
                                    #if ((not StatTimeTminTmaxInVecsOnly) or (cSNAPSHOTTYPE=='STAT')):         
                                    #    h5DumpLog="{:s} - H5Dump: YES: key=/{!s:>20s} not in keys".format(h5DumpLogSST,h5Key)  
                                    #else:
                                    #    h5DumpLog="{:s} - H5Dump:  NO ".format(h5DumpLogSST)  

                                if h5Dump and cSNAPSHOTTYPE in ['TIME']:
                                    if (not StatTimeTminTmaxInVecsOnly) or time==firstTime:                             
                                        try:                                            
                                        ...
                                        h5DumpLog="{:s} - Nr. {:6d}".format(h5DumpLog,timesWrittenToMxsVecs) 
                                        keysInH5Store.append('/'+str(h5Key))
                              
                    # store vecs in dfVecAggs                                    
                    if ((cSNAPSHOTTYPE in ['STAT','TMIN','TMAX']) or (cSNAPSHOTTYPE=='TIME' and time == firstTime)):                                                    
                        self._processToDfVecAggs(cVerso, time, cSNAPSHOTTYPE, values, valuesVecs, firstTime)
                           
                    # Ausgabe
                    logger.debug("{:s}TimeNr. {:>6d} read and processed finally={!s:s} Values (without TIMESTAMP): {:d} - {:s}.".format(logStr
                                    ,recsReadFromFile+1
                                    ,time_read_finally                                    
                                    ,len(values)
                                    ,h5DumpLog))  
                    ...
                    if maxRecordsLimit:
                        if recsReadFromFile == maxRecords:
                            logStrTmp="{0:s}maxRecords={1:d} read!".format(logStr,maxRecords)
                            logger.debug(logStrTmp)  
                            fPos = file_size # Ende While                            
                                                   
                except EOFError:                    
                    logger.debug("{0:s}Last Time read={1!s}. File finished (by EOFError).".format(logStr,time_read_finally))                                                                                             
    ...
    def _processToDfVecAggs(self,cVerso, time, cSNAPSHOTTYPE, values, valuesVecs, firstTime):                                                       
        """
        Speichern von vecs in dfVecAggs.
        
        Args:
            * cVerso, time, cSNAPSHOTTYPE, values, valuesVecs: der von _readMxsFileSingleRecord gelesene Record
            * firstTime: der erste gelesene Record (wirde verwendet als TIMESTAMPL fuer TMIN, TMAX)
        """
        ...
        try:
            ...
            if cSNAPSHOTTYPE == 'TIME':
                timeL=time
            elif cSNAPSHOTTYPE == 'STAT':
                timeL=time
            elif cSNAPSHOTTYPE in ['TMIN','TMAX']:    
                timeL=firstTime

            arrays=[[cSNAPSHOTTYPE]*len(self.mxColumnNamesVecs),self.mxColumnNamesVecs,[timeL.tz_localize(None)]*len(self.mxColumnNamesVecs),[time.tz_localize(None)]*len(self.mxColumnNamesVecs)]
            tuples=list(zip(*arrays))        
            ...
            df=pd.DataFrame(df.values,index=mIndex,columns=df.columns) # MemError?!

            if self.dfVecAggs.empty:                                                    
                self.dfVecAggs=df
                logger.debug("{:s}cSNAPSHOTTYPE {:s}     processed to (empty) dfVecAggs with TIMESTAMPL: {!s:s}".format(logStr,cSNAPSHOTTYPE,timeL))                  
            else:  
                dfVecAggsAdd=True
                if self.dfVecAggs.index.isin([x[0] for x in df.index],level=0).any(): # cSNAPSHOTTYPE existiert schon ...
                    if self.dfVecAggs.loc[(cSNAPSHOTTYPE,slice(None),slice(None),slice(None)),:].index.isin([x[2] for x in df.index],level=2).any(): # ... mit time                                                       
                        logger.debug("{:s}cSNAPSHOTTYPE {:s} not processed to         dfVecAggs with TIMESTAMPL: {!s:s} weil dort schon existierend.".format(logStr,cSNAPSHOTTYPE,time))                          
                        dfVecAggsAdd=False
                if dfVecAggsAdd:                           
                    self.dfVecAggs=pd.concat([self.dfVecAggs,df]) # MemError ?!
                    logger.debug("{:s}cSNAPSHOTTYPE {:s}     processed to         dfVecAggs with TIMESTAMPL: {!s:s}".format(logStr,cSNAPSHOTTYPE,timeL))                                                                                                                         
    ...
    def _readMxsFileSingleRecord(self,mxsFilePtr,MxRecordLength,fPos):
        ...
        Args:
            * mxsFilePtr: .MXS-File
            * MxRecordLength: bytes to read
            * fPos: seek-Argument           
        Returns:
            * cVerso, time, cSNAPSHOTTYPE, values, valuesVecs
        ...
        try:   
                ...
                try: 
                    ...
                    logStrFinal="{:s}Exception: Line: {:d}: {!s:s}: {:s}".format(logStr,sys.exc_info()[-1].tb_lineno,type(e),str(e))                    
                    logger.debug(logStrFinal) 
                    logger.debug("{0:s}record=f.read(MxRecordLength) failed (with struct.error). EOF assumed.".format(logStr))  
                    raise EOFError
                except EOFError as e:
                    logStrFinal="{:s}Exception: Line: {:d}: {!s:s}: {:s}".format(logStr,sys.exc_info()[-1].tb_lineno,type(e),str(e))                    
                    logger.debug(logStrFinal) 
                    logger.debug("{0:s}record=f.read(MxRecordLength) failed (with EOFError). EOF also assumed.".format(logStr))  
                    raise EOFError
                else:
                    timeISO8601=None
            ...
            logger.debug("{0:s}MxError raised.".format(logStr))  
            raise MxError
        except Exception as e:
            logger.debug("{0:s}MxError raised:".format(logStr))  
            logStrFinal="{:s}Exception: Line: {:d}: {!s:s}: {:s}".format(logStr,sys.exc_info()[-1].tb_lineno,type(e),str(e))
    ...
    def setResultsToMxsFile(self,mxsFile=None,add=False,NewH5Vec=False,maxRecords=None,StatTimeTminTmaxInVecsOnly=False):
        ...
        Args:
            ...
            * maxRecords: Anzahl der max. zu lesenden Records
            * StatTimeTminTmaxInVecsOnly: nur die "Zeiten" werden in das VECS-File gelesen
    ...
    def setResultsToMxsZipFile(self,mxsZipFile=None,add=False,NewH5Vec=False,maxRecords=None,StatTimeTminTmaxInVecsOnly=False):
        ...
        Args:
            ...
            * NewH5Vec
            * maxRecords: Anzahl der max. zu lesenden Records
            * StatTimeTminTmaxInVecsOnly: nur die Zeiten werden ins VECS-File geschrieben
            ...
            with pd.HDFStore(h5File) as h5Store:
                ...
                for h5Key in h5Keys:
                    ...
                    if key == 'MXS':                           
                        ...
                        # bei StatTimeTminTmaxInVecsOnly=False muss erste Zeit, letzte Zeit, Anzahl der Zeiten gleich sein
                        firstTime=self.df.index[0]
                        ...
                        if tupleDf != tupleVecH5:                            
                            logger.debug("{:s}{:s}: tupleDf {!s:s} != tupleVecH5 {!s:s} - ggf. war StatTimeTminTmaxInVecsOnly bei init Wahr.".format(logStr,h5File,tupleDf,tupleVecH5))     
    ...
    def getMxsVecsFileData(self,timesReq=None,fastMode=False):
        ...
        try:
            ...
            elif isinstance(timesReq,list):
                #h5Keys ermitteln
                for timeReq in timesReq:
                    key=getMicrosecondsFromRefTime(refTime=firstTime,time=timeReq)
                    h5KeysRequested.append(key)
            else:                
                logStrFinal="{:s}timesReq (List of TIMESTAMPs): Not defined!".format(logStr)
                logger.error(logStrFinal) 
                raise MxError(logStrFinal)                   
                    ...
                    for idx,h5KeyReq in enumerate(h5KeysRequested):                       
                        ...
                        if h5KeyReq in h5KeysAvailable:    
                            ...
                            for h5KeyAva in h5KeysAvailable:                            
                                ...
                                # lesen von Mxs
                                timeReq=timesReq[idx]
                                df=self.readMxsVecsFileData(timeReq=timeReq)
                                if df.empty:
                                    logStrAdditional="{:s} - AND TimeReq {:32s} also as TIMESTAMP NOT available - reading from Mxs failed too ".format(logStrAdditional,str(timesReq[idx])) 
                                else:
                                    logStrAdditional="{:s} - AND TimeReq {:32s} also as TIMESTAMP NOT available - reading from Mxs successfull".format(logStrAdditional,str(timesReq[idx])) 
    ...
    def readMxsVecsFileData(self,timeReq=None):
        """Returns dfVecs - a df with mxsVecsFileData for timeReq read from mxsFile. 

        Args:
            * timeReq
                                              
        Returns:
            * dfVecs - a df with mxsVecsFileData for timeReq
            * empty df if problems occured
        ...
        try:
            ...
            logger.debug("{0:s}Mxs: {1:s}: try reading ...".format(logStr,self.mxsFile)) 

            with open(self.mxsFile,'rb') as  mxsFilePtr:                                               
            
                # Pos. bestimmen
                timesAvailable=self.df.index.to_list()
                idxRecord=timesAvailable.index(timeReq)
                MxRecordLength=struct.calcsize(self.mxRecordStructFmtString)
                fPos=(idxRecord+1)*MxRecordLength
                mxsFilePtr.seek(fPos)
                                                                                 
                # Record lesen                       
                cVerso, time, cSNAPSHOTTYPE, values, valuesVecs = self._readMxsFileSingleRecord(mxsFilePtr,MxRecordLength,fPos)
                if time != timeReq:                    
                    logger.warning("{:s}timeReq: {!s:s} !=?! time(Read): {!s:s} @ fPos: {:d} == idxRecord: {:d} ...".format(logStr,timeReq,time,fPos,idxRecord+1))     
                else:                    
                    logger.debug("{:s}timeReq: {!s:s} read @ fPos: {:d} == idxRecord: {:d} ...".format(logStr,timeReq,fPos,idxRecord+1))     
                dfVecs = pd.DataFrame.from_records([valuesVecs],index=[time],columns=self.mxColumnNamesVecs)         
    ...
    def getRohrVektorkanaeleIpkt(self):
        ...
        try:
            kanalLst=[col for col in self.mxColumnNamesVecs if re.match(regExpSir3sRohrVecAttrType,re.match(regExpSir3sVecID,col).group('ATTRTYPE'))]                               
        except MxError:
            raise
        except Exception as e:
            logStrFinal="{:s}h5File: {:s}: Exception: Line: {:d}: {!s:s}: {:s}".format(logStr,h5File,sys.exc_info()[-1].tb_lineno,type(e),str(e))
    ...
    def getRohrVektorkanaeleVrtx(self):
        """
        Return:
        ['ROHR_VRTX~*~*~*~H',
         'ROHR_VRTX~*~*~*~T',
        ...
        try:
            kanalLst=[col for col in self.mxColumnNamesVecs if re.search('^ROHR_VRTX',col)]                            
        except MxError:
            raise
        except Exception as e:
            logStrFinal="{:s}h5File: {:s}: Exception: Line: {:d}: {!s:s}: {:s}".format(logStr,h5File,sys.exc_info()[-1].tb_lineno,type(e),str(e))
    ...
    def getVecAggs(self,time1st=None,time1stIncluded=True,time2nd=None,time2ndIncluded=True,aTIME=False):
        ...
        * New calced Aggregates are stored in dfVecAggs.
        ...
        try:           
            ...
            logger.debug("{:s}timesReq: {!s:s}".format(logStr,timesReq))   
            ...
            if self.dfVecAggs.index.isin([testTYPE],level=0).any(): # Type (exemplarisch) existiert schon
                logger.debug("{:s}testTYPE: {!s:s} existiert ...".format(logStr,testTYPE))   

                timesL=self.dfVecAggs.loc[(testTYPE,slice(None),slice(None),slice(None)),:].index.get_level_values(2).to_list()
                timesLL=[t.tz_localize(None) for t in timesL]
                if timesReq[0] in timesL or  timesReq[0] in timesLL or timesReq[0].tz_localize(None) in timesLL:
                #if self.dfVecAggs.loc[(testTYPE,slice(None),slice(None),slice(None)),:].index.isin([timesReq[0]],level=2).any(): # mit dieser ZeitL
                    logger.debug("{:s}testTYPE: {!s:s} existiert mit ZeitL ...".format(logStr,testTYPE))   

                    timesR=self.dfVecAggs.loc[(testTYPE,slice(None),timesReq[0],slice(None)),:].index.get_level_values(3).to_list()
                    timesRL=[t.tz_localize(None) for t in timesR]
                    if timesReq[-1] in timesR or  timesReq[-1] in timesRL or timesReq[-1].tz_localize(None) in timesRL:
                    #if self.dfVecAggs.loc[(testTYPE,slice(None),timesReq[0],slice(None)),:].index.isin([timesReq[-1]],level=3).any(): # mit dieser ZeitR
                        logger.debug("{:s}testTYPE: {!s:s} existiert mit ZeitL und ZeitR ...".format(logStr,testTYPE))   
                        inDfVecAggs=True

            if not inDfVecAggs:
                logger.debug("{:s}testTYPE: {!s:s} existiert nicht mit ZeitL und ZeitR ...".format(logStr,testTYPE))   
            ...
            if not inDfVecAggs:         
                ...
                logger.debug("{:s}df nach unPack: {!s}".format(logStr,df.head().to_string()))                 
                ...
                if not aTIME:
                    ...
                    df.rename(index={timesReq[0]:'TIME'},inplace=True)    # setzt den TYPE-Teil des MIndex auf 'TIME'                             

                logger.debug("{:s}df vor dem Speichern: {!s}".format(logStr,df.head().to_string()))           
                # store in dfVecAggs
                ...
                for aggType in df.index.unique(level=0).values:
                    try:
                        ...
                        tuples=list(zip(*arrays))        
                        mIndex=pd.MultiIndex.from_tuples(tuples,names=['TYPE','Sir3sID','TIMESTAMPL','TIMESTAMPR'])
                        logger.debug("{:s}df direkt vor dem Speichern: {!s}".format(logStr,df.head().to_string()))               
    ...
    def getVecAggsResultsForObjectType(self,Sir3sVecIDReExp='^KNOT~\*~\*~\*~PH'):
        """
        #'^KNOT~\*~\*~\*~PH$' - genau 1 Kanal des ObjektTyps KNOT
        #'^KNOT~\*~\*~\*~PH' - ggf. mehrere
        #'^KNOT' - alle


        # Sir3sVecIDReExp definiert einen oder mehrere oder alle Kanaele eines OBJTYPEs 
        # OBJTYPE wird aus Sir3sVecIDReExp ermittelt
        # bei ROHRen werden matchende Innenpunktkanaele ignoriert
        # sind auch ROHR_VRTX Kanaele verfuegbar, muss '^ROHR~' fuer das Ergebnis alle ROHR-Kanaele ausser Innenpunktkanaele angegeben werden 
        # da '^ROHR' auch ROHR_VRTX Kanaele liefert und das hier dann nicht funktioniert
    
        # df
            # rows: pk/tk (d.h. mx2Idxe des OBJTYPEs)
            # cols: all vecAggs for Sir3sIDs (Vector-Channels) matching Sir3sIDReExp
            # cols == row Idx (ein Multiindex) von dfVecAggs; use .columns.to_flat_index() to flatten the col Idx (ein Multiindex)
            # empty wenn keine entsprechenden Kanaele gefunden oder Fehler
        """
        ...
        try:           
            ...
            IDs=[ID for ID in Sir3sIDsMatching]    
            if OBJTYPE=='ROHR':
                # keine Innenpunktkanaele liefern        
                for ID in IDs:            
    ...
    def readMxsVecsResultsForObjectType(self,Sir3sVecIDReExp='^KNOT~\*~\*~\*~PH$' ,dfVecAggCols=False ,dfVecAggsFormat=True ,flatIndex=True ):
        ...
        Sir3sVecIDReExp:  gewuenschte Kanaele (gewuenschter Kanal) eines Objekttyps als regexp; kann eine Liste sein
        dfVecAggCols: True: 0,1,2,... sind Labels (sonst (default): pks sind Labels) 
        dfVecAggsFormat:  True: .index (default) ist Multiindex (sonst: .columns ist Multiindex)    
        flatIndex: True: Multiindex zu flachem Index


        # Sir3sVecIDReExp definiert einen oder mehrere oder alle Kanaele eines OBJTYPEs 
        # OBJTYPE wird aus Sir3sVecIDReExp ermittelt
        # bei ROHRen werden matchende Innenpunktkanaele ignoriert
        # sind auch ROHR_VRTX Kanaele verfuegbar, muss '^ROHR~' fuer das Ergebnis alle ROHR-Kanaele ausser Innenpunktkanaele angegeben werden da '^ROHR' auch ROHR_VRTX Kanaele liefert und das hier dann nicht funktioniert



        Return:
            df:
                                        4611806584464347245 4611810179412330323 ...
                TIME
                2022-01-21 09:00:00.000 7.144244            3.430746
                2022-01-21 09:00:00.200 7.143443            3.418180        
                ...          
        """
        ...
        try:           
            ...
            if isinstance(Sir3sVecIDReExp,list):
                ...
                Sir3sVecIDReExpLst=Sir3sVecIDReExp
            else:
                Sir3sVecIDReExpLst=[Sir3sVecIDReExp]
                
            Sir3sIDsMatching=[]
            OBJTYPEs=[]
            for aSir3sVecIDReExp in Sir3sVecIDReExpLst:
                        
                aSir3sIDsMatching=[Sir3sID for Sir3sID in self.mxColumnNamesVecs if re.search(aSir3sVecIDReExp,Sir3sID) != None]
                ...
                if len(aSir3sIDsMatching) > 0:
                    ...
                    if OBJTYPE=='ROHR':
                        # keine Innenpunktkanaele liefern   
                        IDs=[ID for ID in aSir3sIDsMatching]         
                        for ID in IDs:            
                            ATTRTYPE=re.match(regExpSir3sVecID,ID).group('ATTRTYPE')
                            if re.search(regExpSir3sRohrVecAttrType,ATTRTYPE) != None:                                        
                                logger.debug("{:s}: Innenpunktkanal: {!s:s} eliminiert".format(logStr,ID)) 
                    ...
                    # Liste ergänzen
                    Sir3sIDsMatching=Sir3sIDsMatching+aSir3sIDsMatching
                    
                    # Liste der voneinander verschiedenen OBJTYPEs nachfuehren
            ...
            # ueber alle Zeiten 
            dfLst=[]             
            for idx,time in enumerate(timesAvailable):
            
                dfVecs=self.readMxsVecsFileData(timeReq=time)
                dfVecs=dfVecs.filter(items=Sir3sIDsMatching)
                ...
                if idx==0:
                    ...
                    #logger.debug("{:s}mIndex2: {!s:s}".format(logStr,mIndex2)) 
        
                df=self.unPackMxsVecsFileDataDf(dfVecs,mIndex2)

                #logger.debug("{:s}df nach unPack: {!s:s}".format(logStr,df.to_string())) 
                
                arrays=[['TIME']*len(cols),cols,[time.tz_localize(None)]*len(cols),[time.tz_localize(None)]*len(cols)]                                           
                tuples=list(zip(*arrays))        
                mIndex=pd.MultiIndex.from_tuples(tuples,names=['TYPE','Sir3sID','TIMESTAMPL','TIMESTAMPR'])

                #logger.debug("{:s}mIndex: {!s:s}".format(logStr,mIndex)) 

                df=pd.DataFrame(df.values,index=mIndex,columns=df.columns)

                #logger.debug("{:s}df: {!s:s}".format(logStr,df.to_string())) 
            ...
            #logger.debug("{:s}df alle Zeiten: {!s:s}".format(logStr,df.to_string())) 
    
            # Labels
            if not dfVecAggCols: # labels: pks
                ...
                if len(OBJTYPEs) > 1:                    
                    ...
                    # pks/tks 
                    mx2RecordDesc=self.mx2Df[self.mx2Df['ObjType'].str.contains('^'+OBJTYPE)].iloc[0]                                                                                 # klappt auch fuer ...
                    # ... ROHRe, wenn ROHR	N_OF_POINTS hinter ROHR kommt 
                    mx2Idx=mx2RecordDesc['Data']
    
                    # Spalten umbenennen in pks  
                    df.rename(columns={i:mx2Idx[i] for i in df.columns.to_list()},inplace=True)
    
            # Transponieren
            if not dfVecAggsFormat: # columns: Multiindex
                df=df.transpose()
            else: # index: Multiindex
                pass
            ...
            if flatIndex:
                if not dfVecAggsFormat: # columns: Multiindex
                    ...
                    if len(Sir3sIDsMatching)==1:
                        df.index=df.index.get_level_values(2).rename('TIME')
                    else:                
                        # bei mehreren Kanälen würde bei Reduktion auf TIME Information verloren gehen
                        df.index=df.index.to_flat_index()            
                        # use pd.MultiIndex.from_tuples(df.index) to reconstruct Multiindex
                        # then i.e. .loc[(slice(None),'ROHR~*~*~*~DSI',slice(None),slice(None)),:] and .index.get_level_values(2).rename('TIME') to achieve a flat Timeindex
