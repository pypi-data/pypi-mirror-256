...
def write_prefs(config):
    ...
    if not os.path.isdir(yangsuite_dirs.user_config_dir):
        if not os.path.isfile(yangsuite_prefs):
...
def main():
    ...
    basic_group.add_argument(
        '-config', '--config-file',
        default=yangsuite_prefs,
        help="config file path to store the yangsuite settings")
    ...
    django.setup()
    from .celery_init import app as celery_app # noqa
...
def configure_interactively(args):
    ...
    args.config_file = configure_settings(default_value=yangsuite_prefs)
    ...
    if args.save_settings:
        yangsuite_prefs = args.config_file
...
def configure_settings(default_value):
    """YANG Suite settings file."""
    while True:
        path = get_input("YANG Suite stores default settings "
                         "(Port, allowed hosts, data_path etc.)\n"
                         "Set new path or use:",
                         default_value=yangsuite_prefs)
        if path.startswith('~'):
            path = os.path.expanduser(path)
        else:
            path = os.path.abspath(path)
        if not os.path.isfile(path):
            create = confirm("File {0} does not exist. create it?"
                             .format(path))
            if not create:
                continue
            open(path, 'w').close()
