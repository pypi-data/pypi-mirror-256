...
try:
    # prefer django version as it does not require tzlocal
...
def is_celery_working():
    ...
    try:
        ...
        logger.exception(f"cannot connect to celery broker: {err}")
        return False

    result: list[dict] = current_app.control.ping()
    if not result:
        logger.error(f"no celery workers found")
        return False
...
def report_progress(message: str = None, progress: Progress = None, *, index: int|float = None, total: int|float = None, details: str = None):
    ...
    if index is not None:
        ...
        if total is not None:
            meta["total"] = total
            if total != 0:
                log_message += f"{100*index/total:.0f}%, "
...
class TaskInfo:
    ...
    def handle(self):
        ...
        if self.state == CeleryState.PROGRESS.value and self._last_sent is not None and t - self._last_sent < 0.5:
            # Do not send progress messages more than once every 500 ms.
            # Instead, delay the message for the remaining time, and send it only if no new message appeared
            self._delayed_timer = Timer(0.5 - (t - self._last_sent), self._timer_callback, [self._handle_num])
...
def convert_task_string_args(task, *args):
    if not args:
        return tuple(), dict()
