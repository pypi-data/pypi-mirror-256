...
from MedicalAutomationService.const import SD, MODULES
                            ...
                            if new_document_is_present or forced:
                                with user_output.personalized():
                                    ...
                                    doctor_user: User = one(
                                        A.R_U.by_group(A.CT_AD.Groups.FunctionalDiagnostics)
                                    )
                    ...
                    if event == A.CT_E.NEW_FILE_DETECTED:
                        ...
                        if path.startswith(A.PTH.path(A.PTH.SCAN_TEST.VALUE)):
                            ...
                            if n(polibase_document):
                                while True:
                                    try:
                                        polibase_document = A.D.fill_data_from_source(
                                            ...
                                            MIO.waiting_for_result(
                                                js(
                                                    (
                                                        mio_command(COMMAND_KEYWORDS.CHECK),
                                                        mio_command(COMMAND_KEYWORDS.SCAN),
                                                    )
                                                ),
                                    ...
                                    if nn(polibase_document):
                                        break
                            A.E.new_polibase_scanned_document_detected(polibase_document)
                            polibase_person_pin: int = polibase_document.polibase_person_pin
                            ...
                            if document_name in [
                                ...
                                is_holter: bool = document_name == A.CT_P_DT.HOLTER_JOURNAL
                                ...
                                path_destination: str = A.PTH.join(
                                    ...
                                    A.PTH.add_extension(
                                        "holter_journal" if is_holter else "abpm_journal",
                                        ...
                                        (
                                            ...
                                            A.D_F.whatsapp_send_message_to(
                                                ...
                                                js(
                                                    (
                                                        "День добрый,",
                                                        A.D.to_given_name(doctor_user.name),
                                                    )
                                                ),
                                            ),
                                            ")",
