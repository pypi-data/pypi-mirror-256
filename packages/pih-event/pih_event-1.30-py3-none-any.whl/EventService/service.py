...
from pih import A
from EventService.const import SD, MODULES
    ...
    if A.U.for_service(SD, MODULES, as_standalone):
        ...
        def service_call_handler(sc: SC, pl: ParameterList) -> bool | None:
            if sc == SC.send_log_message:
                ...
                Api.send_log_event(event_description, event_parameters, event_flags, image_path)
                event_flags = event_flags or BM.set(event_description.flags)
        ...
        A.SRV_A.serve(SD, service_call_handler, lambda: print("!!!"), isolate=ISOLATED, as_standalone=as_standalone)
