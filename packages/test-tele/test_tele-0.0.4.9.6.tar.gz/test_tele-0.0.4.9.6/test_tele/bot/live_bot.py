...
async def get_images_from_inline(event, is_premium:bool = False):
    ...
    if query.startswith('.px'):
        from test_tele.features.extractors.pixiv import get_pixiv_list, PIXIV_MODE
    ...
    as_file = mode.get('as_file', False)
...
async def image_search_handler(app, message: Message):
    ...
    try:
        msg_to_send = "Sorry, no matches were found"
        if message.text == "/sauce":
            if not message.reply_to_message_id:
                raise ValueError(f"{notes}\n")
            
            replied_msg = await app.get_messages(message.from_user.id, message.reply_to_message_id)
            file = await app.download_media(replied_msg, in_memory=True)
            ...
            if resp:
                msg_to_send = (
                    ...
                    f"Author: [{resp[0]['author']}]({resp[0]['author_url']})\n"
                )
            ...
            if resp:
                msg_to_send = (
                    f"The full version is found in :\n"
                    f"**{resp[0]['title']}**\n"
...
async def get_reply_markup(results:list) -> InlineKeyboardMarkup:
...
def get_handlers():
    ...
    pyrogram_handler = {
        ...
        "image_search": MessageHandler(image_search_handler, filters.command("sauce") | filters.photo & filters.incoming & filters.private)
    }
