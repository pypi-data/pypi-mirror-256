...
class PD(object):
    ...
    def optvol_optimizer_builder(self, cutoff_radius, num_retry, lp_solver):
        if self.pd.boundary_map_chunk is None:
            raise (
                ValueError(
                    'Optimal/Stable volume requires boundary_map: "save_boundary_map=True" is requried when a PD is computed.'
                )
            )
...
class Pair(object):
    ...
    def optimal_1_cycle(self, weighted=False, torelance=None):
        ...
        The algorithm in optimal_1_cycle computes the minimal loop that includes a birth edge (alpha, rips, abstract) or birth pixel (bitmap).
        ...
        * Bitmap filtration by :meth:`PDList.from_bitmap_levelset`: `save_suppl_info=True`
        ...
        Args:
            ...
            torelance (None or float): Noise bandwidth parameter for Reconstructed Shortest Cycles.
              ...
              See https://mtsch.github.io/Ripserer.jl/dev/generated/cocycles/#Reconstructed-Shortest-Cycles if you know more about Reconstructed shortest cycles.
        Returns:
