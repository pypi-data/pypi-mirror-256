...
class PDGM(PDGMBase):
    ...
    def geometry_resolver(self, use_symbol, pbc_ratio=None):
        if self.filtration_type == "alpha":
            if use_symbol:
                return self.alpha_symbol_resolver
            else:
                if pbc_ratio is None:
            ...
            raise RuntimeError("Geometry resolver is unavailable for {}".format(self.filtration_type))
    ...
    def essential_birth_positions(self):
        return self.geometry_resolver(False).resolve_cells(self.essential_birth_indices)
