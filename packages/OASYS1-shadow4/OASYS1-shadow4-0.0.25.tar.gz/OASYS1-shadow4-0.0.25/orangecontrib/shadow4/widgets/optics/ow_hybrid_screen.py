...
class HybridScreenNew(AutomaticElement, HybridListener):
    ...
    outputs = [{"name":"Output Data (Far Field)",
                "type":ShadowData,
                "doc":"Shadow Data",
                "id":"data_ff"},
               {"name":"Output Data (Near Field)",
                "type":ShadowData,
                "doc":"Shadow Data",
                "id":"data_nf"},
               {"name":"Trigger",
    ...
    def plot_xy(self, beam_out, progressBarValue, var_x, var_y, plot_canvas_index, title, xtitle, ytitle, xum="", yum=""):
        ...
        self.plot_canvas[plot_canvas_index].plot_xy(beam_out, var_x, var_y, title, xtitle, ytitle, xum=xum, yum=yum)
    ...
    def plot_xy_hybrid(self, progressBarValue, scaled_matrix, plot_canvas_index, title, xtitle, ytitle, var1, var2):
        ...
        factor1 = ShadowPlot.get_factor(var1) # results are in m
        factor2 = ShadowPlot.get_factor(var2) # results are in m
    ...
    def plot_histo_hybrid(self, progressBarValue, scaled_array, plot_canvas_index, title, xtitle, ytitle, var):
        ...
        factor = ShadowPlot.get_factor(var) # results are in m
    ...
    def set_shadow_data(self, input_data: ShadowData):
        if ShadowCongruence.check_empty_data(input_data):
            if ShadowCongruence.check_empty_beam(input_data.beam):
                self.__input_data = input_data.duplicate()
    ...
    def run_hybrid(self):
        try:
            ...
            if ShadowCongruence.check_empty_data(self.__input_data):
                if ShadowCongruence.check_empty_beam(self.__input_data.beam):
                    ...
                    try:
                        ...
                        if self.send_original_beam==1:
                            self.send("Output Data (Far Field)", self.__input_data.duplicate())
                            self.send("Trigger", TriggerIn(new_object=True))
                        else:
                            raise e
                else:
