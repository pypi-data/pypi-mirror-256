...
class AndroidQQ:
    ...
    def loginH(self, **kwargs):
        """登录 混合模式"""
        self.info.uin = kwargs['uin']
        self.info.password = kwargs['password']
        self.info.device.client_type = 'AndH'
        data = wt_login.login_H(self.info, **kwargs)
    ...
    def get_level_status(self, query: bool = False):
    ...
    def lv_Friend(self):
        ...
        try:
            ...
            taskList = self.get_qq_level().get('taskList', {})
            task = next(item for item in taskList if item['title'] == '加一位好友+0.5天')
            finishedAccelerateDays = 0
            if task:
                del task['iconUrl']
                del task['jumpSchema']
                finishedAccelerateDays = task['finishedAccelerateDays']

            return Box(status=True, message=f'新增活跃天数{finishedAccelerateDays}',
                       finishedAccelerateDays=finishedAccelerateDays,
                       echo=task)
    ...
    def lv_extra_task_up(self):
        ...
        def tcp_mount() -> bool:
            ...
            if not mount:
                if self.no_tail_login()['status'] == 0:
                    mount = True
                    return True
                else:
                    return False
        ...
        task_status = self.get_level_status(True)
        for task in execution_task:
            is_done = task_status.get(task, {}).get('is_done', None)
            print(task, is_done)
            if task == '发布一条空间说说' and not is_done:
                tcp_mount()
                self.Qzone_publishmood('偶尔的停顿和修整，对于人生是非常必要的。')
                logger.info('空间任务')
            elif task == '连续登录QQ' and not is_done:
                tcp_mount()
                self.get_summary_card()
            elif task == '去超级QQ秀更新装扮并保存' and not is_done:
                tcp_mount()
                self.qq_show_replace()
            elif task == '去日签卡打一次卡' and not is_done:
                self.sign_in()
            elif task == '去QQ音乐听歌30分钟+0.5天' and not is_done:
                self.Music_task()
            elif task == '加一位好友' and not is_done:
                self.lv_Friend()
            elif task == '去超级QQ秀更新装扮并保存' and not is_done:
                self.qq_show_replace()
