...
from AndroidQQ.http import qq_level_index, WriteMindJar, change_psw_getsms, Friends, bot_uin, delete_qq_friend, \
    GetLevelStatus, GetLevelTask, RefreshTask
...
class AndroidQQ:
    ...
    def lv_music(self):
        ...
        try:
            ...
            finished_accelerate_days = 0
            for i in range(10):
                logger.info('正在等待音乐任务完成，剩余次数{}'.format(10 - i))
                time.sleep(1)
    ...
    def lv_extra_task_up(self):
        ...
        logger.info(f'已完成任务数量{done_count}')
        return Box(status=True, level=level, task_status=task_status)
