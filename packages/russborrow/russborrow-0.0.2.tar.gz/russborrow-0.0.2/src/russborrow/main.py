...
class BorrowedWordAnalyzer:
    def __init__(self, words, found_borrowed):
        self._dictionary = dict(sorted(found_borrowed.items(), key=lambda x: x[1].get("Repeats", 1), reverse=True))  # order by repeats
        self._len = len(words)
        self._totalbor = sum(len(value["Instances"]) for value in found_borrowed.values())
        self._percent = round(self._totalbor / self._len * 100, 2)
...
def analyze_borrowed_words(the_text=None, output_file=None):
    ...
    ready_dictionary = BorrowedWordAnalyzer(words, found_borrowed)

    if output_file and output_file.endswith(".txt"):
        with open(output_file, "w", encoding="utf-8") as formatted_file:
            formatted_file.write(f"Out of a total of {ready_dictionary.len} words, {ready_dictionary.totalbor} are borrowed, comprising {ready_dictionary.percent}% of the total.\n")
            for key, value in ready_dictionary.dictionary.items():
