...
def checksum_array(
    ...
    """
    if len(arr.shape) > 2:
        raise exceptions.ValidationException("Unsupported array shape: %s" % arr.shape)

    logger.debug("Starting checksum for arr: %s" % arr)
    checksum = initial
    for slice in _array_slicer(arr, chunk_size_0, chunk_size_1):
        checksum = zlib.crc32(slice.tobytes(), checksum)
