...
from pankmer.index import (load_metadata, get_regional_scores_expanded,
                           get_regional_scores_summarized)
...
def get_regional_scores(
        ...
        return (get_regional_scores_summarized if summarized else get_regional_scores_expanded)(
            str(index),
...
def anchor_region(
    *indexes,
    ...
    anchor_dicts = (
        Pool(processes=threads).map if threads > 1 and not low_memory
        else map
    )(
        partial(
            get_regional_scores,
            ...
            threads=threads if low_memory else max(1, floor(threads/len(indexes))),
            summarized=(summary_func is None)
        ),
        indexes 
    )
    if output_file:
    ...
    if legend:
        ...
        if groups is None:
            ...
            plotting_data[f"{legend_title}_chrom"] = tuple(
                "_".join((groups_dict["_".join(x.split("_")[:-1])], x.split("_")[-1]))
