...
class PiiFilterer:
    ...
    def __init__(self, filters_config: Optional[Dict], include_metrics: bool = True):
        self._config = PiiActiveFiltersConfig.from_dict(filters_config) \
    ...
    def _metrics_to_final(metrics: Dict, elapsed_time_ns: int) -> Dict:
        final_metrics = {
            k: PiiFilterMetrics(
                replacements=v,
                time_taken_ms=t / 1000000).to_dict()
        ...
        final_metrics[PiiFilterer._TOTAL_KEY] = PiiFilterMetrics(
            ...
            time_taken_ms=elapsed_time_ns / 1000000
        ).to_dict()
    ...
    def filter_message(self, msg: Union[List, Dict]) -> Union[List, Dict]:
        """
        Filters a dictionary or a list. If the object is a dictionary it includes metrics in a pii_metrics
        attribute in the result dictionary (only if the filterer was created with include_metrics=True which
    ...
    def _do_filter(self, data: Any, filter_function: Callable[[Any], Any]) -> Any:
        ...
        try:
            ...
            if self._config.fail_closed:
                raise
            _logger.exception(f'Failed to evaluate PII filters: {exc}, ignoring because fail_closed=False')
            result = data
        return result

    def filter_str(self, msg: str) -> str:
        """
        Filters a string, please note metrics are not included as we don't have a way to include metrics
        in the resulting string.
        We might return metrics in the future if we decide to use them where we call this (dbtv2 collection)
        """
    ...
    def filter_data(self, msg: bytes, encoding: str = 'utf8') -> bytes:
        ...
        we don't have a way to include metrics in the resulting bytes array.
        We might return metrics in the future if we decide to use them where we call this (dbtv2 collection)
        """
    ...
    def _filter_object(self, o: Any, metrics: Optional[Dict] = None) -> Any:
        if isinstance(o, Dict):
            return {self._filter_text(k, metrics=metrics): self._filter_object(v, metrics=metrics) for k, v in o.items()}
