...
from pycarlo.features.metadata import AllowBlockList, FilterEffectType, MetadataFilter, FilterType
...
class MetadataFiltersContainer:
    ...
    MetadataFiltersContainer class that includes a metadata_filters list that works in the following way:
    ...
    - there's a default effect (allow/block) configured in the list, those elements with no matching filter
      will be resolved with the default effect in the list.
    - This class supports filtering objects in memory or generating SQL conditions for filtering, for SQL
      generation an encoder function is required that maps the different filter types to SQL, also a dictionary
      mapping from property name to column name is required (for example to map 'project' to 'database' or
      'dataset' to 'schema'.
    - The order in which elements are added to the list is not relevant, priority is assigned based on the
      effect, rules with the default effect have the higher priority and stop the iteration.
    ...
    """
    metadata_filters: Optional[AllowBlockList] = field(default_factory=AllowBlockList)

    @property
    def is_metadata_filtered(self) -> bool:
        return self.metadata_filters.filters

    @property
    def is_metadata_blocked(self):
        """
        Helper method for detecting an edge case where everything is blocked because the default effect is block
        and all filters are also blocking, in this case it doesn't make sense to run queries or filter data.
        """
        return self.metadata_filters.default_effect == FilterEffectType.BLOCK and \
            all(f.effect == FilterEffectType.BLOCK for f in self.metadata_filters.filters)

    def is_project_with_datasets_filtered(self, project: str) -> bool:
        """
        Returns True if there's at least one filter configured for the specified project filtering on
        datasets, this can be used to check if get_sql_conditions for the project is going to return
        an empty query or not.
        """
        return self.is_metadata_filtered and any(f.matches(project=project) and f.dataset is not None
    ...
    def is_whole_project_blocked(self, project: str) -> bool:
        ...
        False as prj_1 is not fully blocked, there might be a (prj_1, ds_2) that is allowed, so prj_1
        still needs to be iterated.
        """
        # the condition parameter below is to include blocking filters only if applied to the whole project,
        # we don't want to exclude a project just because a dataset is excluded
        effect = self._get_effect(metadata_filters=self.metadata_filters,
                                  force_regexp=False,
                                  condition=lambda f: not f.dataset or f.effect == FilterEffectType.ALLOW,
                                  project=project)
        return effect == FilterEffectType.BLOCK

    def is_metadata_element_allowed(self, **kwargs) -> bool:
        """
        ...
        When the search completes, if there was a match then the result effect will be the one in the
        matched filter (must be the non-default effect).
    ...
    @staticmethod
    def _get_effect(metadata_filters: AllowBlockList,
                    force_regexp: bool,
        ...
        If there's an explicit filter matching (a filter is explicit if the effect is the default one)
        then default effect is returned.
        If there's a match in the "other effect" list then the "other effect" is returned, we're calling "other effect"
        to the effect that is not the default one.
        If no matching filter, the default effect is returned.
        """
        if not metadata_filters.filters or \
                any(f.matches(force_regexp, **kwargs)
                    for f in metadata_filters.get_default_effect_filters(condition=condition)):
            return metadata_filters.default_effect

        if any(f.matches(force_regexp, **kwargs)
    ...
    def is_dataset_allowed(self, project: Optional[str], dataset: str) -> bool:
        ...
        It returns True if the dataset in the given project is allowed (not blocked), this is equivalent
        to call is_metadata_element_allowed(project=project, dataset=dataset)
        """
    ...
    def get_sql_conditions(self,
                           ...
                           encoder: Callable[[str, str, bool], str],
                           project: str = None,
                           ) -> Optional[str]:
        """
        Helper method that returns a SQL query fragment with conditions for the current filters.
        If project is specified this will return conditions only for the specified project and this is
        supposed to be called after checking that is_project_with_datasets_filtered and
        is_project_allowed returned True.
        column_mapping is used to map filter fields (like project and dataset) to the actual database columns
        (like database and schema).
        encoder is used to encode a filter in the SQL dialect, it needs to encode to expressions like
        "database = 'db_1'" or "database REGEXP 'db_.*'".
        Examples:
        ...
        Basically we first put all filters matching the default condition joined by AND/OR (block/allow),
        and then all filters with the other effect joined by OR/AND.
        """
        ...
        default_effect_filters = self.metadata_filters.get_default_effect_filters(condition=project_condition)
        other_effect_filters = self.metadata_filters.get_other_effect_filters(condition=project_condition)
    ...
    def _get_sql_field_condition(mf: MetadataFilter,
        ...
        conditions = ' AND '.join([
            ...
            for (field, column) in column_mapping.items() if getattr(mf, field) is not None
        ])
