...
def test_with_mutation(client: Client):
    mutation = Mutation()
    mutation.create_or_update_user_settings(key="test", description="test description", value=json.dumps({"value":"test value"}))
    result = client(mutation).create_or_update_user_settings
