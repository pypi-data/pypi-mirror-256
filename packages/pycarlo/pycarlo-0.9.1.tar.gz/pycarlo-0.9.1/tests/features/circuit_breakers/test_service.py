...
SAMPLE_TRIGGER_V2 = Box({'trigger_circuit_breaker_rule_v2': {'job_execution_uuids': [SAMPLE_JOB_EXECUTION]}})
SAMPLE_ERROR = Box(
...
def sample_breach():
    return Box({'payload': {'breach_count': SAMPLE_BREACH_COUNT}}, default_box_attr=None, default_box=True)
...
def sample_payload():
    return Box({'stage': 'FREESTYLE', 'payload': {'me': 'silly'}}, default_box_attr=None, default_box=True)
...
class MockState:
    get_circuit_breaker_rule_state_v2 = [Box(
...
class MockOutOfOrderState:
    get_circuit_breaker_rule_state_v2 = [Box(
        {'status': 'PROCESSING_COMPLETE', 'log': json.dumps([sample_payload(), sample_breach(), sample_stage()])},
        default_box=True
    )]
...
class CircuitBreakerServiceTest(TestCase):
    ...
    @patch.object(CircuitBreakerService, 'poll_all')
    def test_trigger_and_poll_by_rule_name_no_breach(self, poll_all_mock):
        poll_all_mock.return_value = 0
        self.assertFalse(self._test_trigger_and_poll_with_rule_name(poll_all_mock=poll_all_mock))

    @patch.object(CircuitBreakerService, 'trigger_all')
    def _test_trigger_and_poll(self, trigger_all_mock, poll_mock) -> bool:
        trigger_all_mock.return_value = [SAMPLE_JOB_EXECUTION]

        in_breach = self._service.trigger_and_poll(rule_uuid=SAMPLE_RULE_ID)
        trigger_all_mock.assert_called_once_with(rule_uuid=SAMPLE_RULE_ID, namespace=None, rule_name=None)
        poll_mock.assert_called_once_with(job_execution_uuids=[SAMPLE_JOB_EXECUTION], timeout_in_minutes=5)

        return in_breach

    @patch.object(CircuitBreakerService, 'trigger_all')
    def _test_trigger_and_poll_with_rule_name(self, trigger_all_mock, poll_all_mock) -> bool:
        trigger_all_mock.return_value = [SAMPLE_JOB_EXECUTION]

        in_breach = self._service.trigger_and_poll(rule_name='test_rule')
        trigger_all_mock.assert_called_once_with(rule_uuid=None, namespace=None, rule_name='test_rule')
        poll_all_mock.assert_called_once_with(job_execution_uuids=[SAMPLE_JOB_EXECUTION], timeout_in_minutes=5)
    ...
    def test_trigger_all(self):
        ...
        self.assertEqual(self._service.trigger_all(rule_uuid=SAMPLE_RULE_ID), [SAMPLE_JOB_EXECUTION])
        self._client_mock.assert_called_once()
        self.assertEqual(
            self._trim_whitespace(self._client_mock.call_args[0][0]),
            self._trim_whitespace(
                f'mutation {{triggerCircuitBreakerRuleV2(ruleUuid: "{SAMPLE_RULE_ID}") {{jobExecutionUuids}}}}')
        )
    ...
    def test_poll_in_breach(self, internal_poll_mock):
        ...
        internal_poll_mock.assert_called_once_with(job_execution_uuids=[SAMPLE_JOB_EXECUTION], timeout_in_minutes=5)
    ...
    def test_internal_poll_with_timeout(self, mock_time):
        ...
        self.assertIsNone(self._service._poll(job_execution_uuids=[SAMPLE_JOB_EXECUTION], timeout_in_minutes=10))
