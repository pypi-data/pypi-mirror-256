...
from pycarlo.common.settings import DEFAULT_MCD_API_ID_HEADER, DEFAULT_MCD_API_TOKEN_HEADER, \
    ...
    DEFAULT_MCD_SESSION_ID, DEFAULT_MCD_TRACE_ID, DEFAULT_MCD_USER_ID_HEADER, DEFAULT_IGW_TIMEOUT_SECS
...
class Client:
    ...
    def session_scope(self):
        """
        Retrieves the scope from the client's current session, when a scope is set the client can be used
        only to call REST endpoints and not GraphQL endpoints.
        """
    ...
    def __call__(
        ...
        if self._session.scope:
            raise InvalidSessionError("A session initialized with a scope cannot be used for GraphQL calls")
    ...
    def make_request(
        ...
        :return: Returns the JSON dictionary returned by the endpoint or None if the response was empty.
        """
        if not self._session.scope:
            raise InvalidSessionError("A session initialized with a scope is required to call REST endpoints")
