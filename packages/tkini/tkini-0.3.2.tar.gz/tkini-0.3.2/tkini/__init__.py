...
class Window(Tk):
    ...
    def use_widgets_config(self, widgets_config: dict) -> None:
        ...
        for widget_id, widget_config in widgets_config.items():
            ...
            master_id = widget_config.pop("master_id", None)
            widget_layout = widget_config.pop("layout")
            widget_class = self._get_widget_class(widget_type)
            widget: AnyWidget = widget_class(self, **widget_config)
            widget.id = widget_id

            if master_id:
                master_widget = self.get_widget_by_id(master_id)

                if master_widget:
                    widget.grid(in_ = master_widget, **widget_layout)

                else:
                    logging.warning(f"Master widget with id '{master_id}' not found for widget with id '{widget_id}'.")
