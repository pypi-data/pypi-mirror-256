...
def _validate_env_vars():
    ...
    if _JOHNSNOWLABS_ENV_JSON_LICENSE_KEY not in os.environ:
        if not env_var_missing_logged:
            ...
            # TODO if endpoint env, this should raise exception.. but how2detect?! Or jsut awlays I guess
    else:
        _set_env_vars()
...
def _mlflowdbfs_path(run_id, artifact_path):
    ...
    return "{}:///artifacts?run_id={}&path=/{}".format(
        _MLFLOWDBFS_SCHEME,
        run_id,
        posixpath.join(artifact_path, _JOHNSNOWLABS_MODEL_PATH_SUB),
    )
...
def log_model(
    ...
    if auto_infer_signature:
        try:
            signature = infer_jsl_signature(spark_model)
        except Exception as err:
            _logger.warning(
                f"Could not auto-infer signature for your model. Error :\n", err
            )
...
def _save_jars_and_lic(dst_dir, store_license=True, gpu=False):
    ...
    # TODO DOWNLOAD JAR IF MISSING!?! OR FAIL?!
    ...
    if suite.nlp and suite.nlp.get_java_path():
        shutil.copyfile(
            suite.nlp.get_java_path(),
            deps_data_path / f"spark_nlp_{jar_type.value}.jar",
        )
    else:
        print("WARNING: No NLP jar found!")

    if (
        _JOHNSNOWLABS_ENV_VISUAL_SECRET in os.environ
        and suite.ocr
...
def save_model(
    ...
    inferred_signature = _unpack_and_save_model(spark_model, tmp_path)
    ...
    _save_model_metadata(
        ...
        conda_env=conda_env,
        code_paths=code_paths,
        signature=signature if signature else inferred_signature,
        input_example=input_example,
        pip_requirements=pip_requirements,
        extra_pip_requirements=extra_pip_requirements,
        store_license=store_license,
...
def infer_jsl_signature(spark_model):
    # Fit Pipe if not fitted. May fail on complex pipelines with multiple input cols
    if (
        hasattr(spark_model, "has_span_classifiers")
        and spark_model.has_span_classifiers
    ):
        data = "Init|||this"
    elif (
        hasattr(spark_model, "has_table_qa_models") and spark_model.has_table_qa_models
    ):
        data = "Init|||this"
    else:
        data = "Init|||this"

    os.environ['DB_ENDPOINT_ENV'] = 'True'
    ...
    output_data = spark_model.predict(input_data)
    signature = infer_signature(input_data,output_data)
    print('CREATED SIGNATURE: ', signature)
    return signature
...
def _unpack_and_save_model(spark_model, dst):
    ...
    try:
        ...
        print(
            f"Could  not save your model, use a different path for saving or remove {dst}. \n"
...
def _maybe_unpack_and_save_model(spark_model, dst):
    try:
        signature = _unpack_and_save_model(spark_model, dst)
        return True
    except Exception as err:
        return False
