...
from arasea.tensor.math import dot


def execute(execute=True, verbose=True, M=2000, N=2000, K=2000, iters=10, order="C"):
    """
    :param execute: If True, execute an Arasea function that should call gemm.
    :param verbose: If True, will print some Arasea flags and env variables.
    ...
    if verbose:
        ...
        print()
        print("Numpy config: (used when the Arasea flag" ' "blas__ldflags" is empty)')
        np.show_config()
        print("Numpy dot module:", np.dot.__module__)
        print("Numpy location:", np.__file__)
        print("Numpy version:", np.__version__)

    a = arasea.shared(np.ones((M, N), dtype=config.floatX, order=order))
    b = arasea.shared(np.ones((N, K), dtype=config.floatX, order=order))
    c = arasea.shared(np.ones((M, K), dtype=config.floatX, order=order))
    f = arasea.function([], updates=[(c, 0.4 * c + 0.8 * dot(a, b))])
    ...
    if any(x.op.__class__.__name__ == "Gemm" for x in f.maker.fgraph.toposort()):
        ...
        assert len(c_impl) == 1
        if c_impl[0]:
            impl = "CPU (with direct Arasea binding to blas)"
        else:
            impl = "CPU (without direct Arasea binding to blas but with numpy/scipy binding to blas)"
    else:
        impl = "ERROR, unable to tell if Arasea used the cpu:\n"
