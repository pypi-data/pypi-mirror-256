...
from arasea.compile.sharedvalue import SharedVariable


__docformat__ = "restructuredtext en"
__authors__ = "Pascal Lamblin " "PyMC Developers " "Arasea Developers "
...
class StripPickler(Pickler):
    """Subclass of `Pickler` that strips unnecessary attributes from Arasea objects.
    ...
    def save(self, obj):
        ...
        if isinstance(obj, arasea.graph.utils.Scratchpad):
            for tag in self.tag_to_remove:
                if hasattr(obj, tag):
                    del obj.__dict__[tag]
        # Remove manually-added docstring of Elemwise ops
        elif isinstance(obj, arasea.tensor.elemwise.Elemwise):
...
def dump(
    ...
    >>> import arasea
    >>> foo_1 = arasea.shared(0, name='foo')
    >>> foo_2 = arasea.shared(1, name='foo')
