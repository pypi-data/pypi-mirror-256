...
class ffpr:
    """
    >>> A CLASS FOR INTERFACING WITH FFPROBE TO ANALYZE MULTIMEDIA FILES.

    ... ATTRIBUTES
    ----------
    ... METHODS
    -------
    PROBE(INPUT_FILE) -> OPTIONAL[FFPROBERESULT]:
    ---------------------------------------------
        >>> ANALYZE MULTIMEDIA FILE USING FFPROBE AND RETURN THE RESULT.
    PRETTY(INFO):
    ----------------------------
        >>> PRINT READABLE SUMMARY OF THE FFPROBE ANALYSIS RESULT, MAKE BEAUTIFY CONTENT.
    ...
    def probe(self, input_file) -> Optional[FFProbeResult]:
        """
        >>> ANALYZE A MULTIMEDIA FILE USING FFPROBE AND RETURN THE RESULT.
        ...
        ... RETURNS
        -------
        OPTIONAL[FFPROBERESULT]
        -----------------------
            >>> RESULT OF THE FFPROBE ANALYSIS.
        """
        try:
            command = [
                self.ffprobe_path,
    ...
    def pretty(self, info: FFProbeResult):
        """
        >>> PRINT READABLE SUMMARY OF THE FFPROBE ANALYSIS RESULT.
