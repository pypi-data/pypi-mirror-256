...
class ProcessTopic:
    SMALL = 'PUBSUB_TOPIC_BATCH_AGGREGATE_SMALL'
    MEDIUM = 'PUBSUB_TOPIC_BATCH_AGGREGATE_MEDIUM'
    LARGE = 'PUBSUB_TOPIC_BATCH_AGGREGATE_LARGE'
...
class BatchWriteDetectAggregateOperator(BaseEventOperator):
    ...
    def execute(self, data, topic):
        ...
        for b in self.gcs_hook.list(bucket, prefix):
            if b.time_deleted is None:
                ...
                if tables[key]['size'] > threshold['size_large']:
                    self.send_to_process(bucket=bucket, directory=key, files=tables[key]['files'], size=ProcessTopic.LARGE, data=data)
                    tables[key] = None
                    partially_processed_tables.append(key)
                ...
                if (len(tables[key]['files']) > threshold['file_quantity']):
                    ...
                    elif tables[key]['size'] < threshold['size_medium']:
        ...
        for directory, v in tables.items():
            if v is not None:
                if (v['size'] < threshold['size_small']) and ((directory in partially_processed_tables) or (v['min_time_created'].strftime('%Y-%m-%d %H:%M') < time_threshold)):
                    self.send_to_process(bucket=bucket, directory=directory, files=v['files'], size=ProcessTopic.SMALL, data=data)
                elif (v['size'] < threshold['size_medium']) and ((directory in partially_processed_tables) or (v['min_time_created'].strftime('%Y-%m-%d %H:%M') < time_threshold)):
                    self.send_to_process(bucket=bucket, directory=directory, files=v['files'], size=ProcessTopic.MEDIUM, data=data)
                elif (v['size'] < threshold['size_large']) and ((directory in partially_processed_tables) or (v['min_time_created'].strftime('%Y-%m-%d %H:%M') < time_threshold)):
                    self.send_to_process(bucket=bucket, directory=directory, files=v['files'], size=ProcessTopic.LARGE, data=data)
...
class BatchAggregateParquetFilesOperator(BaseEventOperator):
    ...
    def execute(self, data, topic):
        ...
        self.gcs_hook.upload_parquet_from_memory(
            data=tables_union,
        ...
        self.reprocess_directory(directory, metadata)
    ...
    def read_parquet_from_gcs(self, bucket, directory, files):
        ...
        concat = None
        for f in files:
            # t = parquet.read_table(f'gs://{bucket}/{directory}/{f}', filesystem=gcs)
            t = parquet.read_table(f'{bucket}/{directory}/{f}', filesystem=gcs)
            concat = t if not concat else concat.join(t, ['_event_id', '_resource', '_json', '_created_at'], join_type='full outer')
    ...
    def reprocess_directory(self, directory, metadata):
        data = metadata['data']
        data['prefix'] = directory

        self.pubsub_hook.publish(
            project=get_config('GCP_PROJECT'),
            topic=get_config('PUBSUB_TOPIC_AGGREGATE_DETECT'),
            data=data)
