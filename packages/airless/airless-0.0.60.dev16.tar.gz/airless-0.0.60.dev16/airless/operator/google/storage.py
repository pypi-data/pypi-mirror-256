...
class BatchWriteDetectAggregateOperator(BaseEventOperator):
    ...
    def execute(self, data, topic):
        ...
        for b in self.gcs_hook.list(bucket, prefix):
            ...
            if (b.time_deleted is None) or (b.size < threshold['size']):
                filepaths = b.name.split('/')
                key = '/'.join(filepaths[:-1])  # dataset/table
                filename = filepaths[-1]
                ...
                if tables[key]['size'] > threshold['size']:
                    self.send_to_process(bucket=bucket, directory=key, files=tables[key]['files'], size='PUBSUB_TOPIC_BATCH_AGGREGATE', data=data)
                    tables[key] = None
                    partially_processed_tables.append(key)
        ...
        for directory, v in tables.items():
            if v is not None:
                if (len(v['files']) == 1) and (directory not in partially_processed_tables):
                    ...
                    self.send_to_process(bucket=bucket, directory=directory, files=v['files'], size='PUBSUB_TOPIC_BATCH_AGGREGATE', data=data)
