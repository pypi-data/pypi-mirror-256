...
class Party(WMLResource):
    ...
    def run(self, aggregator_id=None, experiment_id=None, asynchronous=True, verify=True, timeout: int = 60 * 10):
        ...
        **Examples**
        ...
        if details[0]["entity"]["federated_learning"]["fusion_type"] == "crypto_iter_avg":
            logger.info("Encryption is set as required")
            if not is_crypto_supported():
                logger.error("Encryption is required, but the '%s' module required for encryption was not found", CRYPTO_LIBRARY)
