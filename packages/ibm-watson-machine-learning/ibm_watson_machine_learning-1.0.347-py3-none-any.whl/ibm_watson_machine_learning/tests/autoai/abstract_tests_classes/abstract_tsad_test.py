...
class AbstractTestTSADAsync(abc.ABC):
    ...
    def test_34_score_deployed_model_with_df(self):
        ...
        print(self.scoring_df.shape)
        print(self.scoring_df)

        predictions = AbstractTestTSADAsync.service.score(payload=self.scoring_df)
        print(predictions)
        self.assertIsNotNone(predictions)

        self.assertEqual(len(predictions['predictions'][0]['values']), len(self.scoring_df))
        self.assertEqual(len(predictions['predictions'][0]['values'][0][0]),
                         len(self.experiment_info.get('feature_columns', [])))
