...
from .exceptions import InvalidChecksumError, ServerError
...
class SentinelApi:
    ...
    def _find_query(
        ...
        end_date = (
            f" and ContentDate/Start lt {end_date.isoformat()}" if end_date else ""
        )
        cloud_cover = (
        ...
        search_query = f"{self.catalogue_odata_url}/Products?$filter=contains(Name, '_T{tile}_'){cloud_cover} and Online eq true and Attributes/OData.CSC.StringAttribute/any(att:att/Name eq 'productType' and att/OData.CSC.StringAttribute/Value eq '{productType.value}'){init_date}{end_date}&$expand=Attributes{order_by}{limit}"
        return search_query
    ...
    def find(
        ...
        search_query = self._find_query(
            productType, tile, init_date, end_date, cloud_cover, limit, order_desc
        )
    ...
    def _get_access_token(self) -> str:
        ...
        if self._access_token is not None and current_timestamp - self._access_token_timestamp < self._access_token_duration:
            return self._access_token
        
        if self._refresh_token is not None and current_timestamp - self._refresh_token_timestamp < self._refresh_token_duration:
            data = {
