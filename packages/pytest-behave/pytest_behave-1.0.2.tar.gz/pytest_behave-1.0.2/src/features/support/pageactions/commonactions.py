"""
Common Actions is a pre-set collection of actions to achieve a repeated bit=
 of functionality.
"""

from features.support.locators.locator import Locator
from features.support.pageactions.actionenum import Action
...
class CommonActions(BasePage):
    ...
    def select_option_from_dropdown(self, locator: Locator, text: str, delay=0.5):
        ...
        self.select_option_with_action(
            locator=locator, text=text, action=Action.ENTER_KEY, delay=delay
        )

    def select_option_with_action(
        self, locator: Locator, text: str, action: Action = Action.ENTER_KEY, delay=0.5
    ):
        """utility method that will check locator displayed, enter text with action chains, and then press enter"""
