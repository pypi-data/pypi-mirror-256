...
def run(
    ...
    trigger_run_request = create_trigger_run_request(project_dir, id, materialize, branch_prefix, args)
    ...
    run_state = create_run_state(job_id, project_dir)
    process_logs(log_stream, run_state)
    return run_state


def create_trigger_run_request(
    project_dir: str,
    id: str,
    ...
    upload_files = [
        file for file in files if file.endswith(('.py', '.sql', 'requirements.txt', 'bauplan_project.yml'))
    ]

    if 'bauplan_project.yml' not in upload_files:
...
def create_run_state(job_id: str, project_dir: str) -> RunState:
    return RunState(
        job_id=job_id,
...
def process_logs(log_stream: grpc.Call, run_state: RunState) -> None:
    for log in log_stream:
        if os.getenv('BPLN_DEBUG'):
            print(log)
        if handle_log(log, run_state):
            break


def handle_log(log: RunnerInfo, run_state: RunState) -> bool:
