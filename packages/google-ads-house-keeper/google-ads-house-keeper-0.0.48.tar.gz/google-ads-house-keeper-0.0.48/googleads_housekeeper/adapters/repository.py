...
class AbstractRepository(abc.ABC):
    ...
    def get_by_conditions(self, conditions: dict[str, Any]) -> Task:
    ...
    @abc.abstractmethod
    def _get_by_conditions(self, conditions: dict[str, Any]) -> Task:
        ...
...
class FirestoreRepository(AbstractRepository):
    ...
    def _add(self, element):
        ...
        self.client.collection(
            self.collection_name).document(str(element_id)).set(element_dict)

    def _get(self, task_id: str):
        doc = self.client.collection(
            self.collection_name).document(task_id).get()
        if doc.exists:
    ...
    def _get_by_conditions(self, conditions: dict[str, Any]):
        try:
            ...
            results = [
                self.entity(**result.to_dict()) for result in query.stream()
            ]
            return results
        except Exception:
            return []
    ...
    def _update(self, task_id: str, update_dict: dict[str, Any]):
        if doc := self.client.collection(
                self.collection_name).document(task_id):
