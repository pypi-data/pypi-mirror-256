...
class ThumbnailCreator:
    ...
    def preprocess_image(self, img_arr: np.ndarray, modality: str, body_part_examined: str) -> np.ndarray:
        """
        ...
        if modality == 'CT':
            if body_part_examined == 'HEAD':
                img_arr = np.clip(img_arr, a_min=0, a_max=80)  # 0 and 80 are used to highlight Head
            elif body_part_examined == 'ABDOMEN' or body_part_examined == 'PELVIS':
                img_arr = np.clip(img_arr, a_min=-150, a_max=250)
