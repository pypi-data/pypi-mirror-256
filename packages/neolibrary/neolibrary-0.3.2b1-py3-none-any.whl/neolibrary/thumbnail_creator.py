...
class ThumbnailCreator:
    ...
    def clamp_CT(self, img_arr: np.ndarray) -> np.ndarray:
        """
        Clamps the values in a NumPy array representing a CT scan image.
        ...
        """
        img_arr = np.clip(img_arr, a_min=0, a_max=80)  # 0 and 80 are used to highlight Head
        return img_arr

    def is_ct_check(self, img_arr: np.ndarray) -> bool:
        """
        Checks if the values in a NumPy array are indicative of a CT scan image.

        Args:
        img_arr (np.ndarray): Input NumPy array to be checked.

        Returns:
        bool: True if the values suggest a CT scan image, False otherwise.
        """
        values = np.unique(img_arr)
        lower_than_threshold = -500
        is_lower = np.any(values < lower_than_threshold)
        return is_lower
        ...
        try:
            ...
            if msk_arr is not None:
                for counter, label in enumerate(labels):
                    ...
                    colormap = self.hex_to_rgba(colorhex)
    ...
    def create_img(
        ...
        try:
            ...
            thumbnail_slice, seg_slice = self.transform_rgb_array(
            ...
            is_ct: bool = self.is_ct_check(img_arr=thumbnail_slice)
            if is_ct:
                thumbnail_slice = self.clamp_CT(img_arr=thumbnail_slice)
    ...
    def hex_to_rgba(self, hex_string: str, alpha: float = 1.0) -> List[int]:
