...
class Filters:
    ...
    @staticmethod
    def has_1_segment():
        return Filters.has_n_segments(1)

    @staticmethod
    def has_2_segments():
        return Filters.has_n_segments(2)

    @staticmethod
    def has_3_segments():
        return Filters.has_n_segments(3)
...
def get_sitemaps_urls(request_options, urls):
    ...
    def sitemap_json(req, data):
        ...
        parsed = sitemap_json(wrap_in_sitemap(locs))
...
def get_sitemaps_from_robots(request_options, urls):
    ...
    for url in urls:
        temp = sitemap_json(clean_robots_txt_url(url)) if is_empty_path(url) else url
        ls.append(temp)
...
def clean_url(base_url, url: str) -> bool:
    ...
    print(f"Testing if URL '{url}' is HTTP(s) URL")
...
def parse_sitemaps_from_robots_txt(base_url, robots_txt_content):
    """
    Parses sitemap URLs from the content of a robots.txt file.

    :param robots_txt_content: The content of the robots.txt file as a string.
    :return: A list of unique sitemap URLs found in the robots.txt content.
    """
