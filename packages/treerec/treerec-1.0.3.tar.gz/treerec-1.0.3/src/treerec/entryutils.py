...
def print_os_file_info(filename,
    ...
    pathlike = Path(filename).resolve().absolute()
    ...
    printouts = {
        ...
        'owner': '  Owner: ' + owner,
        'type': '  Type: ' + ('dir' if pathlike.is_dir() else 'file'),
...
def new_entry_in_dir(parent_dict, objname, device=None, os_type=None, session=None, quiet=False, do_hash=True):
    ...
    obj_pathlike = Path(parent_localpath).resolve().absolute() / objname
...
def tie_files(dict1, dict2, session=None):
