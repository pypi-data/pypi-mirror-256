...
class TreeObj_Cursor():
    ...
    def prompt(self, default=False):
        if self.engine.CTREE==None or default:
            return "   :> "
    ...
    def set_session_name(self, parameters):
        ...
        if name=='':
            print("Cannot have blank session name.")
            return
    ...
    def ls(self, parameters):
        ...
        if second is None:
            self.engine.ls_tree(None, first, recorded_only, show_mtime)
        else:
            self.engine.ls_tree(first, second, recorded_only, show_mtime)

    def cd(self, parameters):
        first = retrieve(parameters, 'first')
        second = retrieve(parameters, 'second')
        if second is None:
            self.engine.cd_tree(None, first)
        else:
            self.engine.cd_tree(first, second)
    
    def cdls(self, parameters):
        """cdls: cd, then ls."""
        if retrieve(parameters, 'input2'):
            treename = retrieve(parameters, 'input1')
            filename = retrieve(parameters, 'input2')
        else:
            treename = None
            filename = retrieve(parameters, 'input1')
        cd_success = self.engine.cd_tree(treename, filename)
    ...
    def rec(self, parameters):
        """Record entries
        If -a: default to current directory if no file is put in
        If -r: record recursively.
        """
        ...
        if use_localpath and recursive:
            ...
            return
        if use_localpath:
            rec_success = self.engine.lrec(None, path, verbose=bool(verbose))
        elif recursive:
            rec_success = self.engine.rrec(None, path, levels=levels, verbose_levels=verbose)
        else:
            rec_success = self.engine.rec(None, path, verbose=bool(verbose))
        if not rec_success:
            return
    ...
    def tie(self, parameters):
        ...
        if retrieve(parameters, 'all'):
            self.engine.a_tie(treename, source, destination)
        else:
            self.engine.tie(treename, source, destination)
    ...
    def note(self, parameters):
        ...
        if action=='show':
            if retrieve(parameters, 'input2'):
                ...
                print('Error: further input required.')
                return
            self.engine.notes(treename, filename)
            return
        elif action=='add':
            filename = retrieve(parameters, 'input1')
            note_text = retrieve(parameters, 'input2')
            self.engine.add_note(None, filename, note_text)
        elif action=='remove':
            filename = retrieve(parameters, 'input1')
            ...
            try:
                ...
                return
            self.engine.remove_note(None, filename, idx)
        else:
...
def print_help(cmd):
    ...
    if cmd==cmd_help:
        ...
        helptext = (
            ...
            '',
            '-a  --all   Tie together all entries which share a name inside of the *directory* FILE A',
            '            and the *directory* FILE B.'
        )
