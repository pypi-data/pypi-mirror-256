...
def ls_dict(dirin:dict, recorded_only:bool=False, show_mtime:bool=False):
    ...
    if dirin['type']==DEVINFO:
        for rootentry in dirin['dir_list']:
            ...
            print('  <> ' + rootentry.get('nickname', 'ERROR') +
                  '  //  path: (' + pathprint + ')' +
                  '   local: [' + localprint + ']')
        return
    for entry in dirin['dir_list']:
        if not (recorded_only and entry['type']=='unrecorded'):
            print(' ' + 
                  ('x' if entry['type']=='unrecorded' else ' ') +
                  ('*' if len(entry.get('notes', [])) > 0 else ' ') +
                  '[] ' + entry['name'] + 
                  ('     ' + time_formatted(entry['mtime']) 
                    if (show_mtime and entry['type']!='unrecorded')
                     else '')
                )
    for entry in dirin['file_list']:
        if not (recorded_only and entry['type']=='unrecorded'):
            print(' ' + 
                  ('x' if entry['type']=='unrecorded' else ' ') +
                  ('*' if len(entry.get('notes', [])) > 0 else ' ') +
...
class TreeObj():
    ...
    The top-level dictionary is named DEVINFO (defined in dictutils, defaults to 'devinfo'), and has device informaiton.
    ...
    def set_savefile(self, filename=None, quiet=False, force=False):
        ...
        if os.path.exists(filename):
            ...
            if not force:
                ...
                return False
        self.savefile = Path(filename).resolve().absolute()
        self.savefile_active = True
    ...
    def ls(self, dirpath:str=None, recorded_only:bool=False, show_mtime:bool=False):
        if not dirpath:
            dirin = self.CDIR
        else:
        ...
        ls_dict(dirin, recorded_only, show_mtime)
    ...
    def recroot(self, root_localpath:str, nickname:str=None, treepath:str=None):
        ...
        """
        root_localpathlike = Path(root_localpath).resolve()
        if not root_localpathlike.is_dir():
        ...
        if treepath_tup in current_root_treepaths:
            ...
            return False
        newentry = create_dict(root_localpathlike, rootname=nickname, nickname=nickname, session=self.session, do_hash=self.hash_files)
    ...
    def root_set_localpath(self, rootname:str, localpath:str):
        ...
        if root['type']==DEVINFO:
            ...
            return
        pathparts = Path(localpath.strip()).resolve().absolute().parts
    ...
    def lrec(self, pathin:str, verbose:bool=False, quiet:bool=False) -> bool:
        ...
        if type(pathin) is Path:
            ...
            pathtuple = Path(pathin).resolve().absolute().parts
        else:
