...
class TypedColumns:
    ...
    ------
        ...
        e.g. feature_column_names=TypedColumns(
                untyped_columns=["feature_1", "feature_2"],
                categorical_columns=["feature_3"],
                integer_columns=["feature_4"]
            )

    Fields:
    -------
        untyped_columns (Optional[List[str]]): List of columns that will not be altered at all.
            There's no difference between passing in all column names as untyped_columns
            vs. not using TypedColumns at all.
        categorical_columns (Optional[List[str]]): List of columns that should be cast to pandas StringDType.
        integer_columns (Optional[List[str]]): List of columns that should be cast to pandas Int64DType.
        float_columns (Optional[List[str]]): List of columns that should be cast to pandas Float64DType.
    ...
    untyped_columns: Optional[List[str]] = None
    ...
    def get_all_typed_names(self) -> Set[str]:
        return {name for name in self.get_all_column_names() if name not in self.untyped_columns}

    def has_duplicate_columns(self) -> bool:
