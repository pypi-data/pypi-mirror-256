...
def main(simname, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
    ...
    if option not in ["0","1","2"]:
        print("ERROR: Error in grid_connection.py. Please contact Support.")
        return False
    ...
    if isinstance(setting_df, pd.DataFrame):
        ...
        print("Data Reading Failed. Check your settings.")
        return False
    ...
    if option == "2":
        ...
        if not SharedMethods.check_existing_file(filename):
            print("ERROR: d4 file {} do not exist. Select option 1 or check Supply Point ID to proceed.".format(feeder))
            return False
...
def result_rvc_update(simname, result_df, pri_v, fault, event_f,event_if,event_vif):
    ...
    vo_impedance = 2* fault / (pri_v*pri_v)  #vision oslo equivalent source impedance
    ...
    if isinstance(event_f, (int, float)) and not np.isnan(event_f):
        temp = (event_f * nominal_v / pri_v) * vo_impedance / pri_v # current at primary * impedance / primary nominal voltag
    ...
    if isinstance(event_if, (int, float)) and not np.isnan(event_if):
        temp = (event_if * nominal_v / pri_v) * vo_impedance / pri_v # current at primary * impedance / primary nominal voltag
    ...
    if isinstance(event_vif, (int, float)) and not np.isnan(event_vif):
        temp = (event_vif * nominal_v / pri_v) * vo_impedance / pri_v # current at primary * impedance / primary nominal voltag
...
def check_input_info(sec_v,time_increment,feeder,pri_v,max_op_range,min_op_range,fault):
    ...
    if 0 <= time_increment <= 5:
        print("Simulation Time Increment was Set to {} second.".format(time_increment))
    else:
        print("ERROR: Please set simulation time increment between 1-5 and restart the process....")
        return False
    
    if isinstance(sec_v, str) or np.isnan(sec_v):
        print('ERROR: Please check input in Modelled Supply Voltage. Value Required...')
        return False
    else:
        if sec_v < 10 or sec_v > 450:
            print('WARNING: Norminal Voltage in kV. Abnormal norminal votage detected.')
    
    if isinstance(pri_v, str) or np.isnan(pri_v):
        print('ERROR: Please check input in Nominal Supply Voltage. Value Required...')
        return False
    else:
    ...
    if isinstance(max_op_range, str) or np.isnan(max_op_range):
        print('ERROR: Check Maximum Operating Range. Value Required...')
        return False
    
    if isinstance(min_op_range, str) or np.isnan(min_op_range):
        print('ERROR: Check Minimum Operating Range. Value Required...')
        return False
    
    if isinstance(fault, str) or np.isnan(fault):
        print('ERROR: Please check input in Fault Level. Value Required...')
        return False
    
    if len(feeder) > 4:
        print('WARNING: Feeder ID larger than 4 digit. Extraction is not possible...')
