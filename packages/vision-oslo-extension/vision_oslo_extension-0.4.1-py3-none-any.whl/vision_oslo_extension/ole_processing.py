...
def main(simname, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
    ...
    if option not in ["0","1","2"]:
        print("ERROR: Error in ole_processing.py. Please contact Support...")
        return False
    ...
    if option == "1":
        ...
        for branch_id in branch_list:
            oslo_extraction.branch_step(simname, time_start, time_end, "1", branch_id)
    ...
    if option == "2":
        ...
        for branch in branch_list_new:
            ...
            if not SharedMethods.check_existing_file(filename):
                print("ERROR: d4 file {} do not exist. Select option 1 or check OSLO ID to proceed.".format(branch))
                return False
    ...
    for branch_id in branch_list:
            if len(branch_id) <= 6:
                if branch_id[-2:-1] =="/":
                    ...
                    print('ERROR:Branch OSLO ID {} does not contains proper symbol. Check Input'.format(branch_id))
                    return False
            else:       
                print('ERROR:Branch OSLO ID {} is NOT in correct format. Check Input'.format(branch_id))
                return False
...
def check_rating_list(sheet):
    ...
    if sheet.cell(row=index, column=column).value is not None:
        ...
        print("Wrong data format. No information at G12")
        return False
...
def check_table_list(sheet):
    ...
    if sheet.cell(row=index, column=column+2).value is not None:
        ...
        print("Wrong data format. No information at C12")
        return False
