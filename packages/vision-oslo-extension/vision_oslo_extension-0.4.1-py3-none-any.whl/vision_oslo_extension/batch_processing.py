...
def main(simname, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
    ...
    if main_option == "9":
        ...
        substation_process(simname, option_select,time_start, time_end,time_windows,time_increment)

    if main_option == "10":
        text_input = 'BranchNodeList.txt'
        time_increment = 5
        branch_data_process(simname, option_select,time_start, time_end, time_step,text_input,time_increment)
...
def substation_process(simname, option_select,time_start, time_end,time_windows,time_increment):
    ...
    if option_select == "2":
        ...
        if not os.path.isfile(name):
            print("Error: Please provide the GridAllocation.csv. Process terminated...")
            return False 
        
        file = simname + '_30min_AVEpower_sum.csv'
        if not os.path.isfile(file):
            print("Error: Please select option 1 First to calculate the Summary First. Process terminated...")
            return False 
        
        columns = ['Substation','OSLO ID','Grid Feeder']
        grid_df = pd.read_csv(name, usecols=[0, 1, 2])  # Specify column indices
        ...
        feeder_list,sum_df_list = grid_power_processing(grid_df,time_increment)

        # Ready data prepare:
        sum_df_1, sum_df_2 = grid_summary_output(grid_df,sum_df_list,feeder_list,sum_df)

        # print result to excel
        result_to_excel(simname, sum_df_1,sum_df_2)
...
def d4_file_process(simname,feeder,time_windows,time_increment):
    ...
    # power process (30min power)
    powersec = 1800
    column_name = 'P_30min_Ave (MW)'
    column_name_list.append(column_name)
    df[column_name] = df['P_inst'].rolling(window=int(powersec/time_increment)).mean()

    # insert empty columns
    df.insert(df.columns.get_loc('I_Square'),'New_C_1', np.nan)
    df.insert(df.columns.get_loc('P_30min_Ave (MW)'),'New_C_2', np.nan)
...
def rms_summary_output(simname,df_list,sum_df_list,time_windows,feeder_list):
    ...
    for time in time_windows:
        print("Processing {} RMS Current Summary...".format(time))
        # dataframe column name
        if time[0] == "P":
            time = time[1:]
            column = 'P_30min_Ave (MW)'
...
def grid_power_processing(grid_df,time_increment):
    # Group by Grid Feeder and list OSLO IDs for each feeder
    ...
    for feeder, oslo_ids in grouped.items():
        ...
        for oslo in oslo_ids:
            ...
            if not os.path.isfile(filename):
                print("Warning: OSLO Step Output file {} does NOT exist. Skipping....".format(filename))
        ...
        time_window = 1800
        merged_df['P30min'] = pd.to_numeric(merged_df['Total'].rolling(window=int(time_window/time_increment)).mean(), errors='coerce')
...
def grid_summary_output(grid_df,sum_df_list,feeder_list,sum_df):
# sum_df
#          OSLO ID                PRLY                COUL
...
# 4       07:00:10                 NaN                 NaN
    ...
    for index, df in enumerate(sum_df_list):
        ...
        time = df.loc[0,"P30min"]
        power = df.loc[1,"P30min"]
    ...
    for index, df in enumerate(sum_df_list):
        ...
        df_oslo = df.drop(columns=['Time', 'Total', 'P30min'])
        ...
        for oslo in columns:
            ...
            if oslo in sum_oslo_list:
                ...
                oslo_time_abs  = pd.to_numeric(sum_df.loc[0,oslo], errors='coerce')
                oslo_power_abs = pd.to_numeric(sum_df.loc[1,oslo], errors='coerce')
                if power != 0:
                    oslo_contribute = oslo_power_at/power
                    oslo_diff = oslo_power_at/power
                else:
                    oslo_contribute = 1
                    oslo_diff = 1
            else:
    ...
    columns = ['Grid Name','at Time (hh:mm:ss)','Max Power (MW)','Substation','OSLOID', \
               'Power at Max Grid (MW)','Sub Contribute %', 'Time at Sub Abs(hh:mm:ss)', \
...
# print result to excel
def result_to_excel(simname,sum_df_1,sum_df_2):
    print('Writting Summary to Excel Ready Format....')

    filename = simname + '_30min_Grid_Summary.xlsx'
    ...
    with pd.ExcelWriter(filename, engine='openpyxl') as writer:
        ...
        sum_df_1.to_excel(writer, sheet_name='summary', startrow=2, startcol=1, index=False)
        
        # Write sum_df_2 to cell G3
        sum_df_2.to_excel(writer, sheet_name='summary', startrow=2, startcol=6, index=False)

        # formatting
        sheet = wb['summary']
        sheet['B2'].value = 'Grid 30 min Average Power Summary'

        sheet['G2'].value = 'Detailed 30min Maximum Average Power Summary'

        minrow = 2 + 1
        maxrow1 = len(sum_df_1) + minrow
        maxrow2 = len(sum_df_2) + minrow
...
def branch_data_process(simname, option_select,time_start, time_end, time_step, text_input, time_increment):
        ...
        for node in branch_input:
            if node[-2:] == '/S' or node[-2:] == 'E':
               pass
                ...
                print("ERROR: BranchNodeList.txt has information {} which is not the supported format.",format(node))
                print("ERROR: Choose Option 1-3, or modify BranchNodeList.txt")
                return False
    ...
    if option1 in ["4","5","6"]:
        ...
        for item in branch_input:
            if item in step_df.columns:
                ...
                print("WARNING: Input File Node {} does NOT exist in the simulation. Ignored....".format(item))
    ...
    if option1 == "3":
        print("\nProcessing RMS calculation...")
        rms_df,max_df = branch_list_RMS_process(simname,branch_df,second,time_increment)
        print('\nSaving branch RMS Summary output summary...')
...
if __name__ == "__main__":
    ...
    time_end = "0093000"  # Adjust as needed
    option_select = "3"  # Adjust as needed
    text_input = "BranchNodeList"  # Adjust as needed
