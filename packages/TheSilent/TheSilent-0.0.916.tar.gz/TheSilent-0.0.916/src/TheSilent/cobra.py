...
def cobra(host,delay=0,crawl=1):
    ...
    for mal in mal_command:
        new_mal.append("$(echo -e '" + str(" " + " ".join(format(ord(char), "x") for char in mal)).replace(" ", r"\x") + "')")
        new_mal.append(urllib.parse.quote("$(echo -e '" + str(" " + " ".join(format(ord(char), "x") for char in mal)).replace(" ", r"\x") + "')"))
        new_mal.append(urllib.parse.quote(urllib.parse.quote("$(echo -e '" + str(" " + " ".join(format(ord(char), "x") for char in mal)).replace(" ", r"\x") + "')")))
    mal_command = new_mal[:]
