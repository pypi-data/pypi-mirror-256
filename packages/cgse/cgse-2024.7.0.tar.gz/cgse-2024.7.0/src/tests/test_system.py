...
def test_timer():
    ...
    # Confirm that the elapsed time is not increased anymore
...
def test_read_last_line():
    ...
    line = read_last_line(prefix / "data/empty_data_file.txt")
    assert line == ""

    line = read_last_line(prefix / "data/data_file.txt")
...
# The following is a slow test, it takes about 2'.
...
def test_get_module_location():
    ...
    assert get_module_location(egse) == Path(egse.__file__).parent
    assert get_module_location("egse") == Path(egse.__file__).parent

    # egse.system is a module (.py file) and its location is in the `egse` module
    assert get_module_location(egse.system) == Path(egse.__file__).parent
    assert get_module_location('egse.system') == Path(egse.__file__).parent
    assert get_module_location(get_module_location) == Path(egse.__file__).parent

    # egse.logger is a package, so its location is in the logger folder
    assert get_module_location(egse.logger) == Path(egse.logger.__file__).parent
    assert get_module_location("egse.logger") == Path(egse.logger.__file__).parent
