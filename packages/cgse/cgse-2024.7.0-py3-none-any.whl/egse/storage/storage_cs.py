...
def status(full):
    ...
    rich.print("Storage Manager:")
    if is_storage_manager_active():
        rich.print("  Status: [green]active")
        with StorageProxy() as sm:
            rich.print(f"  Hostname: {sm.get_ip_address()}")
            rich.print(f"  Monitoring port: {sm.get_monitoring_port()}")
            rich.print(f"  Commanding port: {sm.get_commanding_port()}")
            rich.print(f"  Service port: {sm.get_service_port()}")
            rich.print(f"  Storage location: {sm.get_storage_location()}")
            rich.print(f"  Loaded Setup: {sm.get_loaded_setup_id()}")
            rich.print(f"  Registrations: {sm.get_registry_names()}")
            if full:
                rich.print("Filenames for all registered items:")
                for origin in sm.get_registry_names():
                    fn = [x for x in sm.get_filenames({"origin": origin})]
                    rich.print(f"  {origin:10s} ->  {fn}")
                if obsid := sm.get_obsid():
                    rich.print(f"An observation is registered: {obsid}")
                else:
                    rich.print("No observation is registered.")

                total, used, free = sm.get_disk_usage()
                rich.print(f"Total disk space: {humanize_bytes(total)}")
                rich.print(f"Used disk space: {humanize_bytes(used)} ({(used / total * 100):.2f}%)")
                rich.print(f"Free disk space: {humanize_bytes(free)} ({(free / total * 100):.2f}%)")
    else:
...
def check_prerequisites():
    ...
    # logger.debug(f"Storage location for data = {location}")
