...
from egse.stages.arun.smd3 import Smd3Protocol, Smd3Proxy
...
class Smd3ControlServer(ControlServer):
    """
    ...
    * a PUB socket that serves as a monitoring server. It will send out Lamp status
    ...
    def __init__(self):
        ...
        self.device_protocol = Smd3Protocol(self)
...
@cli.command()
@click.option("--simulator", "--sim", is_flag=True, help="Start the SMD3 Simulator as the backend.")
def start(simulator):
    """Start the SMD3 Control Server."""

    if simulator:
        Settings.set_simulation_mode(True)
    try:
        controller = Smd3ControlServer()
        controller.serve()
        ...
        logger.exception("Cannot start the SMD3 Control Server")
