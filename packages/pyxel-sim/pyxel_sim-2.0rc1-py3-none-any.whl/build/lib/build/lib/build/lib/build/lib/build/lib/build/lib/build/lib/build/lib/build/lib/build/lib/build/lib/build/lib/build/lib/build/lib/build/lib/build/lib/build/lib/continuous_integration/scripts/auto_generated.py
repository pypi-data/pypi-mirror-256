...
class ModelSceneGenerationLoadStarMapArguments(Mapping[str, Any]):
    ...
    band: Literal['blue_photometer', 'gaia_band', 'red_photometer'] = field(
        ...
        metadata=schema(
            title='band'
            ,description=(
                    "Define the band to use. * 'blue_photometer' is the band from 330 nm to"
                    " 680 nm * 'gaia_band' is the band from 330 nm to 1050 nm * "
...
class ModelPhotonCollectionOpticalPsfArguments(Mapping[str, Any]):
    ...
    wavelength: Union[int, float, tuple[float, float], None] = field(
        default=None,
        metadata=schema(
            title='wavelength'
    ...
    pixelscale: Optional[float] = field(
        default=None,
        metadata=schema(
            title='pixelscale'
            ,description='Pixel scale of detector, default is None.'
        )
    )
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('fov_arcsec', 'optical_system', 'wavelength', 'pixelscale', 'apply_jitter', 'jitter_sigma'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
class ModelPhotonCollectionSimpleCollectionArguments(Mapping[str, Any]):
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('aperture', 'filter_band', 'resolution', 'pixelscale', 'integrate_wavelength'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
class ModelChargeGenerationRadiationInducedDarkCurrentArguments(Mapping[str, Any]):
    ...
    shot_noise: bool = field(
        metadata=schema(
            title='shot_noise'
            ,description='True to enable shotnoise.'
        )
    )
    seed: Optional[int] = field(
        default=None,
    ...
    express: int = field(
        ...
        metadata=schema(
            ...
            ,description=(
                    ...
                    '     * ``express = 2`` means the transfers are re-computed half-way '
                    'through readout.     * ``express = N`` where ``N`` is the total number'
                    ' of pixels.  Default ``express = 0`` is a convenient input for '
                    'automatic ``express = N``.'
                )
        )
    )
