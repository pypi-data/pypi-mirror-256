...
class EntityExtractor(LoadModel, PreprocessText, LoadSecondModel, LoadThirdModel):
    ...
    @staticmethod
    def split_before_mv(text):
        split_text = text.split(" mv")
        output = []
        for fragment in split_text:
            output.append("mv" + fragment)
        return output
    ...
    def extract_entities(self, text):
        ...
        if cleaned_text_parts is not None:
            text_fragments = self.split_before_mv(cleaned_text_parts[1])
            futures = []
            for fragment in text_fragments:
                entities_dict = defaultdict(list)
                for nlp_func in [self.__nlp_first, self.__nlp_second, self.__nlp_third]:
                    future = self.__executor.submit(self.process_nlp, nlp_func, entities_dict, fragment)
                    futures.append(future)
                all_entities.append(entities_dict)
