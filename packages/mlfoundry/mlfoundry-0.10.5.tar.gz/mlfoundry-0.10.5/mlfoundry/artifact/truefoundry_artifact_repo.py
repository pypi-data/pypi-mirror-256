...
class ArtifactIdentifier(BaseModel):
    ...
    dataset_fqn: Optional[str]

    @root_validator
    def _check_identifier_type(cls, values: dict):
...
class MlFoundryArtifactsRepository(ArtifactRepository):
    ...
    def get_signed_urls_for_read(
        ...
        if artifact_identifier.artifact_version_id:
            return self._tracking_client.get_signed_urls_for_artifact_version_read(
                version_id=artifact_identifier.artifact_version_id,
                paths=paths,
            )
        else:
            return self._tracking_client.get_signed_urls_for_dataset_read(
                fqn=artifact_identifier.dataset_fqn,
                paths=paths,
            )
    ...
    def get_signed_urls_for_write(
        ...
        if artifact_identifier.artifact_version_id:
            ...
            return self._tracking_client.get_signed_urls_for_dataset_write(
                fqn=artifact_identifier.dataset_fqn,
                paths=paths,
            )
    ...
    def create_multipart_upload_for_identifier(
        ...
        if artifact_identifier.artifact_version_id:
            return self._tracking_client.create_multipart_upload(
                artifact_version_id=artifact_identifier.artifact_version_id,
                path=path,
                num_parts=num_parts,
            )
        else:
            return self._tracking_client.create_multipart_upload_for_dataset(
                fqn=artifact_identifier.dataset_fqn,
                path=path,
                num_parts=num_parts,
            )
    ...
    def list_files(
        ...
        if artifact_identifier.dataset_fqn:
            return self._tracking_client.list_files_for_dataset(
                fqn=artifact_identifier.dataset_fqn,
                path=path,
