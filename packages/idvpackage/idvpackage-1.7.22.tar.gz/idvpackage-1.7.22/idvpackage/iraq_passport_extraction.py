...
def iraq_passport_extraction(passport_text):
    ...
    patterns = {
        ...
        'dob': (r'(\d+)[MF]', lambda match: convert_dob(match.group(1)) if match else ''),
        'expiry_date': (r'[MF](\d+)', lambda match: convert_expiry_date(match.group(1)) if match else ''),
    ...
    mrz2_keys = ['passport_gender', 'passport_number_mrz', 'dob', 'expiry_date']
    ...
    if not (passport_details.get('dob') or passport_details.get('expiry_date')):
        dob_pattern = r"(\d{7})[MF]"
        dob_match = re.search(dob_pattern, passport_text_clean)
        if dob_match:
            dob = dob_match.group(1)
            passport_details['dob'] = convert_dob(dob)
        else:
            dob_pattern = r'.*?[\S]R[\S](\d{9,})\b'
            dob_match = re.search(dob_pattern, passport_text_clean)
            if dob_match:
                dob = dob_match.group(1)[:7]
                passport_details['dob'] = validate_date(convert_dob(dob))
    
        doe_pattern = r"[MF](\d+)"
        doe_match = re.search(doe_pattern, passport_text_clean)
        if doe_match:
            expiry = doe_match.group(1)
            passport_details['expiry_date'] = validate_date(convert_expiry_date(expiry))
        else:
