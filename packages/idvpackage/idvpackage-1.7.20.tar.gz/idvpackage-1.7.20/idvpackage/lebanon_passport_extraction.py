...
def lebanon_passport_extraction(passport_text):
    ...
    patterns = {
        'legal_id': (r"([A-Za-z]\d{8}|[A-Za-z]\d{7}|[A-Za-z]{2}\d{7})", lambda match: match.group(1) if match else ''),
    ...
    if passport_details['passport_number_mrz'] and (passport_details['passport_number_mrz']!=passport_details['legal_id']):
        passport_details['legal_id'] = passport_details['passport_number_mrz']
    ...
    passport_details['date_of_birth'] = get_dates_to_generic_format(dob)
    passport_details['register_expiry_date'] = get_dates_to_generic_format(expiry)
    passport_details['register_issue_date'] = get_dates_to_generic_format(issue_date)
    
    ## HANDLE MISSED DATE FIELDS
    if not passport_details['date_of_birth']:
        passport_details['date_of_birth'] = passport_details['date_of_birth_mrz']
    if not passport_details['register_expiry_date']:
        passport_details['register_expiry_date'] = passport_details['date_of_expiry_mrz']
    
    ## HANDLE MISSED NAME FIELDS
    if not passport_details['first_name']:
        passport_details['first_name'] = passport_details['first_name_mrz']
    if not passport_details['last_name'] or passport_details['last_name'] != passport_details['surname']:
