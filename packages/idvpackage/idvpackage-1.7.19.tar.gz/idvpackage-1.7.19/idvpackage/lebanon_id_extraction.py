...
def detect_id_card(client, image_data, id_text):
    ...
    if id_text:
        ...
        return id_text, id_card, second_part_text
    else:
...
def lebanon_front_id_extraction(client, image_data, text, desc):
    ...
    place_of_birth_pattern = r"محل الولادة (.+)|محل الولادة:\s*(.+)"

    place_of_birth = None
    place_of_birth_match = re.search(place_of_birth_pattern, desc)
    if place_of_birth_match:
        place_of_birth = place_of_birth_match.group(1) or place_of_birth_match.group(2)
        place_of_birth = place_of_birth.replace(":", "").replace(" ", "")
    
    print(f"place_of_birth: {place_of_birth}")

    id_data['dob'] = dob
    id_data['place_of_birth'] = place_of_birth

    ## CASE 1 - HANDLING
    if not id_data.get('fathers_name', '') or not id_data.get('last_name', '') or not id_data.get('name', '') or id_data.get('fathers_name', '') == 'اسم':
        id_text, id_card, second_part_text = detect_id_card(client, image_data, text)
        # print(f"\n\nDATA: {second_part_text}\n")
        id_data_new = extract_personal_info(second_part_text)
        # print(f"\nID DATA NEW: {id_data_new}\n")
        # print(f"\nORIGINAL DATA: {desc}\n")
        
        if not id_data.get('fathers_name') or id_data.get('fathers_name')=='اسم':
            id_data['fathers_name'] = id_data_new['fathers_name']
    ...
    if not id_data.get('fathers_name') or id_data.get('fathers_name')=='اسم':
        if len(extracted_words) >= 4:
            ...
            if not id_data.get('fathers_name') or id_data.get('fathers_name')=='اسم':
                id_data['fathers_name'] = fathers_name

    ## CASE 3 - HANDLING WITH ORIGINAL DATA
    if not id_data.get('fathers_name', '') or not id_data.get('last_name', '') or not id_data.get('name', '') or id_data.get('fathers_name', '') == 'اسم' or id_data.get('name', '') == 'الشهرة': 
    ...
    if id_data['fathers_name']:
        id_data['fathers_name'] = id_data['fathers_name'].replace(":", "").replace("-", "").replace("_", "").replace("الاب", "").replace("الأب", "").replace(" ", "")
        id_data['fathers_name_en'] = GoogleTranslator(dest = 'en').translate(id_data['fathers_name'])
    ...
    name = f"{id_data['first_name']} {id_data['fathers_name']} {id_data['last_name']}"
...
def extract_gender_normalized(extracted_text):
    ...
    elif re.search(r'انثى', extracted_text) or re.search(r'أنثى', extracted_text) or re.search(r'انتی', extracted_text):
        gender_ar = 'انثى'
        gender = 'F'
...
def lebanon_back_id_extraction(extracted_text):
    ...
    back_data = {
        ...
        'issue_date_en': eastern_arabic_to_english(issue_date) if issue_date else ''
    }
