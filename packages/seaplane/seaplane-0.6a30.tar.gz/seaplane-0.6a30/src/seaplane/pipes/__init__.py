...
EdgeFrom = Union[Flow, FlowTopic, "Dag", Subscription, "Bucket"]
...
class Dag:
    ...
    def flow(
        ...
        ret = Flow(self, work, prefixed_name, topic, replicas, ack_wait_secs)
    ...
    def edge(
        ...
        # Users only deal with Nodes and Edges. Not Subscriptions and Subjects.
        if isinstance(source, Subscription) and not isinstance(source, App.InEndpoint):
            raise SeaplaneError(
                "Cannot set an edge from a Subscription that is not App In Endpoint"
            )
        if isinstance(dest, Subject) and not isinstance(dest, App.OutEndpoint):
            raise SeaplaneError("Cannot set an edge to a Subject that is not App Out Endpoint")
        ...
        if isinstance(source, Flow) and isinstance(dest, Subject):
            ...
            source_node_id = Representation.get_flow_node_id(self.name, source.instance_name)
            self.edges.append(Representation.get_edge(source_node_id, "out"))
            return
        ...
        target_node_id = Representation.get_flow_node_id(self.name, dest.instance_name)

        if isinstance(source, FlowTopic):
            if source_node_id == "":
                source_node_id = Representation.get_flow_node_id(
                    source.dag_name, source.instance_name
                )
        ...
        if isinstance(source, Subscription):
            ...
            if source_node_id == "":
                self.edges.append(Representation.get_edge("in", target_node_id))
            else:
...
class App:
    ...
    def represention(self) -> Dict[str, Any]:
        ...
        for k_dag in sorted(self.dag_registry.keys()):
            dag = self.dag_registry[k_dag]
            # Add all flow nodes
            for k_flow in sorted(dag.flow_registry.keys()):
                flow = dag.flow_registry[k_flow]
