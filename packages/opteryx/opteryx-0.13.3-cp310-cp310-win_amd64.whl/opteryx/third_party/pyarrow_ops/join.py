...
def preprocess_left_side(left, left_on):
    l_array = columns_to_array(left, left_on)
    l_distinct, lc, l_sort_idxs, lbi = groupify_array(l_array)
    return l_distinct, lc, l_sort_idxs.astype(np.int64), lbi


def inner_join_with_preprocessed_left(left, right, right_on, l_distinct, lc, l_sort_idxs, lbi):
    # Gather join columns - create arrays of the hashes of the values in the column
    # updated for Opteryx
    r_array = columns_to_array(right, right_on)
