    ...
    ->> key-word argument
        ...
        for simu in self.find_simulations(simulation_names):
            ...
            if kwargs.get('No_till', False):
                self.organic_calcualted.FBiom[:2]= self.organic_calcualted.FBiom[:2] + 0.2 * self.organic_calcualted.FBiom[:2]
                print(self.organic_calcualted.FBiom[:2])
        ...
        for simu in self.find_simulations(simulation_names):
            ...
            for cropLL in soil_crop:
                ...
                if kwargs.get('No_till', False):
                    cropLL.KL =  cropLL.KL + 0.2* cropLL.KL
                cropLL.XF = XF
                cropLL.Thickness = self.thickness_replace
        return self
