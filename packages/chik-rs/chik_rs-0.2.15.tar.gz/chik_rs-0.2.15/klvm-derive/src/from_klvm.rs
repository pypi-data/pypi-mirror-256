...
pub fn from_klvm(mut ast: DeriveInput) -> TokenStream {
    ...
    add_trait_bounds(&mut ast.generics, parse_quote!(#crate_name::FromKlvm));
    let (impl_generics, ty_generics, where_clause) = ast.generics.split_for_impl();

    quote! {
        #[automatically_derived]
        impl #impl_generics #crate_name::FromKlvm for #struct_name #ty_generics #where_clause {
            fn from_klvm(a: &klvmr::Allocator, node: klvmr::allocator::NodePtr) -> #crate_name::Result<Self> {
                let #destructure_macro!( #( #field_names, )* ) = <#match_macro!( #( #field_types ),* ) as #crate_name::FromKlvm>::from_klvm(a, node)?;
                Ok(#initializer)
            }
        }
    }
