...
from panoptica_genai_protection.settings import SIGN_URL, SCOPE, ESCHER_TOKEN_URL, JWT_TIMEOUT_MS

logger = getLogger("marvin_auth")


class MarvinAuthException(Exception):
...
class MarvinAuth:
    ...
    def get_instance(cls, access_key=None, secret_key=None, host=None,
        ...
        access_key = access_key if access_key else os.getenv("MARVIN_ACCESS_KEY")
        secret_key = secret_key if secret_key else os.getenv("MARVIN_SECRET_KEY")
        host = host if host else os.getenv("MARVIN_HOST")
        if not access_key or not secret_key or not host:
            raise ValueError('please provide valid MARVIN_ACCESS_KEY, MARVIN_SECRET_KEY, and MARVIN_HOST')
    ...
    def get_token(self):
        ...
        if not signed_resp.ok:
            raise MarvinAuthException(signed_resp.text)
