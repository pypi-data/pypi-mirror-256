...
def rf_azure_sync_patch():
    ...
    for robot_file in path_robot_files:
        if "todo_organize.robot" not in robot_file:
            content = sync_utils.read_robot_file(robot_file)
            test_cases_data, test_tags = sync_utils.extract_test_tags_and_test_cases(content)
            cases = sync_utils.extract_test_cases(test_cases_data, sync_config)
            for case in cases:
                print("\n" + "=" * 70)
