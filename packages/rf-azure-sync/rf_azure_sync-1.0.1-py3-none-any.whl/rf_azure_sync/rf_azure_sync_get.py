    ...
    for root, _, files in os.walk(folder_path):
        for rf_file in files:
            if rf_file.endswith(".robot"):
                ...
                content = sync_utils.read_robot_file(file_path)
                test_cases_data, _ = sync_utils.extract_test_tags_and_test_cases(content)
                cases = sync_utils.extract_test_cases(test_cases_data, config)
...
def process_new_test_cases(new_test_case_ids, robot_test_case_ids, url, headers):
    ...
    for test_case_id in new_test_case_ids:
        if test_case_id not in robot_test_case_ids:
            url_workitems = f"{url}workitems?ids={test_case_id}&$expand=all&api-version=7.1-preview.3"
            ...
            if response_workitems.status_code == 200:
                ...
                print(
                    f"Error in request: {response_workitems.status_code} - {response_workitems.text}"
                )
...
sync_config = sync_utils.load_sync_config()
