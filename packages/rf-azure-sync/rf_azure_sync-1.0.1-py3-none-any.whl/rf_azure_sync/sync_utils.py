...
def create_sync_config():
    ...
    tests_folder = input(
        "Folder with Robot Framework tests to be synchronized (default: tests): "
    )
    personal_access_token = input(
        "Azure personal access token with read and write permission: "
    )
    organization_name = input(
        "Organization name (the one that comes after https://dev.azure.com/): "
    )
    project_name = input(
        f"Project name (the one after https://dev.azure.com/{organization_name}/): "
    )
    test_case = input(
        "Prefix used to identify the Test Case id example (TC, TestCase): "
    )
    user_story = input(
        "Prefix used to identify the User Story id related to the Test Case example (US, UserStory): "
    )
    bug = input(
        "Prefix used to identify the Bug id related to the Test Case example (Bug): "
    )
    title = input(
        "Prefix used to identify the title of the Test Case example(Title, Scenario): "
    )
    tested_by_reverse = input(
        "Reverse the 'Tested By' relationship between Test Cases and User Stories: "
    )
    ...
    ignore_sync = input(
        "Azure DevOps field to mark Test Cases that should be ignored during synchronization: "
    )
    ...
    settings_section = input(
        "Section in the Robot Framework settings file to store Azure-related settings: "
    )
    test_cases_section = input(
        "Section in the Robot Framework settings file to store synchronized Test Cases: "
    )
...
def extract_test_tags_and_test_cases(rf_content):
    """
    Function to extract test tags and test cases from Robot Framework content.
    ...
    settings_match = re.search(
        rf"{re.escape(settings_section)}(.*?)\*\*\*", rf_content, re.DOTALL
    )
    ...
    test_cases_match = re.search(
        rf"{re.escape(test_cases_section)}(.*?)(?=\*\*\*|$)", rf_content, re.DOTALL
    )
...
def extract_test_cases(raw_test_cases_data, config_settings):
    ...
    Returns:
        List[dict]: A list of dictionaries representing extracted test cases.
    """
    test_cases = []
    current_test_case = {}
