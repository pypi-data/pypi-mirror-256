...
def get_canonical_name(name,type=None,quoted=True,external=_D):
	D='public';B=quoted;A=name;A=A.strip()
	...
	if not external and type in[None,NameType.SCHEMA]and A.lower()==D:return D
	C=A.upper()
	if B:return f'"{C}"'
	return C
