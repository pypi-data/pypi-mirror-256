...
class HandlePutQuery(QueryProcessor):
	REGEX='^\\s*PUT\\s+.+'
	def should_apply(A,query):return bool(re.match(A.REGEX,query.original_query,flags=re.I))
...
class HandleCopyIntoQuery(QueryProcessor):
	def should_apply(A,query):return bool(re.match('^\\s*COPY\\s+INTO\\s.+',query.original_query,flags=re.I))
	def transform_query(B,expression,query):
		A=expression
