...
class HandleCreateStream(QueryProcessor):
	...
	def transform_query(T,expression,query):
		S='TEXT';M=query;H=expression;D=', '
		if isinstance(H,(exp.Create,exp.Command)):
			I=M.get_database();E=[];N=T.REGEX.match(str(H));U=re.match('.+APPEND_ONLY\\s*=\\s*TRUE',M.original_query,flags=re.I)
			if U:LOG.info('Please note that APPEND_ONLY=TRUE semantics for table streams are not fully supported yet')
			J=get_canonical_name(N.group(5));A=get_canonical_name(N.group(3));V,O=get_trigger_function_for_stream(A);B=get_canonical_name(COL_NAME_RECORD_ID);K=f"\n            ALTER TABLE {J} ADD {B} VARCHAR DEFAULT gen_random_uuid()\n            ";State.server.run_query(K,database=I);W=MetadataUtils.get_table_schema(J,database=I);P=[A for A in W.columns if A.name.lower()!=COL_NAME_RECORD_ID];L=list(P);C=list(P);C.append(TableColumn(name='METADATA$ACTION',type_name=S));C.append(TableColumn(name='METADATA$ISUPDATE',type_name='BOOLEAN'));C.append(TableColumn(name='METADATA$ROW_ID',type_name=S));X=D.join([f"{get_canonical_name(A.name)} {A.type_name}"for A in C]);E.append(f"CREATE TABLE {A}({X})");F=D.join([get_canonical_name(A.name)for A in C]);G=D.join(['%L'for A in L]);Q=D.join([f"NEW.{get_canonical_name(A.name)}"for A in L]);R=D.join([f"OLD.{get_canonical_name(A.name)}"for A in L]);E.append(f"""
            CREATE OR REPLACE FUNCTION {O}() RETURNS trigger AS $$
                ...
                BEGIN
                    ...
                    inserted := (
                        SELECT count(*) FROM {A} WHERE \"METADATA$ROW_ID\" = ref_row.{B} and \"METADATA$ACTION\" = 'INSERT'
                    );
                    deleted := (
                        SELECT count(*) FROM {A} WHERE \"METADATA$ROW_ID\" = ref_row.{B} and \"METADATA$ACTION\" = 'DELETE'
                    );

                    -- delete existing row updates in this stream
                    EXECUTE format('DELETE FROM {A} WHERE \"METADATA$ROW_ID\" = %L', ref_row.{B});

                    -- handle logic for each row, depending on the operation in TG_OP (INSERT/UPDATE/DELETE)
                    if TG_OP = 'INSERT' then
                        EXECUTE format(
                            'INSERT INTO {A}({F}) VALUES ({G}, ''INSERT'', FALSE, %L)',
                            {Q}, NEW.{B}
                        );
                        ...
                        if not inserted then
                            EXECUTE format(
                                'INSERT INTO {A}({F}) VALUES ({G}, ''DELETE'', TRUE, %L)',
                                {R}, NEW.{B}
                            );
                        end if;
                        EXECUTE format(
                            'INSERT INTO {A}({F}) VALUES ({G}, ''INSERT'', %L, %L)',
                            {Q}, CASE WHEN inserted THEN FALSE ELSE TRUE END, NEW.{B}
                        );
                ...
                END;
            $$ LANGUAGE plpgsql
            """);E.append(f"\n            CREATE TRIGGER {V} BEFORE INSERT OR UPDATE OR DELETE ON {J}\n                FOR EACH ROW EXECUTE FUNCTION {O}()\n            ");K='; '.join(E);State.server.run_query(K,database=I);return parse_one(_D)
		return H
class HandleDropStream(QueryProcessor):
	...
	def transform_query(C,expression,query):
		A=expression
		if isinstance(A,(exp.Drop,exp.Command)):D=query.get_database();E=C.REGEX.match(str(A));B=E.group(2);H,F=get_trigger_function_for_stream(B);G=f"DROP FUNCTION IF EXISTS {F} CASCADE";State.server.run_query(G,database=D);return parse_one(f"SELECT '{get_canonical_name(B,quoted=_A)} successfully dropped.'")
		return A
...
def get_trigger_function_for_stream(stream_name):B=get_canonical_name(stream_name);A=B.strip('"').lower();C=TRIGGER_NAME_PATTERN.replace(_C,A);D=STREAM_FUNC_NAME_PATTERN.replace(_C,A);return C,D