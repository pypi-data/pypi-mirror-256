...
class HandleCreateWarehouse(QueryProcessor):
	REGEX=re.compile('^\\s*CREATE.*\\s+WAREHOUSE(\\s+IF\\s+NOT\\s+EXISTS)?\\s+(\\S+)',flags=re.I)
	def should_apply(A,query):return bool(re.match(A.REGEX,query.original_query))
	def transform_query(C,expression,query):
		B=query;A=expression
		if isinstance(A,(exp.Create,exp.Command)):D=C.REGEX.match(B.original_query);B.session.warehouse=D.group(2);return parse_one("SELECT 'Warehouse successfully created.'")
		return A
