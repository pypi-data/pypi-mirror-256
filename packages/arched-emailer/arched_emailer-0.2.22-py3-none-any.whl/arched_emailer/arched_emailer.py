...
class ArchedEmailer:
    ...
    def init_app(self, app, intercept_errors: Optional[bool] = True, add_current_user: bool = True, **kwargs):
        """
        ...
        if intercept_errors:
            ...
            def update_visited_urls():
                ...
                if "static" not in current_url and ".js" not in current_url and ".css" not in current_url:
                    # Update the session with the current URL, keeping only the last 3 URLs
                    visited_urls = session['visited_urls']
                    visited_urls.append(current_url)
                    session['visited_urls'] = visited_urls[-5:]  # Keep only the last 3 URLs
            ...
            def handle_exception(error):
                # Log the exception via email
                error_text = f"<br><strong style='font-size:larger'>Request url</strong>:<br>{request.url}<br>"

                # Include the last visited URLs
                visited_urls = session.get('visited_urls', [])
                if visited_urls:
                    visited_urls.reverse()
                    error_text += (f"<br><strong style='font-size:larger'>Last Visited URLs</strong>:<br>"
                ...
                if add_current_user:
                    ...
                    error_text += (f"<br>"
                                   f"<strong style='font-size:larger'>User Details: </strong>:"
                                   f"<br>"
                                   f"{f'email: {email} - ' if email else ''}"
                                   f"{f'name: {name} - ' if name else ''}"
                                   f"{f'id: {user_id}' if user_id else ''}"
                                   f"<br>")

                if request.method.lower() == "POST" and request.json:
                    error_text += f"<br><strong style='font-size:larger'>Request JSON</strong>:<br>{format_dict_for_email(request.json)}<br>"
                if request.method.lower() == "POST" and request.form:
                    error_text += f"<br><strong style='font-size:larger'>Request Form</strong>:<br>{format_dict_for_email(dict(request.json))}<br>"

                self.send_error_email(["lewis@arched.dev"], error_text=error_text, exception=error, allowed_minutes=60)

                # Re-raise the exception to propagate it to other error handlers or default Flask handler
                raise error
    ...
    def send_success_email(self, recipients: Union[str, list], dump_time_taken: Optional[bool] = True,
        ...
        try:
            ...
            self._send_to_db(success=True, recipients=recipients, sender=self.success_mailer.sender.email,
                             html=html_content, task_id=task_id if task_id else self.task_id,
                             **kwargs)
    ...
    def send_error_email(self, recipients: Union[str, list],
        ...
        try:
            ...
            if dump_globals:
                template_data["globals"] = {str(k): str(v) for k, v in obfuscate_sensitive_info(globals()).items() if
                                            k != "request"}
            if dump_locals:
                template_data["locals"] = {str(k): str(v) for k, v in obfuscate_sensitive_info(locals()).items() if
                                           k != "request"}
            ...
            if issue:
                if self._allowed_to_send(issue, allowed_minutes=allowed_minutes):
                    ...
                    self._send_to_db(success=False, recipients=recipients, sender=self.mailer.sender.email,
                                     html=self._get_html_content(self.mailer),
        ...
        def decorator(func):
            def wrapper(*func_args, **func_kwargs):
                ...
                try:
                    ...
                    success_text = f"<strong style='font-size:larger'>Function Result: </strong>: <br> {str(result) if result else 'None'}."
                    if send_success and success_recipients:
                        self.send_success_email(success_recipients, success_text=success_text, task_id=task_id, **kwargs)
                    if not send_success and send_to_db:
                        self.send_success_email(success_recipients, success_text=success_text, task_id=task_id, update_db_only=True, **kwargs)
                except Exception as e:
