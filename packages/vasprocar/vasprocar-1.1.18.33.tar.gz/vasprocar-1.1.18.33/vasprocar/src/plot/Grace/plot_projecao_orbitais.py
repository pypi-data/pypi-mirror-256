...
for i in range (1,(loop+1)):     # Loop for analysis of projections
    ...
    if (dest_k == 2):
       ...
       for ii in range (contador2):
           projection.write(f'@    xaxis  tick major {ii}, {dest_pk[ii]} \n')
           temp_r = label_pk[ii]
           for j in range(34):
               if (temp_r == '#' + str(j+1)): temp_r = r_grace[j]
           if (DFT == '_QE/' and l_symmetry == 1):
              temp_r = temp_r + '\\f{Times-Roman}(' + symmetry_pk[ii] + ')'
           #--------------------------------------------------------------                  
           projection.write(f'@    xaxis  ticklabel {ii}, "{temp_r}" \n')
