...
from py_flower_crane.meeting_numba import find_meeting_numba
...
def bench_py_flower_crane_numba():
    # run once to compile
    data = get_data()
    _ = find_meeting_numba(*data)
    start = time.time()
    for _ in range(N):
        _ = find_meeting_numba(*data)
    end = time.time()
    print(f"Numba: {N} iterations in {end-start:.4f} seconds")
