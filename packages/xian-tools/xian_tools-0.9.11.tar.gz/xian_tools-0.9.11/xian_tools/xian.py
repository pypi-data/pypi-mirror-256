...
class Xian:
    ...
    def send_tx(
            ...
            kwargs: dict,
            stamps: int = 500,
            chain_id: str = None) -> tuple[bool, Dict[str, Any]]:
        """
        ...
        if 'error' in data:
            return False, {'error': data['error']['data']}

        if data['result']['check_tx']['code'] == 1:
            data['error'] = 'Transaction check not successful'
            return False, data

        if data['result']['deliver_tx']['code'] == 1:
            data['error'] = 'Transaction delivery not successful'
            return False, data

        if data['result']['deliver_tx']['data']['status'] == 1:
            data['error'] = data['result']['deliver_tx']['data']['result']
            return False, data
