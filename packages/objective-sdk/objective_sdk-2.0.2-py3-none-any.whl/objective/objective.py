...
class ObjectStore:
    ...
    def _get_status_for_object(self, object):
        object.status = self.objective_client.request("GET", f"objects/{object.id}/status")
        return object
    ...
    def list_all_objects(
        ...
        while True:
            list_objects_response = self.list_objects(
                limit=512, cursor=cursor, include_object=include_object, include_status=include_status
            )
...
class Index:
    ...
    def status(self, status_type: Optional[str] = None, watch: bool = False):
        ...
        for _ in range(repeat):
            ...
            if watch:
                total = obj_status['PENDING'] + obj_status['PROCESSING'] + obj_status['LIVE'] + obj_status['ERROR']
                live = obj_status['LIVE']
                progress = int((live / total) * 100)
