...
from aryahelpers.utils.genericutils import elapsedTime
...
class PreprocessText():
    ...
    def ascii_filter(self, text_str):
        ...
        text_str = unicodedata.normalize('NFKD', text_str).encode('ascii', 'ignore').decode('utf-8', 'ignore')
        return text_str

    def spchars_remove(self, text_str, remove_digits=True):
    ...
    def stopwords_clean(self, text_str: str, stop_words: list):
        ...
        text_str = " ".join([wd for wd in words_list if self.spchars_remove(wd.lower()).strip() not in stopw_dict])
        return text_str
    ...
    @elapsedTime
    def preprocess_text(self, text_str, stop_words=[], remove_spchars=True, remove_digits=True, print_el_time=True):
        """This function preprocesses a text string"""    
        text_str = self.ascii_filter(text_str)
        text_str = self.spchars_remove(text_str, remove_digits) if remove_spchars else text_str
        text_str = self.stopwords_clean(text_str, stop_words)
        text_str = self.spaces_clean(text_str)
        return text_str
    
    @elapsedTime
    def preprocess_textlist(self, text_list, stop_words=[], remove_spchars=True, remove_digits=True,
                            remove_nulls=True, remove_dupli=True, print_el_time=True):
        ...
        clean_list = [self.spchars_remove(s, remove_digits) for s in clean_list] if remove_spchars else clean_list
