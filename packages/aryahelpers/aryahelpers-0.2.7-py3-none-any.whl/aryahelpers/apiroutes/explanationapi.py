...
def expl_func(base_env=None):
    try:
        ...
        accessor = AccessExplanations(
            base_env=base_env, job_id=job_id, explanation_type=explanation_type,
            num_candidates=num_candidates, candidate_name=candidate_name, candidate_guid=candidate_guid,
            recommendation_status_id=recommendation_status_id)
        accessor.access_explanations()
        response = accessor.explanations
