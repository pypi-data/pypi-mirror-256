...
def initk8s(ctx: typer.Context,
    ...
    for tmplfile in os.listdir("./templates"):
        ...
        with open(ofpath, "w") as outfile:
            ...
            tf = tf.replace("{{MINIKUBE_MOUNT_ROOT}}", minikube_mount_root)
    ...
    # Save daglib too
    with open(os.path.join(dest_dir, "daglib.py"), "w") as daglibfile:
        # Todo - get this from repo?
        daglibfile.write(open("../outpost/jobs/daglib.py").read())
