...
def scroll(scrollAmount=1):
    ...
    Sometimes games dont take the scroll data that gets sent and just takes thes scroll input to scroll at a fixed amount
...
def remove_hotkey(key:str):
    """
    Unregisters and removes the specified hotkey if it was previously registered
...
def typeWrite(message:str, interval=0.0, _pause=True):
    """
    Types out a given message, supports lowercase and uppercase letters\n
...
def controlClick(x:int=None, y:int=None, className:str=None, windowTitle:str=None, button='left'):
    ...
    Note:
    -------
    Some games/windows ignore your lparam in PostMessageW and they instead use the return from GetPhysicalCursorPos instead\n

    Example: every game made with the unity game engine uses GetPhysicalCursorPos to determine were the user has clicked\n
    ...
    if button == 'left':
        ...
        ctypes.windll.user32.PostMessageW(handle, WM_LBUTTONDOWN, MK_LBUTTON, lparam)
        ctypes.windll.user32.PostMessageW(handle, WM_LBUTTONUP, MK_LBUTTON, lparam)
...
class LowLevelKeyboardHook():
    ...
    def callback(self, nCode, wParam, lParam):
        if nCode < 0:
            return ctypes.windll.user32.CallNextHookEx(0, nCode, wParam, ctypes.wintypes.LPARAM(lParam))
    ...
    def start(self):
        ...
        self.hook_id = ctypes.windll.user32.SetWindowsHookExW(13, CBTProc, 0, 0)
...
class LowLevelMouseHook():
    ...
    def example(x, y, is_down)
        if is_down:
            print(x, y)
        
        elif not is_down:
            do cleanup or something else here
    ...
    def start(self):
        ...
        HOOKProc = ctypes.WINFUNCTYPE(ctypes.wintypes.LPVOID, ctypes.c_int, ctypes.wintypes.WPARAM, ctypes.wintypes.LPARAM)
        CBTProc = HOOKProc(self.callback)
        self.hook_id = ctypes.windll.user32.SetWindowsHookExW(14, CBTProc, 0, 0)
