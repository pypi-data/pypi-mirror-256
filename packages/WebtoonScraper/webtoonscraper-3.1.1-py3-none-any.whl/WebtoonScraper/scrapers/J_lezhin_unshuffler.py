...
def unshuffle_episode(
    source_episode_directory: Path, target_episode_directory: Path, episode_id_int: int
) -> str | None:
    try:
...
def get_episode_directory_no(episode_directory_name: str) -> int | None:
    try:
        ...
        raise ValueError(
            "`episode_directory_name` is invalid. Maybe you tried to unshuffle merged webtoon directory. "
            "`unshuffle_webtoon` does not support merged webtoon."
        ) from e
