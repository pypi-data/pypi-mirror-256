...
class WorkerPool:
    async def run(self, tasks: t.Iterable[WorkerTask], mode: t.Literal["async", "sync"] = "async"):
        """
    ...
    async def _run_sync(self, tasks: t.Iterable[WorkerTask]):
        while True:
            await self._load_messages(tasks)
            for task in tasks:
                await self._process_task_messages(task)

    async def _run_async(self, tasks: t.Iterable[WorkerTask]):
    ...
    async def _load_messages(self, tasks: t.Iterable[WorkerTask]):
        get_messages_tasks = [
    ...
    async def _process_task_messages(task: WorkerTask):
        ack_ids, nack_ids = [], []
        for coro in asyncio.as_completed([task.handler(message) for message in task.messages_pool]):
            try:
                result: HandlerResult = await coro
            except Exception:
                logger.exception("Exception during task processing.")
                continue
