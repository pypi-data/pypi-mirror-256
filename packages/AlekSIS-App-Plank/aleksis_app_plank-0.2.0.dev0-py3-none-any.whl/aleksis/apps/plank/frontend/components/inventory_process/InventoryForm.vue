<template>
  <div>
    <h1 class="text-h3 mb-4">
      {{ $t("plank.item.inventory_process.title") }}
    </h1>
    <ApolloMutation
    ...
    >
      <template #default="{ mutate, loading, error }">
        ...
        <v-form v-model="valid" id="form" ref="form">
          ...
          <v-card v-if="selectedInventory" class="mb-4">
            <v-card-title>
  ...
  computed: {
    filteredCategories() {
      return this.categories.filter(
        (category) => category.inventory.id === this.selectedInventory,
      );
    },
    filteredItemTypes() {
      return this.itemTypes.filter(
        (itemType) => itemType.category.inventory.id === this.selectedInventory,
      );
    },
    filteredLocations() {
      return this.locations.filter(
        (location) => location.inventory.id === this.selectedInventory,
      );
    },
  },
  methods: {
