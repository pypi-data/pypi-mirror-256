...
class CategoryType(PermissionsForTypeMixin, DjangoObjectType):
    ...
    class Meta:
        model = Category
        fields = ("inventory", "name", "notes", "colour", "icon", "id")
...
class ManufacturerType(PermissionsForTypeMixin, DjangoObjectType):
    ...
    class Meta:
        model = Manufacturer
        fields = ("inventory", "name", "notes", "id")
...
class LocationType(PermissionsForTypeMixin, DjangoObjectType):
    ...
    class Meta:
        model = Location
        fields = ("name", "notes", "parent", "inventory", "id")
            ...
            return self.children.filter(
                Q(
                    inventory__in=get_objects_for_user(
                        info.context.user, "plank.view_location_for_inventory", Inventory
                    )
                )
                | Q(
                    pk__in=get_objects_for_user(
...
class CheckInMutation(graphene.Mutation):
    ...
    checked_out_item = graphene.Field(CheckedOutItemType, required=False)
    ...
    def mutate(root, info, id_or_barcode, **kwargs):  # noqa
        ...
        checked_out_item = item.current_checked_out_item
        item.check_in(person=info.context.user.person)
        return CheckInMutation(checked_out_item=checked_out_item, status="checked_in")
...
class Query:
    ...
    def resolve_item_types(self, info, permission=None, **kwargs):
        if permission:
            permission = f"{permission}_for_inventory"
            return ItemType.objects.filter(
                category__inventory__in=get_objects_for_user(
                    info.context.user, permission, Inventory
                )
            )
        if not info.context.user.has_perm("plank.view_itemtype"):
            return ItemType.objects.filter(
                Q(
                    category__inventory__in=get_objects_for_user(
                        info.context.user, "plank.view_itemtype_for_inventory", Inventory
                    )
                )
                | Q(pk__in=get_objects_for_user(info.context.user, "plank.view_itemtype", ItemType))
            )
    ...
    def resolve_locations(self, info, **kwargs):
        if not info.context.user.has_perm("plank.view_location"):
            return Location.objects.filter(
                ...
                | Q(pk__in=get_objects_for_user(info.context.user, "plank.view_location", Location))
            )
    ...
    def resolve_manufacturers(self, info, permission=None, **kwargs):
        if permission:
            ...
            return Manufacturer.objects.filter(
                inventory__in=get_objects_for_user(info.context.user, permission, Inventory)
            )
        if not info.context.user.has_perm("plank.view_manufacturer"):
            return Manufacturer.objects.filter(
                Q(
                    inventory__in=get_objects_for_user(
                        info.context.user, "plank.view_manufacturer_for_inventory", Inventory
                    )
                )
                | Q(
                    pk__in=get_objects_for_user(
    ...
    def resolve_categories(self, info, permission=None, **kwargs):
        ...
        if not info.context.user.has_perm("plank.view_category"):
            return Category.objects.filter(
                Q(
                    inventory__in=get_objects_for_user(
                        info.context.user, "plank.view_category_for_inventory", Inventory
                    )
                )
                | Q(pk__in=get_objects_for_user(info.context.user, "plank.view_category", Category))
            )
