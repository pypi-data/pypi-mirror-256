...
from tenacity import Retrying, AsyncRetrying, RetryError, stop_after_delay, retry_if_not_exception_type, wait_random
...
class APIException(Exception):
    def __init__(self, url: str, status_code: int = 0, message: str = None, request_body: dict = None, response=None):
        self.url = url
...
class APIClientBase:
    ...
    async def _async_request(
        self, method: str, url: str, body: dict, success_codes: list, error_subject: str, on_response_callback
    ):
        try:
                ...
                with attempt:
                    return await asyncio.wait_for(
                        self._unsafe_async_request(
                            method, url, body, success_codes, error_subject, on_response_callback
                        ),
    ...
    def _sync_request(
        ...
        except (aiohttp.ServerTimeoutError, requests.Timeout, futures.TimeoutError, RetryError) as e:
            logger.warning(f"Failed {method} request to {url}, network timeout: {str(e)}")
            error = self._append_url_if_not_local_service("network timeout")
            raise APIException(url=url, message=f"{error_subject} has failed ({error})", request_body=body) from e
