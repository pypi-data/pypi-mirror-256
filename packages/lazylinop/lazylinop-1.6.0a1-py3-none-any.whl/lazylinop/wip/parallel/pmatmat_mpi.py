from lazylinop import LazyLinearOp, binary_dtype
import numpy as np
...
def pmatmat_mpi(L, use_matvec=False, scatter_A=False):
    ...
    .. note:: Do not make confusion with :py:class:`.MPILop`
        which parallelizes by blocks of ``L`` rows (which must hence
        hide a full array). Besides, the function ``pmatmat_mpi`` should not
        be used to parallelize furthermore a :py:class:`.MPILop`.
    ...
    """
    return PMMPILazyLinearOp(L, use_matvec=use_matvec, scatter_X=scatter_A)


class PMMPILazyLinearOp(LazyLinearOp):
    ...
    def _pmatmat_mpi(self, L, X, use_matvec=False, scatter_X=False):
        ...
        if scatter_X:
            ...
            ncols = Xcols.shape[1]
        out_dtype = binary_dtype(L.dtype, X.dtype)
        out = np.empty((L.shape[0], ncols), dtype=out_dtype, order='F')
        if use_matvec:
            # could be parallelized using threads but
            # it is the pretty the same to increase the number of MPI slots,
            # isn't it?
