...
class MPILop(LazyLinearOp):
    """
    Implements a LazyLinearOp based on MPI to compute the multiplication
    (matmat and rmatmat).
    ...
    .. note:: This operator is not to be confound with :py:func:`.pmatmat_mpi`
    ...
    **See also**: :py:func:`.pmatmat`.
    ...
    def __init__(self, shape, mat_npz, mat_dtype, bcast_op=True):
        ...
        **Note**: This operator should be used without OpenMP enabled or it
        won't be efficient to compute the distributed multiplication. Why is
        that?
        Well, mixing multithreading (from numpy/BLAS) with multiprocessing is
        counterproductive.
        ...
        **See also** bm_np_mul.py_ and bm_mpilop.py_ for valuable examples and
        ...
        self.Ls = None
        self.LHs = None
        self.slot_inds = [np.empty((self.size+1), dtype='int')
                          for _ in range(2)]
        self.n_per_slot = [0, 0]
