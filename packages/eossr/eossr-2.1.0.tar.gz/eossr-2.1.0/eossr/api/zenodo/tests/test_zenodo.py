...
from eossr.api.zenodo.zenodo import PendingRequest
...
from eossr.api.zenodo.zenodo import is_live, query_records
from eossr.metadata.tests import ZENODO_TEST_FILE

# test deactivated at the moment
test_record_sandbox = 780  # test library in sandbox (owner: eossr)
...
class TestZenodoAPITokenSandbox(unittest.TestCase):
    ...
    def test_pending_request(self):
        ...
        assert '2bb18da8-730f-4a1d-aca3-fda535950908' in requests_ids

    def test_find_similar_records_sandbox(self):
        self.zenodo.parameters.update({'size': 100})
        existing_record = Record.from_id(eossr_record_id)
...
def test_upload_package(temp_dir_with_file):
    ...
    zenodo = ZenodoAPI(access_token=os.getenv(
        'SANDBOX_ZENODO_TOKEN'), sandbox=True)
    # create new record
    ...
    new_deposit_id = zenodo.upload_dir_content(
        tmpdirname, record_id=deposit_id, publish=False)
...
def test_check_upload_to_zenodo(temp_dir_with_file):
    ...
    updated_metadata = zenodo.set_deposit_metadata(
...
def test_write_record_zenodo(record_4923992, tmpdir):
    record_4923992._write_zenodo_deposit(
        filename=tmpdir / '.zenodo.json', validate=False)
...
def test_search_funders():
    funders = zenodo.search_funders(
        'name:European+Commission', timeout=tests_default_timeout)
...
def test_get_associated_versions():
    ...
    for recid in eossr_record_versions.keys():
        assert eossr_record.data['conceptrecid'] == Record.from_id(
...
def test_query_deposits():
    ...
    sandbox = True
    deposits = query_deposits(search, access_token,
                              sandbox=sandbox, timeout=tests_default_timeout)
...
def test_get_deposit():
    deposit = get_deposit(test_record_sandbox, sandbox=True,
                          access_token=os.getenv('SANDBOX_ZENODO_TOKEN'))
...
def test_post_message():
    ...
    timeline = pending_request.get_timeline(force_refresh=True)['hits']['hits']
    last_comment = timeline[-1]
