...
zenodo_sandbox_api_base_url = "https://sandbox.zenodo.org/api/"
...
class ZenodoAPI:
    ...
    def _raise_token_status(self):
        ...
        if self.access_token is None or self.access_token == '':
            raise ValueError(
                "No access token was provided. This method requires one.")
    ...
    def create_new_deposit(self):
        ...
        req = requests.post(url, json={}, headers=headers,
                            params=self.parameters, timeout=_default_timeout)
    ...
    def upload_file_deposit(self, deposit_id, name_file, path_file):
        ...
        with open(path_file, 'rb') as upload_file:
            upload = requests.put(
                url, data=upload_file, params=self.parameters, timeout=_default_timeout)
    ...
    def upload_dir_content(self, directory, record_id=None, metadata=None, erase_previous_files=True, publish=True):
        ...
        if record_id is not None:
            ...
            new_deposit_id = new_deposit.json(
            )['links']['latest_draft'].rsplit('/')[-1]
            print(f" * Preparing a new version of record {record_id}")
            # TODO: log
            if erase_previous_files:
                old_files_ids = [file['id']
                                 for file in new_deposit.json()['files']]
        ...
        if metadata is not None:
            ...
            print(
                f"   - Record metadata based on zenodo file {path_zenodo_file}")
        ...
        for file in dir_to_upload.iterdir():
            self.upload_file_deposit(
                deposit_id=new_deposit_id, name_file=file.name, path_file=file)
        ...
        if publish:
            ...
            if record_id:
                print(
                    f" * New version of {record_id} published at {new_deposit_id} !")
            else:
                print(f" * Record {new_deposit_id} published")
            print(f" * The new doi should be 10.5281/{new_deposit_id}")

        print(
            f" * Check the upload at {self.api_base_url[:-4]}/deposit/{new_deposit_id} *")
    ...
    def check_upload_to_zenodo(self, directory):
        ...
        updated_metadata = self.set_deposit_metadata(
            test_deposit_id, json_metadata=metadata_entry)
        ...
        try:
            ...
            print(
                f" !! ERROR erasing dummy test entry: {delete_test_entry.json()}")
            print(
                f"Please erase it manually at {self.api_base_url[:-4]}/deposit")
            sys.exit(-1)
    ...
    def find_similar_records(self, record):
        ...
        for user_rec in user_records:
            ...
            if 'related_identifiers' in user_rec.data['metadata'] and 'related_identifiers' in record.data['metadata']:
                ...
                relid1 = [r['identifier']
                          for r in user_rec.data['metadata']['related_identifiers']]
                relid2 = [r['identifier']
                          for r in record.data['metadata']['related_identifiers']]
    ...
    def get_community_pending_requests(self, community, **params):
        ...
        """
        self._raise_token_status()
        community_json = get_community(
            community, sandbox=self.sandbox, token=self.access_token)
        community_requests_url = community_json['links']['requests']
        parameters = deepcopy(self.parameters)
        parameters.update(params)
        response = requests.get(community_requests_url,
                                params=parameters, timeout=_default_timeout)
        http_status.ZenodoHTTPStatus(response)
        response_json = response.json()['hits']['hits']
        response_json = [PendingRequest(hit, sandbox=self.sandbox, access_token=self.access_token) for hit in response_json if hit['is_open']]
    ...
    def answer_to_pending_request(self, community: str, record_id: Union[str, int], accept: bool):
        ...
        if pending_record is None:
            raise ValueError(
                f"Record {record_id} is not in the pending requests of community {community}")
...
class Record:
    ...
    def __init__(self, data: dict):
        ...
        for meta_key in ['title', 'doi']:
            if meta_key not in data['metadata'].keys():
                raise ValueError(
                    f"Mandatory key {meta_key} not in data['metadata']")
        self.data = data
    ...
    def _write_zenodo_deposit(self, filename='.zenodo.json', overwrite=False, validate=True):
        ...
        if 'communities' in metadata:
            metadata['communities'] = [{'identifier': c['id']}
                                       for c in metadata['communities']]
        if 'zenodo' in metadata['doi']:
            metadata.pop('doi')
        metadata['license'] = metadata['license']['id']

        write_zenodo_metadata(metadata, filename=filename,
    ...
    def get_associated_versions(self, size=_default_size_query, **kwargs):
        ...
        ----------
        size : int, optional
            Number of results to return. Default is 50 (`_default_size_query`)
        **kwargs : dict
    ...
    def _summary(self, linebreak='\n'):
        ...
        description = re.sub('<p>', linebreak, re.sub(
            '</p>', linebreak, description))
    ...
    def get_codemeta(self, **zipurl_kwargs):
        ...
        if 'files' not in self.data:
            raise FileNotFoundError(
                f'The record {self.id} does not contain any file')

        codemeta_paths = [s for s in self.filelist if Path(
            s.rsplit('/content', maxsplit=1)[0]).name == 'codemeta.json']
        ...
        if len(codemeta_paths) >= 1:
            ...
            for zipurl in ziparchives:
                try:
                    ...
                    pass
            raise FileNotFoundError(
                f"No `codemeta.json` file found in record {self.id}")
        else:
            raise FileNotFoundError(
                f"No `codemeta.json` file found in record {self.id}")
...
def get_supported_licenses(size=1000):
    ...
    ----------
    size : int, optional
        The number of licenses to retrieve (default is 1000)

    Returns
    -------
    list
        A list of license IDs for Zenodo supported licenses
    """
...
def _search(field, search='', sandbox=False, **kwargs):
    """
    Text based search base function.
    Returns a list of hits (JSON dict containing the object metadata).
    ...
    **kwargs
    ...
    -------
    list[dict]
        A list of hits (JSON dict containing the object metadata).
    """
...
def search_funders(search='', sandbox=False, **kwargs):
    ...
    **kwargs : dict
    ...
    -------
    list of dict
        A list of funders matching the search query.

    Examples
    --------
    >>> search_funders('research')
    [{'name': 'National Science Foundation'}, {'name': 'European Research Council'}, {'name': 'Wellcome Trust'}, {'name': 'Bill & Melinda Gates Foundation'}, {'name': 'National Institutes of Health (NIH)'}]
    """
...
def search_communities(search='', sandbox=False, **kwargs):
    """
    Text based search of communities.
    ...
    **kwargs : dict
        ...
        https://developers.zenodo.org/#list36. Common arguments are:
            - size : int, optional
                Number of results to return. Default is 5.
    ...
    -------
    list of dict
        A list of communities matching the search query.
    """
    kwargs.setdefault('size', 5)
...
def search_licenses(search='', sandbox=False, **kwargs):
    ...
    **kwargs : dict
    ...
    Common query arguments include:
    - size : int, optional
        Number of results to return. Default is 5.
    """
    kwargs.setdefault('size', 5)
...
class PendingRequest:
    ...
    def url(self):
        ...
        if not community_id:
            ...
            return self.data.get('links', {}).get('self_html', '').replace('requests', f'communities/{community_id}/requests')
    ...
    def accept(self):
        ...
        """
        accept_url = self.data.get('links', {}).get(
            'actions', {}).get('accept')
        if accept_url:
    ...
    def decline(self):
        ...
        """
        decline_url = self.data.get('links', {}).get(
            'actions', {}).get('decline')
        if decline_url:
