...
class ZipUrl(remotezip.RemoteZip, ZipFile):
    ...
    def find_files(self, filename):
        ...
        """
        matching_files = [
            f for f in self.namelist() if Path(f).name == filename]
...
def zip_repository(repository_path, zip_filename=None, overwrite=True):
    ...
    print(
        f" * Zipping the content of {directory.absolute()} into {zip_filename}")
...
def spdx_licenses(open_only=True):
    ...
    try:
        ...
        raise requests.exceptions.HTTPError(
            f"Error while fetching spdx licenses: {e}") from e

    licenses = req.json()['licenses']
    if open_only:
        licenses = [lic['licenseId']
                    for lic in licenses if lic['isOsiApproved']]
    else:
        licenses = [lic['licenseId']
                    for lic in licenses if not lic['isOsiApproved']]
    return licenses
