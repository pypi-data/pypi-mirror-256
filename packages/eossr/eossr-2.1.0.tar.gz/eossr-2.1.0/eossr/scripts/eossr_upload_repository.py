...
def upload(
    ...
    if path_zenodo_file.exists():
        print(
            f"Record metadata based on zenodo file {zenodo.path_zenodo_file}")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        zip_filename = Path(upload_directory).absolute(
        ).name + '.zip' if archive_name is None else archive_name
        zip_repository(upload_directory, zip_filename=Path(
            tmpdir).joinpath(zip_filename))
