...
class GnomesAtNightEnv9A(gym.Env):
    ...
    def __init__(self, render_mode=None, round=1):
        ...
        self.SCREEN_WIDTH = (self.size * self.CELL_SIZE) * 2 + self.OFFSET + (self.MARGIN * 2)
        ...
        self.treasure = {'onWhichSide': (self.round-1)%2, 'pos': np.array(self.treasure_pos_this_round)}

        # Define the observation space
        # self.observation_space = spaces.Tuple([spaces.Dict(
        #         {
        #             "current_player": spaces.Discrete(2),   # 0 or 1
        #             "token": spaces.Box(low=np.array([0, 0]), high=np.array([8, 8]), dtype=np.int32),
        #             "walls": spaces.MultiBinary(8*9*2),   # num_vertical = 8 * 9, num_horizontal = 9 * 8, total_walls = num_vertical + num_horizontal
        #             "treasure": spaces.Box(low=np.array([-1, -1]), high=np.array([8, 8]), dtype=np.int32)  # (-1, -1) if not visible
        #         }
        ...
        self.observation_space = spaces.Tuple(
            [gym.spaces.Box(low=obs_low, high=obs_high, dtype=np.int32) for _ in range(self.n_agents)])
    ...
    def is_action_valid(self, current_player, action):
        ...
        if action == 1:
            ...
            elif self.one_hot_walls[current_player]['vertical'][self.token_pos[1],self.token_pos[0]] == 1:
        ...
        if action == 2:
            ...
            if self.one_hot_walls[current_player]['horizontal'][self.token_pos[1] - 1, self.token_pos[0]] == 1:
                # print("wall above")
                return False
        ...
        if action == 3:
            ...
            if self.one_hot_walls[current_player]['vertical'][self.token_pos[1], self.token_pos[0] - 1] == 1:
        ...
        if action == 4:
            ...
            if self.one_hot_walls[current_player]['horizontal'][self.token_pos[1], self.token_pos[0]] == 1:
                # print("wall below")
                return False
    ...
    def get_player_observation(self, player):
        ...
        if self.treasure['onWhichSide'] == player:
            treasure_pos = self.treasure['pos']
        else:
    ...
    def get_infos(self):
        return {
            ...
            "distance": np.linalg.norm(
                self.token_pos - self.treasure['pos'], ord=1    # Manhattan distance from token to treasure
            )
        }
    ...
    def _render_frame(self):
        ...
        self.draw_walls_offset(canvas, self.one_hot_walls[0], offset_x=0, MARGIN=self.MARGIN, CELL_SIZE=self.CELL_SIZE, OUT_WALL_WIDTH=self.OUT_WALL_WIDTH, WALL_WIDTH=self.WALL_WIDTH, WALL_COLOR=self.WALL_COLOR)
        self.draw_walls_offset(canvas, self.one_hot_walls[1], offset_x=self.size * self.CELL_SIZE + self.OFFSET, MARGIN=self.MARGIN, CELL_SIZE=self.CELL_SIZE, OUT_WALL_WIDTH=self.OUT_WALL_WIDTH, WALL_WIDTH=self.WALL_WIDTH, WALL_COLOR=self.WALL_COLOR)

        if self.current_player == 0:
            solid_center_x = int((self.token_pos[0] + 0.5) * self.CELL_SIZE + self.MARGIN)
            empty_center_x = int((self.token_pos[0] + 0.5) * self.CELL_SIZE + self.MARGIN + self.size * self.CELL_SIZE + self.OFFSET)
        else:
            solid_center_x = int((self.token_pos[0] + 0.5) * self.CELL_SIZE + self.MARGIN + self.size * self.CELL_SIZE + self.OFFSET)
            empty_center_x = int((self.token_pos[0] + 0.5) * self.CELL_SIZE + self.MARGIN)
        ...
        if self.treasure['onWhichSide'] == 0:
            ...
            pygame.draw.circle(
                ...
                (255, 255, 0),
                (
                    int((self.treasure['pos'][0] + 0.5) * self.CELL_SIZE + self.MARGIN + self.size * self.CELL_SIZE + self.OFFSET),
                    int((self.treasure['pos'][1] + 0.5) * self.CELL_SIZE + self.MARGIN),
                ),
                int(self.CELL_SIZE / 3),
            )
    ...
    def draw_walls_offset(self, screen, one_hot_walls, offset_x, MARGIN, CELL_SIZE, OUT_WALL_WIDTH, WALL_WIDTH, WALL_COLOR):
        offset_x += MARGIN
        offset_y = MARGIN  # Define the vertical offset as well 

        # Draw the outer boundary of the maze
        pygame.draw.rect(screen, WALL_COLOR, (offset_x, offset_y, self.size * CELL_SIZE, OUT_WALL_WIDTH))  # Top
        pygame.draw.rect(screen, WALL_COLOR, (offset_x, offset_y, OUT_WALL_WIDTH, self.size * CELL_SIZE))  # Left
        pygame.draw.rect(screen, WALL_COLOR, (offset_x, offset_y + self.size * CELL_SIZE - OUT_WALL_WIDTH, self.size * CELL_SIZE, OUT_WALL_WIDTH))  # Bottom
        pygame.draw.rect(screen, WALL_COLOR, (offset_x + self.size * CELL_SIZE - OUT_WALL_WIDTH, offset_y, OUT_WALL_WIDTH, self.size * CELL_SIZE))  # Right
            ...
            for x in range(self.size - 1):
                if one_hot_walls['vertical'][y][x] == 1:
                    ...
                    pygame.draw.line(screen, WALL_COLOR,
                                    ...
                                    ((x + 1) * CELL_SIZE + offset_x, (y + 1) * CELL_SIZE + offset_y), WALL_WIDTH)
