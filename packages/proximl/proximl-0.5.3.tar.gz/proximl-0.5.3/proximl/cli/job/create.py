...
@click.option(
    "--model-id",
    type=click.STRING,
    help="proxiML Model ID to use as the model data",
)
@click.option(
    "--model-dir",
    type=click.Path(exists=True, file_okay=False, resolve_path=True),
...
def notebook(
    ...
    if model_dir:
        config.proximl.run(job.wait_for("waiting for data/model download"))
...
@click.option(
    "--output-type",
    type=click.Choice(
        ["aws", "gcp", "local", "proximl", "azure", "wasabi"],
        case_sensitive=False,
    ),
...
def from_json(config, attach, connect, file):
    ...
    job = config.proximl.run(config.proximl.client.jobs.create_json(payload))
