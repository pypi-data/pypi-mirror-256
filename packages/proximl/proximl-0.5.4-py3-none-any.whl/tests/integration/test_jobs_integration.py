...
class JobLifeCycleTests:
    ...
    async def test_get_jobs(self, proximl, job):
        jobs = await proximl.jobs.list()
        assert len(jobs) > 0

    async def test_get_job(self, proximl, job):
        response = await proximl.jobs.get(job.id)
...
class JobAPIResourceValidationTests:
    async def test_invalid_gpu_type(self, proximl):
        with raises(ApiError) as error:
    ...
    async def test_invalid_disk_size(self, proximl):
        with raises(ApiError) as error:
            await proximl.jobs.create(
    ...
    async def test_combine_cpu_and_gpu_types(self, proximl):
        with raises(ApiError) as error:
            await proximl.jobs.create(
    ...
    async def test_missing_cpu_count(self, proximl):
        with raises(ApiError) as error:
            await proximl.jobs.create(
    ...
    async def test_invalid_cpu_count(self, proximl):
        with raises(ApiError) as error:
            await proximl.jobs.create(
    ...
    async def test_invalid_cpu_count_for_gpu(self, proximl):
        with raises(ApiError) as error:
            await proximl.jobs.create(
    ...
    async def test_invalid_cpu_count_for_gpu_2(self, proximl):
        with raises(ApiError) as error:
            await proximl.jobs.create(
    ...
    async def test_invalid_cpu_count_for_gpu_3(self, proximl):
        with raises(ApiError) as error:
            await proximl.jobs.create(
...
class JobAPIDataValidationTests:
    async def test_invalid_output_type_for_notebook(self, proximl):
        with raises(ApiError) as error:
            await proximl.jobs.create(
...
class JobAPIWorkerValidationTests:
    async def test_missing_workers_for_training(self, proximl):
        with raises(ApiError) as error:
            await proximl.jobs.create(
    ...
    async def test_too_many_workers_for_inference(self, proximl):
        with raises(ApiError) as error:
            await proximl.jobs.create(
    ...
    async def test_invalid_worker_spec_for_endpoint(self, proximl):
        with raises(ApiError) as error:
            await proximl.jobs.create(
...
class JobIOTests:
    ...
    async def test_job_model_input_and_output(self, proximl, capsys):
        ...
        assert (
            new_model.name
            == "Job - CLI Automated Tests - Training With proxiML Model Output"
        )
...
class JobTypeTests:
    async def test_endpoint(self, proximl):
        job = await proximl.jobs.create(
            ...
            model=dict(
                source_type="git",
                source_uri="https://github.com/proxiML/simple-tensorflow-classifier.git",
            ),
            endpoint=dict(
                routes=[
                    dict(
    ...
    async def test_job_custom_container(self, proximl, capsys):
        job = await proximl.jobs.create(
            ...
            data=dict(
                ...
                output_type="wasabi",
                output_uri="s3://proximl-example/output/resnet_cifar10",
                output_options=dict(
...
class JobFeatureTests:
    ...
    async def test_inference_job(self, proximl, capsys):
        ...
        job = await proximl.jobs.create(
            ...
            data=dict(
                input_type="wasabi",
                input_uri="s3://proximl-example/input/cifar-10",
                input_options=dict(endpoint_url="https://s3.wasabisys.com"),
                output_type="local",
                output_uri=temp_dir.name,
                output_options=dict(archive=False),
            ),
            model=dict(git_uri="git@github.com:proxiML/environment-tests.git"),
        )
        assert job.id
        await job.wait_for("running")
        await job.connect()
