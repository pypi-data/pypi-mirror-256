...
def execute_install():
    ...
    if os.path.exists(plugin_base.package_dek_filepath):
        plugin_base.save_json(plugin_base.package_standard_filepath,
                              copy_package(plugin_base.load_json(plugin_base.package_dek_filepath)))
    walk(project_dir)
...
def execute_publish(cmd_publish):
    if os.path.exists(plugin_base.package_dek_filepath) and not os.path.exists(plugin_base.package_standard_filepath):
        plugin_base.save_json(plugin_base.package_standard_filepath,
                              copy_package(plugin_base.load_json(plugin_base.package_dek_filepath)))
    return run_cmd(cmd_publish, False)
...
def execute_server(begin=False):
    ...
    if os.path.isdir(dir_servers) and os.listdir(dir_servers):
        ...
        if not os.path.exists(dir_material):
            os.system(f'djcreator wrapper clean --project={project_name}')
            ret = os.system(f'djcreator project {project_name} {dir_work}')
            if ret != 0:
        ...
        os.chdir(dir_work)
        os.system(f'dekshell rf .shell.initshell.pysh')
        if begin:
            os.chdir(dir_material)
            os.system(f'dekshell rf .shell.generate.{project_name}.pysh')
            os.chdir(project_dir)
            os.system(f'deknp install')
