...
from dektools.dict import flat, assign
...
class PluginBase:
    ...
    def get_package_json(self, filepath):
        data = {}
        package_path = os.path.join(filepath, self.package_standard_name)
        package_dek_path = os.path.join(filepath, self.package_dek_name)
        if os.path.isfile(package_path):
            data = self.load_json(package_path)
        elif os.path.isfile(package_dek_path):
            data = self.load_json(package_dek_path)
        return data
...
class Plugin(PluginBase):
    ...
    def template_full(self):
        return Template(self.flat_variables(self.get_variables()))
