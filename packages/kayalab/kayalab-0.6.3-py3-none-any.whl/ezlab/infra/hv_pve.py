...
from ezlab.infra.authkey import get_ssh_pubkey_from_privatekey, get_privatekey_from_string
from ezlab.infra.remote import wait_for_ssh
...
def clone(
    ...
    queue.put(f"[{vm_name}] assigned id {nextid}")
    ...
    if dryrun:
        ...
        try:
            ...
            if result == TASK_FINISHED:
                ...
                queue.put(f"PVE CLONE FAILED for {vm_name}: {result}")
                queue.put(TASK_FINISHED)
                return False
        except Exception as error:
            queue.put(f"PVE VMCLONE EXCEPTION for {vm_name}: {error}")
            queue.put(TASK_FINISHED)
            return False

    if dryrun:
        queue.put(f"""Would update VM config with:
            cores={eznode["cores"]},
        ...
        # configure vm
        queue.put(f"[ {vm_name} ] reconfigure")
        try:
            new_vm.config.post(
                cores=eznode["cores"],
                memory=eznode["memGB"] * 1024,
            ...
            queue.put(f"PVE VMCONFIG EXCEPTION for {vm_name}: {error}")
            queue.put(TASK_FINISHED)
            return False
    ...
    if dryrun:
        ...
        try:
            ...
            queue.put(f"[{vm_name}] {SWAP_DISK_SIZE}GB swap disk added")
            ...
            queue.put(f"PVE VMUPDATE EXCEPTION for {vm_name}: {error}")
            queue.put(TASK_FINISHED)
            return False
