...
from .common import (
    ...
    start_tasks_delayed,
    technitium_add_record,
    technitium_delete_record,
    wait_for_ssh,
    wait_tasks,
)
...
def get_connection(host: str, username: str, password: str):
    ...
    if not service_instance:
        raise SystemExit(
            f"Unable to connect to host with supplied credentials. username={username} password={password}"
        )
...
def get_resources(si: vim.ServiceInstance):
    ...
    hostfolder: vim.Folder = select_from_objects(
        datacenter.hostFolder.childEntity, "Host Folder"
    )
