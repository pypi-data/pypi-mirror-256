...
from ..constants import path_work_dir, dir_name_venv
from ..utils.common import get_py_pkg_name, shell_line
from ..utils.file import write_file, read_text, sure_dir, remove_path
from ..utils.sys import sys_paths_relative, path_sys_exe
from ..main.command import build, is_pdm_project, get_pkg_path
from .core import get_pkg_entry_points_paths

path_scripts = os.path.join(path_work_dir, 'scripts')
path_scripts_info = os.path.join(path_work_dir, '.scripts.json')
...
@app.command()
def add(name: str = '', pkg: str = '', path: str = '.'):
    if name and not pkg:
        pkg = name
    if pkg:
        req = Requirement(pkg)
    ...
    path_venv = os.path.join(path_scripts_item, dir_name_venv)
    ...
    if pkg:
        cmd = f'pdm add "{pkg}" -v -p {path_scripts_item}'
        ...
        cmd = f'pdm add file:///{file} -v -p {path_scripts_item}'
    ...
    path_sys_exe_scripts = sys_paths_relative(os.path.dirname(path_sys_exe()))['scripts']
    dict_path_cmd_proxy = {}
...
path_bin_work = sys_paths_relative(os.path.dirname(path_sys_exe()))['scripts']
...
def pop(name: str = '', pkg: str = ''):
    ...
    if os.path.exists(path_scripts_item):
        cmd = f'pdm remove "{pkg}" -v -p {path_scripts_item}'
        shell_line(cmd)
    else:
        raise
