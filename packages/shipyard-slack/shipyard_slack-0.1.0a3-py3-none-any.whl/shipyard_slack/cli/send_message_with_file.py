...
def main():
    try:
        ...
        if len(files) > 1:
            ...
            logger.error(f"No files found with name {args.source_file_name}")
            raise ExitCodeException(f"No files found with name {args.source_file_name}",
        ...
        if args.destination_type == "dm":
            ...
            responses.append(
                send_slack_message_with_file(slack_client, create_name_tags(user_id_list) + message, upload,
                                             args.channel_name, include_in_thread))

        artifact.responses.write_json(os.getenv("SHIPYARD_BLUEPRINT_NAME", "send_message_with_file"), responses)
