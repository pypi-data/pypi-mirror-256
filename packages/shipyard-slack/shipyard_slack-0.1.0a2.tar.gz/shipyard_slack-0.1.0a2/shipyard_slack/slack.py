...
class SlackClient(Messaging):
    ...
    def _handle_slack_error(self, slack_error: SlackApiError):
        ...
        if error_type == 'invalid_auth':
            ...
            raise ExitCodeException("Invalid Slack token", self.EXIT_CODE_INVALID_CREDENTIALS)
            ...
            logger.error(f"Failed to validate credentials. Response from Slack servers: {slack_error}")
            raise ExitCodeException(f" Response from Slack servers: {slack_error}",
    ...
    def connect(self):
        try:
            ...
            if e.response.get("error") == 'invalid_auth':
                ...
                logger.authtest(f"Failed to validate credentials. Response from Slack servers: {e}")
            return 1
        else:
            return 0
    ...
    def send_message(self, message: str, channel_name: str, download_link: str = ""):
        ...
        message_response = self.web_client.chat_postMessage(
            channel=channel_name, link_names=True, text=message,
            blocks=self._create_blocks(message, download_link=download_link)
        )
        logger.debug(f'Your message of "{message}" was successfully sent to {channel_name}')
    ...
    def user_lookup(self, lookup, lookup_method):
        ...
        if lookup_method not in {"email", "real_name", "display_name"}:
            raise self.EXIT_CODE_INVALID_INPUT(
    ...
    def search_user_by_email(self, email_address):
        logger.debug("Attempting to look up user by email...")
        try:
        ...
        try:
            ...
            for user in response.data['members']:
                if user.get('real_name') == name:
                    ...
                    return user
            raise ExitCodeException(f"User {name} not found", self.EXIT_CODE_USER_NOT_FOUND)
        except SlackApiError as e:
            self._handle_slack_error(e)
    ...
    def search_user_by_display_name(self, display_name):
        ...
        try:
            ...
            raise ExitCodeException(f"User {display_name} not found", self.EXIT_CODE_USER_NOT_FOUND)
