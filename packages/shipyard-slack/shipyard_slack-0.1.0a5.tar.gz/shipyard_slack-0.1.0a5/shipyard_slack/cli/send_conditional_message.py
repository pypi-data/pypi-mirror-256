...
def main():
    try:
        ...
        if args.destination_type == "channel" and not args.channel_name:
            raise ExitCodeException("--destination-type channel requires --channel-name",
                                    SlackClient.EXIT_CODE_INVALID_INPUT)
        elif args.destination_type == "dm" and not args.users_to_notify:
            raise ExitCodeException("--destination-type dm requires --users-to-notify",
                                    slack_client.EXIT_CODE_INVALID_INPUT)

        if args.users_to_notify and not args.user_lookup_method:
            raise ExitCodeException("--users-to-notify requires a --user-lookup-method",
                                    slack_client.EXIT_CODE_INVALID_INPUT)
        ...
        files = file_utils.find_matching_files(args.source_file_name, source_folder_name,
        ...
        if args.destination_type == "dm" and file_upload:
            ...
            response = send_slack_message_with_file(slack_client, create_name_tags(user_id_list) + message, upload,
