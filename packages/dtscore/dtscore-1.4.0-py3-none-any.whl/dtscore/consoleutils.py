...
def promptfor_int(prompt:str) -> int:
    ...
    if intvalue != None: return intvalue
...
def promptfor_text(prompt:str, toupper:bool=False) -> str:
    """ prompt for text, convert to upper case if toupper is true """
    print(prompt + ': ', end='')
    response = input()
...
def promptfor_optionaltext(prompt:str, toupper:bool=False) -> Optional[str]:
    """ prompt for option text. return None if user enters return  """
    response = promptfor_text(prompt, toupper)
