...
class CSVReader(_Generic[_T]):
    ...
    * self.Headers  : A list of column names that must match the column count on the rest of csv rows.
    ...
    def Export(self, filepath:str, mergeExistingFile=False):
        """
        Exports the CSV instance to a specified file path.

        :param filepath: The path to the CSV file where the data will be exported.
        :param mergeExistingFile: If True, appends the data to an existing file. \
                                  If False, creates a new file or overwrites an existing one.
        """
        import os

        # For mergeExistingFile strategy, non-existing or empty files can be simply redirected to regular export where headers are attached as well.
        if (mergeExistingFile) and (os.path.exists(filepath)) and (os.path.getsize(filepath) > 0):
            with open(filepath, 'a', newline='', encoding=self._encoding) as csv_file:
