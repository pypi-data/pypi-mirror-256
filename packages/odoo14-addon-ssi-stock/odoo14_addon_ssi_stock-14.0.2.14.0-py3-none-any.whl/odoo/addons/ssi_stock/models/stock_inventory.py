...
class StockInventory(models.Model):
    ...
    def _prepare_inventory_filter(self):
        ...
        if self.filter == "moving_product":
            ...
            current_datetime = pytz.UTC.localize(datetime.now()).astimezone(pytz.timezone(user_tz))
            ...
            start_date = datetime.strptime(start_date, datetime_format) - relativedelta(hours=hours)
            end_date = datetime.strptime(end_date, datetime_format) - relativedelta(hours=hours)
            criteria = [
                ("date", ">=", start_date),
                ("date", "<=", end_date),
                ("state", "=", "done"),
