...
class AnalysisContext:
    ...
    def get_qc_status(self, patient_secret_name=None, ssid=None, analysis_id=None):
        ...
        data_sessions = self.__comm.send_request(
            "patient_manager/get_patient_list", req_headers={"X-Range": "items=0-4500"}
        )
        self.logger.info(data_sessions[0])
        self.logger.info("Getting QC for {patient_secret_name} / {ssid}".format(**self.analysis_data))
        to_return = None
        for session in data_sessions:
            patient_secret_name = session.get("patient_secret_name", patient_secret_name)
            ssid = session.get("ssid", ssid)
            self.logger.info("Matches QC for {} / {}?".format(patient_secret_name, ssid))

            if str(self.analysis_data["patient_secret_name"]) == str(patient_secret_name) and str(
                    self.analysis_data["ssid"]
            ) == str(ssid):
        ...
        if analysis_id:
            try:
                to_return = [
                    analysis["qa_data"] for analysis in self.__comm.send_request(
                        "analysis_manager/get_analysis_list", req_headers={"X-Range": "items=0-4500"}
    ...
    def upload_file(
        ...
        if not direct:
            ...
            while not completed_upload and retry_count < self.__max_upload_retries:
                with open(source_file_path, "rb") as fp:
                    upload_response = self.__comm.send_request(
                        method="POST", url=signed_url, req_data=fp, return_json=False
                    )
            ...
            verification_response = self.__comm.send_request(
                ...
                req_data={"container_id": container_id, "path": destination_path, "md5": md5_hash},
            )
    ...
    def download_resource(self, resource_path, destination_path):
        ...
        """
        self.logger.info("Downloading resource {!r} to {!r}".format(resource_path, destination_path))

        data_to_send = {"analysis_id": self.analysis_id, "type": type, "path": resource_path}
        try:
        ...
        with open(destination_path, "wb") as fp:
            for chunk in res.iter_content(chunk_size=self.__comm.chunk_size):
                if chunk:  # filter out keep-alive new chunks
                    fp.write(chunk)
                    fp.flush()
        self.logger.debug("Finished downloading resource {!r}".format(destination_path))
    ...
    def set_metadata_value(self, key, value, title=None, readonly=False, patient_secret_name=None, ssid=None):
        ...
        data_to_send = {
            ...
            "key": key,
            "value": value,
            "patient_secret_name": patient_secret_name or self.analysis_data["patient_secret_name"],
    ...
    def get_metadata_value(self, key, patient_secret_name=None, ssid=None):
        """
        ...
        """
        self.logger.info(
            "Getting value from field with id {} from subject {}/{}".format(key, patient_secret_name, ssid)
        )
...
class File:
    ...
    def __download_file(self, destination_file_path):
        ...
        download_response = self.__comm.send_request(url=url, return_json=False, stream=True, method="GET")
