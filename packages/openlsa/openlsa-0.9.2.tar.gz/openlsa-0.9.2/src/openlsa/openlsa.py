...
class OpenLSA():
    ...
    def __compute_vec_k(self,
        ...
        if self.options['display']:
            plt.subplots(1)
            plt.imshow(np.log10(fft_img_abs), alpha=0.5,
        ...
        # init_angle
        if (np.abs(np.angle(vec_k))-init_angle) > np.pi/2:
        ...
        if self.options['display']:
            ...
            for ivec in range(2):
                ...
                plt.text(1.1*self.vec_k[ivec].real,
                         ...
                         texts[ivec], color=colors[ivec])
            plt.xlabel("frequency along ${e}_1$ [$pixel^{-1}$]")
            plt.ylabel("frequency along ${e}_2$ [$pixel^{-1}$]")
            plt.show()
    ...
    def compute_phases_mod(self, img, kernel=None, roi_coef=0.2, unwrap=True):
        ...
        mod = np.zeros(img.shape, dtype=complex)
        for i in range(len(self.vec_k)):
            mod += mods[i]*self.vec_dir(i)
        mod_abs = np.abs(mod)
        ...
        if self.temp_unwrap['roi_75percent'] is None:
            self.temp_unwrap['roi_75percent'] = mod_abs > mod_abs.max()*0.75

        return phi, mods

    def temporal_unwrap(self, img1, img2, phi_1, phi_2, uinit=None):
        ...
        point, uinit = self.rough_point2point(img1, img2, dis_init=uinit)
        return self.jump_correction(phi_1, phi_2, point), uinit
    ...
    def rough_point2point(self, img1, img2,
                          point1=None,
                          dis_init=None):
    ...
    def jump_correction(self, phi_1, phi_2, point2,
                        point1=None):
        ...
        rphi_1 = phi_1.interp(point1)
        rphi_2 = phi_2.interp(point2)
    ...
    def compute_displacement(self, phi_1, phi_2,
        ...
        if disp is None:
            ...
            disp = map_coordinates(disp, (z_roi.imag.ravel(),
    ...
    # %% extra function
    def compute_refstate_from_im_stack(self, kernel,
                                       im_folder=None,
                                       im_extensions='.tif',
                                       im_stack=None,
                                       s3_dictionary=None,
        ...
        s3_dictionary is formated as
            s3_dictionary = {'s3_bucket_name': s3_bucket_name,
                             's3_path_2_im': s3_path_2_im,
                             's3_path_2_folder': s3_path_2_folder,
                             's3_endpoint_url': s3_endpoint_url}
        # TODO : estimate noise properties from the data obtained with this stack of images
                 the strain can be written as convolution of the displacement -> to be
                 extended to the variance calculation."""
        ...
        if im_folder is not None:
            ...
            for ext in im_extensions:
                im_stack.extend(glob.glob(os.path.join(im_folder, f"*{ext}")))
            print(f"A path to a folder is given: {len(im_stack):d} images are found.")

        if im_stack is None:
            if s3_dictionary is None:
                print('Error - I need paths to images or at least to a folder')
            else:
                s3_flag = True
                im_stack, bucket = provide_s3_path(s3_dictionary, im_extensions)

        print(f"Reference coordinate system built with: \n    -> {im_stack[0]}")
        print(f"Computing reference phase: 1/{len(im_stack):d}")
        if s3_flag:
        ...
        phi_ref, __ = self.compute_phases_mod(img_ref, kernel, roi_coef=roi_coef)
        phi_ref_av = phi_ref.copy()
        for comp in [0, 1]:
            phi_ref_av[comp].data[:] = phi_ref_av[comp].data[:]/len(im_stack)
        self.check_temp_unwrap(img_ref, point1=point_2_follow)

        i = 0
        for img_name in im_stack[1:]:
            i += 1
            print(f"Computing reference phase: {i+1:d}/{len(im_stack):d}")
            if s3_flag:
            ...
            phi_loc = self.compute_phases_mod(img_loc, kernel,
                                              roi_coef=roi_coef/2)[0]
            phi_loc, __ = self.temporal_unwrap(img_ref, img_loc, phi_ref, phi_loc)
            uxy_loc = self.compute_displacement(phi_ref, phi_loc)
            uxy_loc_rbm = compute_rbm(uxy_loc[self.roi],
                                      self.__px_z[self.roi].real,
                                      self.__px_z[self.roi].imag)
            phi_loc_rbm = phi_loc.interp(self.__px_z[self.roi] + uxy_loc_rbm)
            u01_loc_rbm = axy_2_a01(self.vec_dir(), uxy_loc_rbm)
            for comp in [0, 1]:
                phi_u01_loc_rbm = (2*np.pi/self.pitch()[comp])*u01_loc_rbm[comp]
                phi_ref_av[comp].data[self.roi] += (phi_loc_rbm[comp].data
...
def provide_s3_path(s3_dictionary, im_extensions):
    """ This function reads the item s3_dictionary to provide a list of paths to a set of images,
    """
    if s3_dictionary['s3_path_2_im'] is not None:
    ...
    s3_resource = boto3.resource('s3', endpoint_url=s3_dictionary['s3_endpoint_url'])
