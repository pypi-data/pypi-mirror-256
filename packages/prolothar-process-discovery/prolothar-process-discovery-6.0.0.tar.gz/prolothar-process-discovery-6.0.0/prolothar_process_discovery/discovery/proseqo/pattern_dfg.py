...
class PatternDfg(DirectlyFollowsGraph):
    ...
    def add_node(self, activity ):
        super().add_node(activity)
        if not hasattr(self.nodes[activity], 'pattern'):
            self.nodes[activity].pattern = Singleton(activity)
    ...
    def add_pattern(self, activity: str, pattern: Pattern):
        ...
        self.nodes[activity].pattern = pattern
    ...
    def get_patterns_with_activity(self, activity: str) -> List[Pattern]:
        matching_patterns = []
        for node in self.nodes.values():
            if node.pattern.contains_activity(activity):
                matching_patterns.append(node.pattern)
    ...
    def copy(self) -> 'PatternDfg':
        ...
        copy.join(self)
        for node in self.nodes.values():
            copy.add_pattern(node.activity, node.pattern.copy())
        return copy
    ...
    def remove_degenerated_patterns(self):
        ...
        for node in list(self.get_nodes()):
            ...
            if node.activity != node.pattern.get_activity_name():
                self.rename_activity(node.activity, node.pattern.get_activity_name())
    ...
    def compute_activity_set(self) -> Set[str]:
        ...
        activity_set = set()
        for node in self.get_nodes():
            activity_set.update(node.pattern.get_activity_set())
        return activity_set
    ...
    def generate_log(
            ...
            start_activities: List[str] = None,
            end_activities: List[str] = None) -> EventLog:
