...
class ParallelCandidateGenerator(PatternCandidateGenerator):
    """finds parallel pattern candidates in a pattern_dfg"""
    ...
    def _generate_patterns(
        ...
        if self.__only_perfect_matches:
            candidates = find_perfect_matching_parallels_in_dfg(pattern_dfg)
        else:
            candidates = find_parallel_candidates_in_dfg(pattern_dfg)
            candidates.update(find_binary_parallel_candidates_in_dfg(pattern_dfg))
        return candidates
