...
class SumOfTraceAttributeOnNodeLevel(AbstractNumericProjection):
            ...
            for trace in self.__log.traces:
                for activity in node.pattern.get_activity_set():
                    if trace.contains_activity(activity):
                        summed_value += trace.attributes[self.__attribute]
                        break
            node_value_dict[node.activity] = summed_value
