...
RE_APPLICATION = r"^([A-Za-z]+/)+v\d+r\d+(p\d+)?(\@[a-z0-9_\-\+]+)?"
RE_JOB_NAME = r"^[a-zA-Z0-9][a-zA-Z0-9_\-]+$"
RE_OUTPUT_FILE_TYPE = r"^([A-Za-z][A-Za-z0-9_]+\.)+((ROOT|root|HIST|hist)|.?(DST|dst))$"
    ...
    "bk_query": Map(
        {
            ...
            Optional("keep_running", default=False): Bool(),
        }
    ),
    "job_name": Map(
