...
class dataprocessing:
    ...
    def help(self):
        ...
        print("   - Usage: aggregate_daily_to_wc(df, date_column, group_columns, sum_columns, wc, aggregation='sum', include_totals=False)")
        print("   - Example: aggregate_daily_to_wc(df, 'date', ['platform'], ['cost', 'impressions', 'clicks'], 'mon', 'average', True)")
        ...
        print("\n13. merge_dataframes_on_column")
        print("    - Description: Merge a list of DataFrames on a common column.")
        print("    - Usage: merge_dataframes_on_column(dataframes, common_column='OBS', merge_how='inner')")
        print("    - Example: merge_dataframes_on_column([df1, df2, df3], common_column='OBS', merge_how='inner')")
        ...
        print("    - Usage: classify_within_column(df, col_name, to_find_dict, default_value = 'other')")
    ...
    def aggregate_daily_to_wc(self, df, date_column, group_columns, sum_columns, wc, aggregation='sum', include_totals=False):
        """
    ...
    def classify_within_column(self, df, col_name, to_find_dict, default_value = "other"):
        # Define the inner function for classification
        def find_word_in_string(x_string):
            for key, value in to_find_dict.items():
                if key.lower() in x_string:
                    return value
            return default_value

        # Apply the inner function to the specified column and add the result as a new column
