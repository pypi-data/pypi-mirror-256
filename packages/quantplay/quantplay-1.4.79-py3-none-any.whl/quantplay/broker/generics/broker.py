...
class Broker:
    ...
    def verify_rms_square_off(self, stoploss, target, keep_hedges=False, ticks=5):
        ...
        if len(positions[positions.quantity != 0]) == 0:
            return {"should_exit": False, "pnl": pnl, "sleep_time": sleep_time}
        ...
        if stoploss is not None and pnl < stoploss:
            ...
            return {"should_exit": True, "pnl": pnl, "sleep_time": sleep_time}
    ...
    def square_off_all(
        ...
        for order in orders_to_close:
            ...
            order["price"] = self.market_protection_price(
                order["price"], order["transaction_type"]
            )
            print(
                order["symbol"],
                order["exchange"],
