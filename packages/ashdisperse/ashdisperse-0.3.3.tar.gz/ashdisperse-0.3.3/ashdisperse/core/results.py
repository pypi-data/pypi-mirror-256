...
from ashdisperse.params.params import Parameters
...
class AshDisperseResult:
    ...
    def __init__(self, params, x, y, C0, Cz):
    ...
    def from_netcdf(r, filename):
        ...
        if isinstance(da.diameter, np.float64):
            p.grains.fromvalue(da.diameter, da.density, da.proportion)
        else:
            p.grains.fromlists(da.diameter, da.density, da.proportion)
        ...
        p.source = SourceParameters(
            ...
            Suzuki_k=da.Suzuki_k,
            Suzuki_peak=da.Suzuki_peak,
            name=da.name,
        )
        ...
        p.model.from_params(p.solver, p.met, p.source, p.grains, p.physical)

        p.output = OutputParameters(start=da.start, stop=da.stop, step=da.step)

        r = AshDisperseResult(p, da.x.values, da.y.values, da.C0.values, da.Cz.values)
    ...
    def to_netcdf(self, filename="AshDisperse.nc", compress=True, complevel=5):
        ...
        da = xa.Dataset(
            data_vars=dict(
                C0=(["x", "y", "grains"], self.C0_dimless),
                Cz=(["x", "y", "z", "grains"], self.Cz_dimless),
            ),
            coords=dict(
            ...
            attrs=dict(
                ...
                Suzuki_k=self.params.source.Suzuki_k,
                Suzuki_peak=self.params.source.Suzuki_peak,
                ...
                SettlingScale=self.params.model.SettlingScale,
                Velocity_ratio=self.params.model.Velocity_ratio,
                xyScale=self.params.model.xyScale,
                Lx=self.params.model.Lx,
                Ly=self.params.model.Ly,
                cScale=self.params.model.cScale,
                QScale=self.params.model.QScale,
                Peclet_number=self.params.model.Peclet_number,
                Diffusion_ratio=self.params.model.Diffusion_ratio,
                sigma_hat=self.params.model.sigma_hat,
                sigma_hat_scale=self.params.model.sigma_hat_scale,
