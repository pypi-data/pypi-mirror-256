...
class Camera:
        ...
        try:
            ...
            try:
                if interface is not None:
                    ...
                    self._interface = _scanner(self.sdk, [
                        "FireWire",
                        "Camera Link MTX",
                        "GenICam",
                        "Camera Link NAT",
                ...
                raise CameraException(sdk=self, code=error_msg)
    ...
    def record(self, number_of_images=1, mode="sequence", file_path=None):  # file_type=None   mode=file & file non blocking
        ...
        if not number_of_images:
            raise CameraException(sdk=self.sdk, message="number_of_images for record(...) must not be 0", code=0xA0160001)
    ...
    def images(
        ...
        if not self.rec.recorder_handle.value:
            if self.has_ram:
                raise CameraException(ssdk=self.sdk, message="Camera has no memory allocated for the active segment", code=0x00160000|0xA0000006)
            raise CameraException(sdk=self.sdk, message="Camera has not recorded yet. Call \"record(...)\" before \"images(...)\"", code=0x00160000|0x80003024)
