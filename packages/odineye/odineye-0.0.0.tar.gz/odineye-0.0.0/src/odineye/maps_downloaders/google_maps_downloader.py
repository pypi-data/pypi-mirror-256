...
from odineye.url_downloaders.url_downloader import UrlDownloader
...
class GoogleMapsDownloader(MapsDownloader):
	...
	def __call__(
		...
		"""
		super().__call__(
			north_latitude, east_longitude, south_latitude, west_longitude, zoom, map_style
		)
		assert (
		...
		if resize:
			map_image = resize_image(map_image, resize)

		map_image = map_image.convert(image_format)
