...
class DynamoDBAPI:
    def __init__(self, group_name, credentials=None, region=shared.DEFAULT_REGION):
        """
        :param table_name: "테이블 이름"
        :param credentials: {
            "aws_access_key_id": "str",
        ...
        self.boto3_session = shared.get_boto_session(credentials)
        self.cache = {}
        self.dynamoDB = wrapper.DynamoDB(self.boto3_session, region, group_name)
    ...
    def put_item(self, table_name:str, item:Dict, can_overwrite:bool=False)->Dict:
        table_name = self._wrap_table_name(table_name)
        response = self.dynamoDB.put_item(table_name, item, can_overwrite)
        response = encode_dict(response)
        return response

    def get_item(self, table_name: str, id: str, consistent_read: bool) -> Dict[str, Any]:
        """
        ...
        table_name = self._wrap_table_name(table_name)
        item = self.dynamoDB.get_item(table_name, key, consistent_read)
        item = encode_dict(item)
        return item
    ...
    def update_item(self, table_name:str, id:str, new_item:Dict)->Dict:
        """
            아이탬을 업데이트합니다.
        ...
        table_name = self._wrap_table_name(table_name)
        response = self.dynamoDB.update_item(table_name, key, new_item)
    ...
    def query_items(self, table_name:str, pk_field:str, pk_value:Any,
                    ...
                    recursive_filters:Dict=None)->Tuple[List[Dict], Dict]:
        """
