...
class Coupon(PromotionBaseModel):
    ...
    def clean(self):
        ...
        if current_time < self.start_date or current_time > self.end_date:
            raise ValidationError(_("The current date is outside the valid time range."))

        if self.is_percentage and (self.value < 0 or self.value > 100):
            raise ValidationError(_("The percentage value must be between 0 and 100 if is_percentage is True."))
        elif not self.is_percentage and self.value < 0:
            raise ValidationError(_("The value must be greater than or equal to 0 if is_percentage is False."))
