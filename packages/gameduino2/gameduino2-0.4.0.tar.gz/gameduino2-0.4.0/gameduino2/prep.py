...
def preview(np, fmt, size, data):
    ...
    if fmt == gd2.L1:
        ...
        (r, g, b) = (a, a, a)
        if fmt == gd2.ARGB4:
            d16 = np.array(array.array('H', data.tobytes()))
            a = (15 & (d16 >> 12)) / 15.
            r = (15 & (d16 >> 8)) / 15.
            g = (15 & (d16 >> 4)) / 15.
            b = (15 & (d16 >> 0)) / 15.
...
class AssetBin(gameduino2.base.GD2):
    ...
    def load_font(self, name, ims, widths, fmt, **args):
        ...
        dblock = array.array('B', widths).tobytes() + struct.pack("<5i", fmt, stride, sz[0], sz[1], src)
        self.alldata += dblock
