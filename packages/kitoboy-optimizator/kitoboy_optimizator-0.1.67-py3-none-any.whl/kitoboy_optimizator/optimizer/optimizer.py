...
class Optimizer:
    ...
    def pass_results_filters(self, backtest_results: BacktestResults, filters: dict|None) -> bool:
        if filters is None:
            return True
        ...
        if filters.get("min_profit") is None:
            ...
            min_profit_filter_result = backtest_results.net_profit >= filters.get("min_profit")

        if filters.get("max_drawdown") is None:
            max_drawdown_filter_result = True
        else:
            max_drawdown_filter_result = backtest_results.max_drawdown <= filters.get("max_drawdown")
