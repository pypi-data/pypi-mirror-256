...
class Reporter:
    ...
    def backtest_results_report(
        ...
        self.jobs.append(
            asyncio.create_task(
                self.handle_backtest_results_report(result_id, best_params, backtest_results, start_timestamp, end_timestamp)
            )
        )



    async def finish_optimisation(self):
        # finish all tasks
    ...
    async def send_backtest_results(self, title: str, start_timestamp: int, end_timestamp: int, msg: str | None = None, population: dict | None = None):
        params = {
            "title": title,
            "exchange_name": self.exchange_name,
            "symbol": self.symbol,
