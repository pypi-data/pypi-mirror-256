...
class BitgetAPI:
    ...
    async def fetch_futures_ohlcv(
        ...
        # prev_timestamp = start_timestamp - 60*60*1000
        all_ohlcv = []
        try:
            while since < end:
                # print(f"since: {since} end: {end}")
                ...
                since = cls.__get_end_time_for_historical_data_request(start_time=int(ohlcv[-1][0]), timeframe=interval, limit=1)  # Set 'since' to the timestamp of the last fetched candle + 1 millisecond
    ...
    @classmethod
    def __get_end_time_for_historical_data_request(cls, start_time: int, timeframe: str, limit:int = DATA_SIZE_LIMIT) -> int:
