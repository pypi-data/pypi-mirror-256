...
class Optimizer:
    ...
    async def execute(self):
        ...
        for i in range(self.number_of_starts):
            print(f"{self.strategy.name} {self.symbol} {self.interval} loop #{i+1}")
            try:
    ...
    async def process_results(self):
        ...
        result_id = f"{int(dt.datetime.now().timestamp())}_{random.randint(0, 100)}"
        ...
        if self.pass_backtest_results_filter(backtest_results):
            print(
                f"{self.symbol} {self.interval} {self.exchange_name} { self.strategy.name}\n Backtest results passed!\nProfit:{backtest_results.net_profit} Drawdown: {backtest_results.max_drawdown}"
            )
            if self.start_forwardtest_after_optimization:
                ...
                if self.pass_forwardtest_results_filter(forwardtest_results):
                    ...
                    print(
                        f"{self.symbol} {self.interval} {self.exchange_name} { self.strategy.name}\n Forwardtest results passed!\nProfit:{forwardtest_results.net_profit} Drawdown: {forwardtest_results.max_drawdown}"
                    )
                else:
                    print(
                        f"{self.symbol} {self.interval} {self.exchange_name} { self.strategy.name}\n Forwardtest results not passed!"
                    )
                    del self.population[self.best_score]
                    self.best_score = self.__get_best_score_of_population(
                        self.population
                    )
                    return 0
            ...
            self.reporter.backtest_results_report(
                ...
                best_params,
                backtest_results,
                self.backtest_options.get("start_timestamp"),
                self.backtest_options.get("end_timestamp"),
            )
    ...
    async def get_backtest_results(self, strategy_params: dict, initial_capital: float, start_timestamp: int, end_timestamp: int):
    ...
    def pass_backtest_results_filter(self, backtest_results: BacktestResults):
        return (
                backtest_results.net_profit >= self.backtest_options["filters"].get("min_profit") 
                and (backtest_results.max_drawdown <= self.backtest_options["filters"].get("max_drawdown"))
            )
    
    
    def pass_forwardtest_results_filter(self, forwardtest_results: BacktestResults):
        return (
                forwardtest_results.net_profit >= self.forwardtest_options["filters"].get("min_profit") 
                and (forwardtest_results.max_drawdown <= self.forwardtest_options["filters"].get("max_drawdown"))
            )
    ...
    async def create_and_save_html_report(
        ...
        start_date = dt.datetime.utcfromtimestamp(
            round(0.001 * start_timestamp)
        ).strftime("%Y-%m-%d")
        end_date = dt.datetime.utcfromtimestamp(round(0.001 * end_timestamp)).strftime(
            "%Y-%m-%d"
        )
