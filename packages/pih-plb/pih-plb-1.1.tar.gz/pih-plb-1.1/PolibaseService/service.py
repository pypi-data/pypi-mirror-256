    ...
    if A.U.for_service(SD, MODULES):
        ...
        def service_call_handler(sc: SC, pl: ParameterList, context) -> Any:
            ...
            if sc == SC.set_polibase_person_card_folder_name:
                return Api.set_card_folder_name_for_person(pl.next(), pl.next(), pl.next())
            ...
            if sc == SC.get_polibase_persons_pin_by_visit_date:
                return A.R.pack(
                    A.CT_FCA.VALUE_LIST,
                    Api.get_person_pin_list_by_visit_date(pl.next(), pl.next(), pl.next()),
                )
            if sc == SC.search_polibase_person_visits:
                ...
                return A.R.pack(
                    ...
                    person_visit_list_mapper(Api.search_person_visits(value, test), test),
                )
            ...
            if ne(person_visit_list):
                person_cache: dict[int, dict] = {
                    ...
                    for person in Api.get_person_list_by_pin(
                        list(set(list(map(lambda item: item.pin, person_visit_list)))), test
                    )
                }
                for person_visit_item in person_visit_list:
                    pin: int = person_visit_item.pin
                    cabinet_id: int = person_visit_item.cabinetID
                    ...
                    if pin not in [
                        ...
                        if (
                            ...
                            if not (n(full_name) or n(telephone_number)):
                                ...
                                telephone_number = A.D_F.telephone_number(telephone_number)
                                ...
                                complete_date2: datetime = person_visit_item.completeDate2
                                if (
                                ...
                                if (
                                    ...
                                    and complete_date2.year != A.CT_P.DATE_IS_NOT_SET_YEAR
                                ):
        ...
        def on_person_saldo_update_handler(
            ...
            id: int = person_saldo_data[A.CT_FNC.ID]
            doctor_id: int = person_saldo_data[A.CT_FNC.DOCTOR_ID]
            bonus_minus: int = person_saldo_data["bonus_minus"] or 0
            bonus_plus: int = person_saldo_data["bonus_plus"] or 0
            doctor_bonus_pin_list: list[int] = A.S.get(A.CT_S.BONUS_DOCTOR_PERSON_PIN_LIST)
            if bonus_plus > 0:
        ...
        def service_starts_handler() -> None:
            ...
            Api.on_person_creation_or_update_handler = on_person_creation_or_update_handler
            Api.on_person_saldo_update_handler = on_person_saldo_update_handler

        A.SRV_A.serve(SD, service_call_handler, service_starts_handler, isolate=ISOLATE, as_standalone=as_standalone)
