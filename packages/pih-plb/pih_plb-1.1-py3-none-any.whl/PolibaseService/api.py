...
from pih.tools import j, js, esc, e, escs, n, ne, nl, BitMask as BM, while_not_do, one
...
def get_dsn(test: bool) -> str:
    return j(
        (
            (A.CT_H.POLIBASE_TEST if test else A.CT_H.POLIBASE1).NAME,
...
class PolibaseApi:
    ...
    def cqn_callback_for_note_emailed_update(message):
        for query in message.queries:
            for table in query.tables:
                for row in table.rows:
                    note_emailed_data: dict[
                        str, int
                    ] | None = PolibaseApi.get_note_emailed_by_rowid(row.rowid)
    ...
    def get_connection(
        ...
        if poolled:
            try:
                ...
                A.ER.global_except_hook(exception)
                if catch_exception:
                    raise Error(exception.args[0].message, None)
        else:
    ...
    def get_person_by_pin(value: int, test: bool | None = None) -> dict[str, Any]:
        return one(PolibaseApi.get_person_list_by_pin([value], test))

    @staticmethod
    def get_person_by_email(value: str, test: bool | None = None) -> dict[str, Any]:
        return one(PolibaseApi.get_person_by_email(value, test))

    @staticmethod
    def get_person_by_rowid(value: str, test: bool | None = None) -> dict[str, Any]:
        return one(PolibaseApi.get_person_list_by_rowid([value], test))

    @staticmethod
    def get_note_emailed_by_rowid(
        value: str, test: bool | None = None
    ) -> dict[str, Any]:
        return one(PolibaseApi.get_note_emailed_list_by_rowid([value], test))

    @staticmethod
    def get_person_saldo_by_rowid(
        value: str, test: bool | None = None
    ) -> dict[str, Any]:
        return one(PolibaseApi.get_person_saldo_list_by_rowid([value], test))
