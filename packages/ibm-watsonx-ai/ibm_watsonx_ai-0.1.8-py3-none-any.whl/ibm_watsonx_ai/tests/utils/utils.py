...
from ibm_watsonx_ai.tests.utils.cleanup import bucket_cleanup
...
def get_space_id(wml_client, space_name, cos_resource_instance_id=None):
    ...
    """
    spaces_details = []
    for entry in wml_client.spaces.get_details(limit=200, asynchronous=True, get_all=True):
        spaces_details.extend(entry.get('resources'))
...
def resources_discovery(wml_client):
    ...
    def check(parent_el, parent_name):
        for r in parent_el.__dict__:
            try:
                if parent_el.__dict__[r].__class__.__module__.startswith('ibm_watsonx_ai.'):
...
def set_wml_client_default_space_wrapper(f):
    def wrapper(*args):
        if args[0].wml_client.default_space_id is None:
            args[0].wml_client.set.default_space(args[0].space_id)
        return f(*args)
