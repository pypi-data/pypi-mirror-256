...
class AbstractDeploymentTest(AbstractClientTest, abc.ABC):
    ...
    a deployment scenario using watsonx-ai-python-sdk
    """
    ...
    def test_03a_list_repository(self):

        repository_list_df = self.wml_client.repository.list()

        self.assertGreater(len(repository_list_df), 0)
        self.assertIn(self.model_id, repository_list_df['GUID'].values)

    def test_03b_list_repository_return_as_df_False(self):
        with io.StringIO() as buf, contextlib.redirect_stdout(buf):
            self.wml_client.repository.list()
            self.wml_client.repository.list(return_as_df=False)
