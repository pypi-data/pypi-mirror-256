...
from ibm_watsonx_ai.tests.utils.assertions import validate_autoai_experiment
from ibm_watsonx_ai.tests.utils.cleanup import space_cleanup
from ibm_watsonx_ai.utils.autoai.enums import PredictionType, Metrics, ImputationStrategy
from ibm_watsonx_ai.utils.autoai.errors import StrategyIsNotApplicable, InvalidImputationParameterNonTS, \
    ...
    def test_31_initialize_optimizer_kb_scenario_string_params_all(self):
        for imputation_strategy in ImputationStrategy:
            ...
            try:
                ...
                params = \
                self.remote_auto_pipelines._engine._wml_stored_pipeline_details['entity']['document']['pipelines'][0] \
                    ['nodes'][0]['parameters']['optimization']
    ...
    def test_32_initialize_optimizer_ts_scenario_string_params_all(self):
        for imputation_strategy in ImputationStrategy:
            ...
            try:
                ...
                ts_experiment_info = self.ts_experiment_info.copy()
                ts_experiment_info['numerical_imputation_strategy'] = (params['imputer_list'] if params[
                    'use_imputation'] else []) + ([params[
                                                       'imputer_fill_type']] if 'imputer_fill_type' in params else [])
