...
from ibm_watsonx_ai.tests.utils import save_data_to_cos_bucket
...
class AbstractTestBatch(abc.ABC):
    ...
    def test_99a_delete_created_assets_with_batch_name(self):
        file_name = AbstractTestBatch.batch_output_filename
        print("\n==========FILE NAME==========")
        print(file_name)

        all_assets = self.wml_client.data_assets.get_details()
        assets_sorted_by_file_name = [asset for asset in all_assets["resources"] if
                                      "batch_output" in asset["metadata"]['name']]

        print("\n==========ACTIONS==========")
        for asset in assets_sorted_by_file_name:
            assets_id = self.wml_client.data_assets.get_id(asset)
            self.wml_client.data_assets.delete(assets_id)
            print(f'Asset: {assets_id} has been deleted!')
