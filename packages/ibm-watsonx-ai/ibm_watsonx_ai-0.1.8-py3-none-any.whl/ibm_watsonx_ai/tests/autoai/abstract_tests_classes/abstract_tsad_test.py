...
from ibm_watsonx_ai.helpers.connections import DataConnection, DeploymentOutputAssetLocation, S3Location
...
from ibm_watsonx_ai.tests.utils.cleanup import space_cleanup, delete_model_deployment
from ibm_watsonx_ai.utils.autoai.enums import PredictionType, RunStateTypes

from ibm_watsonx_ai.tests.utils.assertions import get_and_predict_all_pipelines_as_lale, \
    validate_autoai_tsad_experiment

from ibm_watsonx_ai.utils.autoai.utils import chose_model_output
from ibm_watsonx_ai.utils.autoai.errors import NoAvailableNotebookLocation


class AbstractTestTSADAsync(abc.ABC):
    """
    The abstract tests which covers:
    - training AutoAI TSAD model on a dataset
    ...
    OPTIMIZER_NAME = "AutoAI tsad test"
    ...
    batch_output_filename = "TSAD_Batch_output.csv"
    ...
    def test_00a_space_cleanup(self):
        ...
        AbstractTestTSADAsync.space_id = space_id
    ...
    def test_01_initialize_autoai_experiment__pass_credentials__object_initialized(self):
        if self.SPACE_ONLY:
            AbstractTestTSADAsync.experiment = AutoAI(wml_credentials=self.wml_credentials.copy(),
    ...
    def test_03_initialize_optimizer(self):
        AbstractTestTSADAsync.experiment_info = validate_autoai_tsad_experiment(self.experiment_info,
                                                                                self.pod_version)

        AbstractTestTSADAsync.remote_auto_pipelines = self.experiment.optimizer(**AbstractTestTSADAsync.experiment_info)
    ...
    def test_05_fit_run_training_of_auto_ai_in_wml(self):
        AbstractTestTSADAsync.trained_pipeline_details = self.remote_auto_pipelines.fit(
            training_data_reference=[self.data_connection],
            training_results_reference=self.results_connection,
            background_mode=False)

        AbstractTestTSADAsync.run_id = self.trained_pipeline_details['metadata']['id']
    ...
    def test_06_get_train_data(self):
        # AbstractTestTSADAsync.train_data = self.remote_auto_pipelines.get_data_connections()[0].read()
        #
        # print("train data sample:")
        # print(self.train_data.head())
        # self.assertGreater(len(self.train_data), 0)

        AbstractTestTSADAsync.train_X, AbstractTestTSADAsync.test_X, AbstractTestTSADAsync.train_y, AbstractTestTSADAsync.test_y = \
            self.remote_auto_pipelines.get_data_connections()[0].read(with_holdout_split=True)

        AbstractTestTSADAsync.scoring_df = self.train_X[:10]
    ...
    def test_08c_get_pipeline_notebook(self):
        if not self.NOTEBOOK_CHECK:
            self.skipTest("Skip notebook")
    ...
    def test_10_summary_listing_all_pipelines_from_wml(self):
        ...
        AbstractTestTSADAsync.winning_pipelines_summary = pipelines_summary[pipelines_summary['Winner']]
        AbstractTestTSADAsync.discarded_pipelines_summary = pipelines_summary[pipelines_summary['Winner'] == False]

        self.assertGreater(len(AbstractTestTSADAsync.winning_pipelines_summary), 0,
        ...
        if 'max_number_of_estimators' in self.experiment_info and self.experiment_info:
            ...
            self.assertEqual(len(AbstractTestTSADAsync.winning_pipelines_summary), nb_gen_pipelines,
    ...
    def test_11__get_data_connections__return_a_list_with_data_connections_with_optimizer_params(self):
        ...
        # Need this import during rebranding process
        from ibm_watson_machine_learning.helpers.connections import DataConnection as WMLDataConnection
        self.assertIsInstance(data_connections[0], WMLDataConnection,
    ...
    def test_13_get_pipeline__load_lale_pipeline__pipeline_loaded(self):
        if not self.LALE_CHECK:
            self.skipTest("Skip lale")

        AbstractTestTSADAsync.lale_pipeline = self.remote_auto_pipelines.get_pipeline()
    ...
    def test_15_list_historical_runs_and_get_run_ids(self):
        ...
        if len(runs_completed_df) > 1:
            AbstractTestTSADAsync.prev_run_id = runs_completed_df.run_id.iloc[1]  # prev run_id
            print("Random historical run_id: {}".format(AbstractTestTSADAsync.prev_run_id))
            self.assertIsNotNone(AbstractTestTSADAsync.prev_run_id)
    ...
    def test_16_get_params_of_last_historical_run(self):
        ...
        AbstractTestTSADAsync.historical_opt = self.experiment.runs.get_optimizer(self.run_id)
    ...
    def test_31_deployment_online_setup_and_preparation(self):
        if not self.ONLINE_DEPLOYMENT:
            self.skipTest("Skip online deployment")

        # note: if target_space_id is not set, use the space_id
        if self.target_space_id is None:
            AbstractTestTSADAsync.target_space_id = self.space_id
        else:
            AbstractTestTSADAsync.target_space_id = self.target_space_id
        # end note

        self.wml_client.set.default_space(AbstractTestTSADAsync.target_space_id)
        delete_model_deployment(self.wml_client, deployment_name=self.DEPLOYMENT_NAME)

        if self.SPACE_ONLY:
            AbstractTestTSADAsync.service = WebService(source_wml_credentials=self.wml_credentials,
                                                     source_space_id=self.space_id,
                                                     target_wml_credentials=self.wml_credentials,
                                                     target_space_id=AbstractTestTSADAsync.target_space_id)
        else:
            AbstractTestTSADAsync.service = WebService(source_wml_credentials=self.wml_credentials,
        ...
        self.assertIsInstance(AbstractTestTSADAsync.service, WebService, msg="Deployment is not of WebService type.")
        self.assertIsInstance(AbstractTestTSADAsync.service._source_workspace, WorkSpace,
                              msg="Workspace set incorrectly.")
        self.assertEqual(AbstractTestTSADAsync.service.id, None, msg="Deployment ID initialized incorrectly")
        self.assertEqual(AbstractTestTSADAsync.service.name, None, msg="Deployment name initialized incorrectly")
    ...
    def test_32__deploy__deploy_best_computed_pipeline_from_autoai_on_wml(self):
        ...
        AbstractTestTSADAsync.service.create(
            experiment_run_id=self.remote_auto_pipelines._engine._current_run_id,
            model=pipeline_name,
            deployment_name=self.DEPLOYMENT_NAME)

        self.assertIsNotNone(AbstractTestTSADAsync.service.id, msg="Online Deployment creation - missing id")
        self.assertIsNotNone(AbstractTestTSADAsync.service.name, msg="Online Deployment creation - name not set")
        self.assertIsNotNone(AbstractTestTSADAsync.service.scoring_url,
    ...
    def test_33_score_deployed_model_with_empty_payload(self):
        ...
        predictions = AbstractTestTSADAsync.service.score()
        print(predictions)
    ...
    def test_34_score_deployed_model_with_df(self):
        ...
        predictions = AbstractTestTSADAsync.service.score(payload=self.scoring_df)
        print(predictions)
        self.assertIsNotNone(predictions)
    ...
    def test_35_list_deployments(self):
        ...
        AbstractTestTSADAsync.service.list()
        params = AbstractTestTSADAsync.service.get_params()
        print(params)
        self.assertIsNotNone(params)

        self.assertIn(AbstractTestTSADAsync.service.deployment_id, str(params))

    def test_36_delete_deployment(self):
        if not self.ONLINE_DEPLOYMENT:
            self.skipTest("Skip online deployment")

        print("Delete current deployment: {}".format(AbstractTestTSADAsync.service.deployment_id))
        AbstractTestTSADAsync.service.delete()
        self.wml_client.repository.delete(AbstractTestTSADAsync.service.asset_id)
        self.assertEqual(AbstractTestTSADAsync.service.id, None, msg="Deployment ID deleted incorrectly")
        self.assertEqual(AbstractTestTSADAsync.service.name, None, msg="Deployment name deleted incorrectly")
        self.assertEqual(AbstractTestTSADAsync.service.scoring_url, None,
    ...
    def test_41_batch_deployment_setup_and_preparation(self):
        ...
        self.assertIsNotNone(AbstractTestTSADAsync.target_space_id, "Test issue: target space not set.")

        if self.SPACE_ONLY:
            AbstractTestTSADAsync.service_batch = Batch(source_wml_credentials=self.wml_credentials,
                                                      source_space_id=self.space_id,
                                                      target_wml_credentials=self.wml_credentials,
                                                      target_space_id=AbstractTestTSADAsync.target_space_id)
        else:
        ...
        self.assertIsInstance(AbstractTestTSADAsync.service_batch, Batch, msg="Deployment is not of Batch type.")
        self.assertIsInstance(AbstractTestTSADAsync.service_batch._source_workspace, WorkSpace,
                              msg="Workspace set incorrectly.")
        self.assertEqual(AbstractTestTSADAsync.service_batch.id, None, msg="Deployment ID initialized incorrectly")
        self.assertEqual(AbstractTestTSADAsync.service_batch.name, None, msg="Deployment name initialized incorrectly")
    ...
    def test_42_deploy__batch_deploy_best_computed_pipeline_from_autoai_on_wml(self):
        ...
        pipeline_name = self.winning_pipelines_summary.reset_index()['Pipeline Name'][0]
        print('Deploying', pipeline_name)
        AbstractTestTSADAsync.service_batch.create(
            experiment_run_id=self.remote_auto_pipelines._engine._current_run_id,
            model=pipeline_name,
            deployment_name=self.DEPLOYMENT_NAME + ' BATCH')

        self.assertIsNotNone(AbstractTestTSADAsync.service_batch.id, msg="Batch Deployment creation - missing id")
        self.assertIsNotNone(AbstractTestTSADAsync.service_batch.id, msg="Batch Deployment creation - name not set")
        self.assertIsNotNone(AbstractTestTSADAsync.service_batch.asset_id,
    ...
    def test_43_list_batch_deployments(self):
        ...
        deployments = AbstractTestTSADAsync.service_batch.list()
        print(deployments)
        self.assertIn('created_at', str(deployments).lower())
        self.assertIn('status', str(deployments).lower())

        params = AbstractTestTSADAsync.service_batch.get_params()
        print(params)
        self.assertIsNotNone(params)
    ...
    def test_44_run_job__batch_deployed_model_with_data_frame(self):
        ...
        if self.BATCH_DEPLOYMENT_WITH_DF:
            scoring_params = AbstractTestTSADAsync.service_batch.run_job(
                payload=self.scoring_df,
                background_mode=False)
            print(scoring_params)
            print(AbstractTestTSADAsync.service_batch.get_job_result(scoring_params['metadata']['id']))
    ...
    def test_45_run_job_batch_deployed_model_with_data_assets(self):
        ...
        if not self.BATCH_DEPLOYMENT_WITH_DA:
            ...
            AbstractTestTSADAsync.data_assets_to_delete.append(asset_id)
        ...
        predictions = AbstractTestTSADAsync.service_batch.get_job_result(deployment_job_id)
        print(predictions)
    ...
    def test_46_run_job_batch_deployed_model_with_data_assets_with_cos_connection(self):
        ...
        AbstractTestTSADAsync.bucket_name = save_data_to_cos_bucket(self.batch_payload_location,
        ...
        AbstractTestTSADAsync.connection_id = self.wml_client.connections.get_uid(connection_details)

        self.assertIsNotNone(self.batch_payload_location,
                             "Test configuration failure: Batch payload location is missing")

        asset_details = self.wml_client.data_assets.store({
            self.wml_client.data_assets.ConfigurationMetaNames.CONNECTION_ID: AbstractTestTSADAsync.connection_id,
    ...
    def test_47_run_job_batch_deployed_model_with_connected_data_asset(self):
        ...
        # prepare connection
        if not AbstractTestTSADAsync.connection_id:
            connection_details = self.wml_client.connections.create({
                'datasource_type': self.wml_client.connections.get_datasource_type_uid_by_name(
                    'bluemixcloudobjectstorage'),
                'name': 'Connection to COS for tests',
        ...
        conn_space = DataConnection(
            connection_asset_id=AbstractTestTSADAsync.connection_id,
            location=S3Location(
                bucket=self.bucket_name,
    ...
    def test_49_delete_deployment_batch(self):
        ...
        print("Delete current deployment: {}".format(AbstractTestTSADAsync.service_batch.deployment_id))
        AbstractTestTSADAsync.service_batch.delete()
        self.wml_client.set.default_space(self.target_space_id) if not self.wml_client.default_space_id else None
        self.wml_client.repository.delete(AbstractTestTSADAsync.service_batch.asset_id)

        print(f"Delete all created assets in Batch deployment tests. {AbstractTestTSADAsync.data_assets_to_delete}")
        for asset_id in AbstractTestTSADAsync.data_assets_to_delete:
            self.wml_client.data_assets.delete(asset_id)

        self.assertEqual(AbstractTestTSADAsync.service_batch.id, None, msg="Deployment ID deleted incorrectly")
        self.assertEqual(AbstractTestTSADAsync.service_batch.name, None, msg="Deployment name deleted incorrectly")
        self.assertEqual(AbstractTestTSADAsync.service_batch.scoring_url, None,
                         msg="Deployment scoring_url deleted incorrectly")
