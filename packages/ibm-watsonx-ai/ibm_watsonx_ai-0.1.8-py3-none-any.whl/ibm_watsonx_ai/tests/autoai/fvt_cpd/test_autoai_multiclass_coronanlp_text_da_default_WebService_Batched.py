...
from ibm_watsonx_ai.helpers.connections import S3Connection, S3Location, DataConnection, FSLocation, AssetLocation

from ibm_watson_machine_learning.experiment.autoai.optimizers import RemoteAutoPipelines

from ibm_watsonx_ai.utils.autoai.errors import NotExistingCOSResource
...
from ibm_watsonx_ai.utils.autoai.enums import PositiveLabelClass, TShirtSize, Transformers
...
class TestAutoAIRemote(unittest.TestCase):
    ...
    data_cos_path = 'data/auto_ai_exp_cfpb.csv'
    ...
    OPTIMIZER_NAME = f"TEXT TRANSFORMER {with_text_transformer} multiclass {data_location.split('/')[-1]} "
    ...
    def test_05_fit_run_training_of_auto_ai_in_wml(self):
        ...
        # TestAutoAIRemote.run_id = '939e1252-6a2e-4b88-aa58-3014670b7365' # hist run_id
    ...
    def test_10_summary_listing_all_pipelines_from_wml(self):
        ...
        for column in pipelines_details.columns:
            print(pipelines_details[column])
    ...
    def test_33_score_batch_deployed_model(self):
        scoring_params = self.service_batch.run_job(
            payload=self.train_data.drop(columns=[self.prediction_column]).dropna()[:3],
            background_mode=False)
