...
from ibm_watsonx_ai.helpers.connections import S3Connection, S3Location, DataConnection, FSLocation, \
    AssetLocation

from ibm_watson_machine_learning.experiment.autoai.optimizers import RemoteAutoPipelines

from ibm_watsonx_ai.tests.utils import (get_wml_credentials, get_cos_credentials, bucket_exists,
                                                     create_bucket, get_space_id, is_cp4d)
from ibm_watsonx_ai.tests.utils.cleanup import space_cleanup, delete_model_deployment

from ibm_watsonx_ai.utils.autoai.enums import TShirtSize, PipelineTypes, SamplingTypes
from ibm_watsonx_ai.utils.autoai.errors import CannotReadSavedRemoteDataBeforeFit
...
class TestAutoAIRemote(unittest.TestCase):
    """
    The test can be run on CLOUD, WMLS and CPD
    The test covers:
    - training on space (Cloud) or project (CPD)
    ...
    data_cos_path = 'data/fake_job_position.csv'
    ...
    y_values = None
    elapsed_time = None
    data_location = './autoai/data/fake_job_postings_2MB.csv'

    prediction_column = "fraudulent"

    text_columns = ['description', 'company_profile']
    word2vec_feature_number = 3
    ...
    def test_02a_read_saved_remote_data_before_fit(self):
        with self.assertRaises(ConnectionError, msg="Should fail with adequate error - reading without wml_client"):
            _ = self.data_connection.read()

        self.data_connection.set_client(wml_client=self.wml_client)
    ...
    def test_03_initialize_optimizer(self):
        TestAutoAIRemote.remote_auto_pipelines = self.experiment.optimizer(
            ...
            text_processing=self.with_text_transformer,
            ...
            text_columns_names=self.text_columns
        )
    ...
    def test_05_fit_run_training_of_auto_ai_in_wml(self):
        ...
        # TestAutoAIRemote.run_id = '47111a26-b2bf-4ee2-bcf5-81137dc99b27'
    ...
    def test_06_get_train_data(self):
        ...
        # TestAutoAIRemote.train_data = pd.read_csv(self.data_location)
    ...
    def test_08a_get_feature_importance(self):
        feature_importance_df = self.remote_auto_pipelines.get_pipeline_details(pipeline_name='Pipeline_1').get(
            'features_importance')
        ...
        for column in self.text_columns:
            self.assertIn(f'word2vec({column})', str(str_feature_importance_index_list),
                          msg=f"word2vec({column}) is not in features importance table. Full feature list: {str_feature_importance_index_list}")

        self.assertIn('NewTextFeature_0', str_feature_importance_index_list,
                      msg="Text features were numerated incorrectly.")
        self.assertIn(f'NewTextFeature_{self.word2vec_feature_number * len(self.text_columns) - 1}',
    ...
    def test_33_score_batch_deployed_model(self):
        scoring_params = self.service_batch.run_job(
            payload=self.train_data.drop(columns=[self.prediction_column])[:100],
            background_mode=False)
