...
class TestAutoAIRemote(AbstractTestTSAsync, unittest.TestCase):
    ...
    def test_09ab_predict_using_fitted_pipeline(self):
        pipeline = self.remote_auto_pipelines.get_pipeline(persist="True")
    ...
    def test_09b_get_pipeline_optimizer_from_experiment_metdata(self):
        ...
        print("Delete current deployment: {}".format(service.deployment_id))
        service.delete()
        self.wml_client.repository.delete(service.asset_id)
        self.assertEqual(service.id, None, msg="Deployment ID deleted incorrectly")

    @unittest.skip("Not working outside WS")
    def test_09c_get_pipeline_optimizer_from_experiment_metdata(self):
        from ibm_watsonx_ai.helpers import ContainerLocation, FSLocation
