...
class PromptTuner:
    ...
    def _get_first_and_last_iteration_metrics_for_each_epoch(tuning_details):
        ...
        for ind in range(len(tuning_details['entity']['status']['metrics'])):
            if ind == 0:
                first_and_last_iteration_metrics_for_each_epoch.append(tuning_details['entity']['status']['metrics'][ind])
                first_and_last_iteration_metrics_for_each_epoch.append(tuning_details['entity']['status']['metrics'][ind])
    ...
    def summary(self, scoring: str = 'loss') -> 'DataFrame':
        ...
        **Example**
        ...
        columns = ['Model Name', 'Enhancements', 'Base model', 'Auto store', 'Epochs']
        columns = (columns + [metric_name for metric_name in get_metrics_names() if metric_name != 'epoch'])
        values = []
