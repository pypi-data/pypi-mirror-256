...
class ContactNotesLocal(GenericMapping):
    ...
    def insert_contact_note_text_block_table(self, contact_note_id: int) -> int:
        ...
        for i, text_block in enumerate(text_blocks_list):
            if i == 0:
                ...
                if not text_block.get('is_action_item'):
            ...
            if i > 0 and not text_block.get('is_action_item', False):
    ...
    def _process_text_block(self, text_block: str) -> dict:
        ...
        if '-----' in lines[0]:
            ...
            for line in lines[1:]:
                self._process_insert_action_item(line)
            text_block_dict = {
                'is_action_item': True,
                'text': text_block,
            }
            ...
            for date_format in date_formats:
                try:
                    formatted_date = self._convert_date_format(date_str=milestone_date, date_format=date_format)
    ...
    def _process_insert_action_item(self, action_item: str) -> None:
        data_action_item = {
            'title': action_item,
