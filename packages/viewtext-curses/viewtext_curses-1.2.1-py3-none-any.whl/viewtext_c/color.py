...
import tomllib
import tomli_w
from viewtext_c.color_list import color_list
from viewtext_c.util_io    import file_available_for_read, file_available_for_write

# logging.basicConfig(filename='.viewtext.log', encoding='utf-8', level=logging.DEBUG)
...
# RC_CONFIG_FILE   = '.viewtextrc.toml'
...
class Palette():
	...
	elt_ID      = dict()
	colors      = None
	celts       = None  # completion elements
	...
	def load_themes(cls) -> None:
		...
		path = file_available_for_read(CONFIG_FILES, DEFAULT_CONFIG_FILE)
		if not path:
	...
	def styling_color(cls, elt: str, ground: str) -> str:
		...
		match ground:
			...
			case 'background':
				offset = 1
			case _:
				raise Exception(f': {os.basename(ground)} must be either foreground or background')
