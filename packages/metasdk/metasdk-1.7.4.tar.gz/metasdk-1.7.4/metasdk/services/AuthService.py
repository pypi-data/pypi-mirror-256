...
class AuthService:
    ...
    def verify_access_token(self, token: str, required_scopes: List = None) -> Dict:
        """
        ...
        return self.__set_permissions(auth_info=auth_info)

    def _authenticate_user(self, token) -> Dict:
        """
    ...
    def _check_scopes(self, token_scopes: List, required_scopes: List):
        if not required_scopes or type(required_scopes) != list:
            raise TypeError("Invalid format of the required_scopes")

        if not token_scopes or type(token_scopes) != list:
    ...
    def __set_permissions(self, auth_info):
        """
