...
class FeedService:
    def __init__(self, app):
        ...
        self.__auth_service = app.AuthService
        self.__required_scopes = ["datahub"]
    ...
    def list_feeds(self, filters: Dict, token: str) -> List[Dict]:
        """
        ...
        user_info = self.__auth_service.verify_access_token(token, self.__required_scopes)
        if user_info["is_admin"]:
            query_params = {"company_id": user_info["company_id"], **filters}
        else:
            query_params = {"author_user_id": user_info["auth_user_id"], **filters}

        query = self._generate_list_feeds_query(filters=filters, is_admin=user_info["is_admin"])
    ...
    def get_feed_by_id(self, feed_id: str, token: str, last_tasks: bool = False) -> Dict:
        """
        ...
        if user_info["is_admin"]:
            query_params.update({"company_id": user_info["company_id"]})
        else:
            query_params.update({"author_user_id": user_info["auth_user_id"]})

        query = self._generate_feed_by_id_query(last_tasks=last_tasks, is_admin=user_info["is_admin"])
    ...
    def get_tasks(self, feed_id: str, token: str) -> List[Dict]:
        """
        ...
        query = self._generate_tasks_query(is_admin=user_info["is_admin"])
