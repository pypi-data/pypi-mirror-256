...
class ModelElement:
    def __init__(
        ...
        bounds: Union[None, list[float]] = None,
        ...
        Args:
            ...
            bounds (Union[None, list[float]], optional): Lower and upper bounds. Can be overwritten in add_to_model().
                Defaults to None.
    ...
    def add_to_model(
        ...
        Args:
            ...
            bounds (Union[None, list[float]], optional): Lower and upper bounds.
...
class ModelHistory:
    ...
    def filter_for_entities(
        ...
        new_history = ModelHistory(entities)
    ...
    def filter_for_entity_types(self, entity_types: Union[Type[ModelEntity], List[Type[ModelEntity]]]) -> ModelHistory:
        ...
        new_history = ModelHistory(filtered_entities)
        new_history.history = filtered_history
    ...
    def filter_for_element_names(self, names: Union[str, List[str]]) -> ModelHistory:
        ...
        new_history = ModelHistory(self.model_entities)
        new_history.history = filtered_history
...
class ModelEntity:
    ...
    def get_pyomo_element(self, name: str, model: ConcreteModel) -> Union[Var, Param, Set, Constraint, Objective]:
        ...
        "global" (e.g. "n1.n12.p", i.e. from the perspective of a higher block).
        The given model can also be local (of the calling block) or "global" (of a block higher in the hierarchy).
        For a global name we will first assume the corresponding global model
        and then iteratively try to find the variable bottom-up.
        For a local name we will prepend the entity id and then look for the variable in
        the given model bottom-up iteratively.

        This is so complicated mainly because we want to provide a "local" interface to model elements
        a certain entity within a hierarchical, nested pyomo model.
        We know the exact id of elements in the global model, but if we need to access an element from
        a scope lower in the hierarchy (e.g. a parent node) we need to do some work to find the right element.
        Additionally, pyomo has made the (possibly debatable) decision to pass an element's local
        during construction.
        This means the model passed to initialization functions or to expressions is local and
        elements have to be accessed as "local".
        ...
        # fist try to find the correct element in the model tree:
        model_levels = len(self.id.split("."))
        name_levels = len(name.split("."))

        if name_levels > 1:  # name not local e.g. "n0.n01.p" -> "n01.p" -> "p"
            for level in range(name_levels + 1):
                name_for_level = ".".join(name.split(".")[level:])
                if rhasattr(model, name_for_level):
                    return rgetattr(model, name_for_level)

        else:  # local name e.g. "p": Approach: "n0.n01.p" -> "n01.p" -> "p"
            # sanity check: does self even have a variable "name"?
            # Why? Because we have a problem if a variable does not exist on a low level but on a higher level
            # (between calling block and intended block).
            # In this case we would iterate bottom-up and return the local element of an unintended block.
            # TODO: How can we improve this??
            local_name = name.split(".")[-1]
            if local_name not in [e.name for e in self.model_elements]:
                raise EntityError(self, f"The element {local_name} was not defined for this entity")

            for level in range(model_levels + 1):
                name_for_level = ".".join(self.id.split(".")[level:] + [name])
                if rhasattr(model, name_for_level):
                    return rgetattr(model, name_for_level)

        raise EntityError(self, f"The variable {name} could not be found in the given model")
