...
from chaiverse.metrics.baseline import baseline_dict
...
def _adjust_for_baseline(submission_id, submission_data):
    double_thumbs_up = submission_data.get('double_thumbs_up', 0)
    thumbs_up = submission_data.get('thumbs_up', 0)
    thumbs_down = submission_data.get('thumbs_down', 0)
    baseline = baseline_dict.get(submission_id)
    if baseline:
        double_thumbs_up -= baseline.get('double_thumbs_up', 0)
        thumbs_up -= baseline.get('thumbs_up', 0)
        thumbs_down -= baseline.get('thumbs_down', 0)
        total_feedback_count = double_thumbs_up + thumbs_up + thumbs_down
    return {
        **submission_data,
...
def get_leaderboard_row(submission_item, developer_key=None, evaluation_date_range=None, fetch_feedback=False):
    ...
    submission_data = _adjust_for_baseline(submission_id, submission_data)
    if fetch_feedback:
