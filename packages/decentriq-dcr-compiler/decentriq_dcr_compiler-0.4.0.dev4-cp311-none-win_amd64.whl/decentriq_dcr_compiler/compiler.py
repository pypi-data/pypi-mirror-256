...
def compile_media_insights_dcr(dcr: MediaInsightsDcr):
    return _ddc_py.compile_media_insights_dcr_serialized(dcr.json())
...
def get_media_insights_dcr_consumed_datasets(dcr: MediaInsightsDcr) -> RequirementList:
    req_list_serialized = _ddc_py.get_media_insights_dcr_consumed_datasets_serialized(
        dcr.json()
    )
    return RequirementList.parse_raw(req_list_serialized)


def is_data_lab_compatible_with_media_insights_dcr(
        data_lab: DataLab,
        dcr: MediaInsightsDcr,
) -> bool:
    return _ddc_py.is_data_lab_compatible_with_media_insights_dcr_serialized(
        data_lab.json(), dcr.json()
    )


def get_media_insights_dcr_features(dcr: MediaInsightsDcr):
    return _ddc_py.get_media_insights_dcr_features_serialized(lm_dcr.json())


def compile_media_insights_request(
        request: MediaInsightsRequest,
        user_auth_serialized: bytes
) -> bytes:
