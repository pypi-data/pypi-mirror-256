...
from maplestory.types.character import HyperStatLevel, HyperStatType
...
class HyperStatItem(BaseModel):
    ...
    Attributes:
        type (HyperStatType): 스탯 종류
        point (int | None): 스탯 투자 포인트
        level (HyperStatLevel): 스탯 레벨
        increase (str | None): 스탯 상승량
    """

    type: HyperStatType = Field(alias="stat_type")
    point: int | None = Field(alias="stat_point")
    level: HyperStatLevel = Field(alias="stat_level")
