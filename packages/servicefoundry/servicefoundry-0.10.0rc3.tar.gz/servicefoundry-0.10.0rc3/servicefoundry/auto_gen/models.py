...
#   timestamp: 2024-02-01T08:49:13+00:00
...
class Resources(BaseModel):
    ...
    devices: Optional[List[Union[NvidiaGPU, AWSInferentia]]] = Field(
        None,
...
class AsyncServiceAutoscaling(BaseAutoscaling):
    metrics: Union[
        SQSQueueMetricConfig, NATSMetricConfig, KafkaMetricConfig, CronMetric
    ] = Field(
        ...,
        description="+label=Autoscaling metrics\n+usage=Metrics to use for the autoscaler\n+sort=4",
    )
...
class BaseWorkbenchInput(BaseModel):
    ...
    env: Optional[Dict[str, str]] = Field(
        None,
        description="+label=Environment Variables\n+usage=Configure environment variables to be set in the workbench pod(s)\n+sort=10110",
    )
    mounts: Optional[List[Union[SecretMount, StringDataMount, VolumeMount]]] = Field(
        None,
        description="+usage=Configure data to be mounted to workbench pod(s)\n+sort=10111",
    )
...
class Notebook(BaseWorkbenchInput):
    ...
    image: Union[
        TruefoundryImageBase, TruefoundryImageFull, CustomNotebookImage
    ] = Field(
        ...,
...
class WorkerConfig(BaseModel):
    ...
    output_config: Optional[
        Union[
            SQSOutputConfig, NATSOutputConfig, CoreNATSOutputConfig, KafkaOutputConfig
        ]
...
class Application(BaseModel):
    __root__: Union[Service, AsyncService, Job, Notebook, Codeserver, Helm, Volume]
