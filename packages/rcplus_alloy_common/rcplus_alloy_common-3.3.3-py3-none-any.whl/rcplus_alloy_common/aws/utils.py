...
def get_last_p_timestamp_iceberg(table, database=None, partitioned=False):
    """Get the last p_timestamp from an iceberg table."""

    if partitioned:
        raise NotImplementedError("partitioned iceberg tables has to be implemented")
    ...
    sql = f"""
    SELECT MAX(data.p_timestamp.max) AS p FROM {table_name}
    """
...
def get_last_p_timestamp_hive(table, database=None):
    ...
    sql = f"""
    SELECT MAX(p_timestamp) as p FROM {table_name}
    """
    p_timestamp = _read_sql_p_timestamp(sql)
    return p_timestamp
