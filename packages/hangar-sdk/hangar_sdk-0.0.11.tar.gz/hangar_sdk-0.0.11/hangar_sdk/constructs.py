...
class Asset(CompositeResource, Deployable):
    ...
    def sync(self, token=None):
        ...
        if isinstance(self.source, DirPath):
            ...
            ignorefile = pathlib.Path(self.source.ignorefile) if self.source.ignorefile else None

            if ignorefile and ignorefile.exists():
                with tarfile.open(fname, "w") as tar:
                    tar.add(self.source.path, arcname=os.path.basename(self.source.path), filter=lambda x: x if x.name not in ignorefile.read_text().split("\n") else None)
            else:
                with tarfile.open(fname, "w") as tar:
