...
from .constants import MISSING_DEPENDENCIES_TEMPLATE
from .utils import UI  # noqa
...
class BaseLambdaDeploymentPackager(object):
    ...
    def _deployment_package_filename(
        ...
        deployment_package_filename = self._osutils.joinpath(
            project_dir, ".hangar", "deployments", filename
        )
    ...
    def _iter_app_filenames(self, project_dir: str) -> Iterator[Tuple[str, str]]:
        # add all files in project_dir except for dotfiles and dotfolders and what is mentioned in gitignore

        project_dir_path = Path(self._osutils.abspath(project_dir))

        gitignore_path = project_dir_path / ".gitignore"
        ...
        for rootdir, dirs, filenames in self._osutils.walk(
            self._osutils.abspath(project_dir)
        ):
            dirs[:] = [d for d in dirs if not d.startswith(".")]

            # dont include dotfiles and dotfolders
            for filename in filenames:
        ...
        # add current package
        package_path = Path(self._osutils.abspath(__file__)).parent.parent

        for rootdir, dirs, filenames in self._osutils.walk(package_path):
            dirs[:] = [d for d in dirs if not d.startswith(".")]
            ...
            for filename in filenames:
                ...
                if filename in gitignore:
                    continue
                fullpath = self._osutils.joinpath(rootdir, filename)
                zip_path = "hangar_sdk/" + fullpath[len(package_path.as_posix()) + 1 :]
                yield (fullpath, zip_path)
        ...
        # if self._osutils.directory_exists(
        #     self._osutils.joinpath(project_dir, "bin")
        # ):
        #     for rootdir, _, filenames in self._osutils.walk(
        #         self._osutils.joinpath(project_dir, "bin")
        #     ):
        #         for filename in filenames:
        #             yield (self._osutils.joinpath(rootdir, filename),"bin/" + filename)

        # current_path = Path(self._osutils.abspath(__file__))

        # yield str(current_path.parent.parent,"hangar_serverless")
        # yield str(current_path.parent, "hangar_serverless/hangar_serverless")
        # yield ((current_path.parent / "main.py").as_posix(),"hangar_serverless/main.py")
    ...
    def _iter_chalice_lib_if_needed(
        ...
        # add current file and all files in hangar_serverless
        current_path = Path(self._osutils.abspath(__file__))
        for rootdir, _, filenames in self._osutils.walk(current_path.parent.resolve()):
            for filename in filenames:
                yield (
                    self._osutils.joinpath(rootdir, filename),
                    self._osutils.joinpath("hangar_serverless", filename),
                )
...
class PipRunner(object):
    ...
    def download_all_dependencies(
        ...
        if rc != 0:
            ...
            error = err.decode()
            match = re.search(
                r"Could not find a version that satisfies the " r"requirement ([^\s]+)",
                error,
            )
            if match:
...
if __name__ == "__main__":
    ...
    dep_builder = DependencyBuilder(osutils=osutils)

    lp = LambdaDeploymentPackager(
        osutils=osutils, ui=ui, dependency_builder=dep_builder
    )
    lp.create_deployment_package(
        "/Users/mihirchintawar/entropy-monorepo/projects/lambda/package_test",
        "python3.11",
    )
