...
def generate_quiz_widget():
    # List of code statements for the quiz
    statements = [
        "numbers = [1, 2, 3, 4, 5]",
    ...
    # Create dropdown widgets for each line of code
    ...
    submit_button = widgets.Button(description="Check Answers")

    # Function to check the order of the selected options and provide feedback
    def check_answers(button):
        ...
        # Use clear_output to clear the previous output (feedback)
        clear_output(wait=True)
        
        # Check each dropdown selection against the correct order
        for dropdown, correct_answer in zip(dropdowns, correct_order):
            if dropdown.value == correct_answer:
                dropdown.layout.border = '2px solid green'
            else:
                dropdown.layout.border = '2px solid red'

        # Redisplay the dropdowns and the submit button
        display(*dropdowns, submit_button)

        # Provide a final message based on the correctness of all answers
        if all(dropdown.value == correct for dropdown, correct in zip(dropdowns, correct_order)):
            print("Correct! Well done.")
        else:
            print("Some lines are incorrect. The green ones are correct, and the red ones are not.")

    # Assign the check_answers function to execute when the submit button is clicked
    submit_button.on_click(check_answers)

    # Display the dropdowns and the submit button initially
    display(*dropdowns, submit_button)
