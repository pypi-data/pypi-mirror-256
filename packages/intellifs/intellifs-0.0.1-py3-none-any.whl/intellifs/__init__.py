...
class FileSystem:
    ...
    def __create_map(self, base: str):
        ...
        try:
            ...
            self._map_path.write_text(
                json.dumps(self.map | {base: collection_name}))
        except ValueError:
            pass
    ...
    def collection_of(self, __indexable: File | Directory) -> LocalCollection:
        ...
        base = __indexable.base
        collection_name = self.map.get(base)
        if collection_name is None:
            collection_name = self.__create_map(base)
        try:    
            collection = self._storage._get_collection(collection_name)
        except ValueError: # Handling edge cases
            collection = self._storage._get_collection(self.__create_map(base))
