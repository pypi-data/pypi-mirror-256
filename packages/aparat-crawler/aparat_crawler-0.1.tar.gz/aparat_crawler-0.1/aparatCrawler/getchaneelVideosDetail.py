...
def get_video_commnets(video_uid,proxy=None):
    ...
    if proxy != None:
        response = requests.request("GET", url, data=payload,proxies=proxies[0], timeout=timeout_seconds)
    else:
        response = requests.request("GET", url, timeout=10)
    data=response.json()
...
def getvideos(usrname,video_count=None,proxy=None):
    ...
    while True:
        ...
        for included in includeds:
            ...
            # print("********************")
            comment_data=get_video_commnets(included["attributes"]["uid"],proxy=None)

            video={
                "platform": "Aparat",
                "_":"video",
                "id":included["id"] if "id" in included else None,
                "owner_usermane":included["attributes"]["username"] if "username" in included["attributes"] else None,
