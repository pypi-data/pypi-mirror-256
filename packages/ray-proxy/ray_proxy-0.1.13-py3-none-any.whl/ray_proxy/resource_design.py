...
from pinjected.di.injected import MappedInjected, InjectedFunction, ZippedInjected, MZippedInjected, InjectedPure, \
    InjectedByName
...
class ResourceDesign:
    ...
    def to_scheduler(self,ray=None) -> "ResourceSchedulerClient":
        from ray_proxy.resource_scheduler_client import ResourceSchedulerClient
        if ray is None:
            import ray
            if not ray.is_initialized():
                raise RuntimeError("ray is not initialized for making ResourceSchedulerClient")
        sch = ResourceSchedulerClient.create(ray,self)
        return sch
