...
def find_notebook(fullname, path=None):
    ...
    if running_on_databricks:
        ...
        #if subdirectories are present in the filename, add them to the path list
        name = name.split('.')[-1] 
        subdirs = name.split('.')[:-1]      
        path.append(os.getcwd() + '/' + '/'.join(subdirs))
        
        for d in path:
            print("!PATH SUPPLIED")

            #if path does not start with /Workspace, add it 
            if not d.startswith('/Workspace'):
                d = '/Workspace' + d

            #create notebook path ( = location of notebook)
            nb_path = os.path.join(d, name)
