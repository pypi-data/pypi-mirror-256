...
from lino.api import dd, rt, _, gettext, pgettext
...
if False:
  ...
  def bulk_create_with_manual_ids(model, obj_list):
    """
    Originally copied from https://stackoverflow.com/a/13143062/407239
...
Vielleicht mit Fußnoten?"""))
    ...
    def fill_defaults(self, ar=None):
        ...
        if self.partner_id is None or self.printed_by is not None:
            return
        if self.pk is not None and self.entry_set.all().count() > 0:
            return
        self.save()
        entries = []
