...
class Plugin(ad.Plugin):
    ...
    def on_initdb(self, site, verbosity=1):
        ...
        def make_wsdl(template, parts):
            ...
            if fn.exists():
                if fn.stat().st_mtime > site.kernel.code_mtime:
                    site.logger.debug(
            ...
            fn.write_text(s)
            # open(fn, 'wt').write(s)
            site.logger.debug("Generated %s for environment %r.", fn, environment)

        # v1 was stopped in March 2019
        # make_wsdl('RetrieveTIGroups-v1.wsdl', RetrieveTIGroupsRequest.wsdl_parts)
        make_wsdl('RetrieveTIGroups-v2.wsdl', site.models.cbss.RetrieveTIGroupsRequest.wsdl_parts)
        make_wsdl('WebServiceConnector.wsdl', site.models.cbss.SSDNRequest.wsdl_parts)
