...
"""
The `demo` fixture for :mod:`lino_welfare.modlib.welfare`.
...
def objects():
    ...
    melanie = User(
        ...
        coaching_type=DSBE, language='fr',
    ...
    alicia = person(
        first_name=u"Alicia", last_name="Allmanns",
        email=settings.SITE.demo_email,
    ...
    yield alicia
    alicia = User(
        username="alicia", partner=alicia, language='fr',
        user_type=flexible_user_type('100'),
        coaching_type=DSBE,
    ...
    cpas = company(name="ÖSHZ Kettenis", city=kettenis, country=belgium)
    yield cpas
    bisa = company(name="BISA", city=eupen, country=belgium)
    yield bisa
    bisa_dir = role(company=bisa, person=annette, type=1)
    yield bisa_dir
    rcycle = company(name="R-Cycle Sperrgutsortierzentrum",
    ...
    yield company(name="Die neue Alternative V.o.G.", city=eupen, country=belgium)
    proaktiv = company(name="Pro Aktiv V.o.G.", city=eupen, country=belgium)
    yield proaktiv
    proaktiv_dir = role(company=proaktiv, person=hans, type=1)
    # IT manager : no contracts
    yield role(company=proaktiv, person=ulrike, type=4)
    yield proaktiv_dir
    cardijn = company(name="Werkstatt Cardijn V.o.G.", city=eupen, country=belgium)
    yield cardijn
    yield company(name="Behindertenstätten Eupen", city=eupen, country=belgium)
    bwe = company(name="Beschützende Werkstätte Eupen", city=eupen, country=belgium)
    yield bwe
...
Flexibilität: die Termine sind je nach Kandidat anpassbar.""",
    ...
    # 20210415 i = pcsw.Client.objects.all().__iter__()
    p = next(i)
    offset = 0
