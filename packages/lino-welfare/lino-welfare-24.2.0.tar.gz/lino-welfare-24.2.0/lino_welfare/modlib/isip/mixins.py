...
class ContractBase(Signers, Certifiable, EventGenerator, UserAuthored,
    ...
    user_asd = dd.ForeignKey(
        "users.User",
        verbose_name=_("Responsible (GSS)"),
    ...
    def user_choices(cls):
        return rt.models.users.User.get_active_users({IntegUser, SocialUser})
        # return rt.models.users.User.get_active_users([IntegUser, SocialUser], [SiteAdmin])

    @dd.chooser()
    def user_asd_choices(cls):
        return rt.models.users.User.get_active_users({SocialUser})
    ...
    def update_cal_from(self, ar):
        ...
        return rset.get_next_suggested_date(date, ar.logger)
...
dd.update_field(ContractBase, 'user', verbose_name=_("Responsible (IS)"))


class ContractPartnerBase(AbstractContactRelated, UploadController):
    class Meta:
        abstract = True
...
class ContractBaseTable(dd.Table):
    ...
    parameters = mixins.ObservedDateRange(
        user=dd.ForeignKey(settings.SITE.user_model,
            verbose_name=_("Responsible (IS)"), blank=True),
        user_asd=dd.ForeignKey(settings.SITE.user_model,
            verbose_name=_("Responsible (GSS)"), blank=True),
        observed_event=ContractEvents.field(blank=True, default='active'),
    ...
    params_layout = """
    user user_asd type start_date end_date observed_event
