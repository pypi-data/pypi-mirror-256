...
class ClientSummary(Certifiable, DateSummarized, SlaveSummarized):
    ...
    def get_summary_collectors(self):
        ...
        if dd.is_installed('art60'):
            qs = rt.models.art60.Contract.objects.all()
            qs = self.add_date_filter(
                qs, 'applies_from', client=self.master)
            yield (self.collect_from_jobs_contract, qs)
        elif dd.is_installed('jobs'):
    ...
    def reset_summary_data(self):
        if self.user_modified:
            return
        for sf in StatisticalFields.get_list_items():
