...
# extensible = dd.plugins.extensible

from lino_xl.lib.clients.choicelists import ClientStates
from lino_xl.lib.coachings.ui import CoachingsByClient
...
class CreateClientVisit(dd.Action):
    ...
    parameters = dict(
        user=dd.ForeignKey(settings.SITE.user_model),
        summary=models.CharField(verbose_name=_("Reason"), blank=True),
        waiting_number=models.CharField(verbose_name=_("Waiting number"), blank=True))
    params_layout = """
    user
    summary
    waiting_number
    ...
    def create_visit(self, ar, client):
        ...
        e = create_prompt_event(
            ...
            waiting_number=pv.waiting_number)
        # e.after_ui_save(ar, None)

        def msg(user, mm):
            subject = _("{client} now waiting for {user} ({number})")
            subject = subject.format(
                client=client, user=pv.user, number=pv.waiting_number)
            return (subject, '')
...
Tested document about :ref:`welfare.specs.reception.AgentsByClient`
    ...
    def actions(cls, obj, ar):
        ...
        if client.client_state == ClientStates.coached \
                ...
                # or user.newcomer_quota > 0:
            if not dd.plugins.extensible.is_hidden():
                sar = extensible.CalendarPanel.request(
                    subst_user=user,
