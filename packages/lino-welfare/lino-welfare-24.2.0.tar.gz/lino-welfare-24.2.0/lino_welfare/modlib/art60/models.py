...
class Convention(ContractPartnerBase, UploadController):
    ...
    refund_rate = models.CharField(_("refund rate"), max_length=200, blank=True)
    ...
    def get_certifiable_fields(cls):
        return (
            'contract job company contact_person contact_role '
...
class Contracts(ContractBaseTable):
    ...
    parameters = PriceRange('monthly_refund', _("Monthly refund"),
        type=dd.ForeignKey(
    ...
    def get_request_queryset(cls, ar):
        ...
        if pv.start_monthly_refund:
            qs = qs.filter(convention__monthly_refund__gte=pv.start_monthly_refund)
        if pv.end_monthly_refund:
            qs = qs.filter(Q(convention__monthly_refund__isnull=True)
