...
class Plugin(ad.Plugin):
    verbose_name = _("Art60 job supplying")  # Mises à l'emploi art.61
    needs_plugins = ['lino_welfare.modlib.jobs', 'lino_xl.lib.cv']

    def get_quicklinks(self):
        yield "art60.MyContracts"
        yield "art60.MyContractsGSS"

    def setup_main_menu(self, site, user_type, m):
        mg = site.plugins.integ
        m = m.add_menu(mg.app_label, mg.verbose_name)
        m.add_action('art60.MyContracts')
        m.add_action('art60.MyContractsGSS')

    # def setup_config_menu(self, site, user_type, m):
    #     mg = site.plugins.integ
    #     m = m.add_menu(mg.app_label, mg.verbose_name)
    #     m.add_action('art60.ContractTypes')
