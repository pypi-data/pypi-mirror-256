...
def objects():
    ...
    # arbitrary list of isip and jobs contract factories
    ...
    if dd.is_installed('art61'):
        ...
        SUBS_AMOUNTS = Cycler([250, 400, 100, 500])
        SUBS_STORIES = Cycler([
            [Subsidizations.hiring],
            [Subsidizations.tutorat],
            [Subsidizations.hiring, Subsidizations.tutorat],
        ...
        def f(**kw):
            kw.update(
                ...
                company=EMPLOYERS.pop(),
                duration=JOBS_CONTRACT_DURATIONS.pop())
            ss = SUBS_STORIES.pop()
            for sub in ss:
                # compare sub.get_contract_fields()
                k = "sub_{}_amount".format(sub.value)
                kw[k] = SUBS_AMOUNTS.pop()
    ...
    for i, client in enumerate(qs):
        ...
        qs2 = client.get_coachings((af, af), type=DSBE)
        if qs2.count() > 0:
            coaching = qs2[0]
