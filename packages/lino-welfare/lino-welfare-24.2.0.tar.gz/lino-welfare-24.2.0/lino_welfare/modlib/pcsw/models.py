...
class Client(contacts.Person, BiographyOwner, BeIdCardHolder,
    ...
    #     qs = rt.models.pcsw.PersonGroup.objects.filter(active=True)
    #     qs = qs.exclude(ref_name='').order_by('ref_name')
    #     return qs
    ...
    def get_active_contract(self):
        ...
        if dd.is_installed("art60"):
            qs2 = self.art60_contract_set_by_client.filter(flt)
        else:
    ...
    if not dd.is_installed("art60"):
      ...
      def contract_company(obj, ar):
        ...
        if isinstance(c, ContractPartnerBase):
            return c.company
...
class ClientDetail(dd.DetailLayout):
    ...
    remarks
    checkdata.MessagesByOwner:25 contacts.RolesByPerson:20
...
class PersonGroups(dd.Table):
    ...
    required_roles = dd.login_required(SocialStaff)
    order_by = ["ref_name"]
    column_names = "ref_name name active id *"
