...
from bigeye_sdk.model.protobuf_enum_facade import (
    ...
    SimpleMetricTemplateParameterType,
...
def deploy_all_autometrics(
    ...
    lookback_days: int = typer.Option(
        7, "--lookback_days", "-lbd", help="Look back days for the metrics."
    ),
    ops_only: bool = typer.Option(
        True, "--ops_only", help="Deploy only operational metrics"
    ),
):
    ...
    log.info(
        f"Bigeye API Configuration: {bigeye_conf} | Warehouse ID: {warehouse_id} | Table IDs: {table_ids}"
    )
    ...
    if not schema_names:
        mil = client.get_metric_info_batch_post(
    ...
    if schema_names:
        for sn in schema_names:
            submil = client.get_metric_info_batch_post(
                warehouse_ids=[warehouse_id],
                table_ids=table_ids,
                metric_creation_states=[ms],
                schema_name=sn,
    ...
    if ops_only:
        mil.metrics = [
            ...
            if m.metric_configuration.metric_type.predefined_metric in OPS_METRICS
        ]
...
def upsert_template(
    ...
    template_name: str = typer.Option(
        ..., "--name", "-tn", help="The user defined name of the template"
    ),
    template_body: str = typer.Option(
        ..., "--body", "-tb", help="The string to define the template"
    ),
    ...
    template_id: Optional[int] = typer.Option(
        ...
        "--template_id",
        "-id",
        help="The template ID (Only required when updating a template).",
    ),
):
    """Create or update a template for a source."""
    # TODO: Lookup template ID, once template names are unique
    client = cli_client_factory(bigeye_conf, config_file, workspace)
...
def delete_template(
    ...
    template_id: int = typer.Option(
        ..., "--template_id", "-tid", help="The ID of the metric template"
    ),
):
    """Delete a template."""
    client = cli_client_factory(bigeye_conf, config_file, workspace)
...
def upsert_virtual_table(
    ...
    sql: str = typer.Option(..., "--sql", "-s", help="The SQL to define the table"),
    source_name: str = typer.Option(
        ...,
        "--source_name",
        "-sn",
