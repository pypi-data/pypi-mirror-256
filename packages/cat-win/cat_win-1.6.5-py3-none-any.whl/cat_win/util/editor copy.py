...
from cat_win.util.editorhelper import History, Position, Highlights, HIGHLIGHTS, \
    UNIFY_HOTKEYS, KEY_HOTKEYS, ACTION_HOTKEYS, SCROLL_HOTKEYS
    ...
    def _render_scr(self) -> None:
        ...
        try:
            for token in tokenize.tokenize(
                BytesIO(
                    '\n'.join(self.window_content).encode()
                    ).readline
                ):
                if token.end[0]-1 < self.wpos.row or token.start[0] > self.wpos.row + max_y:
                    continue
                if token.type == 1: # keyword
                    for highlight, length, color in HIGHLIGHTS:
                        if token.string == highlight and token.start[1] < max_x:
                            self.curse_window.chgat(token.start[0]-1-self.wpos.row,
                                                    token.start[1], length,
                                                    self._get_color(color))
                            break
                elif token.type == 2: # number
                    self.curse_window.chgat(token.start[0]-1-self.wpos.row, token.start[1],
                                            token.end[1]-token.start[1],
                                            self._get_color(Highlights.BLUE))
                elif token.type == 3: # string
                    str_parts = token.string.split('\n')
                    for i, part in enumerate(str_parts):
                        _row = token.start[0]-1+i
                        if _row < self.wpos.row:
                            continue
                        if _row > self.wpos.row + max_y:
                            break
                        _start_col = token.start[1] if i==0 else 0
                        _end_col = token.end[1] if i==len(str_parts)-1 else len(part)
                        self.curse_window.chgat(_row-self.wpos.row, _start_col,
                                                _end_col-_start_col,
                                                self._get_color(Highlights.GREEN))
                elif token.type == 61: # comment
                    self.curse_window.chgat(token.start[0]-1-self.wpos.row, token.start[1],
                                            token.end[1]-token.start[1],
                                            self._get_color(Highlights.YELLOW))
    ...
    def _init_screen(self):
        ...
        try:
            ...
            if curses.can_change_color():
                ...
                curses.init_pair(5, curses.COLOR_WHITE, curses.COLOR_RED  ) # spechar & quit-prompt
                for color, c_id in filter(
                    lambda item: isinstance(item[1], int), vars(Highlights).items()
                    ):
                    curses.init_pair(c_id, getattr(curses, f"COLOR_{color}"), -1) # default colors
        curses.raw()
        self.curse_window.nodelay(False)
