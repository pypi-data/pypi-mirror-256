...
class RestClient(ApiClient):
    ...
    def __init__(
        ...
        *,
        ...
        """
        if endpoint is None:
            from cdh_lava_core_gems import dbgems
        ...
        from cdh_lava_core.dbx_db_rest.clusters import ClustersClient

        self.clusters = ClustersClient(self)

        from cdh_lava_core.dbx_db_rest.cluster_policies import ClustersPolicyClient

        self.cluster_policies = ClustersPolicyClient(self)

        from cdh_lava_core.dbx_db_rest.instance_pools import InstancePoolsClient

        self.instance_pools = InstancePoolsClient(self)

        from cdh_lava_core.dbx_db_rest.jobs import JobsClient

        self.jobs = JobsClient(self)

        from cdh_lava_core.dbx_db_rest.ml import MlClient

        self.ml = MlClient(self)

        from cdh_lava_core.rest.permissions import Permissions

        self.permissions = Permissions(self)

        from cdh_lava_core.dbx_db_rest.pipelines import PipelinesClient

        self.pipelines = PipelinesClient(self)

        from cdh_lava_core.dbx_db_rest.repos import ReposClient

        self.repos = ReposClient(self)

        from cdh_lava_core.dbx_db_rest.runs import RunsClient

        self.runs = RunsClient(self)

        from cdh_lava_core.dbx_db_rest.scim import ScimClient

        self.scim = ScimClient(self)

        from cdh_lava_core.dbx_db_rest.sql import SqlClient

        self.sql = SqlClient(self)

        from cdh_lava_core.dbx_db_rest.tokens import TokensClient

        self.tokens = TokensClient(self)

        from cdh_lava_core.dbx_db_rest.token_management import TokenManagementClient

        self.token_management = TokenManagementClient(self)

        from cdh_lava_core.dbx_db_rest.uc import UcClient

        self.uc = UcClient(self)

        from cdh_lava_core.dbx_db_rest.workspace import WorkspaceClient
