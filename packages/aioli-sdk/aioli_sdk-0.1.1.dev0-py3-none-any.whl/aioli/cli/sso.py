...
def handle_token(master_url: str, token: str) -> None:
    ...
    me = api.get(master_url, "api/v1/users/me", headers=tmp_auth, authenticated=False).json()
...
def sso(parsed_args: Namespace) -> None:
    master_info = api.get(parsed_args.controller, "info", authenticated=False).json()
    try:
        sso_providers = master_info["sso_providers"]
    except KeyError:
        raise NotFoundException("No SSO providers data")
    ...
    if not parsed_args.headless:
        if webbrowser.open(sso_url):
            ...
            with HTTPServer(
                ...
                make_handler(parsed_args.controller, lambda code: sys.exit(code)),
            ) as httpd:
    ...
    while not token:
        ...
        try:
            ...
            handle_token(parsed_args.controller, token)
    ...
    Cmd("auth", None, "manage auth", [
        Cmd("login", sso, "sign on with an auth provider", [
            Arg("-p", "--provider", type=str,
                help="auth provider to use (not needed if the Aioli master only supports"
                " one provider)"),
