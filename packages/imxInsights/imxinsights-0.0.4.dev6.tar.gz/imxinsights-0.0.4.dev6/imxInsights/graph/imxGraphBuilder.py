...
# from imxInsights import Imx, ImxSituationsEnum
...
class ImxGraphBuilder:
    ...
    Attributes:
        ...
        rail_connections_repo (ImxRailConnectionRepo): Repository of rail connections.
        micro_link_connections (MicroLinkConnectionRepo): Micro link connections extracted from the data.
    """

    def __init__(self, imx_situation: SituationRepo):
    ...
    def _get_rail_connection_repo(self) -> ImxRailConnectionRepo:
        """Creates a rail connection repository from the situation data.

        Returns:
            ImxRailConnectionRepo: The rail connection repository.
        """
        return ImxRailConnectionRepo([ImxRailConnection(_) for _ in self.imx_situation.get_by_types(["RailConnection"])])
    ...
    def _get_linked_micro_links(self, input_micro_link: ImxMicroLink, node_type: str) -> Dict[str, GraphMicroLink]:
        ...
        for jumper in micro_node.jumpers:
            ...
            # todo: no support for v500, code below will not result in passage refs camelcasing is the issue
                ...
                for micro_link in self.micro_link_repo.micro_links:
                    for passage_ref in micro_link.passage_refs:
                        ...
                        if passage_ref in passages_from_other_micro_link and micro_link != input_micro_link:
                            ...
                            # todo: why check both but should be same... make clear in comments
                            if not jumper.is_two_way and node_type == "to_node" and micro_link.from_node.ref == micro_node.puic:
                                continue
                            if not jumper.is_two_way and node_type == "from_node" and micro_link.to_node.ref == micro_node.puic:
                                continue
    ...
    def build_graph(self) -> ImxGraph:
        ...
        for key, value in mapping.items():
            for item in value:
                # todo calculate weight by 0.5 length of source and target
                self.g.add_edge(
                    key,
                    item.imx_micro_link.puic,
                    current_direction=item.from_direction.name,
                    true_node=item.true_node.junctionRef,
...
# imx = Imx(r"C:\Users\marti\OneDrive - ProRail BV\ENL\TVP4\U_concept ENL 4b_000__RVTO_20240212_concept\
#         U_concept ENL 4b_000__RVTO_20240212_compleet_concept_imx500.xml")
# imx_situation = imx.get_situation_repository(ImxSituationsEnum.InitialSituation)
# imx_graph = ImxGraphBuilder(imx_situation).build_graph()
