...
def plot_history(variable):
    ...
    f = open('functionfit.%s' % variable, 'r')
...
def plot_curve(variable):
    ...
    data = load_xy('functionfit.data')
    var = load_xy('functionfit.%s' % variable)
...
def plot_partition_x_histogram():
    ...
    data = load_xy('functionfit.data')
    hist = load_xy('functionfit.partition_x_hist')
...
def plot_partition_count_histogram():
    ...
    f = open('functionfit.partitions', 'r')
    history = list(map(int, f.readlines()))
    f.close()

    f = open('functionfit.partition_hist', 'r')
