...
def plot_curve(variable):
    ...
    var = load_xy('multistep.%s' % variable)
...
def plot_credible():
    ...
    data = load_xy('data.txt')
    lvar = load_xy('multistep.credible_min')
    uvar = load_xy('multistep.credible_max')
...
def plot_partition_x_histogram():
    ...
    data = load_xy('data.txt')
    hist = load_xy('multistep.partition_x_hist')
...
def plot_partition_count_histogram():
    ...
    f = open('multistep.partitions', 'r')
    history = list(map(int, f.readlines()))
    f.close()

    f = open('multistep.partition_hist', 'r')
