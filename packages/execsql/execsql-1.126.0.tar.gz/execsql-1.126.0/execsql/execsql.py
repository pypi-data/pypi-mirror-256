...
# 	MS-Access, SQLite, SQL Server, MySQL, Firebird, or Oracle database, or a DSN, and
...
# 	Copyright (c) 2007, 2008, 2009, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024 R.Dreas Nielsen
...
global sqlite3
# duckdb is imported if a DuckDB database will be used.
global duckdb
...
color_names = ("aliceblue", "antiquewhite", "aqua", "aquamarine", "azure", "beige", "bisque", "black", "blanchedalmond",
		...
		"purplse", "rebeccapurple", "red", "rosybrown", "royalblue", "saddlebrown", "salmon", "sandybrown", "seagreen",
			...
			if not configfile in self.files_read and os.path.isfile(configfile):
				...
				if cp.has_option(self._INPUT_SECTION, "trim_column_headers"):
					...
					if self.trim_col_hdrs not in ('none', 'both', 'left', 'right'):
						raise ConfigError("Invalid argument to trim_column_headers: %s." % self.trim_col_hdrs)
...
def encodings_match(enc1, enc2):
	...
	enc1 = re.sub(rx, u'\\g<tz>', enc1)
	enc2 = re.sub(rx, u'\\g<tz>', enc2)
	if enc1==enc2:
		return True
	equivalents = (
...
class FileWriter(multiprocessing.Process):
	...
	CMD_WRITE, CMD_CLOSE_IF_OPEN, CMD_CLOSE_ALL, CMD_GET_STATUS, CMD_SHUTDOWN, CMD_OPEN_AS_NEW, CMD_CLOSE_ALL_AFTER_WRITE, CMD_CLOSED_STATUS = range(8)

	class FileControl(object):
		STATUS_OPEN, STATUS_WAITING, STATUS_UNOPENED, STATUS_CLOSED, STATUS_OPENFAILURE = range(5)
		def __init__(self, fn, open_timeout, encoding="utf-8"):
			self.filename = fn
		...
		def try_open(self):
			if self.status != self.STATUS_OPEN or self.handle is None:
				...
				if time.time() - self.open_start_time < self.open_timeout:
					try:
						self.handle = io.open(file=self.filename, mode=self.openmode, encoding=self.encoding, errors="backslashreplace")
					except:
					...
					self.status = STATUS_OPENFAILURE
					if not self.fail_message_written:
						sys.stderr.write("Could not open %s for writing after retrying for %s seconds" % (self.filename, self.open_timeout))
						self.fail_message_written = True
		def close(self):
			if self.status == self.STATUS_WAITING:
				qlen = len(self.output_queue)
				sys.stderr.write("Closing %s while still trying to open it (locked by another process?) -- %s item(s) in queue to be written" % (self.filename, qlen))
	...
	def __init__(self, input_queue, return_msg_queue, file_encoding="utf-8", open_timeout=600):
		...
		self.execvec = (self.write, self.close_if_open, self.close_all, self.status, self.shutdown, self.open_as_new, self.close_all_after_write, self.closed_status)
	def __del__(self):
		self.close_all()
	def close_all(self):
		for fc in self.files.values():
			fc.close()
	def close_if_open(self, fn):
	...
	def closed_status(self):
		...
		stat = self.FileControl.STATUS_CLOSED
		for fc in self.files.values():
			if fc.status in(self.FileControl.STATUS_OPEN, self.FileControl.STATUS_WAITING):
				stat = fc.status
				break
		self.return_msg_queue.put(stat)
	def close_all_after_write(self):
		for fc in self.files.values():
			if fc.status != self.FileControl.STATUS_CLOSED:
				fc.clean_close()
	def open_as_new(self, fn):
		filename = os.path.abspath(fn)
		if filename not in self.files:
			self.files[filename] = self.FileControl(filename, open_timeout=self.open_timeout, encoding=self.file_encoding)
		fc = self.files[filename]
		fc.open_as_new()
	def status(self, fn):
		filename = os.path.abspath(fn)
...
filewriter = None
fw_input = multiprocessing.Queue()
fw_output = multiprocessing.Queue()

def filewriter_filestatus(filename):
	fw_input.put((FileWriter.CMD_GET_STATUS, (filename, )))
	return fw_output.get()

def filewriter_write(filename, message):
	fw_input.put((FileWriter.CMD_WRITE, (filename, message)))

def filewriter_open_as_new(filename):
...
def filewriter_close_all_after_write():
...
def filewriter_closeall():
	fw_input.put((FileWriter.CMD_CLOSE_ALL, ()))

def filewriter_shutdown():
...
class Logger(object):
	...
	def __init__(self, script_file_name, db_name, server_name, cmdline_options, user_logfile=False, log_file_name=None):
		...
		for datarow in rowsource:
			...
			if dataitems > 0:
				...
				if dataitems < chkcols:
					...
					if dataitems > chkcols:
						# If all the extra data items are null or empty string *and* conf.del_empty_cols, then OK, else raise an error
						errmsg = "Too many columns (%d) on data row %d" % (dataitems, self.inputrows)
						if conf.del_empty_cols:
							any_non_empty = False
							for c in range(chkcols, dataitems):
								column_value = datarow[c]
								if not ((column_value is None or (not conf.empty_strings and isinstance(column_value, stringtypes) and \
										len(column_value.strip()) == 0)) and conf.del_empty_cols):
									any_non_empty = True
									break
							if any_non_empty:
...
class AccessDatabase(Database):
	...
	def execute(self, sqlcmd, paramlist=None):
		...
		def exec1(sql, paramlist):
			...
			if tqd:
				if self.jet4 or sys.version_info >= (3,):
					qn = tqd.group(3)
					qsql = sql[tqd.end():]
				else:
...
class PostgresDatabase(Database):
	...
	def open_db(self):
		def db_conn(db, db_name):
			try:
				...
				msg = u"Failed to open PostgreSQL database %s on %s; check server and database name, and validity of credentials" % (self.db_name, self.server_name)
	...
	def import_tabular_file(self, schema_name, table_name, csv_file_obj, skipheader):
		...
		if encodings_match(input_enc, self.encoding) \
				...
				and not conf.del_empty_cols and not conf.create_col_hdrs \
			...
			copy_cmd = copy_cmd + ")"
			exec_log.log_status_info(u"IMPORTing %s using Postgres' fast file reading routine" % csv_file_obj.csvfname)
			try:
...
class MySQLDatabase(Database):
	...
	def import_tabular_file(self, schema_name, table_name, csv_file_obj, skipheader):
		...
		if conf.import_common_cols_only:
			...
			import_cols = csv_file_cols
		input_col_list = ",".join(import_cols)
		if data_table_cols == csv_file_cols \
				and conf.empty_strings and conf.empty_rows \
				and not conf.del_empty_cols and not conf.create_col_hdrs \
				and not conf.trim_strings and not conf.replace_newlines:
			...
			exec_log.log_status_info(u"IMPORTing %s using the DBMS' fast file reading routine" % csv_file_obj.csvfname)
...
class CsvFile(EncodedFile):
	...
	def _colhdrs(self, inf):
		...
		if any([x is None or len(x)==0 for x in colnames]):
			if conf.del_empty_cols:
				self.blank_cols = [i for i in range(len(colnames)) if colnames[i] is None or len(colnames[i].strip())==0]
				blanks = copy.copy(self.blank_cols)
				while len(blanks) > 0:
					b = blanks.pop()
					del(colnames[b])
...
class CommandList(object):
	...
	def run_and_increment(self):
		...
		if compiling_loop:
			...
			subvars.add_substitution("$CURRENT_TIME_UTC", utcnow.strftime("%Y-%m-%d %H:%M"))
...
GUI_DISPLAY, GUI_CONNECT, GUI_CONSOLE, GUI_SELECTSUB, GUI_PAUSE, GUI_HALT, GUI_ENTRY, QUERY_CONSOLE, \
	GUI_OPENFILE, GUI_SAVEFILE, GUI_DIRECTORY, GUI_COMPARE, GUI_MAP, GUI_SELECTROWS, GUI_ACTION, \
	GUI_CREDENTIALS = range(16)
		...
		if gui_manager_stop_event.is_set():
			...
			if not gui_manager_queue.empty():
				...
				if msg.gui_type == GUI_DISPLAY:
					...
					btn, rv = ui.activate()
					if "free" not in msg.gui_args or not msg.gui_args["free"]:
						# Nobody is waiting for a value from a 'free' display.
					...
					msg.return_queue.put({"button": btn, "return_value": rv})
					ui = None
				elif msg.gui_type == GUI_MAP:
					ui = MapUI(**msg.gui_args)
					#btn, rv = ui.activate()
					btn = ui.activate()[0]
					#msg.return_queue.put({"button": btn, "return_value": rv})
					msg.return_queue.put({"button": btn })
					...
					msg.return_queue.put({"button": btn, "return_value": rv})
					ui = None
				elif msg.gui_type == GUI_CREDENTIALS:
					ui = CredentialsUI(**msg.gui_args)
					ui.activate()
					msg.return_queue.put({"exit_status": ui.exit_status,
											"user": as_none(ui.user.get()),
											"pw": as_none(ui.pw.get()) })
...
def save_data_as(parent, rowset, column_headers, tablename=None, desc=None):
	try:
...
def add_saveas_menu(parent, mnu, label, rowset, column_headers, tablename=None, desc=None):
	def save_as():
		save_data_as(parent, rowset, column_headers, tablename, desc)
	mnu.add_command(label=label, command=save_as)
...
class DisplayUI(object):
	...
	def __init__(self, title, message, button_list, selected_button=0, no_cancel=False, column_headers=None,
			...
			initialtext=None, free=False, help_url=None):
		...
		# free, if True, creates just a 'Close' button and does not use 'grab_set()', so the main script keeps running.
		try:
			import Tkinter as tk
		except:
			import tkinter as tk
		...
		add_saveas_menu(self.win, mnu, "Save as...", rowset, column_headers, tablename=title, desc=message)
		# Put the button frame in place.
		btnframe.grid(column=0, row=win_row, sticky=tk.EW)
		btnframe.columnconfigure(0, weight=1)
		win_row += 1
		# Create buttons.
		if self.help_url is not None and not self.free:
			help_btn = ttk.Button(btnframe, text="Help", command=self.do_help)
			help_btn.grid(column=0, row=0, sticky=tk.W, padx=3)
		if not no_cancel:
			button_list.append(('Cancel', None, "<Escape>"))
		if self.free:
			ok_btn = ttk.Button(btnframe, text="Close", command=self.close_ok)
			ok_btn.grid(column=1, row=0, sticky=tk.E, padx=3)
		else:
			for colno, btn_spec in enumerate(button_list):
				...
				btn.grid(column=colno+2, row=0, sticky=tk.E, padx=3)
		# Other key bindings
	...
	def do_help(self, *args):
		if self.help_url is not None:
			import webbrowser
			webbrowser.open(self.help_url, new=2, autoraise=2)
...
class CompareUI(object):
	def __init__(self, title, message, button_list, headers1, rows1,
	        headers2, rows2, keylist, selected_button=0, sidebyside=False, help_url=None):
		# button_list is a *list* of 3-tuples where the first item is the button label,
		# 	the second item is the button's value, and the third (optional) value is the key
		# 	to bind to the button.  Key identifiers must be in the form taken by the Tk bind()
		# 	function, e.g., "<Return>" and "<Escape>" for those keys, respectively.
		...
		# Add 'save as' menu items
		mnu = tk.Menu(self.win)
		self.win.config(menu=mnu)
		add_saveas_menu(self.win, mnu, "Save 1 as...", rows1, headers1)
		add_saveas_menu(self.win, mnu, "Save 2 as...", rows2, headers2)
		# Create buttons.
		if self.help_url is not None:
			help_btn = ttk.Button(btnframe, text="Help", command=self.do_help)
			help_btn.grid(column=0, row=0, sticky=tk.W, padx=3)
		...
		for colno, btn_spec in enumerate(button_list):
			...
			if btn_spec[2] is not None:
				self.win.bind(btn_spec[2], btn_action)
			self.buttons.append(btn)
			btn.grid(column=colno+1, row=0, sticky=tk.E, padx=3)
		# Allow resizing.
		self.win.columnconfigure(0, weight=1)
...
class MapUI(object):
	...
	def new_img_del(img_self):
		try:
			...
			pass
	img_patched = False
	def __init__(self, title, message, button_list, headers, rows,
			lat_col, lon_col, label_col=None, symbol_col=None, color_col=None, selected_button=0):
		# Mapping features are only available when using Python 3 (>=3.8), therefore no imports from Python 2.
		import tkinter as tk
		import tkinter.ttk as ttk
		import tkinter.font as tkfont
		import tkinter.filedialog as tkfiledialog
		from tkintermapview import TkinterMapView
		# Patch ImageTk.PhotoImage.__del__ to eliminate exception messages
		from PIL import ImageTk
		if not self.img_patched:
			ImageTk.PhotoImage.__del__ = self.new_img_del
			self.img_patched = True
		# end patch
		self.headers = headers
		self.rows = rows
		self.buttons = []		# A list of ttk.Button objects, with an extra attribute "command_func" (because the Button callback function is not queryable)
		self.focus_button = selected_button
		self.button_clicked_value = None
		...
		self.label_index = headers.index(label_col) if label_col is not None else None
		self.symbol_index = headers.index(symbol_col) if symbol_col is not None else None
		self.color_index = headers.index(color_col) if color_col is not None else None
		# The marker for the selected location
		self.sel_map_marker = None
		# Map bounds
		self.min_lat = None
		self.max_lat = None
		self.min_lon = None
		...
		self.loc_marker_icon = tk.BitmapImage(data=icon_xbm['triangle'], foreground="black")
		self.sel_marker_icon = tk.BitmapImage(data=icon_xbm['wedge'], foreground="red")
		...
		# Use four stacked frames for the main application window components
		msgframe = ttk.Frame(self.win, padding="3 3 3 3")
		mapframe = ttk.Frame(self.win)
		tblframe = ttk.Frame(self.win, padding="3 3 3 3")
		btnframe = ttk.Frame(self.win, padding="3 3 3 3")
		# Allow resizing.
		self.win.columnconfigure(0, weight=1)
		...
		btnframe.columnconfigure(0, weight=1)
		btnframe.grid(row=3, column=0, sticky=tk.E)
		# Populate the map frame
		# The map frame has two rows, one for the basemap selectors (in bm_frame) and one for the map widget
		mapframe.rowconfigure(0, weight=0)
		mapframe.rowconfigure(1, weight=1)
		mapframe.columnconfigure(0, weight=1)
		# Basemap controls
		bm_frame = tk.Frame(mapframe)
		bm_frame.grid(row=0, column=0, sticky=tk.W)
		bm_frame.rowconfigure(0, weight=0)
		bm_frame.columnconfigure(0, weight=1)
		self.basemap_label = ttk.Label(bm_frame, text="Basemap:", anchor="w")
		self.basemap_label.grid(row=0, column=0, padx=(5, 5), pady=(2, 5), sticky=tk.W)
		self.basemap_var = tk.StringVar(self.win, "OpenStreetMap")
		basemap_vals = list(bm_servers.keys())
		self.map_option_menu = tk.OptionMenu(bm_frame, self.basemap_var, *basemap_vals,
			command = self.change_basemap)
		self.map_option_menu.grid(row=0, column=1, padx=(5, 5), pady=(2, 5), sticky=tk.W)
		# Map control buttons
		#zoomsel_btn = ttk.Button(bm_frame, text="Zoom selected", image=zoom_sel_icon, compound=tk.LEFT, command=self.zoom_selected)
		#zoomsel_btn.image = zoom_sel_icon
		#zoomsel_btn.grid(row=0, column=2, sticky=tk.W)
		expand_btn = ttk.Button(bm_frame, text="Zoom full", image=expand_icon, compound=tk.LEFT, command=self.zoom_full)
		expand_btn.image = expand_icon
		expand_btn.grid(row=0, column=2, sticky=tk.W)
		focus_btn = ttk.Button(bm_frame, text="Focus", image=focus_icon, compound=tk.LEFT, command=self.focus_map)
		focus_btn.image = focus_icon
		focus_btn.grid(row=0, column=3, sticky=tk.W)
		unselect_btn = ttk.Button(bm_frame, text="Un-select", image=unselect_icon, compound=tk.LEFT, command=self.unselect_map)
		unselect_btn.image = unselect_icon
		unselect_btn.grid(row=0, column=4, sticky=tk.W)
		# Map widget
		self.map_widget = TkinterMapView(mapframe, height=600, width=600, corner_radius=0)
		#self.map_widget.set_tile_server(bm_servers["OpenStreetMap"])
		self.map_widget.grid(row=1, column=0, sticky=tk.NSEW)
		mapframe.grid(row=1, column=0, sticky=tk.NSEW)
		# Populate the table frame
		tableframe, self.tbl = treeview_table(tblframe, rows, headers, "browse")
		tblframe.rowconfigure(0, weight=1)
		tblframe.columnconfigure(0, weight=1)
		tblframe.grid(row=2, column=0, sticky=tk.NSEW)
		tableframe.grid(column=0, row=0, sticky=tk.NSEW)
		def mark_map(event):
			# Highlight the selected row in the table and get the coordinates to map it
			if self.tbl.selection():
				curr_row = self.tbl.item(self.tbl.selection()[0])["values"]
				lat_val = float(curr_row[self.lat_index])
				lon_val = float(curr_row[self.lon_index])
				if self.sel_map_marker is not None:
					self.map_widget.delete(self.sel_map_marker)
				self.sel_map_marker = self.map_widget.set_marker(lat_val, lon_val, icon=self.sel_marker_icon)
		self.tbl.bind('<ButtonRelease-1>', mark_map)
			...
			if lon_val is not None and lat_val is not None:
				...
				if self.color_index is None and self.symbol_index is None:
					...
					if mkr_key not in custom_icons:
						#custom_icons[mkr_key] = PIL.ImageTk.BitmapImage(PIL.Image.frombytes('1', (16, 16), icon_bmps[symbol]), foreground=color)
				...
				if self.label_index is not None:
					...
					mkr = self.map_widget.set_marker(lat_val, lon_val, text=lbl, icon=marker_icon, command=self.map_sel_table)
				else:
		...
		if m is not None:
			...
			self.win.geometry("%dx%d+%d+%d" % (wwd, wht, xpos, ypos))
		# Limit resizing
		self.win.minsize(width=300, height=200)
	def zoom_full(self):
		self.map_widget.fit_bounding_box((self.max_lat, self.min_lon), (self.min_lat, self.max_lon))
	...
	def focus_map(self):
		if self.sel_map_marker is not None:
			self.map_widget.set_position(self.sel_map_marker.position[0], self.sel_map_marker.position[1])
	...
	def map_sel_table(self, marker):
		...
		for row_id in self.tbl.get_children():
			rowdata = self.tbl.item(row_id)["values"]
			if float(rowdata[self.lat_index]) == lat and float(rowdata[self.lon_index]) == lon:
				self.tbl.selection_add(row_id)
				self.tbl.see(row_id)
				if self.sel_map_marker is not None:
					self.map_widget.delete(self.sel_map_marker)
				self.sel_map_marker = self.map_widget.set_marker(lat, lon, icon=self.sel_marker_icon)
				break
	def cancel(self):
		#status.dialog_canceled = True
		self.win.destroy()
	...
	def activate(self):
		...
		self.win.update_idletasks()
		# Explicitly delete the Tkinter variable to suppress Tkinter error message.
		self.basemap_var = None
		#rv = self.return_value
		#return (self.button_clicked_value, rv)
		return (self.button_clicked_value,)



class SelectRowsUI(object):
	def __init__(self, title, message, button_list, headers1, rows1,
	        headers2, rows2, alias2, table2, selected_button=0, help_url=None):
		# button_list is a *list* of 3-tuples where the first item is the button label,
		# 	the second item is the button's value, and the third (optional) value is the key
		# 	to bind to the button.  Key identifiers must be in the form taken by the Tk bind()
		# 	function, e.g., "<Return>" and "<Escape>" for those keys, respectively.
...
class ActionUI(object):
	def __init__(self, title, message, button_specs, compact=None, include_continue_button=False, help_url=None, column_headers=None, rowset=None):
	...
	def activate(self):
		# Window control
		# Don't use grab_set() so that PROMPT...DISPLAY...FREE windows can function
		self.win._root().withdraw()
		self.win.focus_force()
...
class CredentialsUI(object):
	def __init__(self, message):
		...
		if message:
			...
			self.msg_label.grid(column=0, row=0, sticky=tk.EW)
			msgframe.grid(column=0, row=0, sticky=tk.EW)
		self.userframe = ttk.Frame(master=self.win)
		self.user = tk.StringVar()
		self.user.trace("w", self.clearstatus)
		self.pw = tk.StringVar()
		self.pw.trace("w", self.clearstatus)
		self.status_msg = tk.StringVar()
		ttk.Label(self.userframe, text="User:").grid(column=0, row=0, padx=3, pady=3, sticky=tk.E)
		self.userentry = ttk.Entry(self.userframe, width=30, textvariable=self.user)
		self.userentry.grid(column=1, row=0, padx=3, pady=3, sticky=tk.W)
		ttk.Label(self.userframe, text="Password:").grid(column=0, row=1, padx=3, pady=3, sticky=tk.E)
		ttk.Entry(self.userframe, width=30, textvariable=self.pw, show="*").grid(column=1, row=1, padx=3, pady=3, sticky=tk.W)
		self.userframe.grid(column=0, row=1, sticky=tk.NSEW)
		# Create Connect and Cancel buttons
		btnframe = ttk.Frame(master=self.win, padding="3 3 3 3")
		ttk.Button(btnframe, text='Continue', command=self.cont).grid(row=0, column=0, sticky=tk.E, padx=3)
		...
		statusframe.grid(column=0, row=3, sticky=tk.EW)
		# Bindings
		self.win.bind('<Return>', self.cont)
		self.win.bind('<Escape>', self.cancel)
		# Limit resizing
		self.win.resizable(width=0, height=0)
		self.redraw()
...
class ConnectUI(object):
	...
	def __init__(self, title, message, help_url=None):
		try:
			import Tkinter as tk
		except:
			import tkinter as tk
		...
		if self.help_url is not None:
			help_btn = ttk.Button(btnframe, text="Help", command=self.do_help)
			help_btn.grid(column=0, row=0, sticky=tk.W, padx=3)
		conn_btn = ttk.Button(btnframe, text='Connect', command=self.connect)
		conn_btn.grid(row=0, column=1, sticky=tk.E, padx=3)
		cancel_btn = ttk.Button(btnframe, text='Cancel', command=self.cancel)
...
class ConsoleUI(object):
	...
	def set_height(self, lines):
		...
		if m is not None:
			...
			self.win.update_idletasks()
	def kill(self, return_status=True, *args):
...
class GuiConsole(object):
	...
	def wait_for_user(self):
		...
		exit_status = self.resp_queue.get(True)
		# exit_status True -> console killed; False -> console still active
...
class EntrySpec(object):
	def __init__(self, name, prompt, required=False, initial_value=None, default_width=None, default_height=None,
					lookup_list=None, form_column=None, validation_regex=None, validation_key_regex=None, entry_type=None):
		# The recognized values for entry_type are "checkbox", "listbox", "textarea", "inputfile", and "outputfile".
		self.name = name
		self.prompt = prompt
		...
		if self.form_column is None or self.form_column == '' or (self.form_column is not None and isinstance(self.form_column, int) and self.form_column < 1):
			self.form_column = 1
		if validation_regex:
		...
		if self.entry_type == "checkbox":
			# Checkboxes don't have a null state
			self.required = False
		if self.entry_type == "listbox" and lookup_list is None:
		...
		if not self.height and self.entry_type == "listbox":
			self.height = 4
	def __repr__(self):
		return "EntrySpec(%s, %s, %s, %s, %s, %s, %s, %s, %s)" % (self.name, self.prompt, self.required, self.value,
				self.width, self.height, self.lookup_list, self.form_column, self.validation_rx.pattern if self.validation_rx else None, self.entry_type)


class EntryFormUI(object):
	def __init__(self, title, message, entry_specs, column_headers=None, rowset=None, help_url=None):
		...
		for e in self.entries:
			maxcol = max(maxcol, e.form_column or 1)
		self.win.columnconfigure(0, weight=1)
		# Message frame and control.
		msgframe = ttk.Frame(master=self.win, padding="3 3 3 3")
		msgframe.grid(column=0, row=0, sticky=tk.EW)
		self.msg_label = ttk.Label(msgframe, text=message, wraplength=200*maxcol)
		self.msg_label.bind("<Configure>", wrap_msg)
		self.msg_label.grid(column=0, row=0, sticky=tk.EW)
		# Data entry frame
		entryframe = ttk.Frame(master=self.win, padding="3 3 3 3")
		entryframe.columnconfigure(0, weight=1)
		entryframe.grid(column=0, row=1, sticky=tk.EW, padx=(3,3), pady=(3,6))
		has_textarea = False
		any_required = False
		# Column subframes
		colframes = []
		for fno in range(maxcol):
			frm = ttk.Frame(master=entryframe)
			frm.columnconfigure(0, weight=1)
			frm.grid(row=0, column=fno, padx=3, sticky=tk.NSEW)
			colframes.append(frm)
		# Row indexes for each column subframe
		nextrow = [0] * maxcol
		for i, e in enumerate(self.entries):
			colindex = e.form_column - 1
			itemrow = nextrow[colindex]
			nextrow[colindex] = itemrow + 1
			itementryframe = ttk.Frame(master=colframes[colindex])
			itementryframe.grid(column=1, row=itemrow, sticky=tk.W)
			if e.entry_type is None or e.entry_type.lower() != "radiobuttons":
				e.entrylabel = ttk.Label(master=colframes[colindex], text=e.prompt)
				e.entrylabel.grid(column=0, row=itemrow, sticky=tk.NE, padx=3)
			e.entryvar = tk.StringVar()
			entry_gridded = False
			if e.lookup_list:
				if e.entry_type == 'listbox':
					e.entryvar.set(e.lookup_list)
					e.entryctrl = tk.Listbox(itementryframe, listvariable=e.entryvar, height=e.height or 4, selectmode=tk.MULTIPLE, exportselection=0)
					if e.required:
						e.entryctrl.bind("<ButtonRelease>", self.validate_all)
						any_required = True
					sbar = ttk.Scrollbar(itementryframe, orient=tk.VERTICAL, command=e.entryctrl.yview)
					e.entryctrl.configure(yscrollcommand=sbar.set)
					e.entryctrl.grid(column=1, row=0, sticky=tk.W+tk.NS, padx=(3,0), pady=(3,0))
					sbar.grid(row=0, column=2, sticky=tk.W+tk.NS, padx=(0,3), pady=3)
					hsbar = ttk.Scrollbar(itementryframe, orient=tk.HORIZONTAL, command=e.entryctrl.xview)
					e.entryctrl.configure(xscrollcommand=hsbar.set)
					hsbar.grid(row=1, column=1, sticky=tk.N+tk.EW, padx=(3,0), pady=(0,3))
					entry_gridded = True
				else:
					e.entryctrl = ttk.Combobox(itementryframe, textvariable=e.entryvar, width=e.width, values=e.lookup_list)
					if e.required:
						e.entryctrl.configure(validate='all')
						e.entryctrl.configure(validatecommand=(e.entryctrl.register(validate_item(e.lookup_list, e.required)), '%P'))
						any_required = True
				e.entryctrl.bind("<Return>", self.do_continue)
			elif e.entry_type is not None and e.entry_type.lower() == "checkbox":
				e.entryctrl = tk.Checkbutton(itementryframe, text="", variable=e.entryvar, onvalue="1", offvalue="0")
				if e.value is None or str(e.value).lower() in ("0", "off", "no", "false"):
					e.entryvar.set("0")
				else:
					e.entryvar.set("1")
				# Don't bind <Return> to the 'do_continue' action because it always also toggles the checkbutton on Linux.
			elif e.entry_type is not None and e.entry_type.lower() == "textarea":
				has_textarea = True
				e.entryctrl = tk.Text(itementryframe, width=e.width if e.width else 20, height=e.height if e.height else 5, wrap="word")
				if e.value is not None:
					e.entryctrl.insert(tk.END, e.value.strip())
				if e.validation_rx or e.required:
					e.entryctrl.bind("<KeyRelease>", self.validate_all)
				# Don't bind <Return> to the 'do_continue' action because it is used for text entry.
			elif e.entry_type is not None and e.entry_type.lower() == "radiobuttons":
				# The prompt must be a semicolon-delimited superstring, where the first item is the overall prompt and
				# the remaining items are options.  The return values will be a 1-based integer (string) identifying the chosen option.
				rbutn_labels = [p.strip() for p in  e.prompt.split(";")]
				e.entrylabel = ttk.Label(master=colframes[colindex], text=rbutn_labels[0])
				e.entrylabel.grid(column=0, row=itemrow, sticky=tk.NE, padx=3)
				for i, btn in enumerate(rbutn_labels[1:]):
					ttk.Radiobutton(itementryframe, text=btn, value=str(i+1), variable=e.entryvar).grid(row=i, column=0, sticky=tk.W, padx=(3,3))
				e.entryvar.set('1')
				entry_gridded = True
			elif e.entry_type is not None and e.entry_type.lower() == "inputfile":
				getfile_entryframe = ttk.Frame(master=itementryframe)
				e.entryctrl = ttk.Entry(getfile_entryframe, width=e.width if e.width else 20, textvariable=e.entryvar)
				self.browse_button = ttk.Button(getfile_entryframe, text='Open', width=8, command=self.set_entryinputfile(i))
				e.entryctrl.grid(row=0, column=1, padx=3, pady=3, sticky=tk.W)
				self.browse_button.grid(row=0, column=2, padx=3, sticky=tk.W)
				getfile_entryframe.grid(row=0, column=0)
				entry_gridded = True
				e.entryctrl.bind("<Return>", self.do_continue)
			elif e.entry_type is not None and e.entry_type.lower() == "outputfile":
				getfile_entryframe = ttk.Frame(master=itementryframe)
				e.entryctrl = ttk.Entry(getfile_entryframe, width=e.width if e.width else 20, textvariable=e.entryvar)
				...
				e.entryctrl = ttk.Entry(itementryframe, width=e.width if e.width else 20, textvariable=e.entryvar)
				if e.validation_rx or e.required:
					e.entryctrl.configure(validate='all')
					e.entryctrl.configure(validatecommand=(e.entryctrl.register(validate_entry(e.validation_rx, e.validation_key_rx, e.required)), '%V', '%P'))
					e.entryctrl.configure(invalidcommand=(e.entryctrl.register(setfocus(e))))
				e.entryctrl.bind("<Return>", self.do_continue)
			if e.value is not None and e.value.strip() != '':
				if e.entry_type == "listbox":
					try:
						entry_idx = e.lookup_list.index(e.value)
					except:
						pass
					else:
						e.entryctrl.select_set(entry_idx)
				elif e.entry_type != "checkbox":
					e.entryvar.set(e.value)
			if e.entry_type is None or (e.entry_type.lower() != "textarea" and e.entry_type.lower() != "listbox"):
				e.entryvar.trace("w", self.validate_all)
			if not entry_gridded:
				e.entryctrl.grid(column=1, row=0, sticky=tk.W, padx=3, pady=3)
			if e.required:
				ttk.Label(itementryframe, text="*").grid(column=100, row=0, sticky=tk.W+tk.N, padx=2)
				any_required = True
		# Add data table frame if needed.
		if rowset and column_headers:
		...
		continue_btn.grid(column=1, row=0, sticky=tk.E, padx=3)
		# The <F5> key always does the 'do_continue' action, <Return> does so only when the focus is not on a textarea or checkbutton.
		...
		if m is not None:
			...
			xpos = (swd/2) - (wwd/2)
			ypos = (sht/2) - (wht/2)
			self.win.geometry("%dx%d+%d+%d" % (wwd, wht+12, xpos, ypos))
		# Limit resizing
		self.win.minsize(width=400, height=50)
	...
	def validate_all(self, *args):
		valid_errors = []
		for e in self.entries:
			if e.entry_type is not None and e.entry_type == "listbox":
				if e.required:
					if len(e.entryctrl.curselection()) == 0:
						valid_errors.append("%s requires at least one value selected" % e.name)
			elif e.entry_type is None or e.entry_type not in ("checkbox"):
				if e.entry_type is not None and e.entry_type == "textarea":
	...
	def do_continue(self, *args):
		...
		for e in self.entries:
			...
			elif e.entry_type is not None and e.entry_type == 'listbox':
				sel_idx = e.entryctrl.curselection()
				if len(sel_idx) > 0:
					sel_values = [e.entryctrl.get(i) for i in sel_idx]
					e.value = ",".join(["'" + v.replace("'", "''") + "'" for v in sel_values])
				else:
					e.value = ''
			else:
...
# Undocumented
def x_debug_commandliststack(**kwargs):
	output.write("Command List Stack:\n")
	pfx = "  "
	for cl in commandliststack:
		output.write(pfx + "Name:             %s\n" % cl.listname)
		output.write(pfx + "Parameters:       %s\n" % cl.paramnames)
		output.write(pfx + "Command pointer:  %s\n" % cl.cmdptr)
		pfx = pfx + "  "
	return None

metacommandlist.add(r'^\s*DEBUG\s+WRITE\s+COMMANDLISTSTACK\s*$', x_debug_commandliststack)


#****	DEBUG WRITE IFLEVELS
# Undocumented
def x_debug_iflevels(**kwargs):
	if len(if_stack.if_levels) == 0:
		output.write("If levels: None\n")
	else:
		output.write("If levels: [%s]\n" % ",".join([str(tf.tf_value) for tf in if_stack.if_levels]))
	return None

metacommandlist.add(r'^\s*DEBUG\s+WRITE\s+IFLEVELS\s*$', x_debug_iflevels)



#****	IMPORT FROM PARQUET
def x_import_parquet(**kwargs):
	...
	exec_log.log_status_info(u"IMPORTing from Parquet file %s (%s, %s)" % (filename, sz, dt))
	try:
		import_parquet(dbs.current(), schemaname, tablename, filename, is_new)
	except ErrInfo:
		raise
	except:
		raise ErrInfo("exception", exception_msg=exception_desc(), other_msg="Can't import data from Parquet data file %s" % filename)
	return None

metacommandlist.add(
	ins_table_rxs(r'^\s*IMPORT\s+TO\s+(?:(?P<new>NEW|REPLACEMENT)\s+)?', ins_fn_rxs(r'\s+FROM\s+PARQUET\s+', r'\s*$')),
	x_import_parquet)


#****	IMPORT FROM FEATHER
def x_import_feather(**kwargs):
	# is_new should have values of 0, 1, or 2
	newstr = kwargs['new']
	if newstr:
		is_new = 1 + ['new', 'replacement'].index(newstr.lower())
	...
	if not os.path.exists(filename):
		raise ErrInfo(type="cmd", command_text=kwargs['metacommandline'], other_msg='Input file %s does not exist' % filename)
	sz, dt = file_size_date(filename)
	exec_log.log_status_info(u"IMPORTing from Feather file %s (%s, %s)" % (filename, sz, dt))
	try:
		import_feather(dbs.current(), schemaname, tablename, filename, is_new)
	except ErrInfo:
		raise
	except:
		raise ErrInfo("exception", exception_msg=exception_desc(), other_msg="Can't import data from Feather data file %s" % filename)
	return None

metacommandlist.add(
	ins_table_rxs(r'^\s*IMPORT\s+TO\s+(?:(?P<new>NEW|REPLACEMENT)\s+)?', ins_fn_rxs(r'\s+FROM\s+FEATHER\s+', r'\s*$')),
	x_import_feather)
...
def x_debug_log_config(**kwargs):
	...
	exec_log.log_status_info("Config; Write prefix = %s" % conf.write_prefix)
	exec_log.log_status_info("Config; Write suffix = %s" % conf.write_suffix)
	...
	def write(txt):
		if output_dest is None or output_dest == 'stdout':
			...
			if append is not None:
				filewriter_open_as_new(output_dest)
			filewriter.write(txt)
	write(u"Config; Script encoding = %s\n" % conf.script_encoding)
	write(u"Config; Output encoding = %s\n" % conf.output_encoding)
	write(u"Config; Import encoding = %s\n" % conf.import_encoding)
	write(u"Config; Import common columns only = %s\n" % conf.import_common_cols_only)
	write(u"Config; Use numeric type for Access = %s\n" % conf.access_use_numeric)
	write(u"Config; Max int = %d\n" % conf.max_int)
	write(u"Config; Boolean int = %s\n" % conf.boolean_int)
	write(u"Config; Boolean words = %s\n" % conf.boolean_words)
	write(u"Config; Clean column headers %s\n" % conf.clean_col_hdrs)
	write(u"Config; Create column headers %s\n" % conf.create_col_hdrs)
	write(u"Config; Dedup column headers %s\n" % conf.dedup_col_hdrs)
	write(u"Config; Console wait when done %s\n" % conf.gui_wait_on_exit)
	write(u"Config; Console wait when error %s\n" % conf.gui_wait_on_error_halt)
	write(u"Config; Empty rows = %s\n" % conf.empty_rows)
	write(u"Config; Empty strings = %s\n" % conf.empty_strings)
	write(u"Config: Trim_strings = %s\n" % conf.trim_strings)
	write(u"Config: Replace_newlines = %s\n" % conf.replace_newlines)
	write(u"Config; Only strings = %s\n" % conf.only_strings)
	write(u"Config; Scan lines = %s\n" % conf.scan_lines)
	write(u"Config: Import row buffer size = %s\n" % conf.import_row_buffer)
	write(u"Config; Write warnings to console = %s\n" % conf.write_warnings)
	write(u"Config; Write prefix = %s\n" % conf.write_prefix)
	write(u"Config; Write suffix = %s\n" % conf.write_suffix)
	write(u"Config; Log write messages = %s\n" % conf.tee_write_log)
	write(u"Config; Log data variable assignments = %s\n" % conf.log_datavars)
	write(u"Config; GUI level = %s\n" % conf.gui_level)
	write(u"Config; CSS file for HTML export = %s\n" % conf.css_file)
	write(u"Config; CSS styles for HTML export = %s\n" % conf.css_styles)
	write(u"Config; Make export directories = %s\n" % conf.make_export_dirs)
	write(u"Config: Quote all text on export = %s\n" % conf.quote_all_text)
	write(u"Config: Export row buffer size = %s\n" % conf.export_row_buffer)
	write(u"Config; Text length for HDF5 export = %s\n" % conf.hdf5_text_len)
	write(u"Config; Template processor = %s\n" % conf.template_processor)
	write(u"Config; SMTP host = %s\n" % conf.smtp_host)
	write(u"Config; SMTP port = %s\n" % conf.smtp_port)
	write(u"Config; SMTP username = %s\n" % conf.smtp_username)
	write(u"Config; SMTP use SSL = %s\n" % conf.smtp_ssl)
	write(u"Config; SMTP use TLS = %s\n" % conf.smtp_tls)
	write(u"Config; Email format = %s\n" % conf.email_format)
	write(u"Config; Email CSS = %s\n" % conf.email_css)
	write(u"Config; Zip buffer size (Mb) = %s\n" % conf.zip_buffer_mb)
	write(u"Config; DAO flush delay (seconds) = %s\n" % conf.dao_flush_delay_secs)
	write(u"Config; Configuration files read = %s\n" % ", ".join(conf.files_read))
...
def x_serve(**kwargs):
	...
	if not os.path.isfile(infname):
		raise ErrInfo(type="cmd", command_text=kwargs['metacommandline'], other_msg='Input file %s does not exist' % infname)
...
metacommandlist.add(ins_fn_rxs(r'^\s*SERVE\s+',
			r'\s+AS\s+(?P<format>BINARY|CSV|TXT|TEXT|ODS|JSON|HTML|PDF|ZIP)\s*$'), x_serve)


#****	RESET DIALOG_CANCELED
def x_reset_dialog_canceled(**kwargs):
	status.dialog_canceled = False

metacommandlist.add(r'^\s*RESET\s+DIALOG_CANCELED\s*$', x_reset_dialog_canceled)



#****	SUB_QUERYSTRING
def x_sub_querystring(**kwargs):
	qstr = kwargs['qstr']
	sublist = parse_qsl(qstr)
	global subvars
	for sub, value in sublist:
		subvars.add_substitution(sub, value)
	return None

metacommandlist.add(r'^\s*SUB_QUERYSTRING\s+(?P<qstr>.+)\s*$', x_sub_querystring)
...
def x_break(**kwargs):
	...
	if len(commandliststack) == 1:
		...
		write_warning("BREAK metacommand with no command nesting on line %s of %s" % (line_no, src))
	else:
...
def x_export_query(**kwargs):
	...
	if filefmt == 'txt' or filefmt == 'text':
		...
		write_query_to_cgi_html(select_stmt, dbs.current(), outfile, append, desc=description, zipfile=zipfilename)
	elif filefmt == 'latex':
...
metacommandlist.add(ins_fn_rxs(r'^\s*EXPORT\s+QUERY\s+<<\s*(?P<query>.*;)\s*>>\s+(?P<tee>TEE\s+)?(?P<append>APPEND\s+)?TO\s+', 
		ins_fn_rxs(
			r'(?:\s+IN\s+ZIPFILE\s+',
			r')?\s+AS\s*(?P<format>CSV|TAB|TSV|TABQ|TSVQ|UNITSEP|US|TXT|TXT-ND|PLAIN|ODS|JSON|HTML|CGI-HTML|VALUES|LATEX|RAW|B64|FEATHER)(?:\s+DESCRIP(?:TION)?\s+"(?P<description>[^"]*)")?\s*$', symbolicname='zipfilename'
			)),
...
def x_import_ods_pattern(**kwargs):
	...
	subvars.add_substitution("$SHEETS_IMPORTED", ",".join(impsheets))
	subvars.add_substitution("$SHEETS_TABLES", ",".join(tables))
	if schemaname is None:
		subvars.add_substitution("$SHEETS_TABLES_VALUES", ",".join(["('%s')" % t for t in tables]))
	else:
		subvars.add_substitution("$SHEETS_TABLES_VALUES", ",".join(["('"+schemaname+".%s')" % t for t in tables]))
	return None
...
def x_cd(**kwargs):
	...
	os.chdir(new_dir)
	script, lno = current_script_line()
	exec_log.log_status_info("Current directory changed to %s at line %d of %s" % (new_dir, lno, script))
	return None
...
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+ACTION\s+', ins_table_rxs(r'\s+MESSAGE\s+"(?P<message>(.|\n)*)"(?:\s+DISPLAY\s+', r')?(?:\s+COMPACT\s+(?P<compact>\d+))?(?:\s+(?P<continue>CONTINUE))?(?:\s+HELP\s+(?P<help>[^\s]+))?\s*$', suffix="disp")),
		x_prompt_action)
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+ACTION\s+', ins_table_rxs(r'\s+MESSAGE\s+"(?P<message>(.|\n)*)"(?:\s+DISPLAY\s+', r')?(?:\s+COMPACT\s+(?P<compact>\d+))?(?:\s+(?P<continue>CONTINUE))?(?:\s+HELP\s+"(?P<help>[^"]+)")?\s*$', suffix="disp")),
		x_prompt_action)
...
metacommandlist.add(ins_fn_rxs(r'^\s*PROMPT\s+SAVEFILE\s+SUB\s+(?P<match>~?\w+)(?:\s+(?P<fn_match>~?\w+))?(?:\s+(?P<path_match>~?\w+))?(?:\s+(?P<ext_match>~?\w+))?(?:\s+(?P<fnbase_match>~?\w+))?(?:\s+FROM\s+', r')?\s*$', symbolicname="startdir"), x_prompt_savefile)
...
metacommandlist.add(ins_fn_rxs(r'^\s*PROMPT\s+DIRECTORY\s+SUB\s+(?P<match>~?\w+)(?:\s+(?P<fullpath>FULLPATH))?(?:\s+FROM\s+', r')?\s*$', symbolicname="startdir"), x_prompt_directory)
...
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+SELECT_ROWS\s+FROM\s+', ins_table_rxs(r'(?:\s+IN\s+(?P<alias1>\w+))?\s+INTO\s+', r'(?:\s+IN\s+(?P<alias2>\w+))?(?:\s+HELP\s+(?P<help>[^\s]+))?\s+MESSAGE\s+"(?P<msg>(.|\n)*)"\s*$', suffix='2'), suffix='1'), x_prompt_select_rows)
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+SELECT_ROWS\s+FROM\s+', ins_table_rxs(r'(?:\s+IN\s+(?P<alias1>\w+))?\s+INTO\s+', r'(?:\s+IN\s+(?P<alias2>\w+))?(?:\s+HELP\s+"(?P<help>[^"]+)")?\s+MESSAGE\s+"(?P<msg>(.|\n)*)"\s*$', suffix='2'), suffix='1'), x_prompt_select_rows)
...
#****	PROMPT CREDENTIALS
def x_prompt_credentials(**kwargs):
	message = kwargs["message"]
	user = kwargs["user"]
	pw = kwargs["pw"]
...
#****	WRITE_PREFIX
def x_write_prefix(**kwargs):
	pf = kwargs['prefix']
	if pf.lower() == 'clear':
		conf.write_prefix = None
	else:
		conf.write_prefix = pf
	return None

metacommandlist.add(r'^\s*CONFIG\s+WRITE_PREFIX\s+(?P<prefix>.*)\s*$', x_write_prefix)


#****	WRITE_SUFFIX
def x_write_suffix(**kwargs):
	sf = kwargs['suffix']
	if sf.lower() == 'clear':
		conf.write_suffix = None
	else:
		conf.write_suffix = sf
	return None

metacommandlist.add(r'^\s*CONFIG\s+WRITE_SUFFIX\s+(?P<suffix>.*)\s*$', x_write_suffix)
...
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+ENTER_SUB\s+(?P<match_str>~?\w+)\s+(?:(?P<password>PASSWORD)\s+)?MESSAGE\s+"(?P<message>([^"]|\n)*)"(?:\s+DISPLAY\s+', r')?(?:\s+TYPE\s+(?P<type>INT|FLOAT|BOOL|IDENT))?(?:\s+(?P<case>LCASE|UCASE))?(?:\s+INITIALLY\s+"(?P<initial>[^"]+)")?(?:\s+HELP\s+(?P<help>[^\s]+))?\s*$'),
		x_prompt_enter)
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+ENTER_SUB\s+(?P<match_str>~?\w+)\s+(?:(?P<password>PASSWORD)\s+)?MESSAGE\s+"(?P<message>([^"]|\n)*)"(?:\s+DISPLAY\s+', r')?(?:\s+TYPE\s+(?P<type>INT|FLOAT|BOOL|IDENT))?(?:\s+(?P<case>LCASE|UCASE))?(?:\s+INITIALLY\s+"(?P<initial>[^"]+)")?(?:\s+HELP\s+"(?P<help>[^+]+)")?\s*$'),
		x_prompt_enter)
...
def x_prompt_entryform(**kwargs):
	...
	for r in spec_rows:
		...
		if 'form_column' in colhdrs:
			...
			if entry_col:
				try:
					...
					raise ErrInfo(type="cmd", command_text=kwargs['metacommandline'], other_msg="Entry column %s is not an integer" % entry_height)
				if entry_col < 1:
					entry_col = 1
		...
		entry_list.append(EntrySpec(subvar, prompt_msg, required=bool(v.get('required')), initial_value=initial_value,
							default_width=entry_width, default_height=entry_height, lookup_list=lookups, form_column=entry_col,
...
metacommandlist.add(
	ins_table_rxs(r'^\s*PROMPT\s+ENTRY_FORM\s+', ins_table_rxs(r'(?:\s+HELP\s+(?P<help>[^\s]+))?\s+MESSAGE\s+"(?P<message>(.|\n)*)"(?:\s+DISPLAY\s+', r')?\s*$', suffix="disp")),
	x_prompt_entryform)
metacommandlist.add(
	ins_table_rxs(r'^\s*PROMPT\s+ENTRY_FORM\s+', ins_table_rxs(r'(?:\s+HELP\s+"(?P<help>[^"]+)")?\s+MESSAGE\s+"(?P<message>(.|\n)*)"(?:\s+DISPLAY\s+', r')?\s*$', suffix="disp")),
	x_prompt_entryform)
...
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+SELECT_SUB\s+', r'\s+MESSAGE\s+"(?P<msg>(.|\n)*)"(?:\s+(?P<cont>CONTINUE))?(?:\s+HELP\s(?P<help>[^\s]+))?\s*$'), x_prompt_selectsub)
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+SELECT_SUB\s+', r'\s+MESSAGE\s+"(?P<msg>(.|\n)*)"(?:\s+(?P<cont>CONTINUE))?(?:\s+HELP\s"(?P<help>[^"]+)")?\s*$'), x_prompt_selectsub)
...
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+COMPARE\s+', ins_table_rxs(r'(?:\s+IN\s+(?P<alias1>\w+))?\s+(?P<orient>AND|BESIDE)\s+', r'(?:\s+IN\s+(?P<alias2>\w+))?\s+(?:PK|KEY)\s*\((?P<pks>(("[A-Z_0-9]+")|[A-Z_0-9]+)(\s*,\s*(("[A-Z_0-9]+")|[A-Z_0-9]+))*)\)(?:\s+HELP\s+(?P<help>[^\s]+))?\s+MESSAGE\s+"(?P<msg>(.|\n)*)"\s*$', suffix='2'), suffix='1'), x_prompt_compare)
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+COMPARE\s+', ins_table_rxs(r'(?:\s+IN\s+(?P<alias1>\w+))?\s+(?P<orient>AND|BESIDE)\s+', r'(?:\s+IN\s+(?P<alias2>\w+))?\s+(?:PK|KEY)\s*\((?P<pks>(("[A-Z_0-9]+")|[A-Z_0-9]+)(\s*,\s*(("[A-Z_0-9]+")|[A-Z_0-9]+))*)\)(?:\s+HELP\s+"(?P<help>[^"]+)")?\s+MESSAGE\s+"(?P<msg>(.|\n)*)"\s*$', suffix='2'), suffix='1'), x_prompt_compare)
...
metacommandlist.add(
	ins_table_rxs(r'^\s*PROMPT\s+ASK\s+"(?P<msg>(.|\n)*)"\s+SUB\s+(?P<match>~?\w+)\s+COMPARE\s+', ins_table_rxs(r'(?:\s+IN\s+(?P<alias1>\w+))?\s+(?P<orient>AND|BESIDE)\s+', r'(?:\s+IN\s+(?P<alias2>\w+))?\s+(?:PK|KEY)\s*\((?P<pks>(("[A-Z_0-9]+")|[A-Z_0-9]+)(\s*,\s*(("[A-Z_0-9]+")|[A-Z_0-9]+))*)\)(?:\s+HELP\s+(?P<help>[^\s]+))?\s*$', suffix='2'), suffix='1'), x_prompt_ask_compare)
metacommandlist.add(
	ins_table_rxs(r'^\s*PROMPT\s+ASK\s+"(?P<msg>(.|\n)*)"\s+SUB\s+(?P<match>~?\w+)\s+COMPARE\s+', ins_table_rxs(r'(?:\s+IN\s+(?P<alias1>\w+))?\s+(?P<orient>AND|BESIDE)\s+', r'(?:\s+IN\s+(?P<alias2>\w+))?\s+(?:PK|KEY)\s*\((?P<pks>(("[A-Z_0-9]+")|[A-Z_0-9]+)(\s*,\s*(("[A-Z_0-9]+")|[A-Z_0-9]+))*)\)(?:\s+HELP\s+"(?P<help>[^"]+)")?\s*$', suffix='2'), suffix='1'), x_prompt_ask_compare)


#****	PROMPT MESSAGE MAP
def x_prompt_map(**kwargs):
	db = dbs.current()
	schema = kwargs["schema"]
	table = kwargs["table"]
	message = kwargs["message"]
	...
	gui_args = {"title": table,
				 ...
				 "color_col": color_col}
	gui_manager_queue.put(GuiSpec(GUI_MAP, gui_args, return_queue))
	user_response = return_queue.get(block=True)
	btn = user_response["button"]
	if not btn:
		if status.cancel_halt:
			msg = u"Halted from map of %s" % sq_name
			exec_log.log_exit_halt(script, line_no, msg)
			exit_now(2, None)
	return None

metacommandlist.add(
	ins_table_rxs(r'^\s*PROMPT\s+MESSAGE\s+"(?P<message>(.|\n)*)"\s+MAP\s+', r'\s*LAT\s+(?P<lat_col>\w+)\s+LON\s+(?P<lon_col>\w+)'
					r'(?:\s+LABEL\s+(?P<label_col>\w+))?(?:\s+COLOR\s+(?P<color_col>\w+))?(?:\s+SYMBOL\s+(?P<symbol_col>\w+))?\s*$'), x_prompt_map)
...
#****	EXPORT_METADATA INTO TABLE
def x_export_metadata_table(**kwargs):
	xall = kwargs['all'] is not None
	schemaname = kwargs['schema']
	tablename = kwargs['table']
	newstr = kwargs['new']
	...
	if xall:
		...
		hdrs, rows = export_metadata.get()
	import_data_table(dbs.current(), schemaname, tablename, is_new, hdrs, rows)

metacommandlist.add(
	ins_table_rxs(r'^\s*EXPORT_METADATA(?:\s+(?P<all>ALL))?\s+INTO(?:\s+(?P<new>NEW|REPLACEMENT))?\s+TABLE\s+', r'\s*$'),
...
def x_boolean_words(**kwargs):
	flag = kwargs['yesno'].lower()
	conf.boolean_words = flag in ('yes', 'on', 'true', '1')
	return None
...
#****	TRIM_COLUMN_HEADERS
def x_trim_col_hdrs(**kwargs):
	conf.trim_col_hdrs = kwargs['which'].lower()
	return None

metacommandlist.add(r'^\s*CONFIG\s+TRIM_COLUMN_HEADERS\s+(?P<which>NONE|BOTH|LEFT|RIGHT)\s*$', x_trim_col_hdrs)
...
def x_del_empty_cols(**kwargs):
	flag = kwargs['yesno'].lower()
	conf.del_empty_cols = flag in ('yes', 'on', 'true', '1')
	return None

metacommandlist.add(r'^\s*CONFIG\s+DELETE_EMPTY_COLUMNS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE|0|1)\s*$', x_del_empty_cols)
...
#****	CONNECT to DuckDB
def x_connect_duckdb(**kwargs):
	db_file = unquoted2(kwargs['filename'])
	mk_new = kwargs['new']
	mk_new = unquoted2(mk_new).lower() == 'new' if mk_new else False
	# The duckdb library will automatically create a new database if the referenced one does not exist.
	...
	if not mk_new and not os.path.exists(db_file):
		raise ErrInfo(type="cmd", command_text=kwargs['metacommandline'], other_msg='DuckDB file does not exist.')
	if mk_new:
		check_dir(db_file)
...
metacommandlist.add(
	ins_fn_rxs(r'^CONNECT\s+TO\s+DUCKDB\s*\(\s*FILE\s*=\s*', r'(?:\s*,\s*(?P<new>NEW))?\s*\)\s+AS\s+(?P<db_alias>[A-Z][A-Z0-9_]*)\s*$'), x_connect_duckdb)
...
def x_use(**kwargs):
	...
	subvars.add_substitution("$DB_SERVER", dbs.aliased_as(db_alias).server_name)
	return None
...
metacommandlist.add(ins_table_rxs(r"^\s*PROMPT\s+ASK\s+'(?P<question>[^']+)'\s+SUB\s+(?P<match>~?\w+)(?:\s+DISPLAY\s+", r')?(?:\s+HELP\s+(?P<help>[^\s]+))?\s*$'), x_prompt_ask)
metacommandlist.add(ins_table_rxs(r"^\s*PROMPT\s+ASK\s+'(?P<question>[^']+)'\s+SUB\s+(?P<match>~?\w+)(?:\s+DISPLAY\s+", r')?(?:\s+HELP\s+"(?P<help>[^"]+)")?\s*$'), x_prompt_ask)
metacommandlist.add(ins_table_rxs(r'^\s*PROMPT\s+ASK\s+\[(?P<question>[^\]]+)\]\s+SUB\s+(?P<match>~?\w+)(?:\s+DISPLAY\s+', r')?(?:\s+HELP\s+(?P<help>[^\s]+))?\s*$'), x_prompt_ask)
metacommandlist.add(ins_table_rxs(r'^\s*PROMPT\s+ASK\s+\[(?P<question>[^\]]+)\]\s+SUB\s+(?P<match>~?\w+)(?:\s+DISPLAY\s+', r')?(?:\s+HELP\s+"(?P<help>[^"]+)")?\s*$'), x_prompt_ask)
metacommandlist.add(ins_table_rxs(r'^\s*PROMPT\s+ASK\s+"(?P<question>[^"]+)"\s+SUB\s+(?P<match>~?\w+)(?:\s+DISPLAY\s+', r')?(?:\s+HELP\s+(?P<help>[^\s]+))?\s*$'), x_prompt_ask)
metacommandlist.add(ins_table_rxs(r'^\s*PROMPT\s+ASK\s+"(?P<question>[^"]+)"\s+SUB\s+(?P<match>~?\w+)(?:\s+DISPLAY\s+', r')?(?:\s+HELP\s+"(?P<help>[^"]+)")?\s*$'), x_prompt_ask)
...
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+MESSAGE\s+"(?P<message>(.|\n)*)"\s+DISPLAY\s+', r'(?:\s+HELP\s+(?P<help>[^\s]+))?(?:\s+(?P<free>FREE))?\s*$'), x_prompt)
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+MESSAGE\s+"(?P<message>(.|\n)*)"\s+DISPLAY\s+', r'(?:\s+HELP\s+"(?P<help>[^"]+)")?(?:\s+(?P<free>FREE))?\s*$'), x_prompt)
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+DISPLAY\s+', r'\s+MESSAGE\s+"(?P<message>(.|\n)*)"(?:\s+HELP\s+(?P<help>[^\s]+))?(?:\s+(?P<free>FREE))?\s*$'), x_prompt)
metacommandlist.add(
		ins_table_rxs(r'^\s*PROMPT\s+DISPLAY\s+', r'\s+MESSAGE\s+"(?P<message>(.|\n)*)"(?:\s+HELP\s+"(?P<help>[^"]+)")?(?:\s+(?P<free>FREE))?\s*$'), x_prompt)
...
def x_export(**kwargs):
	...
	if filefmt == 'txt' or filefmt == 'text':
		...
		write_query_to_duckdb(select_stmt, dbs.current(), outfile, append, tablename=queryname)
	elif filefmt == 'sqlite':
		write_query_to_sqlite(select_stmt, dbs.current(), outfile, append, tablename=queryname)
	elif filefmt == 'xml':
...
metacommandlist.add(ins_table_rxs(
	r'^\s*EXPORT\s+', ins_fn_rxs(
		ins_fn_rxs(
			...
			r'(?:\s+IN\s+ZIPFILE\s+'),
		#r')?\s+AS\s+(?P<format>CSV|TAB|TSV|TABQ|TSVQ|UNITSEP|US|TXT|TXT-ND|PLAIN|ODS|JSON|XML|VALUES|HTML|CGI-HTML|LATEX|RAW|B64|FEATHER|HDF5)(?:\s+DESCRIP(?:TION)?\s+"(?P<description>[^"]*)")?\s*$', symbolicname='zipfilename')
		r')?\s+AS\s+(?P<format>CSV|TAB|TSV|TABQ|TSVQ|UNITSEP|US|TXT|TXT-ND|PLAIN|JSON|XML|VALUES|HTML|CGI-HTML|SQLITE|DUCKDB|LATEX|RAW|B64|FEATHER|HDF5)(?:\s+DESCRIP(?:TION)?\s+"(?P<description>[^"]*)")?\s*$', symbolicname='zipfilename')
		),
...
#****	EXPORT multiple tables to ODS
def x_export_ods_multiple(**kwargs):
	...
	check_dir(outfile)
	write_queries_to_ods(table_list, dbs.current(), outfile, append, tee, desc=description)

metacommandlist.add(ins_table_list_rxs(
	r'^\s*EXPORT\s+', 
		ins_fn_rxs(
			r'\s+(?P<tee>TEE\s+)?(?P<append>APPEND\s+)?TO\s+', 
		...
		x_export_ods_multiple, "Write data from tables or views to an ODS file.")
...
def x_write(**kwargs):
	...
	outf = kwargs['filename']
	global conf
	if conf.write_prefix is not None:
		msg = substitute_vars(conf.write_prefix) + " " + msg
	if conf.write_suffix is not None:
		msg = msg[:-1] + " " + substitute_vars(conf.write_suffix) + "\n"
	if outf:
		check_dir(outf)
...
def write_query_to_feather(outfile, headers, rows):
	try:
		...
		raise ErrInfo("exception", exception_msg=exception_desc(), other_msg=u"The pandas and pyarrow Python packages must be installed to export data to the feather format.")
...
def write_query_to_duckdb(select_stmt, db, outfile, append, tablename):
	try:
		hdrs, rows = db.select_rowsource(select_stmt)
	except ErrInfo:
		raise
...
def write_query_to_sqlite(select_stmt, db, outfile, append, tablename):
	try:
		hdrs, rows = db.select_rowsource(select_stmt)
	except ErrInfo:
		raise
...
def write_queries_to_ods(table_list, db, outfile, append=False, tee=False, desc=None):
	...
	inventory_name = u"Datasheets"
	if not inventory_name in wbk.sheetnames():
		inventory_sheet = wbk.new_sheet(inventory_name)
		wbk.add_row_to_sheet(('datasheet_name', 'created_on', 'created_by', 'description', 'source'), inventory_sheet, header=True)
		wbk.add_sheet(inventory_sheet)
	for i, t in enumerate(tables):
		if "." in t:
			st = t.split(".")
			if len(st) != 2:
				raise ErrInfo("cmd", other_msg="Unrecognized table specification in <%s>" % t)
			if len(st) == 1:
		...
		while True:
			if sheet_name not in sheet_names:
				break
			sheet_name = "%s_%s" % (tblname, sheet_no)
			sheet_no += 1
		# Get the data
		select_stmt = "select * from %s;" % t
		try:
			hdrs, rows = db.select_rowsource(select_stmt)
		except ErrInfo:
			raise
		...
		export_metadata.add(ExportRecord(queryname=select_stmt, outfile=outfile, zipfile=None, description=d))
		sheet_no += 1
...
def export_duckdb(outfile, hdrs, rows, append, tablename):
	global duckdb
	try:
		import duckdb
	except:
		fatal_error(u"The duckdb module is required to export data in that format.")
	chunksize = 10000
	...
	if pre_exist:
		...
		curs = ddb.cursor()
		res = curs.execute("select count(*) as rows from information_schema.tables where table_catalog = '%s' and table_name = '%s';" % (catalog, tablename))
		rv = res.fetchone()
		if not (rv is None or rv[0] == 0):
			if append:
				# Don't replace an existing table of the same name
				raise ErrInfo(type="error", other_msg="The table %s already exists in %s." % (tablename, outfile))
			else:
				exec_log.log_status_info("Dropping table %s in %s before re-creating it by export" % (tablename, outfile))
	...
	sql = u"insert into %s (%s) values (%s);" % (tablename, colspec, paramspec)
	n_chunks = math.ceil(len(rowdata)/chunksize)
	#ddb.autocommit(False)
...
def export_sqlite(outfile, hdrs, rows, append, tablename):
	global sqlite3
	import sqlite3
	chunksize = 10000
	pre_exist = os.path.isfile(outfile)
	...
	if pre_exist:
		...
		catalog = os.path.splitext(os.path.split(outfile)[1])[0]
		curs = sdb.cursor()
		res = curs.execute("select name from sqlite_master where type='table' and name='%s';" % tablename)
		rv = res.fetchone()
		if not (rv is None or rv[0] == 0):
			if append:
				# Don't replace an existing table of the same name
...
def export_cgi_html(outfile, hdrs, rows, append=False, querytext=None, desc=None, zipfile=None):
	global conf
	def write_table(f):
		f.write(u'<table>\n')
		if desc is not None:
	...
	if zipfile or not append or (append and not os.path.isfile(outfile)):
		if zipfile is None:
			...
			f = ZipWriter(zipfile, outfile, append)
		f.write(u'Content-Type: text/html\n\n')
		write_table(f)
		if outfile.lower() != 'stdout':
			f.close()
	else:
...
def write_query_to_cgi_html(select_stmt, db, outfile, append=False, desc=None, zipfile=None):
	try:
		hdrs, rows = db.select_rowsource(select_stmt)
	except ErrInfo:
		raise
...
def export_latex(outfile, hdrs, rows, append=False, querytext=None, desc=None, zipfile=None):
	...
	def write_table(f):
		...
		f.write(u'  \\hline\n')
		f.write(u'  ' + u' & '.join([h.replace(u'_', r'\_') for h in hdrs]) + u' \\\\\n')
		f.write(u'  \\hline\n')
		for r in rows:
			f.write(u'  ' + u' & '.join([type(u"")(c).replace(u'_', r'\_') for c in r]) + u' \\\\\n')
		f.write(u'  \\hline\n')
...
def xls_data(filename, sheetname, junk_header_rows, encoding=None):
	...
	if len(alldata) == 1:
		return alldata[0], []
	colhdrs = alldata[0]
	if any([x is None or (isinstance(x, stringtypes) and len(x.strip())==0) for x in colhdrs]):
		if conf.del_empty_cols:
			blanks = [i for i in range(len(colhdrs)) if colhdrs[i] is None or len(colhdrs[i].strip())==0]
			while len(blanks) > 0:
				b = blanks.pop()
...
def import_feather(db, schemaname, tablename, filename, is_new):
	try:
		import numpy as np
		import pandas as pd
		import pyarrow.feather
	except:
		raise ErrInfo("exception", exception_msg=exception_desc(), other_msg=u"The pandas and pyarrow Python libraries must be installed to import data from the Feather format.")
...
def import_parquet(db, schemaname, tablename, filename, is_new):
	try:
		import numpy as np
		import pandas as pd
	except:
		raise ErrInfo("exception", exception_msg=exception_desc(), other_msg=u"The pandas and fastparquet or pyarrow Python libraries must be installed to import data from the Parquet format.")
...
def gui_credentials(message, username, pwtext, cmd=None):
	enable_gui()
	return_queue = queue.Queue()
	gui_args = {"message": message}
	gui_manager_queue.put(GuiSpec(GUI_CREDENTIALS, gui_args, return_queue))
	user_response = return_queue.get(block=True)
	exit_status = user_response["exit_status"]
	user = user_response["user"]
	pw = user_response["pw"]
	if exit_status == 1:
		subvars.add_substitution(username, user)
		subvars.add_substitution(pwtext, pw)
	else:
		if status.cancel_halt:
			msg = u"Halted from prompt for database credentials"
			script, lno = current_script_line()
			if script is None:
				exec_log.log_exit_halt(script, lno, msg)
			else:
...
def gui_connect(alias, message, help_url=None, cmd=None):
	dbs.disconnect(alias)
	def connection_pg():
...
def db_DuckDB(duckdb_fn, new_db=False, encoding=None):
	if new_db:
		check_dir(duckdb_fn)
	else:
		if not os.path.exists(duckdb_fn):
			raise ErrInfo(type="error", other_msg=u'DuckDB database file "%s" does not exist.' % duckdb_fn)
...
def clparser():
  ...
  or
       %prog [options] <SQL_script_file>
The first form is used with PostgreSQL, Microsoft SQL Server, MariaDB, MySQL, Firebird, and Oracle;
the second form is used with SQLite, DuckDB, and Microsoft Access.  The third form is used when
database connection parameters are specified in an 'execsql.conf' file.
Arguments:
  ...
  Database_name        The name of the client-server database to use."""
	...
	parser.add_option("-l", "--user_logfile", action="store_true", dest="user_logfile",
						default=False,
						help="Use execsql.log in the user's home directory.")
	...
	parser.add_option("-t", "--type", type="choice", choices=['a', 'd', 'p', 's', 'l', 'm', 'k', 'o', 'f'], dest="db_type",
						default=None,
						help="Database type: 'a'-MS-Access; 'p'-PostgreSQL; 's'-SQL Server; 'l'-SQLite, 'm'-MySQL/MariaDB, 'k'-DuckDB, 'o'-Oracle, 'f'-Firebird, 'd'-DSN.  Default (without this option) is 'a'.")
...
# Subprocess objects for asynchronous writing to text files.  Initialized in main().
filewriter = None
fw_input = multiprocessing.Queue()
fw_output = multiprocessing.Queue()
...
def main():
	...
	if opts.online_help:
		import webbrowser
		webbrowser.open("http://execsql.readthedocs.io/en/latest/", new=2, autoraise=True)
	if args is None:
		parser.print_help()
		sys.exit(0)
	if len(args) == 0:
	...
	# Create subprocess for the WRITE metacommand
	global filewriter
	filewriter = FileWriter(fw_input, fw_output, file_encoding=conf.output_encoding, open_timeout=conf.outfile_open_timeout)
	filewriter.start()
	...
	global exec_log
	exec_log = Logger(script_name, conf.db, conf.server, opts_used, conf.user_logfile)
	...
	exec_log.log_status_info(u"System user: %s" % getpass.getuser())
	...
	if conf.server is None and conf.db is None and conf.db_file is None:
		...
		# Use DuckDB
			...
			if conf.db_file is None:
				fatal_error(u"Configured to run with DuckDB, but no DuckDB file name is provided.")
			db = db_DuckDB(conf.db_file, new_db=conf.new_db, encoding=conf.db_encoding)
		# Use Oracle
		elif conf.db_type == 'o':
	...
	subvars.add_substitution("$DB_SERVER", db.server_name)
