...
from ._mutagen_handler import get_all_mp3_id3_tags
...
class MusicChecker:
    ...
    def __init__(self, library: str = 'Library', media_list: list | None = None, use_mm: bool = False, cache_dir: str = None, force_recheck: bool = False):
        self.use_mm = use_mm
    ...
    def _get_entry_tags(self, entry: mf):
        ...
        if int(exists[0]) == 0 or self.force_recheck is True:
            entry.tags = get_all_mp3_id3_tags(
                os.path.join(
                    entry.path,
                    entry.file_name
                )
    ...
    def execute_checker(self) -> list[mf]:
