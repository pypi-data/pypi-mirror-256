...
from src.music_checker_micro._database import db_entry_exists, instantiate_sqlite_table
...
def test_all_entries():
    ...
    single_file = mc.execute_checker()
    ...
    mm.reset_library("###TESTING###")
    multi_mp3 = mm.execute("###TESTING###", "./tests/multi_sample")
...
def test_checker_use_mm():
    mm = MM("###TESTING###")
    mm.reset_library("###TESTING###")
    zero_mp3 = mm.execute("###TESTING###", "./tests/empty_folder")
    assert len(zero_mp3) == 0
    single_mp3 = mm.execute("###TESTING###", "./tests/sample_mp3")
    assert len(single_mp3) == 1
    mc = MC(library="###TESTING###")
    media_list = mc.execute_checker()
