...
import shlex
import functools
from ....utils.str import str_format_var
...
class MarkerBase:
    ...
    def translate(self, env, s):
        def missing(key):
            if key.startswith(self.trans_marker_command):
                return shell_output(key[len(self.trans_marker_command):])
            elif key.startswith(self.trans_marker_eval):
                return self.eval(env, key[len(self.trans_marker_eval):])
            elif key.startswith(self.trans_marker_env):
                return os.getenv(key[len(self.trans_marker_env):])
            elif key.startswith(self.trans_marker_ignore):
                try:
                    return self.eval(env, key[len(self.trans_marker_ignore):])
                except NameError:
                    return ''
            else:
                return f'{{{key}}}'

        fx, args = str_format_var(s)
