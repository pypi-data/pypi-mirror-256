...
class DebugLogger:
    ...
    def log_http_object(request_log_entry, response_log_entry):
        ...
        try:
            ...
            # Print the log entry
            original_print(f'<HTTP>{formatted_entry}</HTTP>')
    ...
    def log_function_call(log_entry, function_name=None, app_name=None):
        ...
        try:
            ...
            original_print(f'<DEBUG>{formatted_entry}</DEBUG>')
            
        except Exception as e:
            pass

    @staticmethod
    def from_strada_response(input: dict, response: StradaResponse, function_name=None, app_name=None, latency_ms=None):
        try:
            log_entry = {}
...
def with_debug_logs(app_name):
    def inner_decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            DebugLogger.from_strada_response(input=kwargs, response=result, function_name=self.function_name, app_name=app_name)
