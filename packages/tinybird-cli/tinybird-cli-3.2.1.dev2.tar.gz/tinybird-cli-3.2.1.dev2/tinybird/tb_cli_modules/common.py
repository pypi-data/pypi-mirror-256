...
async def get_current_workspace_branches(config: CLIConfig) -> List[Dict[str, Any]]:
    current_main_workspace: Optional[Dict[str, Any]] = await get_current_main_workspace_temp(config)
        ...
        if not base.exists() or force:
            with open(f"{f}", "w") as t_file:
                t_file.write(
                    """
##############################################################################################################################
###   Visit https://versions.tinybird.co/docs/version-control/implementing-test-strategies.html#data-quality-tests-title   ###
        ...
        if not base.exists() or force:
            with open(f"{f}", "w") as t_file:
                t_file.write(
                    """
############################################################################################################################
###   Visit https://versions.tinybird.co/docs/version-control/implementing-test-strategies.html#regression-tests-title   ###
...
async def create_workspace_branch(
    ...
    try:
        workspace = await get_current_workspace_temp(config)
        if not workspace:
...
async def print_branch_regression_tests_summary(client, job_id, host, response=None):
    ...
    if failed:
        raise CLIException(
            "Check Failures Detail above for more information. If the results are expected, skip asserts or increase thresholds, see 💡 Hints above (note skip asserts flags are applied to all regression tests, so use them when it makes sense).\n\nIf you are using the CI template for GitHub or GitLab you can add skip asserts flags as labels to the MR and they are automatically applied. Find available flags to skip asserts and thresholds here => https://versions.tinybird.co/docs/version-control/implementing-test-strategies.html#fixture-tests"
        )
    ...
    for workspace in response["workspaces"]:
        if config["id"] == workspace["id"]:
            ...
            if workspace.get("is_branch"):
                name = workspace["name"]
                main_workspace = await get_current_main_workspace_temp(config)
...
async def get_host_from_region_temp(
    ...
    try:
        regions = await get_regions_temp(config)
...
async def get_regions_temp(config: CLIConfig) -> List[Region]:
    """Pending to remove the _temp suffix once we fully integrate the class CLIConfig.
    Please, don't work with the previous version any more and make any changes here
    """
...
async def try_update_config_with_remote(
    ...
    try:
        ...
        if raise_on_errors:
            raise CLIAuthException(FeedbackManager.error_exception(error=str(ex)))
        return False
...
async def get_current_main_workspace_temp(config: CLIConfig) -> Optional[Dict[str, Any]]:
    """Pending to remove the _temp suffix once we fully integrate the class CLIConfig.
    Please, don't work with the previous version any more and make any changes here
    """
...
async def print_release_summary(config: CLIConfig, semver: str, info: bool = False, dry_run=False):
    try:
        client = config.get_client()
