# -*- coding: utf-8 -*-
from imio.plausible.testing import IMIO_PLAUSIBLE_FUNCTIONAL_TESTING
from imio.plausible.testing import IMIO_PLAUSIBLE_INTEGRATION_TESTING
from imio.plausible.views.plausible_view import IPlausibleView
from imio.plausible.views.plausible_view import PlausibleView
...
class ViewsIntegrationTest(unittest.TestCase):
    ...
    def set_registry_records(self):
        api.portal.set_registry_record("imio.plausible.site", "site-registry.be")
        api.portal.set_registry_record("imio.plausible.token", "token-registry")
        api.portal.set_registry_record("imio.plausible.url", "url-registry.be")

    @mock.patch.dict(
        os.environ,
        {
            "IMIO_PLAUSIBLE_SITE": "",
            "IMIO_PLAUSIBLE_TOKEN": "",
            "IMIO_PLAUSIBLE_URL": "",
        },
    )
    def test_noenv(self):
    ...
    @mock.patch.dict(
        os.environ,
        {
            "IMIO_PLAUSIBLE_SITE": "site-varenv.be",
            "IMIO_PLAUSIBLE_TOKEN": "token-varenv",
            "IMIO_PLAUSIBLE_URL": "url-varenv.be",
        },
    )
    def test_env(self):
        view = PlausibleView(self.portal, self.request)
...
class ViewsFunctionalTest(unittest.TestCase):
    ...
    layer = IMIO_PLAUSIBLE_FUNCTIONAL_TESTING
