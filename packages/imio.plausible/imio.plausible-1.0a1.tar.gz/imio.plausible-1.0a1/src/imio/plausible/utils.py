...
def get_plausible_vars():
    env_plausible_url = os.getenv("IMIO_PLAUSIBLE_URL", "")
    env_plausible_site = os.getenv("IMIO_PLAUSIBLE_SITE", "")
    env_plausible_token = os.getenv("IMIO_PLAUSIBLE_TOKEN", "")

    plausible_url = (
        env_plausible_url
        if (env_plausible_url and env_plausible_url != "")
        else api.portal.get_registry_record("imio.plausible.url")
    )
    plausible_site = (
        env_plausible_site
        if (env_plausible_site and env_plausible_site != "")
        else api.portal.get_registry_record("imio.plausible.site")
    )
    plausible_token = (
        env_plausible_token
        if (env_plausible_token and env_plausible_token != "")
        else api.portal.get_registry_record("imio.plausible.token")
    )
