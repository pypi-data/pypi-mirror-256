...
class SalesforceClient:
    ...
    def _get_asset_url(
        ...
        if asset_type == SalesforceReportingAsset.DASHBOARDS:
            return (
                f"{self.instance_url}/lightning/r/Dashboard/{asset['Id']}/view"
            )

        if asset_type == SalesforceReportingAsset.FOLDERS:
            return f"{self.instance_url}/{asset['attributes']['url']}"

        if asset_type == SalesforceReportingAsset.REPORTS:
            return f"{self.instance_url}/lightning/r/Report/{asset['Id']}/view"
    ...
    def fetch(self, asset: SalesforceReportingAsset) -> List[Dict]:
        ...
        if asset in SalesforceReportingAsset:
