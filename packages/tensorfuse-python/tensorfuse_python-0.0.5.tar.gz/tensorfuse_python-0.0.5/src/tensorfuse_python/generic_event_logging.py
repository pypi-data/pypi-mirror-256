...
def log_generic_event_base(endpoint:str, token: str, requestDetails, log, baseUrl: str):
...
def log_generic_event(token: str, log, baseUrl=BASE_URL):
...
def log_generic_event_using_ids(token: str, teamId: str, projectId: str, datasourceId: str,  log, baseUrl=BASE_URL):
    ...
    return log_generic_event_base(endpoint=endpoint, token=token, requestBody=requestDetails, log=log, baseUrl=baseUrl)
