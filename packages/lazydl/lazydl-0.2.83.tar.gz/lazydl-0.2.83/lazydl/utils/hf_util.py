...
def load_model_and_tokenizer(model_name_or_path, load_in_4bit=False, 
                             ...
                             lora_dropout=0.05, device_map='auto', backbone_file=None, params=None):
    """加载模型，支持4bit
    ...
    # 加载base model
    if backbone_file:
        model = l.load_class(backbone_file).from_pretrained(
            model_name_or_path,
            params=params,
...
class Pipeline:
    def __init__(self, 
        ...
        self.bot_role_code = bot_role_code
    ...
    def generate(self, 
        ...
        if self.user_role_code:
            model_responses = [resp.split(self.user_role_code, 1)[0].strip() for resp in model_responses]
