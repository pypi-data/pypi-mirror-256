...
def log_file(file_dir, experiment):
    dir_list = os.listdir(file_dir)
    if "__pycache__" in dir_list:
        dir_list.remove("__pycache__")
    for cur_path in dir_list:
        path = os.path.join(file_dir, cur_path)
        if os.path.isfile(path):
            experiment.log_asset(path, file_name=path)
        if os.path.isdir(path):
            log_file(path, experiment)
...
def init_env(config: Union[DictConfig, dict], current_dir: str = "./") -> None:
    ...
    if config.get("start_comet_log", "False"):
        ...
        if experiment is None:
            ...
            experiment.set_name(config.get("memo", "Blank"))
        ...
        experiment.log_asset(
            config.root_dir + "/configs/experiments/" + experiment_config + ".yaml"
        )
        experiment.log_asset(config.root_dir + "/configs/default_config.yaml")
        log_file(config.root_dir + "/modules", experiment)
        log_file(config.root_dir + "/configs", experiment)
        experiment.log_asset(config.root_dir + "/run.py")
