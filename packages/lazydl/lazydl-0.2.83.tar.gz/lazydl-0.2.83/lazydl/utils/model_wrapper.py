...
class BaseLitModel(L.LightningModule):
    ...
    def training_step(self, batch: Any, batch_idx: int):
        self.progress = round(self.global_step / self.total_steps(), 2)
        outputs = self(**batch)
        ...
        for key in outputs.keys():
            if '_loss' in key:
                ...
                self.log("train/" + key, key_loss, prog_bar=True, logger=True, sync_dist=True, on_step=True,
                         on_epoch=True, rank_zero_only=True)
        ...
        self.log("progress", self.progress, prog_bar=True, logger=True, on_step=True,
                 rank_zero_only=True, sync_dist=True)
        return {
            'loss': outputs['loss'],
        }
    ...
    def on_train_epoch_end(self):
        self.progress = round(self.global_step / self.total_steps(), 2)
        self.log("progress", self.progress)
    ...
    def get_lr_scheduler(self):
        lr_scheduler = self.config.get("lr_scheduler_type", "constant")
        get_schedule_func = arg_to_scheduler[lr_scheduler]
        total_steps = self.total_steps()
        warmup_steps = self.config.get("warmup_steps", 0.05)
        if warmup_steps <= 1:
