...
def cleaner(text):
    ...
    tags = {'<p>' : '{p}', '</p>':'{/p}','<span>': '{span}','</span>':'{/span}', '<br>':'{br}','</br>':'{/br}','<b>':'{b}',
            ...
            '<ol>':'{ol}', '</ol>':'{/ol}',
            '<img':'{img',
            '<blockquote class="twitter-tweet"':'{blockquote class="twitter-tweet"',
            '<iframe_ini': '{iframe_ini',
            '<video': '{video',
            '<rep_tx': '{rep_tx',
            '}>':'}}',

            '</blockquote>': '{/blockquote}', '</iframe_ini>': '{/iframe_ini}', '</video>': '{/video}', '</rep_tx>': '{/rep_tx}'}



    # change a tags name inside blockquote
    clean_text = re.sub(r'<blockquote class="twitter-tweet".*?<a href="(?:https://twitter.com|TWEET_(\d+)).*?</a>.*?</blockquote>', lambda x: x.group(0).replace("<a", "<rep_tx").replace("</a>", "</rep_tx>"), text, flags=re.DOTALL)
    # change iframe tag name
    clean_text = re.sub(r'<iframe.*?src="(?:https://www.youtube-nocookie.com|https://www.youtube.com).*?</iframe>', lambda x: x.group(0).replace("<iframe", "<iframe_ini").replace("</iframe>", "</iframe_ini>"), clean_text, flags=re.DOTALL)
    # display(clean_text)
...
#################################
    ...
    # changing the back the custom tags to original
    clean_text = re.sub(r'<rep_tx', "<a", clean_text, flags=re.DOTALL)
    clean_text = re.sub(r'</rep_tx>', '</a>', clean_text, flags=re.DOTALL)
    clean_text = re.sub(r'<iframe_ini', "<iframe", clean_text, flags=re.DOTALL)
    clean_text = re.sub(r'</iframe_ini>', "</iframe>", clean_text, flags=re.DOTALL)
