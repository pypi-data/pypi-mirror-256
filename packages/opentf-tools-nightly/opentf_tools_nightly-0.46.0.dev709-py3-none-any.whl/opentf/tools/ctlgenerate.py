...
from opentf.tools.ctlworkflows import _get_workflows, _get_workflow_events, WAIT_DELAY
...
GENERATE_REPORT_HELP = '''Generate a report from a local Insight Collector configuration file

Example:
  # Generate a HTML summary report using a local configuration file
  opentf-ctl generate report 2cd3365a-6fbe-4d60-9146-b8bd6f42d1c8 --name=detailed-execution-report --using=custom_config.yaml --save-to=/user/home/reports/

Options:
  --name={...}: report name, i.e `name` property of an insight from configuration file (supports Unix file patterns).
  --using={...} or -u {...}: configuration file path.
  --save-to={...}: destination path, must be a directory.

Usage:
  opentf-ctl generate report WORKFLOW_ID [--using=config_file] [options]
...
def _get_workflow_results(
    ...
    while retrieved_results < expected:
...
def _process_insightcollector_result(
    result: Dict[str, Any], workflow_id: str, using: str, name_pattern: str
) -> List[Dict[str, Any]]:
    ...
    return _get_workflow_results(
        len(details['expected']), request_id, workflow_id, time.time() + 60
    )


def _query_insightcollector(
    workflow_id: str, name_pattern: str, using: str, save_to: str
) -> None:
    ...
    workflow_results = _process_insightcollector_result(
        result, workflow_id, using, name_pattern
    )
    if workflow_results:
        ...
        for uuid in attachment_uuids:
            download_attachment(workflow_id, uuid, save_to)
    else:
        _error('Timeout while retrieving generated reports.')
        sys.exit(1)


def _get_report_options() -> Tuple[str, Optional[str], str]:
    return (
        _get_arg('--name=') or '*',
...
def generate_report(workflow_id):
    ...
    name_pattern, using, save_to = _get_report_options()
    if not using:
        _error('Configuration file path not provided.')
        sys.exit(1)
    if os.path.isfile(save_to):
        _error(
            'File path `%s` is a file, not a directory. Can not download reports to a file.',
            save_to,
        )
        sys.exit(1)
    if using and not os.path.exists(using):
        ...
        sys.exit(1)
    try:
        _query_insightcollector(workflow_id, name_pattern, using, save_to)
    except Exception as err:
        _fatal(
