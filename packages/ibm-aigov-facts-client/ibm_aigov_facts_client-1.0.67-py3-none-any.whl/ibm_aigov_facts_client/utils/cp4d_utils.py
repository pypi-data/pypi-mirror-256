...
class CloudPakforDataConfig:
    ...
    :attr str service_url: Host URL of Cloud Pak for Data environment.
    :attr str username: Environment username
    :attr str password: Environment password
    :attr str api_key: Environment api_key if IAM enabled
    :attr bool disable_ssl_verification: Disable SSL verification. Default is True.
    :attr str bedrock_url: (Optional) Foundational services (common-services-route ) url. User needs to get this url from CP4D admin. This url is required only when iam-integration is enabled on CP4D 4.0.x cluster.   
    """
