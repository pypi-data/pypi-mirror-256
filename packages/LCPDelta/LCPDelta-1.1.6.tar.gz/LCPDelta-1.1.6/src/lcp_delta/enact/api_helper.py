...
class APIHelper(APIHelperBase):
    ...
    # Series:
    def get_series_data(self, series_id : str, date_from : datetime, date_to : datetime, country_id : str, option_id : list[str] = None, half_hourly_average : bool = False, request_time_zone_id : str = None, time_zone_id : str = None, parse_datetimes : bool = False) -> pd.DataFrame:
    ...
    def get_history_of_forecast_for_date_range(self, series_id : str, date_from : datetime, date_to : datetime, country_id : str, option_id : list[str] = None) -> dict[str, pd.DataFrame]:
        '''Gets the history of a forecast for a given date
    ...
    def get_bm_data_by_search(self, date : datetime, option : str = 'all', search_string : str = None, include_accepted_times : bool = False) -> pd.DataFrame:
    ...
    #Leaderboard:
    def get_leaderboard_data(self, date_from : datetime, date_to : datetime, type = "Plant", revenue_metric = 'PoundPerMwPerH', market_price_assumption = 'WeightedAverageDayAheadPrice', gas_price_assumption = 'DayAheadForward') -> pd.DataFrame:
    ...
    #Ancillary Contracts:
    def get_ancillary_contract_data(self, ancillary_contract_type : str, option_one : Union[str, int], option_two : Union[int, str] = None) -> pd.DataFrame:
        ...
        response = self.post_request(endpoint, request_details)
        dynamic_containment_contract_types = ['DynamicContainmentEfa', 'DynamicContainmentEfaHF', 'DynamicModerationLF', 'DynamicModerationHF', 'DynamicRegulationLF', 'DynamicRegulationHF']
        if 'data' not in response or not response['data']:
            return pd.DataFrame()
        first_item = response['data'][0]
        contract_type = first_item['contractType']
        if contract_type == 'SFfr':
            df = pd.DataFrame(first_item['DataByRow'])
            return df
        if contract_type == 'ManFr':
            for entry in first_item['DataByPlant']:
                entry.update(entry.pop('data'))
            df = pd.DataFrame(first_item['DataByPlant'])
            if not df.empty:
                unit_column_name = df.columns[0]
                df.set_index(unit_column_name, inplace=True)
            return df
        if contract_type == 'StorDayAhead':
            df = pd.DataFrame(first_item['Data'])
            return df
        if contract_type == 'Ffr':
            df = pd.DataFrame(first_item['DataByTender'])
            df.set_index('tenderNumber', inplace=True)
            return df
        if contract_type in dynamic_containment_contract_types:
