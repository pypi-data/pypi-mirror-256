...
if TYPE_CHECKING:
    ...
    from dql.data_storage import AbstractWarehouse
...
class IndexingStep(StartingStep):
    ...
    def apply(self):
        """Return the query for the table the query refers to."""
        # TODO return metastore and warehouse from this index() call and use them below
        self.catalog.index([self.path], **self.kwargs)
        metastore = self.catalog.metastore
        warehouse = self.catalog.warehouse
        uri, path = self.parse_path()

        partial_id = metastore.get_valid_partial_id(uri, path)
        st = warehouse.clone(uri, partial_id)
        ...
        dataset_columns = warehouse.dataset_row_cls.calculate_all_columns([st.nodes])
...
class SQLQuery:
    ...
    def cleanup(self):
        ...
        self.catalog.metastore.clone().id_generator.delete_uris(self.temp_table_names)
    ...
    def extract(self, *params: UDFParamSpec, **kwargs) -> Iterable[Any]:
        ...
        try:
            query = self.apply_steps().select()
            for row in self.catalog.warehouse.dataset_select_paginated(
...
class DatasetQuery(SQLQuery):
    ...
    def save(self, name: str, **kwargs):
        ...
        try:
            ...
            self.catalog.create_shadow_dataset_record(
                name, create_rows=False, custom_columns=custom_columns
            )
            dataset = self.catalog.metastore.get_dataset(name)
            if dataset is None:
                raise RuntimeError(f"No dataset found with {name=}")
            table_name = self.catalog.warehouse.dataset_table_name(dataset.name)

            self.catalog.warehouse.db.rename_table(temp_table_name, table_name)
