...
from dql.sql.types import DateTime, SQLType, String
...
class DatasetRowsFetcher(NodesThreadPool):
    ...
    def check_for_status(self) -> None:
        ...
        export_status_response = self.studio_client.dataset_export_status(
            self.remote_dataset_name, self.remote_dataset_version
        )
    ...
    def do_task(self, urls):
        ...
        shadow_dataset = metastore.get_dataset(self.shadow_dataset_name)
        ...
        while urls:
            for url in urls:
                ...
                inserted = warehouse.insert_dataset_rows(df, shadow_dataset)
    ...
    for node_group in node_groups:
        ...
        metafile_group = {
            "data-source": (
                ...
                else {"uri": listing.warehouse.uri}
            ),
            "files": [],
        }
...
class Catalog:
    ...
    def get_client(self, uri: StorageURI, **config: Any) -> Client:
        ...
        return cls.from_source(uri, self.metastore, self.cache, **config)
    ...
    def enlist_source(
        ...
        source_warehouse = self.warehouse.clone(client.uri)
        ...
        if skip_indexing:
            ...
            source_metastore.init_db(client.uri, partial_id)

            source_warehouse = self.warehouse.clone(
                uri=client.uri, partial_id=partial_id
            )
            source_warehouse.init_db(client.uri, partial_id)

            return (
                Listing(storage, source_metastore, source_warehouse, client),
                path,
            )
        ...
        for src in sources:  # Opt: parallel
            if src.endswith(DATASET_FILE_SUFFIX) and os.path.isfile(src):
                ...
                for source in dataset_sources:
                    ...
                    st = self.warehouse.clone(uri, None)
                    listing = Listing(
                    ...
                    rows = st.get_dataset_rows(
                        dataset, version=ds_version, limit=None, source=source
                    )
    ...
    def _get_nodes(self, sources: Iterable[str], client_config) -> list:
        ...
        for source in sources:
            ...
            uri = client.uri
            partial_id = self.metastore.get_valid_partial_id(uri, "")
            nodes.append(self.warehouse.nodes_table(uri, partial_id))

        return nodes

    def create_shadow_dataset_record(
        self,
        name: str,
        ...
        """
        Creates shadow database record if it doesn't exist.
        ...
        if create_rows:
            custom_cols = [
                Column(c.name, c.type, nullable=c.nullable) for c in custom_columns
            ]
            table_name = self.warehouse.dataset_table_name(dataset.name)
            self.warehouse.create_dataset_rows_table(
                table_name, custom_columns=custom_cols
            )
            dataset = self.metastore.update_dataset_status(
                dataset, DatasetStatus.PENDING
            )
    ...
    def create_dataset_version_record(
        ...
        if create_rows_table:
            ...
            self.warehouse.create_dataset_rows_table(table_name)

        return dataset

    def remove_shadow_dataset(self, dataset: DatasetRecord, drop_rows=True) -> None:
        self.metastore.remove_dataset(dataset)
        if drop_rows:
            self.warehouse.drop_dataset_rows_table(dataset)
    ...
    def update_dataset(
        ...
        is_shadow = dataset.shadow
        versions = []
        if not is_shadow and dataset.versions:
        ...
        if old_name and new_name:
            ...
            if is_shadow:
                self.warehouse.rename_dataset_table(old_name, new_name)
            else:
    ...
    def create_shadow_dataset(
        ...
        dataset = self.create_shadow_dataset_record(
            name,
            sources=sources,
            query_script=query_script,
        ...
        try:
            for source in sources:
                client, path = self.parse_url(source, **client_config)
                uri = client.uri
                partial_id = self.metastore.get_valid_partial_id(uri, path)
                warehouse = self.warehouse.clone(uri, partial_id)
                warehouse.insert_into_shadow_dataset(
                    dataset, uri, path, recursive=recursive
                )
                storage = self.get_storage(uri)
                metastore = self.metastore.clone(uri, partial_id)
                metastore.add_storage_dependency(
                    dataset.name, uri, storage.timestamp_str
                )
    ...
    def register_shadow_dataset(
        ...
        if not dataset.shadow:
            raise ValueError(f"Dataset {dataset.name} must be shadow")

        try:
            registered_dataset = (
                self.get_dataset(registered_name) if registered_name else None
            )
        ...
        if registered_dataset:
            ...
            registered_dataset = self.metastore.create_dataset_version(
                registered_dataset,
                version,
                sources=dataset.sources,
                query_script=dataset.query_script,
                custom_column_types=dataset.custom_column_types_serialized,
            )
    ...
    def ls_datasets(self, shadow_only=None) -> Iterator[DatasetRecord]:
        yield from self.metastore.list_datasets(shadow_only=shadow_only)
    ...
    def dataset_row(
        ...
        return self.warehouse.get_dataset_row(
            dataset, row_id, dataset_version=dataset_version
        )
    ...
    def dataset_stats(self, name: str, version: Optional[int] = None) -> DatasetStats:
        ...
        return DatasetStats(
            num_objects=self.warehouse.dataset_rows_count(dataset, version),
            size=self.warehouse.dataset_rows_size(dataset, version),
        )
    ...
    def remove_dataset(
        ...
        if dataset.registered and version:
            ...
            self.remove_shadow_dataset(dataset)
    ...
    def edit_dataset(
        ...
        dataset = self.get_dataset(name)
        self.update_dataset(dataset, **update_data)
    ...
    def merge_datasets(
        ...
        if src.shadow and src_version:
            raise ValueError(
                f"Source dataset {src.name} is shadow, cannot use it with versions"
            )
        if not src.shadow and not src_version:
            raise ValueError(f"Source dataset {src.name} is registered, need a version")
        if dst.shadow and dst_version:
            raise ValueError(
                f"Dataset {dst.name} is shadow, cannot use it with versions"
            )
        ...
        dst_dep = self.get_dataset_dependencies(dst.name, dst.latest_version)
        ...
        if dst.shadow:
            ...
            self.update_dataset(dst, custom_column_types=custom_column_types)
            for dep in src_dep:
                if dep and dep not in dst_dep:
                    self.metastore.add_dependency(dep, dst.name)
        else:
            dst_version = dst_version or dst.next_version
            dst = self.create_dataset_version_record(
                dst, dst_version, custom_column_types=custom_column_types
            )
            self.warehouse.merge_dataset_rows(
                src,
                dst,
    ...
    def copy_shadow_dataset(self, src_name: str, dst_name: str) -> DatasetRecord:
        ...
        """
        src = self.get_dataset(src_name)
        dst = self.get_dataset(dst_name)
        ...
        self.warehouse.copy_shadow_dataset_rows(src, dst)
    ...
    def pull_dataset(  # noqa: PLR0911, C901
        ...
        *,
        ...
        shadow_dataset = self.create_shadow_dataset_record(
            shadow_dataset_name,
        ...
        if signed_urls:
            ...
            try:
                ...
                self.remove_shadow_dataset(shadow_dataset)
                raise

        shadow_dataset = self.metastore.update_dataset_status(
            shadow_dataset,
    ...
    def query(
        ...
        if dataset:
            ...
            self.update_dataset(dataset, query_script=query_script)
        ...
        if dataset:
            dataset = self.merge_datasets(returned_dataset, dataset)
    ...
    def du(
        ...
        *,
        ...
        def du_dirs(src, node, subdepth):
            if subdepth > 0:
                subdirs = src.listing.warehouse.get_nodes_by_parent_path(
                    node.path, type="dir"
                )
                for sd in subdirs:
