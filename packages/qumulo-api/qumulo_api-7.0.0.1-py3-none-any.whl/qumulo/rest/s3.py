...
class Policy:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'Version': self.version, 'Statements': [s.to_dict() for s in self.statements]}
...
class S3:
    ...
    def list_access_keys(
        self, limit: Optional[int] = None, start_at: Optional[str] = None
    ...
    # path.
    def create_bucket(
        self, name: str, path: Optional[str] = None, create_path: bool = False
    ) -> BucketDescription:
    ...
    def put_bucket_policy(self, bucket: str, policy: Policy) -> None:
        response = self.client.send_request(
            'PUT', f'/v1/s3/buckets/{bucket}/policy', body=policy.to_dict()
        )
        assert response.data is None
