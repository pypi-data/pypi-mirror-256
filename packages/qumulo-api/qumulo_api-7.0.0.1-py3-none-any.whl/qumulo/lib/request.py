...
class Connection:
    def __init__(
        ...
        self.connection_factory = RealConnectionFactory(host, port, timeout)
...
class RestResponse(NamedTuple):
    ...
    def __str__(self) -> str:
        # qq commands usually make one REST API call and simply print the reponse. Many APIs do not
        # return any data in the response. Avoid printing a confusing 'null'.
        return pretty_json(self.data) if self.data is not None else ''
