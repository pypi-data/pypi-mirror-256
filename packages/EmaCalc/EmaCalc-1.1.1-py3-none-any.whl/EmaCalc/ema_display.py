...
result_path / 'xxx_effects' / 'population_mean' / attributes / ...  (if more than one group)
* Here, 'group' may be a single group dimension, like 'Age', or a combination, like 'AgexGender'
...
class EmaDisplaySet:
    ...
    """
    def __init__(self, group_factors):  #, group_head=None, groups=None, group_effects=None):
        """
    ...
    def show(cls, emm,
        ...
        :param group_factors: (optional) list with selected grouping factors (population "dimensions"),
                given as single key, or tuple of group keys.
        ...
        # NOTE: can show individual Participant results only for gf including ALL group dimensions
...
class GroupFactorDisplaySet:
    ...
    def show(cls, emm, gf):
        ...
        if len(group_displays) > 1:
            # group_effects = GroupEffectSet.display(emm)  # OLD
...
class GroupDisplaySet:
    ...
    or ONE PopulationModel represented by one or several groups.
...
class SituationProfile(Profile):
    ...
    def display(cls, xi, m_xi, sit_keys):
        ...
        # NOTE - v. 1.0.2: Comparing CONDITIONAL probabilities of categories in FIRST sit_keys dimension,
        # GIVEN categories in other dimensions.
        # d_pd_old = cred_pd.cred_diff(u_ds, diff_axis=sit_keys[0], case_axis=sit_keys[1:], p_lim=FMT['credibility_limit'])
        # tab_diff_old = fmt.tab_credible_diff(d_pd_old, diff_head=sit_keys[0:1], cred_head=FMT['credibility'],
        #                                      case_head=sit_keys[1:], y_label=FMT['sit_probability'])
        # NOTE v. 1.1.0- : Comparing CONDITIONAL probabilities of categories sit_keys[1:],
        # GIVEN categories in sit_keys[0], IFF more than one situation dimension.
        if len(sit_keys) > 1:
            ...
            diff_axis = sit_keys
            case_axis = ()
        d_pd = cred_pd.cred_diff(u_ds, diff_axis=diff_axis, case_axis=case_axis, p_lim=FMT['credibility_limit'])
        tab_diff = fmt.tab_credible_diff(d_pd, diff_head=diff_axis, cred_head=FMT['credibility'],
                                         case_head=case_axis, y_label=FMT['sit_probability'])
        # ---------------------------------------------------------------------
...
class AttributeProfile(Profile):
    ...
    def display(cls, xi, m_xi, a_effect):
        ...
        # d_pd_old = cred_pd.cred_diff(theta_ds, diff_axis=sit_keys, p_lim=FMT['credibility_limit'])
        # tab_diff_old = fmt.tab_credible_diff(d_pd_old, diff_head=sit_keys, cred_head=FMT['credibility'],
        #                                  y_label=a,
        #                                  file_label=a)
...
class GroupEffectSet:
    ...
    # @classmethod
    # def display(cls, emm):  # OLD v.1.0.2
    #     """Generate all displays for ONE group
...
class EmaGroupDiff:
    ...
    # @classmethod
    # def display(cls, emm, groups):  # ******** OLD v.1.0.2
    #     """
    #     :param emm: ema_model.EmaModel instance with several groups
...
class SituationDiff(Profile):
    # @classmethod
    # def display(cls, xi, emm, sit_keys):  # *** OLD. v.1.0.2
    #     """Generate a probability-profile display for selected distribution and factor
...
class AttributeDiff(Profile):
    ...
    """
    # @classmethod
    # def display(cls, xi, emm, a_effect):  # OLD v. 1.0.2
    #     """Create displays for a single attribute and requested situation effects
    #     :param xi: list of 2D arrays of parameter-vector samples
    #         len(xi) == len(emm.groups)
...
def _check_situations(situations, emf):
    ...
    missing_sit = [sit_tuple for sit_tuple in situations
                   if any(sit_ij not in emf.situation_dtypes.keys()
                          for sit_ij in sit_tuple)]
    for sit in missing_sit:
        logger.warning(f'Situation dimension in {sit} unknown in the learned model. Removed.')
...
def _check_attributes(attributes, emf):
    ...
    missing_attr = [a_sit for a_sit in attributes
                    if a_sit[0] not in emf.attribute_dtypes.keys()]  # OR if any sit_keys not OK ?
    for a_sit in missing_attr:
        logger.warning(f'Attribute effect {a_sit} unknown in the learned model. Removed.')
...
def _check_grade_counts(grade_counts, emf):
    ...
    missing_counts = [c_sit for c_sit in grade_counts
                      if c_sit[0] not in emf.attribute_dtypes.keys()] # OR if any sit_keys not OK ?
    for c_sit in missing_counts:
        logger.warning(f'Requested count {c_sit} unknown in the data set. Removed.')
...
def _check_group_factors(group_factors, emf):
    ...
    if group_factors is None:
        group_factors = [tuple(emf.group_dtypes.keys())]
    group_factors = [gf if isinstance(gf, tuple) else (gf,)
                     for gf in group_factors]  # ensure tuple elements
    error_gf = list(filter(lambda gf: any([gf_i not in emf.group_dtypes.keys()
                                                for gf_i in gf]),  # *** use itertools.filterfalse, with all()
                             group_factors)
                      )
    for gf in error_gf:
        logger.warning(f'Requested group_factor {gf} includes unknown group dimension(s). Removed.')
        group_factors.remove(gf)
