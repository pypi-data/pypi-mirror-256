    ...
    def attribute_grade_mean(self, a=None, groupby=None):
        ...
        def s_mean(s_data, a, groupby):
            ...
            if len(groupby) == 0:
                ...
                if s_m.size != np.prod(s_m.index.levshape):  # no data in some situation(s)
                    ind = pd.MultiIndex.from_product(list(s_m.index.levels))
                    s_m = s_m.reindex(ind, fill_value=np.nan)
                return s_m
