...
class Client(object):
    ...
    def __init__(self, database, buffer_size=3000, cr_api_url=None, cr_api_token=None, cr_integration_id=None):
        ...
        self.data = []
        self._cr_api_url = cr_api_url
        self._cr_api_token = cr_api_token
    ...
    def get_integration(self):
        ...
        if cr_api_url is None:
            return {'result': 'error', 'info': 'cr_api_url is None'}
        if cr_api_token is None:
            return {'result': 'error', 'info': 'cr_api_token is None'}
        if cr_integration_id is None:
            return {'result': 'error', 'info': 'cr_integration_id is None'}        

        headers = {
                'Authorization': f'cr_api_token {self._cr_api_token}'
            }
        r = requests.get(f'{self._cr_api_url}/{cr_integration_id}', headers=headers)    
    ...
    def set_integration(self, load_date=None, load_rows=None, load_percent=None, tables_qty=None):
        ...
        r = requests.post(f'{self._cr_api_url}/{cr_integration_id}', headers=headers,  json=data)    
