from .bot_functions import safe_get,get_any_value
    ...
    for i,each in enumerate([preTokenBalances,postTokenBalances]):
        for post in each:
            ...
            if index not in dont_use:
                ...
                after = get_token_amount(get_acct_index(postTokenBalances,index))
                change = get_token_amount(post)
...
def get_solana_balances(signature_data):
    ...
    for i, amt in enumerate(preBalances):
        balance_change = [exponent(amt, 9), -exponent(postBalances[i], 9)]
...
def parse_pool_data(logMessages):
    logMessages = get_any_value(logMessages,'logMessages') or logMessages
    new_js={}
    log_messages_string = str(log_messages)
...
input(str(set("""search_for_account_index,get_amount_dict,calculate_change,get_acct_index,get_account_keys,create_token_txns,get_solana_balances,get_solana_balances,get_all_txns,parse_pool_data""".split(','))).replace("'",'').replace('{','').replace('}',''))
