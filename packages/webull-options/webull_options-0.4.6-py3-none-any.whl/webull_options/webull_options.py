...
class WebullOptions:
    def __init__(self, database:str='markets', user:str='chuck'):
            ...
            async with sema:
                try:
                    ...
                    async with session.post(url, data=json.dumps(params)) as resp:
                        ...
                        await self.db_manager.batch_insert_dataframe(option_data.as_dataframe)
    ...
    async def find_plays(self):
        ...
        async with asyncpg.create_pool(host='localhost', user='chuck', database='markets', port=5432, password='fud') as pool:
