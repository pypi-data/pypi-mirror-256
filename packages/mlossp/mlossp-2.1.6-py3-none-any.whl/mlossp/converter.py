...
def netcdf2geojson(config_file, input_file, output_dir, verbose, max_records=None):
    ...
    if conf.get("magnitudeVar") and conf.get("directionVar"):
        if conf["convertMagDir"] is True:
            ...
            data["u"] = df.apply(
                lambda x: magdir2u(x[conf["magnitudeVar"]], x[conf["directionVar"]]),
                axis=1,
            )
            data["v"] = df.apply(
                lambda x: magdir2v(x[conf["magnitudeVar"]], x[conf["directionVar"]]),
                axis=1,
            )
    ...
    if conf.get("uVar") and conf.get("vVar"):
        if conf["convertUV"] is True:
            ...
            data["magnitude"] = df.apply(
                lambda x: uv2magnitude(x[conf["uVar"]], x[conf["vVar"]]), axis=1
            )
            data["direction"] = df.apply(
                lambda x: uv2direction(x[conf["uVar"]], x[conf["vVar"]]), axis=1
            )
    ...
    gdf = geopandas.GeoDataFrame(
        data, geometry=geopandas.points_from_xy(df[conf["lonVar"]], df[conf["latVar"]])
    )
    gdf.crs = "EPSG:4326"
    log(gdf, verbose)
...
def run(INPUT, OUTPUT, CONFIG, VERBOSE=True, MAX_RECORDS=None):
    return netcdf2geojson(CONFIG, INPUT, OUTPUT, VERBOSE, MAX_RECORDS)
