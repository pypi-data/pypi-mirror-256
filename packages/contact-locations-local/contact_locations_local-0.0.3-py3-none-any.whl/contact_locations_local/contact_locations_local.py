...
STATE_THEY_LIVE_IN_GROUP_CATAGORY = 501
CITY_THEY_LIVE_IN_GROUP_CATAGORY = 201
...
class ContactLocationsLocal(GenericMapping):
    ...
    def insert_contact_and_link_to_location(self, location_information: dict, contact_id: int) -> None:
        """
        ...
        try:
            ...
            if state_id is None:
                group_id = self.select_one_dict_by_where(schema_name='group', view_table_name='group_ml_view', select_clause_value='group_id',
                                                         where_clause_value='title = %s', params=(state_name,))
                if group_id is None:
                    group_json = {
                        'is_state': 1,
                        'group_category_id': STATE_THEY_LIVE_IN_GROUP_CATAGORY,
            ...
            if city_id is None:
                group_id = self.select_one_dict_by_where(schema_name='group', view_table_name='group_ml_view', select_clause_value='group_id',
                                                         where_clause_value='title = %s', params=(city_name,))
                if group_id is None:
                    group_json = {
                        'is_city': 1,
                        'group_category_id': CITY_THEY_LIVE_IN_GROUP_CATAGORY,
                        'is_test_data': self.is_test_data,
                        'profile_id': self.profile_id,
                    }
                    group_id = self.insert(
                        schema_name='group', table_name='group_table', data_json=group_json)
                city_id = City().insert(city=city_name, lang_code=self.lang_code.value,
            ...
            if county_id is None:
                group_id = self.select_one_dict_by_where(schema_name='group', view_table_name='group_ml_view', select_clause_value='group_id',
                                                         where_clause_value='title = %s', params=(county_name,))
                if group_id is None:
                    group_json = {
                        'is_county': 1,
            ...
            if region_id is None:
                group_id = self.select_one_dict_by_where(schema_name='group', view_table_name='group_ml_view', select_clause_value='group_id',
                                                         where_clause_value='title = %s', params=(region_name,))
                if group_id is None:
                    group_json = {
                        'is_region': 1,
            ...
            if neighborhood_id is None:
                group_id = self.select_one_dict_by_where(schema_name='group', view_table_name='group_ml_view', select_clause_value='group_id',
                                                         where_clause_value='title = %s', params=(neighborhood_name,))
                if group_id is None:
                    group_json = {
                        'is_neighborhood': 1,
