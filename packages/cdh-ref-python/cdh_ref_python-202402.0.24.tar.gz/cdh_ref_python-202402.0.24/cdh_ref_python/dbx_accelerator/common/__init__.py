...
from cdh_ref_python.dbx_accelerator.common.validator import Validator, ValidationError
...
def combine_var_args(*, first: Any, others: Optional[Tuple]) -> List[Any]:
    """
    Combines multiple variable arguments into a single list.
...
def print_title(title: str, divider: str = "-", length: int = 100) -> None:
    """
    Prints a title with a divider line above and below it.
    :param title: the title to print
    :param divider: the character used for the divider line
...
def print_warning(title: str, message: str, length: int = 100) -> None:
    """
    Prints a warning message with a title.
    :param title: the title of the warning
    :param message: the warning message
    :param length: the length of the warning box
    :return: None
    """
...
def deprecated(reason=None, action="warn") -> Callable:
    ...
    :param action: the action to take when the function is called
    ...
    def decorator(inner_function):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                signature = "(..)"
            function_name = (
                f"{inner_function.__module__}.{inner_function.__name__}{signature}"
            )
...
def overrides(func: Callable = None, check_signature: bool = True) -> Callable:
    ...
    :param check_signature: whether to check the function signature
...
def clean_string(value, replacement: str = "_") -> str:
    """
    Cleans a string by replacing non-alphanumeric characters with a specified replacement.
...
def load_databricks_cfg(path: str):
    """
    Loads a Databricks configuration file and returns the sections as a dictionary.
    :param path: the path to the configuration file
    :return: a dictionary containing the sections and their key-value pairs
    """
