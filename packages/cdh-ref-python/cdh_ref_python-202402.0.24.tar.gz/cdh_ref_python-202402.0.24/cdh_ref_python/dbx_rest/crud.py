...
from cdh_ref_python.dbx_rest import *
...
class CRUD(ApiContainer, metaclass=ABCMeta):
    ...
    * _wrap(item)
    ...
    def get_by_name(self, item_name: str, if_not_exists: IfNotExists = "error") -> Item:
        ...
        """
        result = next(
            (item for item in self._list() if item[self.name_key] == item_name), None
        )
        if result is None and if_not_exists == "error":
            raise DatabricksApiException(
                f"{self.singular} with name '{item_name}' not found", 404
            )
    ...
    def get_by_example(self, item: Item, if_not_exists: IfNotExists = "error") -> Item:
        ...
        if item_id := item.get(self.id_key):
            ...
            raise ValueError(
                f"spec must have either {self.id_key!r} or {self.name_key!r}"
            )
    ...
    def create_by_example(
        self, item: Item, *, fetch: bool = False, if_exists: IfExists = "create"
    ) -> Item:
        """
        ...
        if if_exists == "create":
            if self.name_key not in item:
                raise ValueError(
                    f"{self.singular} cannot be created without the {self.name_key}."
                )
            existing = None
        else:
            existing = (
                self.get_by_example(item, if_not_exists="ignore")
                if if_exists != "create"
                else None
            )
        if existing is None:
        ...
        if if_exists == "error":
            raise DatabricksApiException(
                f"{self.singular}({self.id_key}={existing[self.id_key]!r}, "
                f"{self.name_key}={existing[self.name_key]!r}) "
                "already exists.",
                409,
            )
        raise ValueError(
            "if_exists must be one of 'ignore', 'update', 'error', 'create', or 'overwrite'"
        )
    ...
    def update(
        self, item: Item, *, fetch: bool = False, if_not_exists: IfNotExists = "error"
    ) -> Item:
        """
        Update the {singular}.
