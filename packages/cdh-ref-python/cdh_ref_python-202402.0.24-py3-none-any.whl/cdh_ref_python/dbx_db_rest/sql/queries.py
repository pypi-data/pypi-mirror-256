...
class SqlQueriesClient(ApiContainer):
    ...
    def undelete_by_id(self, query_id):
        return self.client.execute_post_json(
            f"{self.base_uri}/trash/{query_id}", params=None
        )
    ...
    def get_by_name(self, query_name, queries=None, page=1):
        ...
        if json_response.get("count") == len(queries):
            ...
            return self.get_by_name(
                query_name=query_name, queries=queries, page=page + 1
            )
    ...
    def existing_to_create(self, query: dict):
        assert (
            type(query) == dict
        ), f'Expected the "query" parameter to be of type dict, found {type(query)}'
