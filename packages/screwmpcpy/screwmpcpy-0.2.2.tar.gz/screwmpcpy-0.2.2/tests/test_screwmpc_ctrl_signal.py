...
from screwmpcpy.screwmpc import BOUND, ScrewMPC, ScrewMPCManipulability
...
def test_screw_mpc_manipulability():
    Np = 50  # prediction horizon, can be tuned;
    Nc = 10  # control horizon, can be tuned
    R = 10e-3  # weight matirix
    Q = 10e9  # weight matrix
    ...
    mpc = ScrewMPCManipulability(Np, Nc, Q, R, vel_bound, acc_bound, jerk_bound)
    A = mpc.a_matrix
    B = mpc.b_matrix
    C = mpc.c_matrix

    franka_kin = FrankaEmikaPandaRobot.kinematics()
    franka_rtb = rtb.models.Panda()
    q_robot = np.array(
        [0.173898, 0.667434, 0.782032, -1.86421, 1.44847, 1.57491, 0.889156]
    )
    x_goal = franka_kin.fkm(q_robot)
    u_state = np.zeros((6,))
    mpc_state = np.zeros((18,))
    ...
    next_point = x_current * pow(delta_dq, 0.1)
    desired_twist = vec6(log(next_point * x_current.conj()))
    jac = franka_rtb.jacob0(q_robot)
    j_m = np.squeeze(franka_rtb.jacobm(q_robot))
    du = mpc.solve(desired_twist, u_state, mpc_state, jac @ j_m)[:6]
    ...
    error_msg = "Unexpected number of arguments, expected 4!"
    with pytest.raises(ValueError, match=error_msg):
        mpc.solve(desired_twist, u_state, mpc_state)
