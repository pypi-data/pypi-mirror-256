...
class ScrewMPC:
    ...
    def solve(self, *args: List[np.ndarray]) -> np.ndarray:  # noqa: UP006
...
class ScrewMPCManipulability(ScrewMPC):
    r"""Model Predictive Control for Dual Quaternion related control.
    ...
    def solve(self, *args: List[np.ndarray]) -> np.ndarray:  # noqa: UP006
        r"""Perform the optimization with manipulability constraint.
        ...
        :param mpc_state: State of mpc s.t.
            :math:`\boldsymbol{s} = \boldsymbol{As} + \boldsymbol{B}\Delta\boldsymbol{u} \in \mathbb{R}^6`.
        :type mpc_state: np.ndarray
        :type j_geom: np.ndarray
        :param j_m: Manipulabiliy gradient of the Jacobian where :math:`\boldsymbol{J}_m \in \mathbb{R}^n`
            with :math:`n` representing the DoF of the robot arm.
        :type j_m: np.ndarray
        :param j_geom: Geometric Jacobian :math:`\boldsymbol{J}_{geom} \in \mathbb{R}^{6 \times n}`
            where :math:`n` represents the DoF of the robot arm.
        :raises ValueError: If the the number of arguments mismatch.
        :return: Change of action :math:`\Delta\boldsymbol{u} \in \mathbb{R}^{6 n_c}`.
        :rtype: np.ndarray
        """
        if len(args) != 4:
            msg = "Unexpected number of arguments, expected 4!"
            raise ValueError(msg)

        desired_twist: np.ndarray = args[0]
        current_u: np.ndarray = args[1]
        mpc_state: np.ndarray = args[2]
        j_m_task: np.ndarray = args[3]

        lower, upper = self._get_bounds(current_u, mpc_state)
        q_vec = self._get_q_vec(desired_twist, mpc_state) - np.tile(j_m_task, self._n_c)
        self._update_solver(q_vec, lower, upper)
        return self._solver.solve().x
