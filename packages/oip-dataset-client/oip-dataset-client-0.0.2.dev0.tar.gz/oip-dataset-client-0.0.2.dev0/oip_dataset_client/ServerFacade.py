...
class ServerFacade:
    ...
    def save_data(
        ...
        base_url: str = os.path.join(
            os.environ["OIP_API_HOST"], cls.__server_module_name, dataset_entity_name
        )
        base_url = base_url.replace("\\", "/")
        headers: Dict = {"authorization": "APIKey " + os.environ["OIP_API_KEY"]}
        if method == "create":
    ...
    def create_presigned_url(
        cls,
        object_name: str,
        is_post_url: Optional[bool] = False,
        ...
        """
        url: str = os.path.join(os.environ["OIP_API_HOST"], cls.__server_module_name)
        if is_post_url:
