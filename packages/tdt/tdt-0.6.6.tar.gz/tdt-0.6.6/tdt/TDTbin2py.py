...
def read_block(block_path, *, bitwise='', channel=0, combine=None, headers=0,
        ...
        try:
            for sort_id in os.listdir(sort_path):
                if os.path.isdir(os.path.join(sort_path, sort_id)):
                    ...
                    sort_files = tdt.get_files(os.path.join(sort_path, sort_id), '.SortResult', ignore_mac=True)
    ...
    if not use_outside_headers:
        ...
        print('read from t={0}s to t={1}s'.format(np.round(t1,2), np.round(lastRead,2)))
    ...
    for current_name in header.stores.keys():
        ...
        if current_type_str == 'epocs':
            ...
            if current_name in sev_names:
                ...
                d = read_sev(block_path, event_name=current_name, channel=channel, verbose=verbose, ranges=valid_time_range, fs=expected_fs,
