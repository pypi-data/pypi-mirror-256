...
MODEL_URL_TEMPLATE = '{host}/serving-endpoints/databricks-{model}/invocations'
...
def get_url(
    ...
    Args:
        ...
        model (str): The model for the API.
    ...
    return MODEL_URL_TEMPLATE.format(host=host, model=model)
...
class FoundationModelAPIResource(APIResource):
    ...
    def create(cls, client: requests.Session = None, **kwargs):
        ...
        **kwargs: The keyword arguments for the API.
        ...
        return cls._make_query(client, api_input)
    ...
    @classmethod
    def _make_query(cls, client: requests.Session, model_input: FoundationModelAPIInput):
        """
        ...
        """
        w = WorkspaceClient()
        url = get_url(host=w.config.host, model=model_input.model)
        headers = {
    ...
    async def acreate(cls, client: httpx.AsyncClient = None, **kwargs):
        ...
        **kwargs: The keyword arguments for the API.
        ...
        The result of the API query.
        """
        model_input = cls._parse_and_validate_request(**kwargs)
        return await cls._amake_query(client, model_input)

    @classmethod
    async def _amake_query(cls, client: httpx.AsyncClient, model_input: FoundationModelAPIInput):
        """
        Makes a query to the API.
        ...
        """
        w = WorkspaceClient()
        url = f'{w.config.host}/serving-endpoints/databricks-{model_input.model}/invocations'
        headers = {
            'authorization': f'Bearer {w.config.token}',
            'Content-Type': 'application/json',
            'X-Databricks-Endpoints-API-Client': 'Generative AI Inference (Mosaic) SDK'
