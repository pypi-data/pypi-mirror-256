...
pipeline has been created.
    ...
    def signup(self, username, password): 
        ...
        if response.status_code == 200:
            ...
            raise Exception({"error": "Signup failed",
                "status_code": response.status_code,
                "response": response.json()["error"]["message"]})
...
returns the generated API key.
        ...
        if response.status_code == 200:
            ...
            raise Exception({"error": "Generating API key failed",
                "status_code": response.status_code,
                "response": response.json()["error"]["message"]})
    ...
    def list_api_keys(self, username, password):
        ...
        if response.status_code == 200:
            ...
            raise Exception({"error": "Retrieving API keys failed",
                "status_code": response.status_code,
                "response": response.json()["error"]["message"]})
    ...
    def create_pipeline(
        ...
        """
    The primary function for creating pipelines via the VecFlow API. `create_pipeline` takes several inputs to create a hosted chunking, embedding, and vector store pipeline for the user.

    :param name: The name of the pipeline. If this matches the name of an existing pipeline, the existing pipeline will be overwritten.
    :type name: str
    :param splitter_type: The type of splitter to use for chunking data. Defaults to 'NLTKTextSplitter'. Available splitters include: 'NLTKTextSplitter'.
    :type splitter_type: str, optional
    :param splitter_args: Arguments for the splitter, varying based on the splitter type. For 'NLTKTextSplitter', required arguments include 'chunk_size' (int), 'chunk_overlap' (int), and 'length_function' (str, choices: ['len']).
    :type splitter_args: dict, optional
    :param embedder_type: The type of embedder to use. Options include 'OpenAIEmbedder', 'OpenAI_003_Large', and 'OpenAI_003_Small'. Each embedder has specific requirements for `embedder_args`.
    :type embedder_type: str, optional
    :param embedder_args: Arguments for the embedder, varying based on the embedder type. For 'OpenAIEmbedder', required arguments include 'api_key' (str).
    :type embedder_args: dict, optional
    :param vector_store_type: The type of vector store to use. Defaults to 'PineconeVectorStore'. Options and required `vector_store_args` are detailed in the API documentation.
    :type vector_store_type: str, optional
    :param vector_store_args: Arguments for the vector store, varying based on the store type. For 'PineconeVectorStore', required arguments include 'api_key' (str), 'environment' (str), and 'index' (str).
    ...
    """
        ...
        if response.status_code == 200:
            ...
            raise Exception({"error": "Creation of pipeline failed",
                "status_code": response.status_code,
                "response": response.json()["error"]["message"]})
