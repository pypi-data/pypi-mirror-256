...
def getFlutterModules(path: str):
    ...
    if not os.path.exists(path + "/pubspec.yaml"):
    ...
    modules = [
        d
        for d in os.listdir(path + "/lib")
        if os.path.isdir(os.path.join(path + "/lib", d))
    ]
...
def getFlutterPages(path: str, module: str):
    ...
    pages = [
        d
        for d in os.listdir(path + "/lib/" + module + "/ui")
        if os.path.isfile(os.path.join(path + "/lib/" + module + "/ui", d))
    ]
...
def create_page(path, module, page) -> dict[list[str], list[str]]:
    ...
    result = create_file(f"{path}/lib/{module}/ui", f"{pageSnake}_page.dart", pagePayload(page=page))
    files.append(f"{result["file"]}")
    status.append(result["status"])

    result = create_file(f"{path}/lib/{module}/binding", f"{pageSnake}_binding.dart", bindingPayload(page=page))
    files.append(f"{result["file"]}")
    status.append(result["status"])

    result = create_file(f"{path}/lib/{module}/controller", f"{pageSnake}_controller.dart", controllerPayload(page=page))
    files.append(f"{result["file"]}")
    status.append(result["status"])
