...
_subdirectories = [
    "binding",
    "controller",
    f"data{separator}data_sources",
    f"data{separator}models",
    f"data{separator}repo_impls",
    f"domain{separator}entities",
    f"domain{separator}repositories",
    f"domain{separator}use_cases",
    f"ui{separator}components",
]
...
def getFlutterModules(path: str):
    ...
    if not os.path.exists(path + f"{separator}pubspec.yaml"):
        raise Exception("pubspec.yaml not found, this is a flutter project?")

    # Verify if the path contains a lib folder
    if not os.path.exists(path + f"{separator}lib"):
        raise Exception("lib folder not found")

    modules = [
        d
        for d in os.listdir(path + f"{separator}lib")
        if os.path.isdir(os.path.join(path + f"{separator}lib", d))
    ]
...
def getFlutterPages(path: str, module: str):
    ...
    pages = [
        d
        for d in os.listdir(path + f"{separator}lib{separator}" + module + f"{separator}ui")
        if os.path.isfile(os.path.join(path + f"{separator}lib{separator}" + module + f"{separator}ui", d))
    ]
...
def new_module(path, module) -> dict[list[str], list[str]]:
    ...
    result = create_directory(f"{path}{separator}lib{separator}{module}")
    directories.append(f"{result["directory"]} (module)")
    status.append(result["status"])

    for subdirectory in _subdirectories:
        result = create_directory(f"{path}{separator}lib{separator}{module}{separator}{subdirectory}")
        directories.append(f"{separator}" + subdirectory)
        status.append(result["status"])

    return {"Directory": directories, "status": status}

def create_page(path, module, page) -> dict[list[str], list[str]]:
    """
    ...
    result = create_file(f"{path}{separator}lib{separator}{module}{separator}ui", f"{pageSnake}_page.dart", pagePayload(page=page))
    files.append(f"{result["file"]}")
    status.append(result["status"])

    result = create_file(f"{path}{separator}lib{separator}{module}{separator}binding", f"{pageSnake}_binding.dart", bindingPayload(page=page))
    files.append(f"{result["file"]}")
    status.append(result["status"])

    result = create_file(f"{path}{separator}lib{separator}{module}{separator}controller", f"{pageSnake}_controller.dart", controllerPayload(page=page))
    files.append(f"{result["file"]}")
    status.append(result["status"])
...
def createFlutterProj(path, projectName, platforms, description, console):
    with console.status(
        ...
        print(f"flutter create -e {projectName} --description {description if description else "A new Izio Flutter project."} --platforms {platforms} --org br.com.izio --no-pub")
        print(f"no caminho {path}")
