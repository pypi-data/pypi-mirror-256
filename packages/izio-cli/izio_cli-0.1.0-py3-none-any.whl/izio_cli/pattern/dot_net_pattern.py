        ...
        for ref in refs:
            run_command(
                [
                    "dotnet",
                    "add",
                    f"{projectName}.{project}\\{projectName}.{project}.csproj",
                    "reference",
                    f"{projectName}.{ref}\\{projectName}.{ref}.csproj",
                ],
                path=path,
            )
...
def addIzioLib(projectName, path):
    ...
    run_command(
        [
            "dotnet",
            "add",
            f"{projectName}.DataAccess\\{projectName}.DataAccess.csproj",
            "package",
...
def createSubmodules(path, projectName):
    ...
    """
    subMobules = [
        projectName + ".Application\\Extensions",
        projectName + ".Application\\Services",
        projectName + ".Contracts\\DTOs",
        projectName + ".Contracts\\Validators",
        projectName + ".DataAccess\\Services",
        projectName + ".Domain\\Entities",
        projectName + ".Domain\\Interfaces",
        projectName + ".Domain\\Interfaces\\Business",
        projectName + ".Domain\\Interfaces\\DataAccess",
    ]
    for sub in subMobules:
        create_directory(f"{path}/{sub}")
