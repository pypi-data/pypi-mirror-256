  ...
  try:
    with _nltk_lock:
      ...
      if not any(os.path.isfile(os.path.join(path, tokenizer_resource)) for path in nltk.data.path):
        # Attempt to download only if the tokenizer isn't found in any path
        nltk.download('punkt', download_dir=custom_nltk_data_path)
      # Load the tokenizer from the first valid path
      return nltk.data.load(f'nltk:{tokenizer_resource}')
  except Exception as e:
    logging.error(f"Failed to download or load nltk punkt tokenizer: {e}")
    raise  # It's often better to raise the exception after logging to avoid silent failures.
