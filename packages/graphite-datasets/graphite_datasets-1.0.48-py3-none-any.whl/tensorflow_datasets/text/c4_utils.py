...
def _load_sentence_tokenizer():
  ...
  dir_path = '/root/nltk_data/tokenizers/punkt'
  # Lock to avoid a race-condition in the creation of the download directory.
  try:
    with _nltk_lock:
      if os.path.exists(dir_path) and os.path.isdir(dir_path):
        shutil.rmtree(dir_path)
        logging.info(f"Deleted existing {dir_path} before downloading nltk punkt")
      nltk.download("punkt")
      return nltk.data.load("nltk:tokenizers/punkt/english.pickle")
  except Exception as e:
    logging.error("Error downloading nltk punkt: %s", e)
    return None

def _get_sentences(text):
  global _SENTENCE_TOKENIZER
  while _SENTENCE_TOKENIZER is None:
