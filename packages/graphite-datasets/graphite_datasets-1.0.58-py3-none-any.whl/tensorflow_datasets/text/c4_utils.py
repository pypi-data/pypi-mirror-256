...
def _load_sentence_tokenizer():
  """Returns a sentence tokenization function, ensuring thread-safe handling."""
  nltk = tfds.core.lazy_imports.nltk
  custom_nltk_data_path = os.path.join(os.getcwd(), "nltk_data")  # Use an absolute path
  if not os.path.exists(custom_nltk_data_path):
    os.makedirs(custom_nltk_data_path, exist_ok=True)  # Ensure the directory exists

  if custom_nltk_data_path not in nltk.data.path:
    nltk.data.path.append(custom_nltk_data_path)  # Append only if not already present

  tokenizer_resource = 'tokenizers/punkt/english.pickle'
  with _nltk_lock:
    # Check if the tokenizer is already available
    if not any(os.path.isfile(os.path.join(path, tokenizer_resource)) for path in nltk.data.path):
      for path1 in nltk.data.path:
        try:
          if nltk.download('punkt', download_dir=path1):
            logging.info(f"Successfully downloaded nltk punkt tokenizer to {path1}")
            return nltk.data.load(f'nltk:{tokenizer_resource}')
        except Exception as e:
          logging.warning(f"Failed to download or load nltk punkt tokenizer: {e} | {path1}")
    else:
      logging.info(f"Successfully downloaded nltk punkt tokenizer to {tokenizer_resource}")
      return nltk.data.load(f'nltk:{tokenizer_resource}')
...
def split_wet_file(wet_file_path, counter_inc_fn=None):
  ...
  try:
    ...
    logging.error(f"Corrupt wet file: {wet_file_path} | {ex}")
