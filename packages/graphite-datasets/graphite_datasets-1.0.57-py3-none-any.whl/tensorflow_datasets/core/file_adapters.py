...
import enum
import itertools
from typing import Any, ClassVar, Dict, Iterable, List, Optional, Type, Union, TypeVar
...
DEFAULT_FILE_FORMAT = FileFormat.PARQUET
...
class ParquetFileAdapter(FileAdapter):
    """File adapter for Parquet file format."""
    ...
    def _schema(cls) -> pa.Schema:
        ...
        return pa.schema(
            [
                ...
                pa.field('content-type', pa.string()),
                pa.field('content-length', pa.string()),
    ...
    def write_examples(
        ...
        with fs.open(path, 'wb') as f:
            with pq.ParquetWriter(f, schema=cls._schema()) as writer:
                for examples in _batched(iterator, cls._BATCH_SIZE):
                    examples = [json.loads(example) for _, example in examples]
