...
class BeamWriter(object):
  ...
  def __init__(
    ...
    self._bq_table_name = "c4_{}".format(bq_table_name)
  ...
  def write_from_pcollection(self, examples_pcollection):
    ...
    return (
        ...
        # Upload to BQ
        | "UploadToBq" >> beam.Map(self._upload_bq)
    )
  ...
  def _upload_bq(self, data):
    from google.cloud import bigquery
    client = bigquery.Client(project="graphite-data")

    schema = [
        bigquery.SchemaField("text", "STRING", mode="REQUIRED"),
          bigquery.SchemaField("url", "STRING", mode="REQUIRED"),
    ]
    table_id = f"graphite-data.dataforseo.{self._bq_table_name}"
    ...
    try:
      ...
      logging.error(f"Create BQ table failed! | {e}")

    files_uri = str(Path(self._split_info_path).parent) + "/*{split}*.parquet"
    files_uri.format(split="train")
    ...
    try:
      ...
      logging.error(f"Load files to BQ from GCS failed | {e}")
      return None

    load_job.result()
    logging.info("upload to BQ is done!")
