...
from zippy_form.models import FormStep, FormField, Form, Account, Webhook, FormPaymentSettings, FormReCaptcha
...
from zippy_form.validation_rule_schema import get_text_box_field_validation_rule_schema, \
    ...
    get_phone_no_field_validation_rule_schema
...
class FormReCaptchaSerializer(serializers.ModelSerializer):
    recaptcha_enabled = serializers.BooleanField(required=True)
    site_key = serializers.CharField(required=True)
    secret_key = serializers.CharField(required=True)

    class Meta:
        model = FormReCaptcha
        fields = ['id', 'recaptcha_enabled', 'site_key', 'secret_key']
...
class UpdateFieldSettingsSerializer(serializers.ModelSerializer):
    def validate_validation_rule(self, value):
        ...
        elif filed_type == FIELD_TYPES[15][0]:
    ...
    class Meta:
        ...
        fields = ['id', 'label', 'field_size', 'placeholder', 'custom_class_name', 'validation_rule', 'is_mandatory',
                  'is_unique', 'options', 'field_format', 'content', 'content_size', 'content_alignment', 'slug']
        extra_kwargs = {
