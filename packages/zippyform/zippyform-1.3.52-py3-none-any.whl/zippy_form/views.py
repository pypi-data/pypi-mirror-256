...
from zippy_form.models import Account, FormField, Form, FormStep, FormSubmission, FormSubmissionData, FormFieldOption, Webhook,WebhookForm, AccountPaymentSettings,FormPaymentSettings, PaymentGatewayWebhook, FormSubmissionPaymentDetails, \
    FormMetaData, FormMetaDataOption, FormSubmissionMetaDataValue, FormPageViews
from zippy_form.serializers import FormSerializer, FormStepSerializer, MapFieldToFormStepSerializer, UpdateFieldSettingsSerializer, \
    ReOrderFieldSerializer, AccountSerializer, WebhookSerializer, FormPaymentSettingsSerializer, AccountProfileUpdateSerializer, AccountPaymentUpdateSerializer, FormReCaptchaSerializer
from zippy_form.utils import (FORM_STATUS, FORM_FIELD_STATUS, FIELD_TYPES, FORM_SUBMISSION_STATUS, \
    ...
    FORM_META_DATA_TYPE, FORM_META_DATA_OPTION_STATUS, convert_utc_to_timezone,get_stripe_secret_key,get_stripe_public_key,get_stripe_connect_url, \
    format_form_submission_status, generate_url_qrcode, check_key_exists_on_dict)
            ...
            for meta_data_id, meta_data_value in meta_data.items():
                ...
                if meta_data_form_submission:
                    if meta_data_form_submission.id not in meta_data_form_submission_id:
                        meta_data_form_submission_id.append(meta_data_form_submission.id)
            ...
            if form_meta_datas:
                for form_meta_data in form_meta_datas:
                    ...
                    form_submission_meta_data_value = FormSubmissionMetaDataValue.objects.filter(
                        form_meta_data=form_meta_data_id).first()
...
def form_submission_details2(req, form_id, form_submission_id):
    ...
    for form_meta_data in form_meta_datas:
        ...
        form_submission_meta_data_value = FormSubmissionMetaDataValue.objects.filter(form_meta_data=form_meta_data_id).first()
        if form_submission_meta_data_value:
            if form_meta_data_type == FORM_META_DATA_TYPE[1][0]:
                # dropdown
                form_meta_data_value = form_submission_meta_data_value.dropdown_field.option
...
def fields_mapped_to_form_step(req, form_id, step_id = None):
    ...
    form_recaptcha = form.form_recaptcha.first()
    if form_recaptcha:
        recaptcha_enabled = form_recaptcha.recaptcha_enabled
        recaptcha_site_key = form_recaptcha.site_key
        recaptcha_secret_key = form_recaptcha.secret_key
...
@api_view(['GET'])
def get_form_recaptcha_details(req, form_id):
    """
    Get Form ReCaptcha Details
    """
    try:
        form = Form.objects.filter(id=form_id).exclude(status=FORM_STATUS[0][0]).get()
    except Form.DoesNotExist:
    ...
    if form_recaptcha:
        recaptcha_enabled = form_recaptcha.recaptcha_enabled
        site_key = form_recaptcha.site_key
        secret_key = form_recaptcha.secret_key
...
@api_view(['POST'])
def update_form_recaptcha_details(req, form_id):
    """
    Create or Update Form ReCaptcha Details
    """
    try:
        form = Form.objects.filter(id=form_id).exclude(status=FORM_STATUS[0][0]).get()
        form_recaptcha = form.form_recaptcha.first()
    ...
    if form_recaptcha:
        # Update Form ReCaptcha Details
        serializer = FormReCaptchaSerializer(form_recaptcha, data=req.data)
    else:
        # Create Form ReCaptcha Details
        serializer = FormReCaptchaSerializer(data=req.data)

    if serializer.is_valid():
        serializer.save(form=form)
        msg = "Form Captcha Details Updated Successfully"
...
def form_analytics(req, form_id, year):
    ...
    # Form Views
    form_views_queryset = FormPageViews.objects.filter(form_id=form_id).filter(created_date__gte=start_datetime,created_date__lte=end_datetime).values('created_date__month').annotate(
        views=Count('id')).order_by('created_date__month')
    form_total_views_count = FormPageViews.objects.filter(form_id=form_id).count()
    ...
    form_submissions_queryset =  FormSubmission.objects.filter(
        form_id=form_id, status=FORM_SUBMISSION_STATUS[1][0]).filter(created_date__gte=start_datetime, created_date__lte=end_datetime).values('created_date__month').annotate(
        views=Count('id')).order_by('created_date__month')

    form_submissions_count = FormSubmission.objects.filter(
        form_id=form_id, status=FORM_SUBMISSION_STATUS[1][0]).count()
...
def submit_form(req, form_id, step_id, submission_id=None):
    ...
    # Validate ReCaptcha
    form_recaptcha = form.form_recaptcha.first()
    if form_recaptcha and form_recaptcha.recaptcha_enabled:
        ...
        if not next_step_exist:
            ...
            form_recaptcha_secret_key = form_recaptcha.secret_key
