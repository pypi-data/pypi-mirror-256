...
def form_analytics(req, form_id, year=""):
    ...
    if year:
        _queryset = FormSubmission.objects.filter(form_id=form_id, status=FORM_SUBMISSION_STATUS[1][0]).filter(created_date__gte=start_datetime, created_date__lte=end_datetime)
    else:
        _queryset = FormSubmission.objects.filter(form_id=form_id, status=FORM_SUBMISSION_STATUS[1][0])
