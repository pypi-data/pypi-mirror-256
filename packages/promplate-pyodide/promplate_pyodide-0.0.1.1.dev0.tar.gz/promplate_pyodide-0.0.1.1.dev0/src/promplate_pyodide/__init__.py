...
def patch_promplate(patch_openai=False):
    ...
    if patch_openai:
        ...
        llm_openai.TextComplete = llm_openai.AsyncTextComplete = partial(
            llm_openai.AsyncTextComplete, http_client=None
        )
        llm_openai.TextGenerate = llm_openai.AsyncTextGenerate = partial(
            llm_openai.AsyncTextGenerate, http_client=None
        )
        llm_openai.ChatComplete = llm_openai.AsyncChatComplete = partial(
            llm_openai.AsyncChatComplete, http_client=None
        )
        llm_openai.ChatGenerate = llm_openai.AsyncChatGenerate = partial(
            llm_openai.AsyncChatGenerate, http_client=None
        )
