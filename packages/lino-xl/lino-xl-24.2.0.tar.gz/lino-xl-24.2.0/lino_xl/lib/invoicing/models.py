...
class Task(Runnable):
    ...
    target_journal = dd.ForeignKey('ledger.Journal',
    ...
    def __str__(self):
        ...
        # return _("Make {}").format(dd.babelattr(self.target_journal, 'name')
    ...
    @dd.displayfield(_("Invoice generator"))
...
class FollowUpRule(Sequenced):
    ...
    source_journal = dd.ForeignKey('ledger.Journal',
                                   verbose_name=_("Source journal"),
                                   null=True,
                                   blank=True,
                                   related_name="followup_source")
    # target_journal = dd.ForeignKey(
    #     'ledger.Journal', verbose_name=_("Target journal"), null=True, blank=True,
...
class Plan(UserPlan):
    ...
    #     return super().run_start_plan(user, **options)
    ...
    def __str__(self):
        ...
        return "{0} to {1} plan by {2}".format(
            self.invoicing_task, self.invoicing_task.target_journal.ref,
            self.user)
...
class Item(dd.Model):
    ...
    def create_invoice(self, ar):
        if self.plan.invoicing_task is None:
            raise Warning(_("No invoicing_task specified"))
...
def run_invoicing_tasks(ar):
    asyncio.run(Task.start_task_runner(ar, max_count=1))
    # async_to_sync(Task.start_task_runner)(ar, max_count=1)
