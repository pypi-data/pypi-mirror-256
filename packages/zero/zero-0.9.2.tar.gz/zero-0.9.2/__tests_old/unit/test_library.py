...
@pytest.fixture
def opamp_library():
    class MockOpAmpLibrary(OpAmpLibrary):
    ...
    return MockOpAmpLibrary()


def _parse_and_return(library, data):
    ...
    library.parse_opamp_test_data(name, data)

    # Get parsed model.
    return library.find_library_opamp_by_test_name(name)


def test_single_real_poles(opamp_library):
    """Test op-amp with single real pole and zero is parsed properly"""
    model = _parse_and_return(opamp_library, {"poles": ["53.4M"], "zeros": ["76.5M"]})
    assert_array_almost_equal(np.array([53.4e6]), model.poles)
    assert_array_almost_equal(np.array([76.5e6]), model.zeros)


def test_single_complex_poles(opamp_library):
    """Test op-amp with single complex pole and zero is parsed properly"""
    model = _parse_and_return(opamp_library, {"poles": ["53.4M 5.1"], "zeros": ["76.5M 4.7"]})
...
def test_multiple_poles(opamp_library):
    """Test op-amp with multiple complex poles and zeros is parsed properly"""
    model = _parse_and_return(
        opamp_library,
        {"poles": ["13M", "53.4M 5.1"], "zeros": ["19.3M", "76.5M 4.7"]}
    )
    assert_array_almost_equal(
        np.array(
            [
