...
support_types = ('str', 'int', 'bool', 'float', str, int, bool, float)
...
class AppSepc(object):
    ...
    def run(self):
        ...
        if self.common_args_option.get('run_app_config', {}).get('merge_function_args_for_init', False):
    ...
    def get_method_spec(self, method_name):
        ...
        if hasattr(inspect, 'getfullargspec'):
            method_arg_spec = inspect.getfullargspec(method_attr)
        ...
        method_spec['parameters'] = _check_parameters_spec(method_name, method_arg_spec, args, default_values)
        return method_spec
...
def _check_parameter_item_default_value(arg_name, parameter_item, method_arg_spec, default_values):
    annotations = hasattr(method_arg_spec, 'annotations') and arg_name in method_arg_spec.annotations
    if (default_values is None or arg_name not in default_values) and not annotations:
        return parameter_item

    if arg_name in default_values:
        value = default_values[arg_name]
        type = _determine_type(value)
        des = 'type: %s, default value is: %s' % (type, value)
    elif annotations:
        type = method_arg_spec.annotations[arg_name]
        value = None
        des = 'type: %s' % type
    else:
        type = None
...
def _check_parameters_spec(method_name, method_arg_spec, args, default_values):
    parameters = {}
    for arg_name in args:
        ...
        parameter_item = _check_parameter_item_default_value(arg_name, parameter_item, method_arg_spec, default_values)
        if 'type' not in parameter_item or parameter_item['type'] is None:
            parameter_item['type'] = 'str'
        if 'des' not in parameter_item or parameter_item['des'] is None:
...
def _check_parameter_item(arg_name, parameter_item):
    if 'type' not in parameter_item:
        print('Missing type in parameter item: %s, arg_name: %s' % (parameter_item, arg_name))
    # if parameter_item['type'] not in support_types:
    #     print('The type: %s is not in support types: %s, arg_name: %s' % (parameter_item['type'], support_types, arg_name))
    if not parameter_item['des']:
        print('Missing parameter descripion(`des`): %s, arg_name: %s' % (parameter_item, arg_name))
