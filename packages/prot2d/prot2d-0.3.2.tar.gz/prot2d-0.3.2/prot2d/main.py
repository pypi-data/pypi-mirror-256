...
def create_2DSVG_from_pdb(pdb_file:str, result_dir:str, tmp_dir:str, family_vis:bool=True, fam_aligned_splitting:bool = True, drop_family_prob:float = 0.5,foldseek_executable:str=None,fixed_sf:int=None,fam_info:bool=True ,domain_splitted:bool=False, domain_annotation_file:str=None, domain_hull:bool=True, visualisation_type:str ="normal", 
                    cysteins:bool=True, as_annotation:bool=False, mark_endings:bool=True, simple_helix:bool=True, show_lddt_col:bool=False,show_lddt_text:bool=False, find_best_rot_step:int = 30, simple_coil_avg:int=10):
    """
    ...
    print(f"{len(domain_files)} domain(s) were found and will be visualized seperatly:")
    ...
    viewbox = calculate_viewbox(full_prot.residues,300) #TODO should be used for final usage but for now shit because browser view is wrong --> when using make_postive can be false for true coords
    ...
    for dom_prot in dom_proteins: 
        ...
        if domain_splitted:
            dom_prot.add_hull(dwg,dom_prot.get_hull_color(), opacity=0.15) if domain_hull else None
